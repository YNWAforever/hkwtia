...
const missingTranslationExpression = `char_length(btrim(body_mdx_zh_hk, ${ecmascriptWhitespaceSql})) = 0`;

describe("WiseTech localized news schema contract", () => {
  it("adds exactly one nullable Traditional Chinese body column to posts", () => {
    const column = getTableConfig(posts).columns.find((candidate) => candidate.name === "body_mdx_zh_hk");
  ...
  it("keeps every posts kind and the existing English body contract unchanged", () => {
    const columns = new Map(getTableConfig(posts).columns.map((column) => [column.name, column]));
  ...
  it("ships 0022 metadata without a false translation backfill and retains later journal history", () => {
    ...
    const snapshot = JSON.parse(readFileSync(snapshotPath, "utf8")) as {tables?: Record<string, {columns?: Record<string, {notNull?: boolean}>}>};
    expect(snapshot.tables?.["public.posts"]?.columns?.body_mdx_zh_hk).toMatchObject({notNull: false});
    ...
    expect(eventHeroIndex).toBeGreaterThan(localizedNewsIndex);
  });
});
