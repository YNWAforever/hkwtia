...
const translated = vi.hoisted(() => ({
  metaTitle: "AI-Ops in public | WiseTech Hong Kong",
...
describe("AI-Ops public page boundary", () => {
  ...
  it("uses the exact new localized metadata and message bundles", async () => {
    ...
    expect(chinese).toMatchObject({
      metaTitle: "公開 AI-Ops｜WiseTech Hong Kong",
      title: "公開量度 AI 營運",
  ...
  // Re-pinned by C-9 (C2 Task 13 Step 4): `methodologyDescription` now carries
  // the human-takeover caveat in both bundles, because the `aiops_monthly_metrics`
  // fix was DECLINED — drizzle-kit regenerates the materialized view without the
  // unique index `REFRESH … CONCURRENTLY` needs. The sentence is the honest
  // label the plan asks for when the view stays as it is.
