...
describe("WiseTech protected route ownership", () => {
  ...
  it("keeps the explicit inventory bidirectionally equal to the current protected code surface", () => {
    ...
    // Showcase lead email recovery adds one protected cron endpoint.
    expect(codeFiles).toHaveLength(62);
    expect(inventoryFiles).toHaveLength(62);
  ...
  it("classifies all admin, general API, webhook, and job owners explicitly", () => {
    ...
    // Showcase lead recovery adds the twelfth authenticated job.
    expect(count("job-handler")).toBe(12);
    expect(protectedRouteOwnershipInventory.filter(({family}) => family === "admin")).toHaveLength(35);
    expect(protectedRouteOwnershipInventory.filter(({family}) => family === "api")).toHaveLength(27);
  });
