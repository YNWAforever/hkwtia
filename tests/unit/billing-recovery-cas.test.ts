...
const replacementRow = [
  replacementId, membershipId, 2, `membership-checkout:${membershipId}:2`, "price_startup_v1", "active",
  null, null, "recovery-1", now, now, null,
];
...
describe("serialized checkout recovery CAS", () => {
  ...
  it("returns the same replacement for a duplicate recovery request token", async () => {
    ...
    database.current = transactionDatabase([[{membership_id: membershipId}], [replacementRow]], statements);
  ...
  it("rejects a stale different-token CAS without ending the new active attempt", async () => {
    ...
    newer[0] = replacementId;
    database.current = transactionDatabase([[{membership_id: membershipId}], [], [newer]], statements);
    await expect(billingAttemptsRepository.startNewAttempt(
      actor, membershipId, "price_startup_v1", "expired",
  ...
  it("locks membership then orders compare, update, and insert inside one transaction", async () => {
    const statements: string[] = [];
    const current = [...replacementRow];
    current[0] = expectedId;
    current[2] = 1;
    database.current = transactionDatabase([
      [{membership_id: membershipId}], [], [current], [{id: expectedId}], [replacementRow],
    ], statements);
    await expect(billingAttemptsRepository.startNewAttempt(
      actor, membershipId, "price_startup_v1", "abandoned", request,
    )).resolves.toMatchObject({id: replacementId, recoveryRequestId: "recovery-1"});
    const sql = statements.map((value) => value.toLowerCase());
    expect(sql[0]).toContain("for update");
    expect(sql.findIndex((value) => value.includes("update "))).toBeLessThan(sql.findIndex((value) => value.includes("insert ")));
  });
