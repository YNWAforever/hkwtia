...
function service() {
  const register = vi.fn(async (): Promise<GuestRegistrationResult> => ({id: "g1", disposition: "registered", eventTitle: "AI Clinic"}));
...
describe("guest registration service (programme B-4)", () => {
  it("honeypot short-circuits; valid input registers, upserts a contact and sends one confirmation", async () => {
    ...
    const [payload] = send.mock.calls[0] as unknown as [{to: string; eventTitle: string; slug: string; cancelUrl: string}];
    expect(payload).toEqual(expect.objectContaining({to: "ada@example.hk", eventTitle: "AI Clinic", slug: "ai-clinic", disposition: "registered"}));
  ...
  it("does not re-send a confirmation to an already registered guest, and survives a contact failure", async () => {
    const {subject, register, upsertContact, send} = service();
    register.mockResolvedValueOnce({id: "g1", disposition: "already_registered", eventTitle: "AI Clinic"});
