...
import { buildM4CSeedFixture, seedM4C } from "../../scripts/seed-m4c";

const databaseUrl = process.env.DATABASE_URL_TEST;
const enabled =
  process.env.RUN_M4C_SEED_POSTGRES === "true" &&
...
describe.runIf(enabled)("M4C PostgreSQL seed reconciliation", () => {
    ...
    await pool.query(
      `INSERT INTO profiles
       ...
       VALUES ($1, 'm4c-seed-survival-auth', 'm4c-seed-survival@example.test',
         'member', $2, false, ARRAY[]::text[], 'M4C survival sentinel', NULL,
    ...
    for (const sentinel of ownershipSentinels) {
      await pool.query(
        ...
        [
          ...
          sentinel.ownershipKey,
          sentinelAt,
          new Date(sentinelAt.getTime() + 86_400_000),
        ],
      );
      await pool.query(
      ...
      await pool.query(
        ...
        [
          ...
          sentinelProfileId,
          JSON.stringify({
            fixture: sentinel.ownershipKey,
            membershipId: `survival-${sentinel.ownershipKey}`,
            renewalOrdinal: 2,
          }),
          sentinelAt,
        ],
      ...
      await pool.query(
        ...
        [
          sentinel.postId,
          `m4c-${sentinel.ownershipKey}-survival`,
          sentinelAt,
  ...
  it(
    "reconciles twice to exact owned counts, metrics, public logs, and privacy boundaries",
    { timeout: 30_000 },
    async () => {
      ...
      expect({
        ...
        escalatedConversationCount: Number(
          current.escalated_conversation_count,
        ),
      ...
      expect(
        renewals.rows.map((row) => ({
          monthStart: hongKongDateKey(row.month_start),
      ...
      const publicLogs =
        await createPublicPostsRepository().listPublishedBuildLogs(asOf);
      ...
      const dashboardHtml = renderToStaticMarkup(
        createElement(AiOpsDashboard, {
          locale: "en",
          state: buildAiOpsDashboardState(
            dashboardRows,
            new Date(
              Math.max(
                ...dashboardRows.map(({ refreshedAt }) =>
                  refreshedAt.getTime(),
                ),
              ) + 1,
            ),
