...
type Props = Readonly<{locale: AppLocale; labels: SegmentResultsLabels; preview: SegmentPreview; saved: readonly SavedSegmentRecord[]; queueAction: QueueAction; newDraftHref: string}>;

function CampaignQueueForm({action, locale, labels, segmentId}: Readonly<{action: QueueAction; locale: AppLocale; labels: SegmentResultsLabels; segmentId: string}>) {
  const [state, formAction, pending] = useActionState(action, {disposition: null, recipientCount: 0, error: null});
  const message = state.disposition
    ? labelledValue(locale, state.disposition === "created" ? labels.queued : labels.existing, `${state.recipientCount} ${labels.recipients}`)
    : null;
