...
export type AdminNavLabels = Readonly<{brand: string; label: string; members: string; segments: string; atRisk: string; events: string; listingsReview: string; approvals: string; reports: string; automations: string}>;
...
export function AdminNav({locale, labels}: {locale: AppLocale; labels: AdminNavLabels}) {
  const items = [
    ...
    {href: "/admin/at-risk", label: labels.atRisk}, {href: "/admin/events-mgmt", label: labels.events}, {href: "/admin/listings-review", label: labels.listingsReview},
