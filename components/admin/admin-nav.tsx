...
export type AdminNavLabels = Readonly<{label: string; members: string; segments: string; atRisk: string; events: string; approvals: string; reports: string}>;
...
export function AdminNav({locale, labels}: {locale: AppLocale; labels: AdminNavLabels}) {
  ...
  return <nav aria-label={labels.label} className="border-b border-border/70 bg-background/85 px-4 py-4 backdrop-blur sm:px-6">
    <div className="mx-auto flex max-w-6xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <Link className="font-serif text-xl font-semibold text-foreground" href={localizedPath(locale, "/admin")}>WTIA</Link>
