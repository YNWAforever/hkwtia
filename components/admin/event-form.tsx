...
type Labels = Readonly<{slug: string; titleEn: string; titleZh: string; descriptionEn: string; descriptionZh: string; startsAt: string; endsAt: string; venue: string; capacity: string; registrationMode: string; registrationModes: Readonly<{rsvp: string; external: string; ticketed: string}>; ticketPriceHkdCents: string; memberOnly: string; published: string; heroMediaId: string; noHeroMedia: string; save: string; saving: string}>;
type Values = Partial<Readonly<{slug: string; titleEn: string; titleZh: string | null; descriptionEn: string; descriptionZh: string | null; startsAt: Date; endsAt: Date | null; venue: string | null; capacity: number | null; registrationMode: string; ticketPriceHkdCents: number | null; memberOnly: boolean; published: boolean; heroMediaId: string | null}>>;
...
export function EventForm({action, labels, values = {}, mediaRows = []}: Readonly<{action: (state: EventActionState, formData: FormData) => Promise<EventActionState>; labels: Labels; values?: Values; mediaRows?: readonly {id: string; altEn: string; altZh: string}[]}>) {
  ...
  return <form action={formAction} className="glass-card grid gap-4 p-6 md:grid-cols-2" noValidate>
    ...
    <label>{labels.startsAt}<input {...fieldProps("startsAt")} className="mt-1 w-full rounded-md border p-2" defaultValue={value("startsAt", formatHongKongDateTimeLocal(values.startsAt))} name="startsAt" required type="datetime-local"/>{error("startsAt")}</label>
    ...
    <label>{labels.registrationMode}<select {...fieldProps("registrationMode")} className="mt-1 w-full rounded-md border p-2" defaultValue={value("registrationMode", values.registrationMode ?? "rsvp")} name="registrationMode">{(["rsvp", "external", "ticketed"] as const).map((key) => <option key={key} value={key}>{labels.registrationModes[key]}</option>)}</select>{error("registrationMode")}</label>
    <label>{labels.ticketPriceHkdCents}<input {...fieldProps("ticketPriceHkdCents")} className="mt-1 w-full rounded-md border p-2" defaultValue={value("ticketPriceHkdCents", values.ticketPriceHkdCents == null ? "" : String(values.ticketPriceHkdCents / 100))} min="1" name="ticketPriceHkdCents" step="1" type="number"/>{error("ticketPriceHkdCents")}</label>
