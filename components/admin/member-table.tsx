...
export type MemberTableLabels = Readonly<{search: string; caption: string; empty: string; name: string; email: string; company: string; plan: string; status: string; renewal: string; score: string; next: string; previous: string; unavailable: string}>;
...
function pageHref(locale: AppLocale, query: string, cursor: string | null): string {
  ...
  return `${localizedPath(locale, "/admin/members")}${suffix ? `?${suffix}` : ""}`;
}
...
export function MemberTable({locale, labels, page, query}: Props) {
  ...
  return <div className="space-y-6">
    ...
    {(page.nextCursor || query) ? <nav aria-label={labels.caption} className="flex items-center justify-between gap-4 text-sm"><span>{page.nextCursor ? <Link className="rounded-md border border-border px-3 py-2 hover:bg-muted" href={pageHref(locale, query, page.nextCursor)}>{labels.next}</Link> : null}</span><Link className="rounded-md border border-border px-3 py-2 hover:bg-muted" href={pageHref(locale, "", null)}>{labels.previous}</Link></nav> : null}
  </div>;
}
