...
function scheduledEvent(): ScheduledController {
  return {
    cron: "30 0 1 * *",
    scheduledTime: Date.parse("2027-04-01T00:30:00.000Z"),
    noRetry: vi.fn(),
  };
}
...
describe("Board Reporter Worker schedule", () => {
  it("dispatches the dedicated monthly cron to only Board Reporter", async () => {
    ...
    expect(fetch.mock.calls[0]?.[0].toString()).toBe(
      "https://preview.example.test/api/jobs/board-reporter",
    );
  ...
  it("uses 240 seconds for every retry while reusing the same monthly endpoint", async () => {
    vi.useFakeTimers();
    vi.setSystemTime("2027-04-01T00:30:00.000Z");
    const urls: string[] = [];
    ...
    expect(urls.filter((url) => url.endsWith("/board-reporter"))).toEqual([
      "https://preview.example.test/api/jobs/board-reporter",
      "https://preview.example.test/api/jobs/board-reporter",
      "https://preview.example.test/api/jobs/board-reporter",
    ]);
  ...
  it("configures the production Worker monthly cron trigger", () => {
    const wrangler = readFileSync("wrangler.toml", "utf8");
    expect(wrangler).toContain('"30 0 1 * *"');
  });
});
