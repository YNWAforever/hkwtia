...
The body of this file is the v1 spec exactly as delivered on 2026-09-01. WP-0 committed it unchanged, apart from one Markdown escape in the §1.1 table (`open\|past`, so that row renders), and records here the facts that differed when the spec was checked against the repository before the first edit. Where this appendix and the body disagree, later work packages follow the appendix.
