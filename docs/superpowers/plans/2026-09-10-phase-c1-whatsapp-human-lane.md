...
- [ ] **Owner action —** confirm Woztell's Open API host and the `conversationHistory` selection set before calling `/api/admin/woztell/backfill` against production (O-2), and set `WOZTELL_OPEN_API_TOKEN` in Vercel with `api:admin` scope.
