...
- [ ] Submit **both** `wtia_event_reminder_24h` (renamed from `wtia_event_reminder_24h_en` when this reconciled onto Phase C's C-7 registry, 2026-09-12 — see `docs/integration/phase-c-whatsapp-go-live.md` for the current, authoritative activation sequence) and `wtia_event_reminder_24h_zh_hk` to Woztell/Meta with the Phase A template batch (spec §8.3); until a member's own language is approved, that member's reminder goes by email only — the runner will not substitute the other language's template. The guest confirmation is email-only; there is no `wtia_event_confirmation_*` template.
