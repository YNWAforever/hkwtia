...
- [ ] **Step 1: Write failing schema, state-machine, transaction, service, action, and route-handoff tests**
    ...
    - for a company target, do not narrow the existing applicant-or-active-company-member repository authority: require `membership.ownerUserId === null` and `membership.companyId === application.companyId`; authorize the locked applicant directly when `application.applicantUserId === actor.profileId`, otherwise select and row-lock the current actor's exact active company-member row for that company before writes; use the locked `application.applicantUserId`, not a possibly different acting profile, as the activation-journey profile;
    - prove both branches: the company applicant remains authorized without a company-member row, and an active non-applicant company member may resume; an inactive/foreign non-applicant company member and a non-applicant personal actor fail before writes;
...
- Create managed launch boundary: `scripts/managed-runtime-environment.mjs`, `scripts/run-managed-playwright.mjs`, `scripts/run-managed-next.mjs`, `tests/fixtures/managed-process-env-probe.mjs`, `tests/unit/managed-runtime-process-boundary.test.ts`.
- Create: `tests/fixtures/isolated-runtime-env.ts`, `tests/unit/isolated-runtime-environment.test.ts`, `tests/fixtures/managed-auth-session.ts`, `tests/unit/managed-auth-session.test.ts`, `tests/fixtures/m1-live-acceptance.ts`, `tests/unit/m1-live-acceptance-safety.test.ts`, `tests/fixtures/webhook-postgres-safety.ts`, `tests/unit/webhook-postgres-safety.test.ts`, `tests/integration/webhook-join-projection-postgres.test.ts`.
...
- Task 4 owns the shared test-only managed-runtime boundary used by mutating M1, M2, M3, M4B, M5, M6, M7, and final authenticated-Axe suites:
  ...
  Only after neutral capture/validation may the Next-server builder set `DATABASE_URL <- DATABASE_URL_TEST`, `APP_URL`/`NEXT_PUBLIC_SITE_URL <- managed origin`, and the active suite's exact standard server names from guarded test sources. M1, M2, M6, M7, and authenticated Axe map server `NEON_PROJECT_ID` only from their respective `M1_TEST_NEON_PROJECT_ID`, `M2_TEST_NEON_PROJECT_ID`, `M6_TEST_NEON_PROJECT_ID`, `M7_TEST_NEON_PROJECT_ID`, or `PR6_AXE_TEST_NEON_PROJECT_ID` after the independently supplied original `NEON_PROJECT_ID` is exact-equal; other suites leave the server name absent. M1/M2 map all four `STRIPE_TEST_*` sources; M2 maps `M2_TEST_CRON_SECRET` and `M2_TEST_UNSUBSCRIBE_TOKEN_SECRET`; M3 maps `M3_TEST_UNSUBSCRIBE_TOKEN_SECRET`; every authenticated suite maps `NEON_AUTH_TEST_BASE_URL`/`NEON_AUTH_TEST_COOKIE_SECRET` only after canonical non-production HTTPS and exact independent `NEON_AUTH_TEST_ALLOWED_ORIGIN` equality. Test-source names remain available only to the sanitized Playwright runner as enumerated test capability and are absent from the replacement Next environment. Missing suite-required values fail before a Pool, browser, Auth, provider, or mutation client.

  `managed-runtime-process-boundary.test.ts` must exercise OS child processes, not only pure returned objects. With synthetic canaries, it invokes the same runner and Next spawn functions against `managed-process-env-probe.mjs`, which reports only sorted key names and hashes of non-secret synthetic values. Assert the actual runner child has only the operational allowlist plus the active suite's sources, the actual Next child has only the operational allowlist plus exact standard test mappings, and every mixed-case live/standard/inactive/test-source canary is absent at the appropriate boundary rather than `""`. Prove Playwright's three exact default keys are admitted only at the inner-wrapper boundary and are absent from Next; prove the merge cannot reintroduce ambient keys because the inner launcher replaces the Next environment. Prove direct managed CLI bypass, PATH/shim launch, unsafe shell-string path serialization, duplicate-case input, unexpected/default-key drift, invalid attestation, occupied port, child failure, and missing local CLI all fail before provider/database/browser mutation. Import the real environment parsers against the captured actual Next-child map with optional Concierge, Turnstile, Resend, R2, and WOZTELL names absent, and prove M2's invalid-signature WOZTELL request still reaches the existing 401 contract. This makes an M2 auth/order regression run with no ambient/live Resend, AI, R2, WOZTELL, Turnstile, telemetry, database, Stripe, cron, unsubscribe, or Auth capability beyond the exact guarded isolated test mappings it intentionally needs.
...
- [ ] **Step 1: Write failing durable-price, locked-row, completion, and locale tests**
    ...
    Add `webhook-postgres-safety.test.ts` and guarded `webhook-join-projection-postgres.test.ts`. The integration gate requires exact `PR6_WEBHOOK_POSTGRES_ALLOW_DESTRUCTIVE=PR6_ISOLATED_WEBHOOK_FIXTURES_ONLY`, independently matched non-production Neon/TLS host/project identity, `DATABASE_URL_TEST`, an advisory lock, and run-owned application/membership/attempt rows before constructing a Pool or mutating. Against real PostgreSQL, prove the checkout `INNER JOIN ... FOR UPDATE OF` executes, a concurrent relation/application change blocks behind the locks, success commits membership/application/attempt/journey/audit/job atomically, and injected audit/job failure rolls all transactional rows back before exact cleanup. Missing approval/resources skips as `NOT PASSED`; it is never a credential-free PASS.
...
- [ ] **Step 3: Implement durable billing and status projection**
    ...
    Implement `webhook-postgres-safety.ts` as a test-only pure guard with no production/provider import. Require exact sentinel `PR6_WEBHOOK_POSTGRES_ALLOW_DESTRUCTIVE=PR6_ISOLATED_WEBHOOK_FIXTURES_ONLY`, absent `PLAYWRIGHT_BASE_URL`, `DATABASE_URL_TEST`, canonical TLS, exact `PR6_WEBHOOK_TEST_DB_HOST`, independently exact `NEON_PROJECT_ID === PR6_WEBHOOK_TEST_NEON_PROJECT_ID`, and a non-production host/project label before constructing a Pool. The integration test takes a fixed advisory lock, creates only run-ID-owned application/membership/active-attempt fixtures with no provider call, executes the real repository SQL, tests a second connection blocking behind the application/membership locks, and cleans exact rows in FK-safe order in `finally`. It never migrates, seeds shared data, contacts Stripe/Vercel/Neon APIs, or runs without separate database-mutation approval.
...
- [ ] **Step 5: Commit the durable billing slice**
    ...
    git add -- ':(literal)package.json' ':(literal)playwright.config.ts' ':(literal)scripts/managed-runtime-environment.mjs' ':(literal)scripts/run-managed-playwright.mjs' ':(literal)scripts/run-managed-next.mjs' ':(literal)tests/fixtures/managed-process-env-probe.mjs' ':(literal)tests/unit/managed-runtime-process-boundary.test.ts' ':(literal)tests/fixtures/isolated-runtime-env.ts' ':(literal)tests/unit/isolated-runtime-environment.test.ts' ':(literal)tests/fixtures/managed-auth-session.ts' ':(literal)tests/unit/managed-auth-session.test.ts' ':(literal)tests/fixtures/m2-runtime-env.ts' ':(literal)lib/billing/checkout-service.ts' ':(literal)lib/db/repos/billing-attempts.ts' ':(literal)lib/db/repos/jobs.ts' ':(literal)lib/membership/join-billing-state.ts' ':(literal)app/[locale]/(join)/join/checkout/page.tsx' ':(literal)app/[locale]/(join)/join/complete/page.tsx' ':(literal)app/[locale]/(member)/portal/billing/page.tsx' ':(literal)components/billing/checkout-status.tsx' ':(literal)messages/en.json' ':(literal)messages/zh-HK.json' ':(literal)tests/fixtures/m1-live-acceptance.ts' ':(literal)tests/unit/m1-live-acceptance-safety.test.ts' ':(literal)tests/fixtures/webhook-postgres-safety.ts' ':(literal)tests/unit/webhook-postgres-safety.test.ts' ':(literal)tests/integration/webhook-join-projection-postgres.test.ts' ':(literal)tests/unit/checkout-service.test.ts' ':(literal)tests/unit/checkout-recovery-service.test.ts' ':(literal)tests/unit/billing-checkout-locking.test.ts' ':(literal)tests/unit/billing-recovery-cas.test.ts' ':(literal)tests/unit/join-billing-pages.test.tsx' ':(literal)tests/unit/portal-billing-actions.test.tsx' ':(literal)tests/unit/webhook-service.test.ts' ':(literal)tests/unit/webhook-repository-sequential.test.ts' ':(literal)tests/unit/m1-acceptance-services.test.ts' ':(literal)tests/e2e/m1-acceptance.spec.ts'
...
- [ ] **Step 1: Write failing grouped-navigation, source-owner, and CRM rendering tests**
    ...
    Extend the M2 safety/runtime tests to require `M2_ACCEPTANCE_ALLOW_DESTRUCTIVE=M2_ISOLATED_FIXTURES_ONLY`, `M2_E2E_ALLOWED_ORIGIN`, `APP_URL`, `NEON_PROJECT_ID`, `M2_TEST_NEON_PROJECT_ID`, `M2_TEST_NEON_HOST`, `M2_TEST_CRON_SECRET`, and `M2_TEST_UNSUBSCRIBE_TOKEN_SECRET` in the centralized contract while delegating standard DB/Stripe/APP mapping to Task 4's shared builder. The M2 wrapper then unconditionally maps `CRON_SECRET <- M2_TEST_CRON_SECRET` and `UNSUBSCRIBE_TOKEN_SECRET <- M2_TEST_UNSUBSCRIBE_TOKEN_SECRET` after the central scrub; require both test secrets nonempty and distinct, with the unsubscribe secret at least 32 bytes, so ambient production names never win. Assert any present `PLAYWRIGHT_BASE_URL`, missing/wrong sentinel, managed-origin/APP/allowlist mismatch, missing project/test/secret variables, equal or undersized test secrets, production/non-Neon/mismatched/TLS-invalid DB, raw test DB mismatch after mapping, multiple mutating sentinels, or reusable/occupied server fails before `connect` or `seedM2`.
    Extend `resetM2AuthenticatedFixtures` and its unit tests with a complete three-identity lifecycle whose baseline predates every reset/seed write. After pure runtime/provider checks construct the guarded Pool, acquire the M2 advisory lock, resolve and validate the exact configured staff/member/company-admin profile/auth identities, and snapshot every profile column before calling `resetM2AuthenticatedFixtures` or `seedM2`. Only then run the named `beforeAll` reset/seed and require the seeded identities still have the same profile/auth IDs and expected fixture roles. A reset/seed failure retains the pre-reset snapshots for aggregate cleanup.
...
- [ ] **Step 2: Run the focused tests and record the RED reason**
    ...
    npm.cmd test -- tests/unit/isolated-runtime-environment.test.ts tests/unit/managed-runtime-process-boundary.test.ts tests/unit/managed-auth-session.test.ts tests/unit/wisetech-pr6-admin-crm-shell.test.tsx tests/unit/admin-presentational.test.tsx tests/unit/admin-dashboard-tiles.test.tsx tests/unit/admin-member-list.test.ts tests/unit/admin-member-page-boundary.test.ts tests/unit/admin-member-profile.test.ts tests/unit/member-note-server-action-boundary.test.ts tests/unit/segment-query.test.ts tests/unit/segment-save-action.test.ts tests/unit/campaign-server-action-auth.test.ts tests/unit/at-risk-repository-boundary.test.ts tests/unit/admin-page-auth.test.ts tests/unit/m2-auth-reset.test.ts tests/unit/m2-runtime-environment.test.ts tests/unit/m2-browser-acceptance-contract.test.ts
...
- Create: `tests/e2e/wisetech-pr6-internal-journeys.spec.ts`, `tests/fixtures/m5-browser-lifecycle.ts`, `tests/unit/m5-browser-lifecycle.test.ts`, `tests/fixtures/pr6-authenticated-accessibility-lifecycle.ts`, `tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts`, `tests/e2e/wisetech-pr6-authenticated-accessibility.spec.ts`, `scripts/run-lighthouse-preview.mjs`, `tests/fixtures/lighthouse-process-env-probe.mjs`, `tests/unit/lighthouse-process-boundary.test.ts`, `tests/unit/lighthouse-runner.test.ts`, `tests/unit/lighthouse-config.test.ts`, `docs/integration/wisetech-pr6-verification.md`, `docs/integration/wisetech-pr6-pr-body.md`.
...
- Modify the public performance gate: `package.json`, `lighthouserc.js`. The package script delegates to the exact local wrapper; the config uses only the authorized Preview route allowlist below, a run-owned OS-temp working/output directory, no local-server fields, and no noindex/authenticated URL.
...
- [ ] **Step 1: Define final credential-free and authenticated browser verification matrices**
    ...
    Before Step 2, create `scripts/run-lighthouse-preview.mjs`, `tests/fixtures/lighthouse-process-env-probe.mjs`, `tests/unit/lighthouse-process-boundary.test.ts`, `tests/unit/lighthouse-runner.test.ts`, and `tests/unit/lighthouse-config.test.ts`; point `package.json`'s `test:lighthouse` script at the wrapper and replace `lighthouserc.js` with the exact fail-closed configuration specified in Step 6. This is Task 12 configuration work, not an external gate claim: all three unit contracts must be executable without network or Chrome launch in the focused aggregate, including a real child-process environment probe, while the authorized Preview collection remains deferred to Step 6.

- [ ] **Step 2: Run the focused PR6 cross-surface aggregate**

    Run:

    npm.cmd test -- tests/unit/membership-catalog.test.ts tests/unit/membership-public-catalog.test.ts tests/unit/membership-links.test.tsx tests/unit/join-schema.test.ts tests/unit/join-navigation.test.ts tests/unit/join-service.test.ts tests/unit/join-terminal-state.test.ts tests/unit/join-terminal-transaction.test.ts tests/unit/join-actions.test.ts tests/unit/member-login-page.test.tsx tests/unit/portal-sign-out-button.test.tsx tests/unit/isolated-runtime-environment.test.ts tests/unit/managed-runtime-process-boundary.test.ts tests/unit/managed-auth-session.test.ts tests/unit/checkout-service.test.ts tests/unit/checkout-recovery-service.test.ts tests/unit/billing-recovery-cas.test.ts tests/unit/join-billing-pages.test.tsx tests/unit/webhook-service.test.ts tests/unit/webhook-repository-sequential.test.ts tests/unit/webhook-postgres-safety.test.ts tests/unit/m1-live-acceptance-safety.test.ts tests/unit/seat-invitation-routes.test.tsx tests/unit/seat-service.test.ts tests/unit/internal-navigation.test.tsx tests/unit/internal-shell.test.tsx tests/unit/wisetech-pr6-route-inventory.test.ts tests/unit/wisetech-pr6-join-shell.test.tsx tests/unit/locale-switcher.test.tsx tests/unit/wisetech-pr6-portal-shell.test.tsx tests/unit/portal-company-context.test.ts tests/unit/wisetech-pr6-admin-crm-shell.test.tsx tests/unit/m2-auth-reset.test.ts tests/unit/m2-runtime-environment.test.ts tests/unit/m2-browser-acceptance-contract.test.ts tests/unit/m3-e2e-safety.test.ts tests/unit/m3-browser-lifecycle.test.ts tests/unit/authenticated-identity-safety.test.ts tests/unit/wisetech-pr6-admin-cms-shell.test.tsx tests/unit/m7-acceptance-safety.test.ts tests/unit/m7-browser-acceptance-contract.test.ts tests/unit/wisetech-pr6-admin-operations-shell.test.tsx tests/unit/m6-e2e-safety-contract.test.ts tests/unit/m5-browser-lifecycle.test.ts tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts tests/unit/lighthouse-process-boundary.test.ts tests/unit/lighthouse-runner.test.ts tests/unit/lighthouse-config.test.ts
...
- [ ] **Step 5: Prove safety contracts, then run isolated M1-M7 only with separate authority**
    ...
    npm.cmd test -- tests/unit/isolated-runtime-environment.test.ts tests/unit/managed-runtime-process-boundary.test.ts tests/unit/managed-auth-session.test.ts tests/unit/authenticated-identity-safety.test.ts tests/unit/m1-live-acceptance-safety.test.ts tests/unit/webhook-postgres-safety.test.ts tests/unit/m2-auth-reset.test.ts tests/unit/m2-runtime-environment.test.ts tests/unit/m2-browser-acceptance-contract.test.ts tests/unit/m3-acceptance-isolation.test.ts tests/unit/m3-acceptance-safety.test.ts tests/unit/m3-e2e-safety.test.ts tests/unit/m3-browser-lifecycle.test.ts tests/unit/m4b-runtime-guard.test.ts tests/unit/m4b-e2e-safety.test.ts tests/unit/m5-browser-lifecycle.test.ts tests/unit/m6-neon-target.test.ts tests/unit/m6-e2e-safety-contract.test.ts tests/unit/m7-acceptance-safety.test.ts tests/unit/m7-browser-acceptance-contract.test.ts tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts

    Expected: PASS. The actual spawned-process probes prove Playwright's inherited runner cannot carry ambient/live capability into the managed Next child: the outer wrapper and inner launcher each replace, rather than merge, their process environment; the Next child receives only exact guarded test mappings plus its fixed operational allowlist. The PostgreSQL guard, managed identity target-to-DB binding, no-reuse behavior, and every destructive/provider/database/target/identity guard fail closed at their specified pure or guarded-DB phase before unauthorized mutation.
    ...
    Expected without its exact isolated Neon/TLS host/project variables and approval: skip and `NOT PASSED`. With the complete guard: PASS with executable checkout `INNER JOIN`/two-row locking, observed concurrent blocking, event-time activation schedules, atomic membership/application/attempt/journey/audit/job commit, injected rollback, and zero run-owned residue. This result is recorded independently from M1 browser acceptance.
...
- Verification coverage: Task 12 includes exact credential-free and 60-case authenticated matrices, 26-page/19-API denial proof, unit/safety aggregates, and managed M1-M7/M4B/M5/Axe lifecycles whose outer Playwright and inner Next launchers each replace their process environment, with actual child probes proving only exact guarded test mappings and fixed operational keys survive. It also covers the separately gated real-PostgreSQL webhook lock/atomicity/rollback contract, real session revocation with no disk auth artifacts, and complete identity/data restoration; M1 bounded timeout/immutable evidence; M2 pre-reset baseline/test-secret/quiescence safety; M3 pre-seed identity plus complete seed ledger; M4B exact read-only fixture preflight; M6 GET-only provider proof plus conflict-key/all-Showcase restoration; M7 full catalog-derived Privacy namespace/database-clock/audit-object ownership; and an exact four-route authorized-Preview Lighthouse wrapper/config with no local server, a validated hash-bound Chrome executable, a platform allowlist verified through a real child probe, and a unique outside-worktree run root. Stage-before-commit ordering, post-commit immutable range proof, and publication remain bound to the exact zero-finding SHA, reviewed commit body, local/remote refs, captured PR identity, and automatic closure of a mismatched created draft.
