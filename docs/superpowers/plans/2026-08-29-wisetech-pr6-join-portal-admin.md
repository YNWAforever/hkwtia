...
- Create: `scripts/resolve-verified-runtime.ps1`, `scripts/invoke-verified-tool.ps1`, `scripts/managed-process-guardian.ps1`, `scripts/managed-runtime-environment.mjs`, `scripts/managed-runtime-environment.d.mts`, `scripts/run-credential-free-verification.mjs`, `scripts/run-managed-playwright.mjs`, `scripts/run-managed-next.mjs`, `tests/contracts/pr6-unsafe-bootstrap.red.ps1`, `tests/fixtures/credential-free-process-env-probe.mjs`, `tests/fixtures/managed-next-env-probe.mjs`, `tests/unit/verified-runtime-bootstrap.test.ts`, `tests/unit/credential-free-verification-boundary.test.ts`, `tests/unit/credential-free-build-boundary.test.ts`, `tests/unit/managed-next-production-boundary.test.ts`.
...
- Consume without executing application code: the locked `package-lock.json`, an operator-approved non-secret execution-attestation record, installed Node/npm/Git/GitHub CLI distributions, and the exact environment-name sources enumerated below.

**Interfaces:**

- The external execution record is populated by the operator or CI approval system before each shell and is never inferred from PATH, a package shim, the repository, or ambient configuration. Its exact JSON schema is `hkwtia.pr6.execution-attestation.v1`: canonical `repositoryRoot` and `gitDir`; attested repository bootstrap files for the invoker and guardian; Node `{path, allowedRoot, sha256, version}`; npm `{cliPath, packageRoot, allowedRoot, cliSha256, packageJsonSha256, version}`; Git `{path, allowedRoot, sha256, version, authorName, authorEmail}`; and optional publication-only GitHub CLI `{path, allowedRoot, sha256, version, host: "github.com", configDir}`. The approval supplies five independent non-secret bootstrap fields—record path/hash and resolver path/allowed-root/hash—so the repository resolver is checked before it can parse or trust the record. The separately recorded lowercase SHA-256 of the complete UTF-8 record is the integrity anchor. The record and its hashes contain no token, password, cookie, database URL, or provider value; a missing field, duplicate/case-colliding JSON key, alternate host, mutable relative path, or schema/version drift fails.
- Before invoking `resolve-verified-runtime.ps1`, the exact shell preamble below uses PowerShell/.NET only to canonicalize the separately approved resolver path without a reparse component and compare its bytes to the separately approved resolver hash. The now-attested resolver then receives `-ExecutionRecordPath` and `-ExecutionRecordSha256`, hashes and duplicate-key-validates that exact record, canonicalizes the repository/Git directory, verifies the recorded invoker/guardian bytes, resolves every tool/file component without reparse ambiguity, requires regular files strictly beneath approved roots, verifies filesystem identity/SHA-256/version, and binds the current worktree/Git directory to the record. npm is not inferred from Node or `package-lock.json`: its exact `npm-cli.js`, package root, `package.json`, hashes, and version are independently attested. GitHub CLI is optional for local work but mandatory before remote lookup or mutation. The resolver returns only the validated descriptor path/hash and canonical attested invoker path; it never enumerates or prints the environment.
- `invoke-verified-tool.ps1` is the only process-creation entrypoint. It revalidates the descriptor on every call, uses `.NET ProcessStartInfo` with `UseShellExecute = false`, an argument list rather than a command string, and a completely cleared child environment before the first Node/Git/GitHub-CLI instruction can execute. Therefore ambient `NODE_OPTIONS`, `NODE_PATH`, preload/import flags, platform dynamic-loader variables, `GIT_*`, `GH_*`, askpass, helpers, hooks, proxy variables, writable PATH entries, and shell shims cannot act before the JavaScript guard. It then adds only canonical OS/runtime directories, run-owned temp/config paths, and the fixed per-launch source-name allowlist. It passes the non-secret descriptor path/hash through two fixed internal keys; every JavaScript launcher hashes the descriptor again and proves `process.execPath`, repository root, mode, and exact entrypoint before reading any retained capability. PowerShell and JavaScript manifest parity, missing/tampered descriptor, same-path/different-case collisions, executable replacement, and argument injection are source- and subprocess-tested.
- At the start of every later shell, create closed script-block invokers named `$verifiedNode`, `$verifiedGit`, and `$verifiedGh` from the returned descriptor. Existing `& $verifiedNode ...` and `& $verifiedGit ...` examples therefore invoke the attested `invoke-verified-tool.ps1`; they are not executable paths. Node calls receive only exact launcher/mode-specific source variables. Local Git calls receive an empty replacement environment, explicit canonical `--git-dir`/`--work-tree`, disabled system/global includes, hooks, fsmonitor, askpass, signing, arbitrary helpers, optional locks, external diff/textconv, proxies, URL rewrites, and network. The adapter permits only the attested author name/email and a source-tested allowlist of inert repository core settings; before every literal `add`, it checks the exact paths' effective attributes and refuses executable clean/process filters, `ident`, or working-tree encoding. It accepts only command/argument forms used by this plan. Publication Git/GitHub-CLI calls require a second non-secret approval descriptor bound to `github.com`, exact repository/branch/base, and attested `gh`/Git identities. That branch uses a run-owned Git config with only the exact verified `gh auth git-credential` helper and approved canonical GitHub CLI config directory, fixed `GH_HOST=github.com`, no proxy/url rewrite, and no token in arguments or output. Raw Git, raw `gh`, PATH resolution, an alternate host/helper/hook/config, or a local command attempting network fails before execution.
...
- Every child map starts empty. The npm modes execute the independently attested `npm-cli.js` through the attested Node executable with `shell: false`, empty run-owned user/global configs, and the public registry. Before `ci`, require `node_modules` to be absent or a regular non-reparse directory whose complete ancestor chain and filesystem IDs are inside this exact worktree; inventory any existing tree for links/reparse points, then revalidate immediately before and after npm. Require every lockfile `resolved` URL host to equal `registry.npmjs.org`, execute exactly `ci --ignore-scripts --audit=false --fund=false`, never resolve an ancestor/shared dependency tree, then verify every repository-local CLI is a regular file under this worktree's `node_modules`. The separately recorded rebuild executes exactly `rebuild --foreground-scripts --audit=false --fund=false` only after every `hasInstallScript` package name/version/integrity matches the lockfile and the same containment checks pass. Audit is exact `audit --omit=dev --audit-level=high`. A path swap, junction, lifecycle/network failure, or version/hash drift is `NOT PASSED` and never authorizes ambient npm credentials.
...
- Build/start never use `next dev`. They receive `__NEXT_PROCESSED_ENV=true` only after inner and outer dotenv-absence fingerprints agree. `managed-process-guardian.ps1` owns the full process tree and a run lease: on Windows it places the outer Node process and all descendants in a `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` Job Object while an out-of-job guardian watches the invoker PID/lease; on POSIX it owns an equivalent isolated process group and parent-death lease. Normal exit, child failure, SIGINT/SIGTERM, forced termination, parent death, and lease expiry all kill the Next/Playwright/browser tree, wait for the port to close, revalidate the recorded directory identities, and remove only the exact run-owned build. An actual kill-the-outer-invoker subprocess test must prove no process, port, lease, or artifact survives.
...
- The production-boundary test pins Next 16.3.0's loader and dev-watcher sources, proves the plan never selects `next dev`, runs the real build/start path against an isolated miniature Next fixture and the repository artifact, creates every production dotenv candidate after readiness, and proves repeated requests see the original environment. External-process tests declare their own budgets rather than inheriting Vitest's default: 180 seconds for a real build, 30 seconds readiness, 20 seconds requests, 10 seconds graceful shutdown, 5 seconds forced shutdown, and 240 seconds aggregate; Playwright subprocess probes use 120 seconds with 180 seconds aggregate. Hooks and cleanup have separate named budgets. Unit tests inject readiness, child, cleanup, and outer-process timeouts and require bounded aggregate failure plus cleanup.

- [ ] **Step 1: Record an executable dependency-free RED without launching Node, a browser, or provider**

    Add `tests/contracts/pr6-unsafe-bootstrap.red.ps1`. Using PowerShell/.NET file reads only, it must fail on the current raw npm/npx/PATH routes, ambient Node preloads, ambient Git/GitHub configuration, missing executable attestation handoff, absent npm pin, unsafe `node_modules`/`.next` containment, `PLAYWRIGHT_BASE_URL`, `reuseExistingServer: !CI`, missing `forbidOnly`, inherited browser environment, development Next server, late dependency bootstrap, and absent parent-death cleanup. Run:
...
- [ ] **Step 3: Bind the shell and bootstrap worktree-local dependencies before any third-party test**
    ...
    $runtimeInvoker = $verifiedRuntime.InvokerPath
    ...
    $verifiedNode = { & $runtimeInvoker -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -Tool Node -Arguments @($args) }.GetNewClosure()
    $verifiedGit = { & $runtimeInvoker -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -Tool Git -PublicationApprovalPath $publicationApprovalPath -PublicationApprovalSha256 $publicationApprovalSha256 -Arguments @($args) }.GetNewClosure()
    $verifiedGh = { & $runtimeInvoker -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -Tool GitHubCli -PublicationApprovalPath $publicationApprovalPath -PublicationApprovalSha256 $publicationApprovalSha256 -Arguments @($args) }.GetNewClosure()
    ...
    Expected: the exact attested npm distribution installs this worktree-local lockfile without lifecycle scripts, the separately reviewed rebuild runs only the locked install-script set, and every required repository-local entrypoint resolves beneath this worktree. A missing dependency, ancestor fallback, reparse/junction, executable drift, or incomplete lifecycle run is `NOT PASSED` before Vitest.
...
- [ ] **Step 4: Run the boundary probes under the verified pre-entry launcher**
    ...
    Expected: PASS. The actual pre-entry/child key sets, descriptor tamper checks, npm identity, no-shell entrypoints, clean npm/Git configuration, fresh-port/origin ownership, real build-time absolute URLs, production build/start selection, browser isolation, explicit timeout budgets, abrupt-parent cleanup, and real post-readiness dotenv proof all pass. Fake PATH/`ComSpec`, `NODE_OPTIONS`/loader injection, direct CLI, raw config, stale localhost, path-reparse race, dotenv race, install-script leakage, unsupported mode/argument, and child failure all fail closed.

- [ ] **Step 5: Commit the execution prerequisite**

    & $verifiedGit add -- ':(literal)package.json' ':(literal)next.config.ts' ':(literal)playwright.config.ts' ':(literal)README.md' ':(literal)AGENTS.md' ':(literal)CLAUDE.md' ':(literal).github/pull_request_template.md' ':(literal)docs/integration/wisetech-delivery-gates.md' ':(literal)tests/unit/wisetech-delivery-gates.test.ts' ':(literal)scripts/resolve-verified-runtime.ps1' ':(literal)scripts/invoke-verified-tool.ps1' ':(literal)scripts/managed-process-guardian.ps1' ':(literal)scripts/managed-runtime-environment.mjs' ':(literal)scripts/managed-runtime-environment.d.mts' ':(literal)scripts/run-credential-free-verification.mjs' ':(literal)scripts/run-managed-playwright.mjs' ':(literal)scripts/run-managed-next.mjs' ':(literal)tests/contracts/pr6-unsafe-bootstrap.red.ps1' ':(literal)tests/fixtures/credential-free-process-env-probe.mjs' ':(literal)tests/fixtures/managed-next-env-probe.mjs' ':(literal)tests/unit/verified-runtime-bootstrap.test.ts' ':(literal)tests/unit/credential-free-verification-boundary.test.ts' ':(literal)tests/unit/credential-free-build-boundary.test.ts' ':(literal)tests/unit/managed-next-production-boundary.test.ts'
...
- Create: `tests/fixtures/isolated-runtime-env.ts`, `tests/unit/isolated-runtime-environment.test.ts`, `tests/fixtures/managed-auth-session.ts`, `tests/unit/managed-auth-session.test.ts`, `tests/fixtures/m4c-readonly-preview-safety.ts`, `tests/unit/m4c-readonly-preview-safety.test.ts`, `tests/fixtures/m1-live-acceptance.ts`, `tests/unit/m1-live-acceptance-safety.test.ts`, `tests/fixtures/webhook-postgres-safety.ts`, `tests/fixtures/webhook-postgres-test-db.ts`, `tests/unit/webhook-postgres-safety.test.ts`, `tests/integration/webhook-join-projection-postgres.test.ts`.
...
- Task 4 extends Task 0's shared test-only runtime for mutating M1, M2, M3, M4B, M5, M6, M7, final authenticated-Axe, guarded PostgreSQL, and read-only M4C suites. `scripts/managed-runtime-environment.d.mts` remains the one exact sibling declaration for the `.mjs` implementation and owns every exported constant, type, and function below; TypeScript consumers import that single runtime module with no duplicate TS implementation.
  ...
  `managed-runtime-process-boundary.test.ts` exercises the pre-entry invoker, outer runner, capability-free build, production Next start, Playwright child, browser child, guardian, and cleanup as actual subprocesses under explicit test/hook/readiness/shutdown budgets. Assert every complete case-folded map, every exact mode-to-file manifest, original-parent versus runner versus Next ownership, fresh-port behavior, direct-bypass/path-substitution denial, executable/hash/path checks, exit propagation, and absent artifact/auth output. Kill the outer invoker while the managed server/browser are live and prove the Job Object or process group closes the complete tree and port while the guardian removes the exact lease/build without touching a sibling. `run-managed-vitest.mjs webhook-postgres` accepts no forwarded path, resolves only `tests/integration/webhook-join-projection-postgres.test.ts`, uses the same attested Node and replacement runner rules, retains only the webhook sentinel/isolated database sources, and invokes the exact local Vitest CLI with `shell: false`; raw Vitest/npm, an alternate file/config/option, and ambient standard `DATABASE_URL` fail before the guard import.

  `m4c-readonly-preview` is a distinct sanitized branch owned here rather than an assumed legacy guard. It requires exact `M4C_READONLY_PREVIEW_APPROVED=M4C_APPROVED_READONLY_PREVIEW_ONLY`, nonempty non-secret `M4C_READONLY_APPROVAL_REFERENCE`, exact `M4C_TEST_VERCEL_DEPLOYMENT_ID`, canonical non-production HTTPS `M4C_TEST_ALLOWED_ORIGIN`, and `PLAYWRIGHT_BASE_URL` exactly equal to that independently approved origin before optional `VERCEL_SHARE_TOKEN` is retained. It starts no local server and passes no database/provider/Auth/mutation source to the runner or browser. `m4c-readonly-preview-safety.ts` creates the only context, sets `serviceWorkers: "block"`, disables trace/screenshot/video/downloads, confines every navigation/subresource/redirect to the exact origin, permits only GET/HEAD/OPTIONS, aborts and records every write or cross-origin request, forbids Playwright `request`/direct fetch seams in the spec, and fails after the case if any denial occurred. The share token is sent only to the exact approved host and is never forwarded on redirect. The spec still requires the private canary absent. Unit/source/subprocess tests cover wrong/missing sentinel, approval/origin/deployment mismatch, production/loopback/Unicode origin, token exfiltration redirect, POST/PUT/PATCH/DELETE, service worker, extra credential, alternate spec, and direct-context bypass. Any such case fails before or during a read-only request and can never be counted as M4C acceptance.
...
- [ ] **Step 3: Implement and verify the managed launch boundary before any capability-bearing RED**
    ...
    Expected: PASS for pre-entry sanitization, exact mode-to-file denial, build-time origin binding, real child maps, explicit budgets, abrupt-parent process-tree/artifact cleanup, and M4C read-only enforcement. No application/provider/database mutation occurs.
...
- [ ] **Step 5: Prove safety contracts, then run isolated M1-M7 only with separate authority**
    ...
    - M4C: exact `M4C_READONLY_PREVIEW_APPROVED=M4C_APPROVED_READONLY_PREVIEW_ONLY`, nonempty `M4C_READONLY_APPROVAL_REFERENCE`, exact `M4C_TEST_VERCEL_DEPLOYMENT_ID`, canonical non-production HTTPS `M4C_TEST_ALLOWED_ORIGIN`, `PLAYWRIGHT_BASE_URL` exact-equal to that origin, and optional `VERCEL_SHARE_TOKEN` only for that host. The helper blocks service workers, cross-origin/redirect exfiltration, and every method except GET/HEAD/OPTIONS, and the suite must continue to assert the private canary is absent.
...
- [ ] **Step 6: Run the exact authorized-Preview Lighthouse and field-performance gates**
    ...
    if ($lighthouseCheckpointSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_LIGHTHOUSE_CHECKPOINT_HASH_INVALID" }

    Expected: unit contracts PASS and `prepare-index` stages exactly the fixed Lighthouse execution set, executes only staged-blob bytes in its materialized tree, prints one absolute non-secret checkpoint path/hash object, and performs no network/Chrome/provider action. Preserve both fields in the verification record even when no Preview collection is authorized so final commit binding is unconditional.
    ...
    & $verifiedNode scripts/run-lighthouse-preview.mjs collect --checkpoint "$lighthouseCheckpoint" --checkpoint-sha256 "$lighthouseCheckpointSha256"
...
- [ ] **Step 8: Stage the remaining Task 12 paths, commit the complete index, then prove the immutable final range**
    ...
    & $verifiedNode scripts/run-lighthouse-preview.mjs verify-commit --checkpoint "$lighthouseCheckpoint" --checkpoint-sha256 "$lighthouseCheckpointSha256" --head "$finalHead"
    ...
    if ($lighthouseGateStatus -ceq "PASSED") {
      ...
      & $verifiedNode scripts/run-lighthouse-preview.mjs verify-commit --checkpoint "$lighthouseCheckpoint" --checkpoint-sha256 "$lighthouseCheckpointSha256" --run-root "$lighthouseRunRoot" --head "$finalHead"
...
- [ ] **Step 9: Complete independent review and publish the stacked draft PR**
    ...
    The publication descriptor must bind `github.com`, `YNWAforever/wisetech`, the expected head/base branches and base OID, the exact attested Git/GitHub CLI/config-directory identities, and the approval reference. It contains no credential. The invoker clears all ambient Git/GH/proxy/config/helper/hook state and allows the verified GitHub CLI credential helper only for exact GitHub HTTPS lookup/push. Obtain `$reviewedHead` from the verdict—not from current `HEAD`—and define all immutable names:
...
- Verification coverage: Task 0 supplies a pre-entry environment-clear launcher, explicit Node/npm/Git/GitHub-CLI/bootstrap-file attestations, worktree-contained dependency/build ownership, bounded real-process tests, parent-death cleanup, and isolated local/publication Git modes. Task 12 includes exact credential-free and 60-case authenticated matrices, 26-page/19-API denial proof, replacement-environment unit/browser aggregates, exact mode-to-file ownership, managed M1-M7/M4B/M5/Axe lifecycles, and a separately approved M4C origin/deployment sentinel with browser-wide read-only enforcement. The separately gated real-PostgreSQL webhook contract binds fixtures and production SQL to one mock-before-import guarded Drizzle handle, proves two independent row-lock contenders/failpoints/rollback, and closes every connection. It covers real session revocation with no disk auth artifacts and complete identity/data restoration. The four-route authorized-Preview Lighthouse gate executes materialized staged-blob bytes, records raw diagnostics separately, scopes effective Git-transform rejection to its exact paths, uses a hash-bound Chrome executable and isolated run root, and always rebinds the credential-free checkpoint to final commit blobs even without collection authority. Publication remains bound to the exact zero-finding SHA through attested, replacement-environment Git/GitHub CLI invokers, reviewed commit body, local/remote refs, captured PR identity, and automatic closure of a mismatched created draft.
