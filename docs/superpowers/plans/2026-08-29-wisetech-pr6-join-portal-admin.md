...
- [ ] **Step 3: Implement the final Join types and service ordering**
    ...
    Add `billingIntervalSchema = z.enum(BILLING_INTERVALS)`. Validate scalar query values without making plan/interval authoritative over a durable terminal row: parse the optional scalar application ID independently, and defer plan/interval validation until the flow is known to be new or nonterminal. Implement `prepareJoinSubmission` with exact defaults `{applications: applicationsRepository, memberships: membershipsRepository, resolveOption: resolveMembershipOption}`. When an application ID is present, require a member and load the actor-scoped application plus membership first. A valid terminal membership must match `application.planCode` and determines both terminal plan and interval even when query plan/interval is absent, invalid, multi-valued, or conflicting. A terminal application without that membership throws `MEMBERSHIP_NOT_FOUND`. Only a new/nonterminal draft parses scalar plan/interval and calls `resolveOption` exactly once; missing, monthly, unknown, multi-valued, or unavailable input fails before mutation. The prepared option is server-only and is never accepted from form data or a Client Component.
    ...
    3. If a valid terminal membership exists, require `application.planCode === membership.planCode` and derive the terminal outcome from the durable membership plan, billing interval, status, and ID; ignore missing, invalid, multi-valued, or conflicting query plan/interval.
    4. If the application claims a terminal status but its actor-scoped membership is absent, throw `MEMBERSHIP_NOT_FOUND`.
    5. Only for a new/nonterminal draft, parse scalar plan/interval, verify any application plan against that selection, and call `resolveOption` exactly once to produce `PreparedJoinSubmission.kind === "draft"`; no mutation has occurred.
...
- Create: `tests/fixtures/isolated-runtime-env.ts`, `tests/unit/isolated-runtime-environment.test.ts`, `tests/fixtures/m1-live-acceptance.ts`, `tests/unit/m1-live-acceptance-safety.test.ts`.
...
- Task 4 owns the shared test-only managed-runtime boundary used by mutating M1, M2, M6, M7, and final authenticated-Axe suites:
  ...
  Any nonempty value for any sentinel name activates the no-reuse safety posture even when the value is wrong. Reject more than one active mutating suite, reject any present `PLAYWRIGHT_BASE_URL`, validate one canonical decimal `PLAYWRIGHT_PORT` (default `3000`), and derive exactly `http://localhost:<port>`. That origin is the sole `use.baseURL`, `webServer.url`, child `APP_URL`, and suite allowed origin. For a mutating run, `webServer.env` spreads defined ambient values first and then unconditionally overrides `DATABASE_URL <- DATABASE_URL_TEST`, `STRIPE_SECRET_KEY <- STRIPE_TEST_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET <- STRIPE_TEST_WEBHOOK_SECRET`, `STRIPE_STARTUP_PRICE_ID <- STRIPE_TEST_STARTUP_PRICE_ID`, `STRIPE_CORPORATE_PRICE_ID <- STRIPE_TEST_CORPORATE_PRICE_ID`, and `APP_URL <- managed origin`. Missing suite-required test values fail configuration/guard before a Pool, browser, auth, provider, or mutation client is constructed; ambient standard names can never win. `reuseExistingServer` is `false` for any nonempty destructive sentinel so an occupied port fails instead of attaching to a stale server. Credential-free runs retain their existing behavior, and M3 remains an independently guarded external Preview flow.
...
- [ ] **Step 3: Implement durable billing and status projection**
    ...
    Implement the shared managed runtime first and make `tests/fixtures/m2-runtime-env.ts` delegate/re-export its neutral builder without weakening existing M2 names. Implement `m1-live-acceptance.ts` as a test-only fail-closed harness. It requires all Task 12 M1 variables, managed loopback with absent `PLAYWRIGHT_BASE_URL`, exact `APP_URL`/`M1_E2E_ALLOWED_ORIGIN` equality, independently allowlisted non-production Neon/TLS identity, allowlisted HTTPS inbox and auth-link origins, three distinct test emails, and `sk_test_` credentials. Pure checks run before Pool; then one guarded read-only DB preflight proves the two exact member profile mappings and clean owner boundary before any other client or mutation. The durable Startup option must have `seatLimit >= 2`. Replace the unconditional always-skip case with one isolated describe whose skip condition is exactly `missingM1LiveEnvironment(process.env).length > 0`; a skip remains `NOT PASSED`.
...
- [ ] **Step 1: Write failing grouped-navigation, source-owner, and CRM rendering tests**
    ...
    Extend the M2 safety/runtime tests to require `M2_ACCEPTANCE_ALLOW_DESTRUCTIVE=M2_ISOLATED_FIXTURES_ONLY`, `M2_E2E_ALLOWED_ORIGIN`, `APP_URL`, and `NEON_PROJECT_ID` in the centralized contract while delegating standard runtime mapping to Task 4's shared builder. Assert any present `PLAYWRIGHT_BASE_URL`, missing/wrong sentinel, managed-origin/APP/allowlist mismatch, missing project ID/test variables, production/non-Neon/mismatched/TLS-invalid DB, raw test DB mismatch after mapping, multiple mutating sentinels, or reusable/occupied server fails before `connect` or `seedM2`. Assert the browser suite uses the exact managed base URL and guarded reset in both `beforeAll` and `afterAll`; cleanup failure fails.

- [ ] **Step 2: Run the focused tests and record the RED reason**

    Run:

    npm.cmd test -- tests/unit/isolated-runtime-environment.test.ts tests/unit/wisetech-pr6-admin-crm-shell.test.tsx tests/unit/admin-presentational.test.tsx tests/unit/admin-dashboard-tiles.test.tsx tests/unit/admin-member-list.test.ts tests/unit/admin-member-page-boundary.test.ts tests/unit/admin-member-profile.test.ts tests/unit/member-note-server-action-boundary.test.ts tests/unit/segment-query.test.ts tests/unit/segment-save-action.test.ts tests/unit/campaign-server-action-auth.test.ts tests/unit/at-risk-repository-boundary.test.ts tests/unit/admin-page-auth.test.ts tests/unit/m2-auth-reset.test.ts tests/unit/m2-runtime-environment.test.ts tests/unit/m2-browser-acceptance-contract.test.ts

    Expected: FAIL because Admin navigation is flat, Dashboard is brand-only, CRM pages lack shared primitives, the M2 reset/browser path is not yet bound to the shared managed loopback/test-resource runtime, and the browser suite lacks the complete 26-page and 19-API authority matrices.
...
- [ ] **Step 3: Adopt Admin shell and CRM primitives without moving authority**
    ...
    Add `M2_ACCEPTANCE_DESTRUCTIVE_SENTINEL = "M2_ISOLATED_FIXTURES_ONLY"` and the required M2 names. Keep `tests/fixtures/m2-runtime-env.ts` as an M2-specific missing-variable wrapper that delegates/re-exports Task 4's neutral builder. Require `PLAYWRIGHT_BASE_URL` absent, the canonical managed origin equal to `APP_URL` and `M2_E2E_ALLOWED_ORIGIN`, exact child `DATABASE_URL === DATABASE_URL_TEST`, standard Stripe names mapped only from `STRIPE_TEST_*`, canonical non-production Neon/TLS/project identity, and `hkwtia.vercel.app` forbidden. Any nonempty destructive sentinel forces no server reuse; missing/wrong authorization skips as `NOT PASSED` before mutation, and a separately approved valid run performs the same named reset before and after.
...
- [ ] **Step 4: Run GREEN and M2 CRM regressions**
    ...
    Expected: PASS with grouped active navigation, locale retention, unchanged CRM/security assertions, exact shared runtime mappings, and every unsafe M2 reset/target case blocked before connection or seed.
...
- `authenticated-identity-safety.ts` is a test-only full-profile snapshot/quiescence/CAS helper for M3-M7 and final authenticated verification. After pure target/DB guards and before login, it snapshots every column for the exact configured profile. After all browser contexts close, it drains fire-and-forget `touchLastLogin` writes over a bounded quiet window, compare-and-swap restores the full baseline, retries only an allowed in-window late login touch, preserves external drift, and requires a second byte-identical quiet window. It aggregates identities independently and never imports into production.
- [ ] **Step 1: Write failing CMS family source and rendering contracts**
    ...
    Add M7 safety RED cases for `M7_ACCEPTANCE_ALLOW_DESTRUCTIVE=M7_ISOLATED_FIXTURES_ONLY`, exact shared child `DATABASE_URL === DATABASE_URL_TEST`, canonical TLS and matching non-production project/host, absent `PLAYWRIGHT_BASE_URL`, canonical managed origin equal to `APP_URL`/`M7_E2E_ALLOWED_ORIGIN`, one active mutating sentinel, forced no-reuse, and configured Admin credentials. Pure target/runtime failures occur before Pool/auth/browser; guarded DB identity checks occur after only the Pool exists and before auth/browser or mutation.
    Snapshot the configured M7 Admin profile through the generic helper after guarded DB identity validation and before authentication. Unit/source RED requires context closure, pre-restore drain, full-row CAS, post-restore quiet verification, delayed-touch coverage, external-drift preservation, and cleanup aggregation independent of Page Copy/News/Media phases.

    Define `m7-cms.spec.ts` as the isolated bilingual acceptance journey on Task 4's managed server. A guarded fixture inserts only run-ID-owned disposable News, Media registry, and reference rows and snapshots pre-run audit IDs for their exact target IDs. Page Copy has fixed `(locale, namespace, key_path)` identity and no run-ID column: snapshot both complete rows or explicit absence, preexisting Page Copy audit IDs, and both public rendered baselines, failing before writes on any read error. Re-read exact snapshots immediately before submit. Through the real UI, staff edits/publishes bilingual News, writes exact run Page Copy values, and exercises Media visibility/reference-lock/archive. Around every request, record start/end timestamps and reconstruct audit IDs by set difference. Do not claim the DB-only Media URL resolves and do not call storage/provider APIs.

    Track Page Copy mutation/audit state separately. In its independent cleanup phase, recompute audit-ID differences even when flags/captured IDs are missing and classify each locale tuple as exact baseline, exact owned run value, or external drift. A Page Copy audit is owned only when its ID is new, `createdAt` lies inside the exact mutation/restoration UI request window, actor user/type equal the configured Admin profile, action is `page_copy.updated`, target type is `page_copy`, target ID is `Privacy`, and metadata is exactly normalized `{namespace: "Privacy", updated: [expected tuple identities], cleared: [expected tuple identities]}` for that request. There is no tuple target or run-ID metadata. Skip baseline; restore exact run values; preserve/report drift. A fresh authenticated full-namespace UI submit supplies the exact baseline value for every prior-present tuple and the approved blank representation for every prior-absent tuple, including an unchanged locale. Only successful real-action restoration followed by both public locales rendering their exact baselines can pass; then restore original metadata/timestamps. If UI restoration or public verification fails while a tuple still equals its owned run value, guarded direct DB restore may contain persistent data leakage for that exact tuple, but it does not invalidate cache or satisfy public-baseline acceptance: report failure, terminate the isolated managed target, and make no restoration claim. Delete only mutation/restoration audit IDs satisfying the full predicate; preserve/report unexpected or extra candidates.

    In another unconditional independent `finally`, reconstruct and validate News/Media audit ownership before deleting target rows. Owned News audits are new IDs inside their exact UI request window with configured actor, target type `post`, target ID equal to the exact run News ID, action `post.updated` with metadata exactly the sorted submitted field-name set, or action `post.published` with metadata exactly `{slug: runSlug}`. The owned Media audit is a new ID inside its archive request window with configured actor, action `media.archived`, target type `media`, target ID equal to the exact run Media ID, and metadata exactly `{url: runUrl}`. Unexpected, extra, out-of-window, or mismatched audit rows are preserved and fail cleanup. Independently delete exact owned audit IDs, reference, News, and Media rows and verify zero; no phase suppresses another. Unit/source contracts cover partial UI mutation/audit capture, non-cascading audit residue, idempotent reruns, drift preservation, and aggregate errors.

    In `m7-browser-acceptance-contract.test.ts`, require the shared managed guard before Pool/auth, bilingual route markers, run-ID ownership only where fields support it, both lifecycle hooks, nested aggregate cleanup, present/absent Page Copy snapshots, per-locale baseline/run/drift classification, pre-write/pre-restore drift rejection, real-UI restoration, public baseline verification, exact request windows and audit-ID set differences, the real Page Copy `Privacy` target/`{namespace, updated, cleared}` metadata, full metadata restoration, and independently unconditional News/Media audit plus row cleanup. Require direct DB containment to fail acceptance and terminate the isolated target rather than claim revalidation. Require active Media visibility, reference-lock, archive visibility/removal, and no storage imports. In `m7-acceptance-safety.test.ts`, unit-test prior-row/prior-absence including an unchanged absent locale's blank representation, one-locale mutation, success before flag/capture, unexpected/extra/out-of-window audit rejection, exact News `post.updated`/`post.published` and Media `media.archived` predicates, non-cascading audit deletion, idempotency, early/UI restoration/public-verification failure, containment-without-success, external drift, independent phases, aggregate errors, and fail-before-mutation drift. Keep `media-upload-delivery-routes.test.ts` unchanged as provider-double delivery evidence.
...
- [ ] **Step 3: Apply CMS presentation primitives**
    ...
    Implement the generic authenticated-identity helper and then the M7 test-only safety module, run-ID fixture, Page Copy snapshot/classifier, request-window audit ledgers, guarded re-reads, full-namespace UI restore, exact metadata/timestamp containment, and independent audit/row cleanup described in Step 1. The M7 fixture connects only after shared/pure checks, snapshots the configured Admin before login, exposes `prepareM7Fixture`, `snapshotM7PageCopy`, `assertM7PageCopyUnchanged`, `restoreM7PageCopyMetadata`, and `cleanupM7Fixture`, and is forbidden from production imports. After contexts close, its identity restore is an independent aggregate phase. Never infer ownership from a fixed tuple alone, never expect run-ID Page Copy metadata, remove no non-owned record, preserve all unexpected audit candidates, and make cleanup idempotent. Direct DB containment after real-action failure always fails and disposes the managed target.

- [ ] **Step 4: Run GREEN and the complete CMS invariant subset**

    Run the Step 2 command again.

    Expected: PASS with all twelve pages aligned, CMS invariants unchanged, every unsafe M7 runtime rejected at the correct phase, configured Admin profile restored after quiet-window/CAS verification, present/absent Page Copy real-action restore and public revalidation plus exact production audit ownership proven, non-cascading News/Media audit residue independently cleaned, source contract verifying the guarded E2E journey, and provider-double Media delivery only. A direct DB containment fallback can protect data but cannot pass. The mutating browser suite still requires separate isolated-mutation approval; absent approval is `NOT PASSED`.
...
- Create M3 browser lifecycle: `tests/fixtures/m3-browser-lifecycle.ts`, `tests/unit/m3-e2e-safety.test.ts`, `tests/unit/m3-browser-lifecycle.test.ts`.
- Modify M3 browser suite: `tests/e2e/m3-automations.spec.ts`. Consume unchanged retry invariant: `tests/unit/automation-admin.test.ts`.
- Create M6 browser lifecycle: `tests/fixtures/m6-browser-lifecycle.ts`.
...
- [ ] **Step 1: Write failing Operations family source and rendering contracts**
    ...
    Add M3 source/unit RED for a complete browser lifecycle, not only page reads. Under existing destructive/database/Preview guards, open `DATABASE_URL_TEST` only after pure checks, call `seedM3` at one fixed `M3_SEED_NOW` before browser work, and require exact seeded run markers to render in the real Preview before any mutation; this observationally binds the target to the guarded database. Snapshot full configured staff/member profiles before login. Create one unique run-ID failed `journey_state` with matching failed `email_log`, retryable error, and delivery key plus one non-failed/sent ineligible control. Snapshot unsubscribe target profiles, suppression rows, seed-owned rows, and audit IDs. The eligible row exposes one retry; the sent/ineligible row does not.

    Add M6 source/unit RED for one deterministic managed-loopback lifecycle. Require `PLAYWRIGHT_BASE_URL` absent, exact `M6_ACCEPTANCE_SEED=true`, fixed canonical `M6_ACCEPTANCE_AS_OF`, shared child `DATABASE_URL === DATABASE_URL_TEST`, no reuse, and database allowlist before Pool/browser. Under the seed advisory lock, snapshot every full row at all fixed cohort/partner/company/application IDs, the graduation target Showcase row, the exact member-company application boundary, pre-run audit IDs, and full configured member/staff profiles; fail before seed on read ambiguity. Run `seedM6(pool, {asOf})` before browser work and require the managed UI to render exact seeded identities before mutation.

- [ ] **Step 2: Run the focused tests and record the RED reason**

    Run:

    npm.cmd test -- tests/unit/authenticated-identity-safety.test.ts tests/unit/wisetech-pr6-admin-operations-shell.test.tsx tests/unit/admin-events.test.ts tests/unit/event-check-in.test.ts tests/unit/m5-admin-review.test.tsx tests/unit/admin-cohort-management.test.ts tests/unit/m6-admin-cohorts.test.tsx tests/unit/approval-authorization.test.ts tests/unit/approval-server-action-auth.test.ts tests/unit/approval-list.test.tsx tests/unit/report-reconciliation.test.ts tests/unit/board-reporter-render.test.ts tests/unit/automation-dashboard-review.test.tsx tests/unit/automation-retry.test.ts tests/unit/automation-admin.test.ts tests/unit/admin-server-action-boundaries.test.ts tests/unit/m3-e2e-safety.test.ts tests/unit/m3-browser-lifecycle.test.ts tests/unit/m6-seed.test.ts tests/unit/m6-e2e-safety-contract.test.ts

    Expected: FAIL on new shell/presentation assertions and absent M3/M6 browser lifecycle contracts. Existing authorization, retry/audit transaction, seed, cohort-transition, and Showcase controls remain diagnostic baselines.
...
- [ ] **Step 3: Apply Operations presentation primitives**
    ...
    Implement `m3-browser-lifecycle.ts` without production imports. It uses `requireM3AcceptanceDatabase`/`requireM3E2ETarget`, `seedM3`, and Task 10's identity helper. After the exact run-ID row renders, staff clicks retry once through the real Server Action. Verify journey `scheduled`/null error, matching email's exact Admin-authorized code, and exactly one new audit inside the request window with configured actor, action `journey.failed_retry_requested`, target `journey_state`/run ID, and exact `{scheduledAt, deliveryKey}`. Reload/replay and the sent/ineligible control must deny another transition/audit; `automation-admin.test.ts` directly submits non-failed IDs and proves no write. Existing English/Chinese console, Member 360, denial, and unsubscribe cases stay. In aggregate `afterAll`, close contexts; delete exact run retry audit/email/journey controls; rerun the fixed `seedM3` reset for all seed-owned suppression/unsubscribe rows; verify the entire serialized fixture; and quiet-window/CAS restore configured identities. Preserve drift/unexpected audits, run every phase independently, and forbid provider, job-runner, delivery retry, or mail send.

    Implement `m6-browser-lifecycle.ts` and make `m6-launch-pad.spec.ts` call it in `beforeAll`/`afterAll`. The member application is owned only by exact cohort, controlled member company, request window, and pre-run absence/baseline. Each stage audit is owned only by new ID, configured staff actor/type, action `cohort_application.stage_changed`, target type/id, request window, and exact `{fromStage, toStage}` chain. Snapshot and compare-and-swap restore the fixed application including notes/timestamps, the graduation Showcase row including `goneGlobal`/`updatedAt`, and every seed-upserted fixed row; delete only a truly absent-before member application and its exact residue. Close contexts, drain/restore both profiles with the shared helper, then verify byte-identical pre-run database and identity baselines. Partial seed, application, transition, audit, Showcase, or one-identity failure cannot suppress another cleanup phase. Unexpected drift is preserved and fails. No fixed row, audit, member application, Showcase bit, or login touch may remain changed.
...
- Create: `tests/e2e/wisetech-pr6-internal-journeys.spec.ts`, `tests/fixtures/external-authenticated-identity-lifecycle.ts`, `tests/unit/external-authenticated-identity-lifecycle.test.ts`, `tests/fixtures/pr6-authenticated-accessibility-lifecycle.ts`, `tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts`, `tests/e2e/wisetech-pr6-authenticated-accessibility.spec.ts`, `docs/integration/wisetech-pr6-verification.md`, `docs/integration/wisetech-pr6-pr-body.md`.
...
- Modify identity-safe external browser gates: `tests/fixtures/m4b-e2e-safety.ts`, `tests/unit/m4b-e2e-safety.test.ts`, `tests/e2e/m4b-agents.spec.ts`, `tests/e2e/m5-showcase.spec.ts`.
- Consume without weakening: Task 1 `tests/e2e/public-shell.spec.ts`; Task 2 `tests/e2e/join-auth.spec.ts`; Task 4 shared managed runtime plus M1; Task 9 M2 reset/browser; Task 10 authenticated-identity helper plus M7; Task 11 complete M3/M6 browser lifecycles; and existing M4C guard/browser suite.
- Modify only if required by measured indexable public-route coverage: `lighthouserc.js`. Never add noindex `/member-login` or authenticated routes to the global SEO-threshold collection.
...
- [ ] **Step 1: Define final credential-free and authenticated browser verification matrices**
    ...
    This suite has its own managed lifecycle: require `PR6_AUTHENTICATED_AXE_ALLOW_DESTRUCTIVE=PR6_ISOLATED_IDENTITIES_ONLY`, absent `PLAYWRIGHT_BASE_URL`, exact managed `APP_URL`/`PR6_AXE_E2E_ALLOWED_ORIGIN`, guarded `DATABASE_URL_TEST` mapping, non-production DB identity, Neon Auth credentials, and all named M2/M3/M7 login identities. After pure checks, snapshot every distinct full profile before login. Each real sign-in must return the exact expected profile/auth identity and advance that same guarded DB profile's `lastLoginAt` within the request window, observationally binding target and database before continuing. Close every context and quiet-window/CAS restore all identities in an aggregate `afterAll`; a skip, binding failure, drift, or incomplete restoration is `NOT PASSED`.
...
- [ ] **Step 2: Run the focused PR6 cross-surface aggregate**
    ...
    npm.cmd test -- tests/unit/membership-catalog.test.ts tests/unit/membership-public-catalog.test.ts tests/unit/membership-links.test.tsx tests/unit/join-schema.test.ts tests/unit/join-navigation.test.ts tests/unit/join-service.test.ts tests/unit/join-actions.test.ts tests/unit/member-login-page.test.tsx tests/unit/portal-sign-out-button.test.tsx tests/unit/isolated-runtime-environment.test.ts tests/unit/checkout-service.test.ts tests/unit/checkout-recovery-service.test.ts tests/unit/billing-recovery-cas.test.ts tests/unit/join-billing-pages.test.tsx tests/unit/m1-live-acceptance-safety.test.ts tests/unit/seat-invitation-routes.test.tsx tests/unit/seat-service.test.ts tests/unit/internal-navigation.test.tsx tests/unit/internal-shell.test.tsx tests/unit/wisetech-pr6-route-inventory.test.ts tests/unit/wisetech-pr6-join-shell.test.tsx tests/unit/locale-switcher.test.tsx tests/unit/wisetech-pr6-portal-shell.test.tsx tests/unit/portal-company-context.test.ts tests/unit/wisetech-pr6-admin-crm-shell.test.tsx tests/unit/m2-auth-reset.test.ts tests/unit/m2-runtime-environment.test.ts tests/unit/m2-browser-acceptance-contract.test.ts tests/unit/m3-e2e-safety.test.ts tests/unit/m3-browser-lifecycle.test.ts tests/unit/authenticated-identity-safety.test.ts tests/unit/wisetech-pr6-admin-cms-shell.test.tsx tests/unit/m7-acceptance-safety.test.ts tests/unit/m7-browser-acceptance-contract.test.ts tests/unit/wisetech-pr6-admin-operations-shell.test.tsx tests/unit/m6-e2e-safety-contract.test.ts tests/unit/external-authenticated-identity-lifecycle.test.ts tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts
...
- [ ] **Step 5: Prove safety contracts, then run isolated M1-M7 only with separate authority**
    ...
    npm.cmd test -- tests/unit/isolated-runtime-environment.test.ts tests/unit/authenticated-identity-safety.test.ts tests/unit/m1-live-acceptance-safety.test.ts tests/unit/m2-auth-reset.test.ts tests/unit/m2-runtime-environment.test.ts tests/unit/m2-browser-acceptance-contract.test.ts tests/unit/m3-acceptance-isolation.test.ts tests/unit/m3-acceptance-safety.test.ts tests/unit/m3-e2e-safety.test.ts tests/unit/m3-browser-lifecycle.test.ts tests/unit/m4b-runtime-guard.test.ts tests/unit/m4b-e2e-safety.test.ts tests/unit/m6-e2e-safety-contract.test.ts tests/unit/m7-acceptance-safety.test.ts tests/unit/m7-browser-acceptance-contract.test.ts tests/unit/external-authenticated-identity-lifecycle.test.ts tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts

    Expected: PASS with shared managed-loopback/resource mapping, external identity target-to-DB binding, no-reuse behavior, and every destructive/provider/database/target/identity guard failing closed at its specified pure or guarded-DB phase before unauthorized mutation.
    ...
    - M1: `M1_ACCEPTANCE_ALLOW_DESTRUCTIVE=M1_ISOLATED_FIXTURES_ONLY`, `M1_ACCEPTANCE_ALLOW_PROVIDER_CALLS=M1_TEST_PROVIDERS_ONLY`, `PLAYWRIGHT_BASE_URL` absent, optional `PLAYWRIGHT_PORT`, `APP_URL`/`M1_E2E_ALLOWED_ORIGIN` exact managed origin, `DATABASE_URL_TEST`, matching non-production Neon project/host, Neon Auth secrets, all four `STRIPE_TEST_*` values, three exact emails, inbox URL/origin/token, and `M1_TEST_MAGIC_LINK_ALLOWED_ORIGIN`. Standard child DB/Stripe/APP names come only from guarded mappings.
    - M2: `M2_ACCEPTANCE_ALLOW_DESTRUCTIVE=M2_ISOLATED_FIXTURES_ONLY`, `PLAYWRIGHT_BASE_URL` absent, optional `PLAYWRIGHT_PORT`, `APP_URL`/`M2_E2E_ALLOWED_ORIGIN` exact managed origin, `DATABASE_URL_TEST`, matching non-production Neon project/host, Neon Auth secrets, all four `STRIPE_TEST_*` values, and exact staff/member/company-admin credentials.
    - M3: `M3_ACCEPTANCE_ALLOW_DESTRUCTIVE=isolated-preview`, `M3_ACCEPTANCE_SEED=true`, fixed canonical `M3_SEED_NOW`, `DATABASE_URL_TEST`, independently different runner `DATABASE_URL` where required, `M3_ACCEPTANCE_EXPECTED_DB_HOST`, exact non-production `PLAYWRIGHT_BASE_URL`/`M3_E2E_ALLOWED_ORIGIN`, staff/member credentials, both unsubscribe tokens, and `VERCEL_SHARE_TOKEN` when needed. The lifecycle seeds/resets, visibly binds target to fixture, and restores identities.
    - M4B: `M4B_IDENTITY_RESTORE_ALLOW_DESTRUCTIVE=M4B_TEST_IDENTITIES_ONLY`, `DATABASE_URL_TEST` with exact `M4B_TEST_DB_HOST`, non-production `PLAYWRIGHT_BASE_URL`/`M4B_E2E_ALLOWED_ORIGIN`, Preview guard, staff/member credentials, and share token when required. The suite snapshots both profiles before login, proves the target advances those exact DB rows, then quiet-window/CAS restores them.
    ...
    - M5: `M5_IDENTITY_RESTORE_ALLOW_DESTRUCTIVE=M5_TEST_IDENTITIES_ONLY`, `DATABASE_URL_TEST` with exact `M5_TEST_DB_HOST`, non-production `PLAYWRIGHT_BASE_URL`, exact email/password, and share token when required. The read-only product assertions restore the one login-touched profile after target-to-DB binding.
    - M6: `PLAYWRIGHT_BASE_URL` absent, optional `PLAYWRIGHT_PORT`, `M6_ACCEPTANCE_SEED=true`, fixed canonical `M6_ACCEPTANCE_AS_OF`, `DATABASE_URL_TEST` mapped to child `DATABASE_URL`, DB host allowlist, Neon Auth secrets, and exact member/staff/company display-name credentials. Any nonempty seed value disables reuse; lifecycle restores seed rows, member application, audits, Showcase state, and identities.
    - M7: `M7_ACCEPTANCE_ALLOW_DESTRUCTIVE=M7_ISOLATED_FIXTURES_ONLY`, `PLAYWRIGHT_BASE_URL` absent, optional `PLAYWRIGHT_PORT`, `APP_URL`/`M7_E2E_ALLOWED_ORIGIN` exact managed origin, `DATABASE_URL_TEST`, matching non-production Neon project/host, Neon Auth secrets, and exact Admin credentials. It restores the Admin profile as well as CMS data/audits.
    - Authenticated Axe: `PR6_AUTHENTICATED_AXE_ALLOW_DESTRUCTIVE=PR6_ISOLATED_IDENTITIES_ONLY`, `PLAYWRIGHT_BASE_URL` absent, optional `PLAYWRIGHT_PORT`, `APP_URL`/`PR6_AXE_E2E_ALLOWED_ORIGIN` exact managed origin, `DATABASE_URL_TEST` and exact non-production host identity, Neon Auth secrets, and all distinct M2/M3/M7 credentials. It mutates only login timestamps and restores every full profile.

    Each guard requires canonical origins and independent non-production resource identity. M1 handles recoverable provider/webhook/database/profile cleanup; M2 deterministic reset; M3 seeded unsubscribe/retry/audit plus identity reset; M4B/M5 exact external target-to-DB identity restoration; M6 full seed/application/audit/Showcase/identity reset; M7 Page Copy/News/Media/audit/Admin-profile restoration; and authenticated Axe all-identity restoration. Any ambiguity, cleanup, snapshot, drift, audit ownership, identity quiet-window, seat residue, or public-baseline failure fails. M3/M4B/M5 remain external guarded Preview flows; M1/M2/M6/M7/Axe are managed loopback only. No external/provider/database mutation occurs without separate named approval.

    Before M4B or M5, their source/unit identity lifecycle contracts must pass; those E2E files call the lifecycle themselves, so each separate command restores its profiles before exit. The final authenticated Axe suite owns its own managed sentinel and restore lifecycle. Do not run a bare authenticated command outside these wrappers.
    ...
    - M1 proves annual durable pricing, retrieval-only/exactly-once magic links, managed checkout, locally signed real-route webhook plus idempotent replay, activation/Billing Portal, distinct invitee capacity denial, recoverable Session lineage, exact job/audit/journey cleanup, independent DB/provider cleanup, and quiet-window/CAS profile restoration.
    ...
    - M3 proves target-bound seeded automation data, exactly one audited real-UI retry and sent/ineligible denial, isolated unsubscribe changes, exact seed/retry/suppression/audit reset, and full staff/member identity restoration.
    - M4B proves retention approvals and inert Board draft preview, then restores both target-bound identities; M4C proves bilingual privacy-safe AI-Ops without app-login identity mutation.
    - M5 proves canonical listing manager/member permissions and staff review/rejection reason, then restores its target-bound login profile.
    - M6 proves legal cohort transitions/stage audits and Showcase projection under deterministic managed reset, then restores exact seed rows, member application, audit set, Showcase fields, and both identities.
    - M7 proves bilingual News, approved Page Copy, and Media lifecycle/reference locks; Page Copy uses exact request-window/actor/`Privacy`/`{namespace, updated, cleared}` ownership and successful real-action/public restoration, News/Media audits are independently deleted, and the Admin identity is restored. Direct DB containment alone cannot pass.
    - Authenticated accessibility proves the exact 60-case matrix on its own managed target and restores every distinct target-bound M2/M3/M7 profile after all contexts close.
...
- [ ] **Step 6: Run Lighthouse and field-performance gates**
    ...
    Expected: collect only approved indexable public routes. Performance is at least 0.90, accessibility at least 0.95, and SEO at least 0.95; record LCP/CLS and authorized field INP or `NOT PASSED`. Never include noindex `/member-login` or authenticated routes in the global SEO threshold, and never upload private pages to public temporary storage.
...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    - exact disposable cleanup evidence for every authorized suite: M1 provider/webhook/profile ledgers; M3 seed/unsubscribe/retry/audit/identity reset; M4B/M5 target-to-DB identity restoration; M6 seed/application/audit/Showcase/identity reset; M7 Page Copy/News/Media/audit/Admin-profile restoration; and authenticated-Axe all-profile restoration;
    - source-only rollback: revert PR6 commits to PR5 head.
    Before staging, run `git diff --check` for unstaged Task 12 edits and confirm no generated trace, result, credential, or environment file is selected. Do not claim a clean worktree or final head yet; both become meaningful only after the Task 12 commit.
...
- [ ] **Step 8: Stage every Task 12 path, commit, then prove the immutable final range**
    ...
    git add -- ':(literal)tests/e2e/wisetech-pr6-internal-journeys.spec.ts' ':(literal)tests/fixtures/external-authenticated-identity-lifecycle.ts' ':(literal)tests/unit/external-authenticated-identity-lifecycle.test.ts' ':(literal)tests/fixtures/pr6-authenticated-accessibility-lifecycle.ts' ':(literal)tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts' ':(literal)tests/e2e/wisetech-pr6-authenticated-accessibility.spec.ts' ':(literal)tests/fixtures/m4b-e2e-safety.ts' ':(literal)tests/unit/m4b-e2e-safety.test.ts' ':(literal)tests/e2e/m4b-agents.spec.ts' ':(literal)tests/e2e/m5-showcase.spec.ts' ':(literal)docs/integration/wisetech-pr6-verification.md' ':(literal)docs/integration/wisetech-pr6-pr-body.md'
    git diff --cached --name-status
    git diff --cached --check

    Expected: exactly the literal Task 12 paths above plus optional `lighthouserc.js`, with no whitespace errors.
    ...
    Expected: approved PR6 files only, cohesive commits, no range whitespace errors, and a clean worktree. Preserve `$finalHead` in the immutable review package and publication checks; do not amend the committed verification document merely to self-record its own SHA.

    If `lighthouserc.js` changed only for approved indexable public-route collection, stage it with `git add -- ':(literal)lighthouserc.js'` before the commit. Never stage `/member-login` as an SEO target, generated results/traces, credentials, environment files, runtime output, or unrelated changes.
...
- Verification coverage: Task 12 includes exact credential-free and 60-case authenticated matrices, 26-page/19-API denial proof, unit/safety aggregates, dependency/static/full browser gates, separately authorized M1-M7 plus M4B/M5/Axe identity lifecycles, managed resource binding/no reuse, M1 Session/webhook/profile cleanup, M3/M6 deterministic reset, M7 exact audit/public restore, indexable-only Lighthouse thresholds, security/diff gates, stage-before-commit ordering, post-commit immutable range/worktree proof, external final-SHA capture, independent review, and exact remote base/head/body verification.
...
- Placeholder scan: every task names exact files, interfaces, RED/GREEN or final commands, expected evidence, constraints, and staging. M1 names provider/session/webhook/profile ledgers; M2 exact 26/19 matrices; M3 complete seed/unsubscribe/retry/audit/identity reset; M6 full seed/application/audit/Showcase/identity restore; M7 production-shaped Page Copy/News/Media audit predicates plus real-action public restore and failing containment; Task 12 names external and managed identity lifecycles plus executable commit/review order. No implementation step delegates an unspecified safety or ownership decision.
