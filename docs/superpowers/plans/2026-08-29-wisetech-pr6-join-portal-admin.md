...
### Task 0: Install the fail-closed execution boundary used by every later task

**Files:**

- Create: `scripts/resolve-verified-runtime.ps1`, `scripts/managed-runtime-environment.mjs`, `scripts/managed-runtime-environment.d.mts`, `scripts/run-credential-free-verification.mjs`, `scripts/run-managed-playwright.mjs`, `scripts/run-managed-next.mjs`, `tests/fixtures/credential-free-process-env-probe.mjs`, `tests/fixtures/managed-next-env-probe.mjs`, `tests/unit/verified-runtime-bootstrap.test.ts`, `tests/unit/credential-free-verification-boundary.test.ts`, `tests/unit/credential-free-build-boundary.test.ts`, `tests/unit/managed-next-production-boundary.test.ts`.
- Modify: `package.json`, `next.config.ts`, `playwright.config.ts`.
- Consume without executing application code: the locked `package-lock.json`, installed Node/npm, repository-local Vitest/Playwright/Next/ESLint/TypeScript entrypoints, and the exact environment-name sources enumerated below.

**Interfaces:**

- Produces the only trusted command bootstrap. `resolve-verified-runtime.ps1` uses PowerShell/.NET APIs only: it accepts operator-recorded absolute Node and Git paths, their allowed installation roots, and lowercase SHA-256 values; resolves every existing component without following a reparse ambiguity; requires a regular executable strictly beneath the approved root; hashes it before returning; and emits only the two canonical paths as `$verifiedNode` and `$verifiedGit`. It never enumerates or prints the environment. Every command in this plan first obtains those reviewed bootstrap results, and every JavaScript launcher independently proves `process.execPath` has the same canonical path/hash before reading any external capability. A PATH-resolved Node, npm shim, `ComSpec` route, or hash/root mismatch is `NOT PASSED`.
- `managed-runtime-environment.mjs` initially owns the pure credential-free environment builders and exact classified-capability inventory; Task 4 extends the same module with suite manifests. `run-credential-free-verification.mjs` is executable-only and accepts exactly `install`, `rebuild`, `audit`, `audit-strings`, `lint`, `typecheck`, `build`, `unit`, or `e2e` plus each mode's fixed literal arguments. It rejects case-colliding keys, `PLAYWRIGHT_BASE_URL`, every recognized database/provider/Auth/password/token/sentinel/proxy/telemetry destination in the original parent, every repository `.npmrc`, and every root dotenv candidate `.env`, `.env.local`, `.env.test`, `.env.test.local`, `.env.development`, `.env.development.local`, `.env.production`, and `.env.production.local` before any third-party or application import.
- Every child map starts empty. It contains only canonical OS/runtime directories, a rebuilt PATH containing the attested Node directory and fixed system directories, canonical shell values, run-owned temp/cache/config paths, fixed `NEXT_TELEMETRY_DISABLED=1`, and a mode-specific `NODE_ENV`. It excludes arbitrary ambient keys. The install mode locates the npm CLI only beneath the attested Node installation, validates its regular-file identity and locked npm version, uses empty run-owned user/global npm configs plus the public registry, and executes exactly `ci --ignore-scripts --audit=false --fund=false` with `shell: false`. The separately recorded rebuild mode executes exactly `npm rebuild --foreground-scripts --audit=false --fund=false` through the same attested npm CLI, only after verifying every `hasInstallScript` package name/version/integrity against the lockfile, and only inside the same credential-free child. Network or lifecycle failure is `NOT PASSED`; it never justifies restoring ambient npm credentials. Audit is the exact read-only `audit --omit=dev --audit-level=high` command under that same empty npm configuration. The remaining modes launch exact repository-local JavaScript entrypoints through `process.execPath` with `shell: false`; no npm lifecycle, PATH shim, or command string is involved.
- The `build` and `e2e` modes use `run-managed-next.mjs` to create a unique absent `.next/pr6-<uuid>` directory that `next.config.ts` accepts only with an internal attestation. They launch the locked Next CLI as `build --webpack` under a capability-free production map, then `start --hostname 127.0.0.1 --port <ownedPort>` under a separately constructed runtime map. They never launch `next dev`. Both processes receive `__NEXT_PROCESSED_ENV=true` only after inner and outer dotenv-absence fingerprints agree; because build/start never call the development `forceReload` watcher, a dotenv file created after readiness cannot change the process. The build artifact is run-owned, scanned for synthetic capability canaries, and removed in aggregate cleanup; missing cleanup or an occupied port fails.
- The credential-free `e2e` path owns a fresh loopback server, uses an unguessable internal runner attestation instead of `PLAYWRIGHT_BASE_URL`, and invokes the exact local Playwright CLI. `playwright.config.ts` has no ambient/default fallthrough: credential-free mode requires that attestation, sets `forbidOnly: true`, one worker, no reuse, the exact owned base URL, and a separately scrubbed browser `launchOptions.env`; a raw config import or direct CLI launch fails. Task 4 adds guarded mutation suites and M4C's read-only Preview mode without weakening this branch.
...
- The production-boundary test pins Next 16.3.0's loader and dev-watcher sources, proves the plan never selects `next dev`, runs the real build/start path against an isolated miniature Next fixture, creates every production dotenv candidate after readiness with forbidden canaries, and proves repeated requests see the original exact environment until shutdown. It also injects files during the prelaunch/build race and proves the processed marker blocks the non-forced loader. The credential-free process probe contaminates its parent with arbitrary canaries and fake shell/PATH entries, while recognized capability variables are rejected before launch; clean child maps must equal the exact allowlist.

- [ ] **Step 1: Record the unsafe bootstrap baseline without launching a browser or provider**

    Add source/contract tests that fail against the current raw `npm.cmd`/`npx` routes, ambient `PLAYWRIGHT_BASE_URL` branch, `reuseExistingServer: !CI`, missing `forbidOnly` guarantee, inherited browser environment, development Next server, and absent dotenv reload protection. Record that source-level RED only; do not put credentials in the shell to reproduce it.

- [ ] **Step 2: Implement the attested runtime, exact child launchers, and immutable Next server**

    Implement only the interfaces above. The PowerShell bootstrap performs the pre-exec Node check; the JavaScript launchers repeat it, validate repository-local entrypoints and package versions, and import no application/provider/database module before constructing their replacement child environments. Remove the raw `e2e`, `test:e2e`, and `test:lighthouse` package-script entry points; all executable plan commands use the reviewed launchers directly.

- [ ] **Step 3: Run the boundary probes under the verified Node runtime**

    At the start of this and every later shell, read the non-secret runtime attestation fields from the execution record and run:

    $verifiedRuntime = & .\scripts\resolve-verified-runtime.ps1 -NodePath $executionNodePath -NodeAllowedRoot $executionNodeRoot -NodeSha256 $executionNodeSha256 -GitPath $executionGitPath -GitAllowedRoot $executionGitRoot -GitSha256 $executionGitSha256
    $verifiedNode = $verifiedRuntime.NodePath
    $verifiedGit = $verifiedRuntime.GitPath
    ...
    Expected: PASS. The actual child key sets, no-shell entrypoints, clean npm configuration, fresh-port ownership, production build/start selection, browser isolation, artifact cleanup, and real post-readiness dotenv-creation proof all pass; fake PATH/`ComSpec`, direct CLI, raw config, stale localhost, dotenv race, install-script leakage, unsupported mode/argument, and child failure all fail closed.

- [ ] **Step 4: Commit the execution prerequisite**

    & $verifiedGit add -- ':(literal)package.json' ':(literal)next.config.ts' ':(literal)playwright.config.ts' ':(literal)scripts/resolve-verified-runtime.ps1' ':(literal)scripts/managed-runtime-environment.mjs' ':(literal)scripts/managed-runtime-environment.d.mts' ':(literal)scripts/run-credential-free-verification.mjs' ':(literal)scripts/run-managed-playwright.mjs' ':(literal)scripts/run-managed-next.mjs' ':(literal)tests/fixtures/credential-free-process-env-probe.mjs' ':(literal)tests/fixtures/managed-next-env-probe.mjs' ':(literal)tests/unit/verified-runtime-bootstrap.test.ts' ':(literal)tests/unit/credential-free-verification-boundary.test.ts' ':(literal)tests/unit/credential-free-build-boundary.test.ts' ':(literal)tests/unit/managed-next-production-boundary.test.ts'
...
- Task 4 extends Task 0's shared test-only runtime for mutating M1, M2, M3, M4B, M5, M6, M7, final authenticated-Axe, guarded PostgreSQL, and read-only M4C suites. `scripts/managed-runtime-environment.d.mts` remains the one exact sibling declaration for the `.mjs` implementation and owns every exported constant, type, and function below; TypeScript consumers import that single runtime module with no duplicate TS implementation.
  ...
  `managed-runtime-process-boundary.test.ts` exercises the outer runner, capability-free build, production Next start, Playwright child, browser child, and cleanup as actual subprocesses. Assert every complete case-folded map, every suite manifest, original-parent versus runner versus Next ownership, fresh-port behavior, direct-bypass denial, executable/hash/path checks, exit propagation, and absent artifact/auth output. `run-managed-vitest.mjs webhook-postgres` uses the same attested Node and replacement runner rules, retains only the webhook test sentinel/isolated database sources, and invokes the exact local Vitest CLI with `shell: false`; raw Vitest/npm and ambient standard `DATABASE_URL` fail before the guard import.

  `m4c-readonly-preview` is a distinct sanitized branch. It requires only the existing M4C read-only sentinel, canonical approved non-production HTTPS `PLAYWRIGHT_BASE_URL`, and optional exact Vercel share token; starts no local server; passes no database/provider/Auth/mutation source to Next or the browser; disables trace/screenshot/video; and lets the existing M4C guard enforce the private-canary/read-only contract. Any mutating sentinel, loopback target, production origin, unexpected credential, or write request fails.
...
- [ ] **Step 2: Run the focused tests and record the RED reason**
    ...
    & $verifiedNode scripts/run-credential-free-verification.mjs unit -- tests/unit/isolated-runtime-environment.test.ts tests/unit/managed-runtime-process-boundary.test.ts tests/unit/managed-vitest-process-boundary.test.ts tests/unit/managed-webserver-boundary.test.ts tests/unit/managed-next-environment-boundary.test.ts tests/unit/managed-browser-process-boundary.test.ts tests/unit/managed-auth-session.test.ts tests/unit/checkout-service.test.ts tests/unit/checkout-recovery-service.test.ts tests/unit/billing-checkout-locking.test.ts tests/unit/billing-recovery-cas.test.ts tests/unit/join-billing-pages.test.tsx tests/unit/portal-billing-actions.test.tsx tests/unit/webhook-postgres-safety.test.ts tests/unit/webhook-service.test.ts tests/unit/webhook-repository-sequential.test.ts tests/unit/m1-acceptance-services.test.ts tests/unit/m1-live-acceptance-safety.test.ts
    ...
    & $verifiedNode scripts/run-managed-vitest.mjs webhook-postgres -- tests/integration/webhook-join-projection-postgres.test.ts
...
- [ ] **Step 4: Run GREEN and prove the M1 harness can reach an authorized isolated result**
    ...
    & $verifiedNode scripts/run-managed-playwright.mjs m1 -- tests/e2e/m1-acceptance.spec.ts
...
- [ ] **Step 4: Run GREEN and M2 CRM regressions**
    ...
    & $verifiedNode scripts/run-managed-playwright.mjs m2 -- tests/e2e/m2-admin-crm.spec.ts
...
- [ ] **Step 4: Run GREEN and M3-M7 focused regression suites**
    ...
    With separate named approvals and complete managed variables, run `& $verifiedNode scripts/run-managed-playwright.mjs m3 -- tests/e2e/m3-automations.spec.ts` and `& $verifiedNode scripts/run-managed-playwright.mjs m6 -- tests/e2e/m6-launch-pad.spec.ts` separately. Each must use the guarded managed server mapped to `DATABASE_URL_TEST`, exercise its real UI outcomes, restore complete database and identity baselines in `afterAll`, and prove zero unexpected residue. Missing authority/guard, an M3 predicate-ledger mismatch, an M6 alternate pair ID/Showcase cardinality mismatch, or any cleanup uncertainty is `NOT PASSED`.
...
- Modify the public performance gate by renaming the existing ESM-incompatible `lighthouserc.js` to CommonJS `lighthouserc.cjs` and adding sibling `lighthouserc.d.cts`. Consume Task 0's `package.json`, where the ambient npm Lighthouse entry point is already absent. Collection runs only through the attested absolute Node plus executable wrapper; the config uses only the authorized Preview route allowlist below, a run-owned OS-temp working/output directory, no local-server fields, and no noindex/authenticated URL.
...
- [ ] **Step 1: Define final credential-free and authenticated browser verification matrices**
    ...
    Before Step 2, rerun Task 0's verified-runtime, credential-free process, and immutable Next production-server contracts. The already committed launcher accepts only its fixed install/rebuild/audit/static/unit/e2e modes, rejects every `tests/integration` path in credential-free mode, never uses an ambient npm lifecycle, confines dependency scripts to its explicit scrubbed rebuild mode, and never launches a development server for Steps 2-4 and the nonmutating part of Step 5. Existing real-PostgreSQL integrations remain inventoried separate external gates; PR6's webhook contract runs only through Task 4's exact guarded Vitest mode. Do not modify this shared boundary in Task 12; a discovered defect returns to Task 0 or Task 4 for its own reviewed fix.
...
- [ ] **Step 3: Run dependency, static, unit, lint, type, build, security, and diff gates**
    ...
    & $verifiedNode scripts/run-credential-free-verification.mjs rebuild
...
- [ ] **Step 5: Prove safety contracts, then run isolated M1-M7 only with separate authority**
    ...
    & $verifiedNode scripts/run-credential-free-verification.mjs unit -- tests/unit/credential-free-verification-boundary.test.ts tests/unit/credential-free-build-boundary.test.ts tests/unit/verified-runtime-bootstrap.test.ts tests/unit/managed-next-production-boundary.test.ts tests/unit/isolated-runtime-environment.test.ts tests/unit/managed-runtime-process-boundary.test.ts tests/unit/managed-vitest-process-boundary.test.ts tests/unit/managed-webserver-boundary.test.ts tests/unit/managed-next-environment-boundary.test.ts tests/unit/managed-browser-process-boundary.test.ts tests/unit/managed-auth-session.test.ts tests/unit/authenticated-identity-safety.test.ts tests/unit/m1-live-acceptance-safety.test.ts tests/unit/webhook-postgres-safety.test.ts tests/unit/m2-auth-reset.test.ts tests/unit/m2-runtime-environment.test.ts tests/unit/m2-browser-acceptance-contract.test.ts tests/unit/m3-acceptance-isolation.test.ts tests/unit/m3-acceptance-safety.test.ts tests/unit/m3-e2e-safety.test.ts tests/unit/m3-browser-lifecycle.test.ts tests/unit/m4b-runtime-guard.test.ts tests/unit/m4b-e2e-safety.test.ts tests/unit/m5-browser-lifecycle.test.ts tests/unit/m6-neon-target.test.ts tests/unit/m6-e2e-safety-contract.test.ts tests/unit/m7-acceptance-safety.test.ts tests/unit/m7-browser-acceptance-contract.test.ts tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts
    ...
    & $verifiedNode scripts/run-managed-playwright.mjs m1 -- tests/e2e/m1-acceptance.spec.ts
    & $verifiedNode scripts/run-managed-playwright.mjs m2 -- tests/e2e/m2-admin-crm.spec.ts
    & $verifiedNode scripts/run-managed-playwright.mjs m3 -- tests/e2e/m3-automations.spec.ts
    & $verifiedNode scripts/run-managed-playwright.mjs m4b -- tests/e2e/m4b-agents.spec.ts
    & $verifiedNode scripts/run-managed-playwright.mjs m4c-readonly-preview -- tests/e2e/m4c-aiops.spec.ts
    & $verifiedNode scripts/run-managed-playwright.mjs m5 -- tests/e2e/m5-showcase.spec.ts
    & $verifiedNode scripts/run-managed-playwright.mjs m6 -- tests/e2e/m6-launch-pad.spec.ts
    & $verifiedNode scripts/run-managed-playwright.mjs m7 -- tests/e2e/m7-cms.spec.ts
    & $verifiedNode scripts/run-managed-playwright.mjs axe -- tests/e2e/wisetech-pr6-authenticated-accessibility.spec.ts
...
- [ ] **Step 6: Run the exact authorized-Preview Lighthouse and field-performance gates**
    ...
    In the mandatory credential-free `prepare-index` phase, the pure core stages only this exact execution set: `package.json`, `package-lock.json`, `scripts/run-lighthouse-preview.mjs`, `scripts/lighthouse-preview-core.mjs`, `scripts/lighthouse-preview-core.d.mts`, `lighthouserc.js` as a deletion, `lighthouserc.cjs`, `lighthouserc.d.cts`, both Lighthouse probe files, and all four Lighthouse unit-test files. Use the attested absolute Git with `shell: false` and a replacement child environment. First require `git check-attr filter` to be unspecified for every path and reject any external clean/smudge/filter-process driver; then stage with the core's fixed literal path list and verify cached name/status/whitespace. All other Task 12 files remain unstaged. This deliberate index checkpoint applies only Git's built-in clean/line-ending behavior and makes the candidate committed bytes addressable without guessing from checkout bytes.

    The pure core then builds a canonical execution manifest through an exact canonical Git executable whose path/root/hash is attested before the Vercel token is read. For every non-deleted ordered path record (a) raw working-tree byte length and SHA-256 before and after execution; (b) exact staged blob OID from `:<path>`; (c) staged blob byte length and SHA-256 computed from `git cat-file blob <oid>`; and (d) a hash of NUL-delimited `git check-attr --all` output plus the effective `core.autocrlf`, `core.eol`, and `core.safecrlf` values with configuration origins. Record the old `HEAD:lighthouserc.js` OID plus an index-deletion tombstone. Also bind the exact Node version and local `@lhci/cli` entrypoint/package version/entrypoint SHA-256. `prepare-index` writes those non-secret values to one unique absolute OS-temp checkpoint and prints only that path. `collect` revalidates every value before reading the Vercel token and again after LHCI; any raw hash, index OID, staged blob hash, attribute/config fingerprint, or cached name/status drift fails. Unit tests cover CRLF checkout versus LF blob, `core.autocrlf` changes, repository/global attribute changes, filter-process failure, index replacement, raw-file replacement, deleted-file resurrection, and CLI seam drift. Write only the sanitized manifest/aggregate under `<runRoot>` and copy them into the verification record.
    ...
    $lighthouseCheckpoint = (& $verifiedNode scripts/run-lighthouse-preview.mjs prepare-index).Trim()

    Expected: unit contracts PASS and `prepare-index` stages exactly the fixed Lighthouse execution set, prints one absolute non-secret OS-temp checkpoint path, and performs no network/Chrome/provider action. Run it even when no Preview collection is authorized so Task 12's source files still enter the final index.
    ...
    & $verifiedNode scripts/run-lighthouse-preview.mjs collect --checkpoint "$lighthouseCheckpoint"
...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    If collection ran, immediately end the dedicated approval-bearing shell. Start Step 7 in a new shell, rerun Task 0's Node/Git bootstrap, and require a source guard to prove every `LHCI_*` approval/token name and `VERCEL_SHARE_TOKEN` is absent without printing values; any later Git/source command in the approval-bearing shell is `NOT PASSED`. Then, in `docs/integration/wisetech-pr6-verification.md`, record:
    ...
    - the Lighthouse pre/post execution-manifest aggregate hash; each raw checkout length/SHA-256 pair; each staged clean-filtered OID, blob length/SHA-256, and attribute/config fingerprint; the deletion tombstone; and local Git/LHCI executable hashes. Mark final-commit binding pending because this document cannot self-reference its own commit, then record Step 8's `HEAD:path`/`git cat-file` result only in the external immutable review package;
...
- [ ] **Step 8: Stage the remaining Task 12 paths, commit the complete index, then prove the immutable final range**
    ...
    if ($lighthouseGateStatus -ceq "PASSED") {
      ...
      & $verifiedNode scripts/run-lighthouse-preview.mjs verify-commit --run-root "$lighthouseRunRoot" --head "$finalHead"
    ...
    Expected: approved PR6 files only, cohesive commits, no range whitespace errors, and a clean worktree. When the authorized Lighthouse gate is `PASSED`, `verify-commit` revalidates Node, Git, LHCI, and the attribute/config fingerprint; proves every final `HEAD:<path>` OID equals the retained staged OID; reads each final blob through `git cat-file blob <oid>` and requires its byte length/SHA-256 to equal the retained staged-blob values; separately requires the collection's pre/post raw working-tree length/SHA-256 pair to have remained equal; and confirms the deletion tombstone. A raw, filtered-blob, attribute, index, or executable mismatch requires a new collection and evidence. If Lighthouse was not separately authorized, do not invent a run root: record this binding and the external performance gate as `NOT PASSED`. Preserve `$finalHead` in the immutable review package and publication checks; do not amend the committed verification document merely to self-record its own SHA.
