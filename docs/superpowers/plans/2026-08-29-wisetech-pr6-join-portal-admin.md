...
- Task 0 Step 1 is the sole pre-execution-record exception. It uses closed `hkwtia.pr6.initial-red-attestation.v1` issued only after the new contract file exists and before any other Task 0 source is created. That record binds repository root; approved-plan path/hash and full identity; exact PowerShell launch/preamble/channel identity; immutable complete PowerShell distribution; the exact contract path/allowed-root/hash; the exact dependency-free `New-Pr6ApprovedScriptBlock` helper bytes/hash already embedded in the approved preamble; fixed argument `-ExpectedState UnsafeBaseline`; and one-use approval/expiry. It forbids the resolver, generator, invoker, native adapter, guardian, Node/npm/Git/GitHub CLI/browser/provider/database/Preview/publication fields and every other repository path or native child. After the handshake the launcher may frame only the three contract identity fields, the helper captures those bytes once, returns one structured result, and the launcher terminates the shell. Any extra read/frame/invocation or attempt to reuse this record fails.
...
- Exact non-secret operation/evidence records are closed schemas. `hkwtia.pr6.m4c-readonly-input.v1` binds the Task 12 target/helper/exact status-entry identities, M4C approval descriptor, request manifest, repository/branch/PR5/spec/approved-plan/`preTask12Head`, and read-only provider operation. `hkwtia.pr6.lighthouse-collect-input.v1` binds the Task 12 target/helper, exact expected status entries/hash, repository/branch/PR5/spec/approved-plan/`preTask12Head`, Preview approval, and six checkpoint/entrypoint/config identities. `hkwtia.pr6.verification-authoring-input.v1` binds those target/helper/head/checkpoint fields; explicit `preAuthoringExpectedStatusEntries`/hash and `postAuthoringExpectedStatusEntries`/hash, where every ordered entry fixes literal path plus index/worktree status and the post set must equal the pre set plus exactly index-clean/worktree-`?` `docs/integration/wisetech-pr6-verification.md` and `docs/integration/wisetech-pr6-pr-body.md` entries; exact gate status; one `hkwtia.pr6.suite-evidence-manifest.v1` path/hash; one `hkwtia.pr6.lighthouse-checkpoint-evidence.v1` path/hash; and iff collection passed, one `hkwtia.pr6.lighthouse-retained-artifact-manifest.v1` and one `hkwtia.pr6.provider-sanitized-evidence.v1` path/hash. The ordered suite manifest binds repository/head, schema and exact `gateStatus`, and entries containing suite, run ID, sanitized argument hash, start/end timestamps, exit code, totals/warnings/skips, `PASSED | NOT_PASSED`, approval reference, and exact `hkwtia.pr6.cleanup-receipt.v1` path/hash. Each cleanup receipt binds that entry/run/head and closed sanitized session/data/provider/artifact cleanup projections; the checkpoint evidence binds the six prepare-index identities and execution aggregates; retained-artifact evidence binds the authorized run root plus every retained file hash; provider-sanitized evidence binds only token-free deployment/head/Chrome/field projections. All cross-record repository/head/run/suite/status/hash duplicates must be byte-equal. `hkwtia.pr6.lighthouse-commit-verification.v1` binds repository/branch/PR5/spec/approved-plan/final head, literal final verification-document path and its `HEAD:path` blob OID/raw blob-byte SHA-256/length, the six checkpoint identities, and `gateStatus: "PASSED" | "NOT_PASSED"`; run-root/artifact/collection identities are required iff `PASSED` and forbidden otherwise. `hkwtia.pr6.zero-finding-review-package.v1` binds repository/branch, reviewed head, PR5/spec OIDs, approved-plan record/commit/literal path/blob OID/raw blob-byte SHA-256/approved-checkout identity/docs-only origin, reviewer evidence hash, and exact totals `{critical: 0, important: 0, minor: 0}`. `hkwtia.pr6.publication-input.v1` binds that review package, exact approved-plan record, publication descriptor, repository/head/base branches, base/spec/approved-plan/reviewed-head OIDs, and literal plan/blob identity. Unknown fields, missing referenced schemas, unresolved paths, or duplicate mismatch fail. No record contains a token, credential, raw provider response, or raw review prose. Trusted launcher/CI issues each only after its prerequisite exists; no record is reused for another schema, head, or shell.
...
- [ ] **Step 4: Run the boundary probes under the verified pre-entry launcher**
    ...
    Expected: PASS. Reproduce the trusted no-profile/noninteractive PowerShell launch, exact preamble bytes, separate framed control endpoint/peer/sequence, replacement environment, pre-field handshake, fake-profile/console/clipboard denial, approved-plan binding, and schema-owned immutable operation-input/script records; exact structured RED/GREEN contract results without native exit state; install/runtime schemas; captured bootstrap/native blocks; complete PowerShell/Node/npm/Git/GitHub-CLI/helper/whole-`node_modules`/browser closures; binary-safe Git-blob hashing; synthetic child-only native Git/`gh` credential delivery and zeroing; `--no-bin-links` with direct regular CLI targets; restricted-token/read-only-mount directory immutability; source/materialization/output graphs; run-owned Vitest cache and Playwright `.last-run.json`; non-inheritable Node capability delivery/zeroing; guarded distDir/generated tsconfig; controlled `next-env.d.ts` and redirected build-info writes; generated/traced closure; full browser isolation; atomic containment; and cleanup. Wrong kind/stale record, ambient profile/secret, preamble/channel/approved-plan drift, fake host/PATH, `.bin` link, source/module Vitest cache, worktree Playwright output, credential/config leakage, late file/directory/DLL/PAK/locale/package creation, DACL/mount relaxation, unexpected source write, root build-info, next-env replacement, loader/config/child drift, or capability/process escape fails before acceptance.
...
- [ ] **Step 6: Run the exact authorized-Preview Lighthouse and field-performance gates**
    ...
    if ($task12LiveResult.schema -cne "hkwtia.pr6.task12-live-assertion-result.v1" -or $task12LiveResult.head -cne $preTask12Head -or $task12LiveResult.gitTreeClosed -ne $true -or [string]$task12LiveResult.expectedStatusEntriesSha256 -cne [string]$verificationAuthoringInput.preAuthoringExpectedStatusEntriesSha256 -or [string]$task12LiveResult.evidenceSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_TASK12_LIVE_RESULT_INVALID" }
    $verifiedGit = $null
...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    End every suite/provider shell. The trusted launcher/CI first canonicalizes the immutable per-command and cleanup receipts into the ordered non-secret suite-evidence manifest, then issues exact `hkwtia.pr6.verification-authoring-input.v1`; when Lighthouse was not approved its closed `NOT_PASSED` branch contains no invented run root/provider artifact. Start Step 7 in a fresh capability-free shell, rerun Task 0's core bootstrap, and resolve every value from that operation record:
    ...
    $task12LiveResult = $task12LiveResults[0]
    if ($task12LiveResult.schema -cne "hkwtia.pr6.task12-live-assertion-result.v1" -or $task12LiveResult.head -cne $preTask12Head -or $task12LiveResult.gitTreeClosed -ne $true -or [string]$task12LiveResult.evidenceSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_VERIFICATION_TASK12_LIVE_INVALID" }
    ...
    if ([string]$suiteEvidenceManifest.repository -cne [string]$task12Target.repository -or [string]$suiteEvidenceManifest.head -cne $preTask12Head -or [string]$suiteEvidenceManifest.gateStatus -cne $lighthouseGateStatus) { throw "PR6_SUITE_EVIDENCE_MANIFEST_BINDING_INVALID" }
    ...
    foreach ($suiteEntry in @($suiteEvidenceManifest.entries)) {
      ...
      if ($receiptResults.Count -ne 1) { throw "PR6_CLEANUP_RECEIPT_COUNT_INVALID" }
      $receipt = $receiptResults[0]
      if ([string]$receipt.suite -cne [string]$suiteEntry.suite -or [string]$receipt.runId -cne [string]$suiteEntry.runId -or [string]$receipt.head -cne $preTask12Head -or [string]$receipt.status -cne [string]$suiteEntry.status -or [string]$receipt.evidenceSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_CLEANUP_RECEIPT_BINDING_INVALID" }
      $cleanupReceipts += $receipt
    }
    if ($cleanupReceipts.Count -ne @($suiteEvidenceManifest.entries).Count) { throw "PR6_CLEANUP_RECEIPT_SET_INVALID" }
    ...
    if ($lighthouseGateStatus -ceq "PASSED") {
      ...
      if ($retainedResults.Count -ne 1 -or $providerResults.Count -ne 1) { throw "PR6_LIGHTHOUSE_AUTHORIZED_EVIDENCE_COUNT_INVALID" }
      $retainedArtifactManifest = $retainedResults[0]
      $providerSanitizedEvidence = $providerResults[0]
      if ([string]$retainedArtifactManifest.head -cne $preTask12Head -or [string]$providerSanitizedEvidence.head -cne $preTask12Head -or [string]$retainedArtifactManifest.runRoot -cne [string]$verificationAuthoringInput.runRoot -or [string]$providerSanitizedEvidence.runRoot -cne [string]$verificationAuthoringInput.runRoot) { throw "PR6_LIGHTHOUSE_AUTHORIZED_EVIDENCE_BINDING_INVALID" }
    } elseif ($lighthouseGateStatus -ceq "NOT_PASSED") {
      if ($null -ne $verificationAuthoringInput.runRoot -or $null -ne $verificationAuthoringInput.retainedArtifactManifestPath -or $null -ne $verificationAuthoringInput.providerSanitizedEvidencePath) { throw "PR6_LIGHTHOUSE_UNAUTHORIZED_AUTHORING_EVIDENCE_PRESENT" }
    } else {
      throw "PR6_LIGHTHOUSE_AUTHORING_GATE_STATUS_INVALID"
    }

    The launch transcript proves the shell's replacement environment contains no `LHCI_*`, `VERCEL_SHARE_TOKEN`, provider credential, or approval value, and every resolved schema rejects such fields; do not enumerate or print an ambient environment. The authoring input, suite manifest, every cleanup receipt, checkpoint evidence, and conditional retained/provider records are held immutable through the edit and exact Git checks. Author the document solely from those validated objects—never by reopening an untyped receipt/artifact path. Missing/drifted target/helper/checkpoint/suite/cleanup/retained/provider evidence, a cross-record repository/head/run/status/hash mismatch, an unauthorized run-root branch, or any extra/missing status entry is `NOT PASSED`.

    - immutable PR5 base, the exact `hkwtia.pr6.verification-authoring-input.v1` and suite-evidence-manifest path/hashes, the exact `hkwtia.pr6.task12-target.v1` record/helper path/hashes and clean local `preTask12Head` captured before Task 12 editing, and the reviewed implementation head that existed before evidence authoring; the final PR6 commit SHA is intentionally captured after commit in the external immutable review package, not self-referenced inside this commit;
    ...
    if ($postAuthoringLiveResult.schema -cne "hkwtia.pr6.task12-live-assertion-result.v1" -or $postAuthoringLiveResult.head -cne $preTask12Head -or $postAuthoringLiveResult.gitTreeClosed -ne $true -or [string]$postAuthoringLiveResult.expectedStatusEntriesSha256 -cne [string]$verificationAuthoringInput.postAuthoringExpectedStatusEntriesSha256) { throw "PR6_POST_AUTHORING_TASK12_LIVE_INVALID" }
...
- [ ] **Step 8: Stage the remaining Task 12 paths, commit the complete index, then prove the immutable final range**
    ...
    Send only the exact sanitized `$finalHead` plus `{verificationDocumentPath, verificationDocumentBlobOid, verificationDocumentBlobSha256, verificationDocumentBlobByteLength}` over the launcher-owned control channel, then close the commit shell. The trusted launcher/CI requires the literal path, recomputes the record field hash, combines those exact values with the retained checkpoint and optional collection evidence, issues one exact `hkwtia.pr6.lighthouse-commit-verification.v1`, and starts a fresh capability-free verification shell. Rerun Task 0's runtime bootstrap, then resolve the launcher-delivered input record without console or environment input:
