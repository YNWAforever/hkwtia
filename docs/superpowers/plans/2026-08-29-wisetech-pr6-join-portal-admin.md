...
- Create: `scripts/generate-pr6-execution-candidate.ps1`, `scripts/resolve-verified-runtime.ps1`, `scripts/invoke-verified-tool.ps1`, `scripts/pr6-native-process-boundary.ps1`, `scripts/managed-process-guardian.ps1`, `scripts/pr6-publication-state-assertions.ps1`, `scripts/managed-runtime-environment.mjs`, `scripts/managed-runtime-environment.d.mts`, `scripts/run-credential-free-verification.mjs`, `scripts/run-managed-playwright.mjs`, `scripts/run-managed-next.mjs`, `tests/contracts/pr6-unsafe-bootstrap.red.ps1`, `tests/fixtures/powershell-profile-probe.ps1`, `tests/fixtures/credential-free-process-env-probe.mjs`, `tests/fixtures/managed-next-env-probe.mjs`, `tests/fixtures/managed-process-creation-window-probe.mjs`, `tests/fixtures/materialization-late-file-probe.mjs`, `tests/unit/verified-runtime-bootstrap.test.ts`, `tests/unit/credential-free-verification-boundary.test.ts`, `tests/unit/credential-free-build-boundary.test.ts`, `tests/unit/managed-next-production-boundary.test.ts`, `tests/unit/pr6-publication-continuation.test.ts`.
...
- Exact non-secret operation/evidence records are closed schemas. `hkwtia.pr6.m4c-readonly-input.v1` binds the Task 12 target/helper/exact status-entry identities, M4C approval descriptor, request manifest, repository/branch/PR5/spec/approved-plan/`preTask12Head`, and read-only provider operation. `hkwtia.pr6.lighthouse-collect-input.v1` binds the Task 12 target/helper, exact expected status entries/hash, repository/branch/PR5/spec/approved-plan/`preTask12Head`, Preview approval, and six checkpoint/entrypoint/config identities. `hkwtia.pr6.verification-authoring-input.v1` binds those target/helper/head/checkpoint fields; explicit `preAuthoringExpectedStatusEntries`/hash and `postAuthoringExpectedStatusEntries`/hash, where every ordered entry fixes literal path plus index/worktree status and the post set equals the pre set plus exactly index-clean/worktree-`?` `docs/integration/wisetech-pr6-verification.md` and `docs/integration/wisetech-pr6-pr-body.md` entries; exact Lighthouse attempt/completion, field-INP, and derived overall-gate state; the literal verification-document path and permission to return only its post-commit `HEAD:path` OID proposal; the independently approved root command inventory and the complete ordered `PRE_AUTHORING` phase-projection set/count/aggregate; the exact Task 0 command-evidence manifest/review closure; one `hkwtia.pr6.suite-evidence-manifest.v3`; one `hkwtia.pr6.lighthouse-checkpoint-evidence.v1`; and the conditionally present attempt/provider/run/retained/field records below. The Preview branch is a closed discriminated union: `lighthouseAttemptStatus: "UNAUTHORIZED"` requires `previewApprovalPath`, `previewApprovalSha256`, `deploymentId`, `projectId`, `teamId`, and `generatedUrl` all canonical null and forbids every attempt/provider/run/retained/field field; `AUTHORIZED_FAILED` or `PASSED` requires an exact `hkwtia.pr6.lighthouse-preview-approval.v1` path/hash plus byte-equal deployment/project/nullable-team/canonical-generated-URL fields, and the attempt receipt must duplicate all five identities even when failure occurs before provider proof. `providerProofCompleted` then requires provider-sanitized evidence with those same identities; `runCreated` requires the canonical run root; `retainedManifestCompleted` requires the retained manifest; and non-`UNAUTHORIZED` field INP requires field evidence. `PASSED` Lighthouse requires the attempt receipt and all three completion booleans true, but the overall gate is `PASSED` only when Lighthouse and field INP are both `PASSED`; `retainedManifestCompleted` implies `runCreated`, `runCreated` implies `providerProofCompleted`, and non-`UNAUTHORIZED` field INP implies provider proof. The independently approved `hkwtia.pr6.required-command-matrix.v2` root is issued outside the repository only after the approved-plan commit exists and before Task 0. It is a literal audit of every executable command block in Tasks 0-12 and contains exactly five ordered, nonempty partitions: `TASK0`, `PRE_AUTHORING`, `POST_COMMIT_REVIEW`, `PUBLICATION`, and `TERMINAL_ATTESTATION`, each with its own entry count/aggregate plus one root count/aggregate. `TASK0` covers every Task 0 command including the one-use initial RED; `PRE_AUTHORING` covers every Task 1-11 command and every Task 12 command completed through Step 6, stopping before the Step 7 shell that consumes its manifest; `POST_COMMIT_REVIEW` covers every Step 7 authoring/commit command, all Step 8 final-commit verification, and Step 9 final-review closure commands, stopping before the first publication shell that consumes its manifest; `PUBLICATION` covers every Step 9 remote/local publication, conditional push/create/view, and reconciliation branch through the sanitized publication-result handoff, stopping before the capability-free completion shell that consumes its manifest; `TERMINAL_ATTESTATION` covers that final completion-evidence shell. No evidence manifest is ever required to equal a future partition. Each root entry fixes `requiredCommandIdentitySha256`, root/partition ordinal, task/step, partition, `commandClass: "INITIAL_RED" | "RED" | "GREEN" | "GATE" | "CONTROL" | "AGGREGATE" | "REVIEW" | "PUBLICATION"`, verified tool/mode, canonical argument template and SHA-256, an ordered closed typed-runtime-slot schema/aggregate, `separateAuthorityPolicy`, canonical nullable `separateAuthorityScope`, `separateAuthorityIssuer`, and `authorizedOperationClass` fields, applicability policy, and exact allowed `{launchDisposition, outcome, nativeExitClass}` tuples. A runtime slot fixes name, primitive/OID/path/hash type, canonicalization, nullability, exact source schema/path field, and producing earlier entry when applicable; no slot may read ambient state. Runtime-generated checkpoint paths/hashes, materialized identities, final/reviewed heads, blob OIDs, PR number, and operation-receipt identities therefore remain typed placeholders in the pre-Task-0 root rather than impossible concrete values. Before the first command of each dependency-ready epoch, trusted launcher/CI issues immutable `hkwtia.pr6.command-phase-projection.v1` resolving only that epoch's slots from already validated prerequisite records; it binds the root path/hash/count/aggregate, partition count/aggregate, exact ordered root entries, template hashes, typed source fields/values, slot-value aggregates, and resolved sanitized argument-vector hashes. A projection never names or hashes the operation record that later embeds it, its own evidence/result, or any future record; trusted launcher/CI issues the projection first and then the operation input, so every dependency edge is acyclic. Every projection exists before its commands, cannot alter command class/order/policy, and all ordered projections must exactly and without overlap exhaust their partition. Task 0, pre-authoring, post-commit-review, publication, and terminal-attestation evidence are respectively closed by `hkwtia.pr6.task0-command-evidence-manifest.v2`, `hkwtia.pr6.suite-evidence-manifest.v3`, `hkwtia.pr6.post-commit-command-evidence-manifest.v1`, `hkwtia.pr6.publication-command-evidence-manifest.v1`, and `hkwtia.pr6.terminal-attestation-command-evidence-manifest.v1`; each must match only its root partition in exact count/order/identity/template/projection/resolved-vector identity. In every manifest, `entryAggregateSha256` is the ordered root-partition identity/template aggregate and is deliberately distinct from the recomputed `fullEntryEvidenceAggregateSha256`, which covers the ordered entry/source/result/cleanup plus authority/refusal/applicability evidence tuples. Every command-evidence entry contains unique entry/source/cleanup identities; `commandSourceKind: "INITIAL_RED" | "EXECUTION"`; root and partition identity; template, slot-value, and resolved-vector hashes; task/step/class/tool/mode; run identity, nullable execution head, plus nullable child-start/end fields; `launchDisposition: "LAUNCHED" | "EVALUATED_IN_LAUNCH_SHELL" | "REFUSED_BEFORE_LAUNCH" | "PROVEN_NOT_APPLICABLE"`; nullable native exit; `outcome: "EXPECTED_RED" | "PASSED" | "NOT_PASSED" | "NOT_APPLICABLE"`; nullable RED cause; immutable `hkwtia.pr6.command-result-projection.v1`, `hkwtia.pr6.command-source-manifest.v1`, and `hkwtia.pr6.cleanup-receipt.v3` path/hash pairs; and conditionally exact separate-authority/refusal/applicability receipt path/hash. `LAUNCHED` requires a positive child-process identity, non-null canonical start/end instants with end not before start, a native child exit, and no refusal/applicability receipt. If its root entry has `separateAuthorityPolicy: "REQUIRED"`, it additionally requires one unique immutable `hkwtia.pr6.separate-authority-receipt.v1`; otherwise every authority field is canonical null. `EVALUATED_IN_LAUNCH_SHELL` is legal for the sole Task 0 `INITIAL_RED` entry and for a root-declared `executionMode: "IN_PROCESS"` `CONTROL`, `AGGREGATE`, or `REVIEW` entry. It requires canonical-null native-exit/child/timing fields, no authority/refusal/applicability receipt, and a no-child cleanup receipt. `INITIAL_RED` additionally requires `EXPECTED_RED` plus the exact structured `UNSAFE_BASELINE` result with a nonempty assertion set; other in-process classes must match their root allowed outcome tuple. `REFUSED_BEFORE_LAUNCH` is legal only for `GATE` with separate authority required and requires every positive-authority field canonical null; it requires `NOT_PASSED`, canonical-null native exit/child/timing fields where no child exists, and immutable `hkwtia.pr6.launcher-refusal-receipt.v1` proving the exact entry/template/projection/result/cleanup tuple, policy reason `MISSING_SEPARATE_AUTHORITY`, `childCreated: false`, `nativeExitCode: null`, and no provider/data mutation. `PROVEN_NOT_APPLICABLE` is legal only for a matrix-declared conditional `CONTROL` or `PUBLICATION` branch and requires `NOT_APPLICABLE`, null native exit/child/timing and null authority fields, and an immutable applicability receipt bound to its exact entry/template/projection/result/cleanup tuple and the exact earlier entry/result pair that made the branch unreachable. Even refused or inapplicable entries retain a prelaunch source manifest and no-child cleanup receipt, so every required matrix entry has one unique reviewable source/result/cleanup projection without fabricating a process. `INITIAL_RED` requires `EVALUATED_IN_LAUNCH_SHELL`/`EXPECTED_RED`/`NO_NATIVE_EXIT` and the structured baseline cause; every ordinary `RED` requires launched `EXPECTED_RED`, nonzero exit, and a nonempty exact RED cause; `GREEN` requires launched `PASSED`, exit 0, and null RED cause; launched `GATE` requires `PASSED`/0 or `NOT_PASSED`/nonzero; other classes must match their exact root allowed tuple. The resolver validates the Task 0 manifest against the complete `TASK0` partition, requires exactly one `INITIAL_RED` entry with its special source/disposition/result tuple, and binds one zero-finding Task 0 review package before accepting later evidence. The pre-authoring suite matches only `PRE_AUTHORING`; Tasks 1-11 entries require `ZERO_FINDING`, at least one unique RED and GREEN per task, no AGGREGATE, and may also contain GATE/CONTROL; exactly 11 ordered designated final task-review closures cover Tasks 1-11. Each closure and resolved `hkwtia.pr6.task-review-package.v3` binds exact ordered arrays and recomputed aggregates for entry identities, command-source hashes, result-projection hashes, cleanup-receipt hashes, and nullable separate-authority/refusal/applicability receipt hashes plus one canonical full-entry-evidence aggregate; the resolver compares every array element to all task entries and forbids reused source/cleanup/refusal/applicability identities. Task 12 pre-authoring entries require `PENDING_FINAL_REVIEW`, may be GATE/CONTROL/AGGREGATE, require at least one AGGREGATE, and have no task-review package; the resolver binds the complete nonempty ordered Task 12 set by entry/source/result/cleanup and nullable authority/refusal/applicability arrays, counts, aggregates, and one full-entry-evidence aggregate into final review. The command-source manifest is a closed discriminated union. `sourceKind: "INITIAL_RED"` is legal only for the sole Task 0 `INITIAL_RED` entry and binds repository, entry/matrix identity, approved-plan identity, exact initial-red-attestation and baseline-record path/hashes, the attestation-bound root matrix/`INITIAL_RED` entry/phase-projection identities, captured contract/baseline member bytes/aggregate, template/slot/resolved-command hashes, and command projection while requiring `executionHead` and `executionAttestationSha256` canonical null. `sourceKind: "EXECUTION"` is required for every other entry and binds repository, entry/matrix identity, execution head, exact prelaunch source bytes/aggregate, execution-attestation hash, template/slot/resolved-command hashes, command projection, and the exact nullable separate-authority-receipt hash while requiring all initial-RED-only fields null. `hkwtia.pr6.command-result-projection.v1` is uniquely addressable by path/hash and binds repository, entry/matrix identity, run identity, nullable execution head, disposition, nullable child/times/native exit, outcome, RED cause, exact nullable authority/refusal/applicability hashes, closed integer totals, ordered warning/skip codes, sanitized result fields, and its recomputed identity; it never contains raw output or review prose. `hkwtia.pr6.separate-authority-receipt.v1` binds a unique identity, exact entry/template/projection/resolved-vector, approval reference and scope, issuer, issued/expiry instants, authorized provider/data operation class, and no secret. The result projection, command-source manifest, cleanup receipt, and full-entry aggregate must all repeat its hash when required and canonical null otherwise. Each cleanup receipt binds repository, unique receipt identity, exact entry/matrix identity, task/step/class/disposition/run/nullable execution head/outcome/nullable child/times/native exit, exact result-projection and nullable authority/refusal/applicability hashes, and closed sanitized session/data/provider/artifact/no-child cleanup projections. Checkpoint and retained-artifact evidence retain their existing exact head/path/hash/run bindings. Provider-sanitized evidence binds repository, `evidenceHead`, exact `previewApprovalSha256`, Vercel `deploymentId`, `projectId`, nullable `teamId`, canonical `generatedUrl`, and only token-free head/Chrome projections. Field-INP evidence binds repository, `evidenceHead`, the exact provider-sanitized-evidence hash and `previewApprovalSha256`, byte-equal deployment/project/nullable-team/generated-URL identity, authorization/sample window, sample sufficiency, p75, threshold 200, and status. `sampleSufficient: true` requires a non-null finite nonnegative p75; `sampleSufficient: false` requires canonical-null p75 and `NOT_PASSED`; `PASSED` is legal iff the non-null p75 is at most 200. Null, NaN, either infinity, a negative value, or an insufficient sample can never pass. `hkwtia.pr6.final-head-verification-input.v1` is issued only after the Task 12 commit entry closes; it binds that immutable command-result-projection path/hash and its exact sanitized post-commit head/branch, repository/base/spec/approved-plan identities, literal verification-document path, and a dependency-ready `POST_COMMIT_REVIEW` projection sourced from that earlier result. `hkwtia.pr6.verification-document-hash-input.v1` retains its exact final document/head/blob binding and the dependency-ready `POST_COMMIT_REVIEW` command-phase projection for that hash shell. `hkwtia.pr6.lighthouse-commit-verification.v1` uses the same conditional Preview union as authoring, binds the exact final document raw blob identity plus pre-Task-12 evidence head/checkpoint and the dependency-ready `POST_COMMIT_REVIEW` command-phase projection for that shell, and revalidates every conditionally completed attempt/provider/run/retained/field record; an authorized attempt receipt always closes the Preview approval/target chain before provider proof. `hkwtia.pr6.zero-finding-review-package.v2` binds repository/branch/reviewed head, PR5/spec/approved-plan identity, reviewer evidence, the root command matrix, all five partition counts/aggregates, the completed `TASK0`, `PRE_AUTHORING`, and `POST_COMMIT_REVIEW` manifests, complete pending Task 12 entry/source/result/cleanup/authority/refusal/applicability/full-evidence aggregates, final verification-document blob identity, exact reviewed PR-body path/blob OID/raw SHA-256/raw byte length/canonical-text SHA-256/canonical-text byte length, exact PR title, and exact zero findings. `hkwtia.pr6.publication-input.v2` is a closed `epoch: "INITIAL" | "POST_CREATE"` union. Both epochs duplicate that closure—including the exact PR-body/title identity whose canonical body argument is resolved into the create-entry projection—and bind the dependency-ready `PUBLICATION` projection and publication descriptor. Both carry the exact hash-bound publication-state-assertion-helper identity; `INITIAL` retains it only as a future equality commitment and does not evaluate it, while `POST_CREATE` must resolve and evaluate those exact bytes in its empty fresh scope. `POST_CREATE` additionally requires the exact create-operation completion receipt/PR identity and `INITIAL` forbids it. On uninterrupted success, the launcher freezes `hkwtia.pr6.publication-final-state.v1` only after all final reads complete. That closed receipt binds repository, reviewed head, PR number/URL, exact title, reviewed PR-body path/blob OID/raw SHA-256/raw byte length/canonical-text SHA-256/canonical-text byte length, `state: "OPEN"`, `isDraft: true`, exact base name/OID, head name/OID/owner, remote-head OID, unique open-head-PR count, create-completion receipt, and exact final view/base/head/open-set result-receipt identities. After either uninterrupted success or a stopped path with any required read-only reconciliation, trusted launcher/CI freezes the exact `PUBLICATION` manifest. That manifest carries canonical nullable final-state, stopped-command-entry/result, uncertain-mutation-operation, reconciliation-descriptor/result, and last-successful-mutation identities. Its reconciliation branch also carries the exact ordered reconciliation-read receipt path/hash/classification arrays and recomputed receipt/classification aggregates; every non-reconciliation branch requires empty arrays and canonical-null aggregates. Its successful final-read result aggregate is required only on PASSED and canonical null on NOT_PASSED, so the later completion record cannot select evidence outside the closed partition. It first issues the dependency-ready `TERMINAL_ATTESTATION` projection solely from the already immutable review package, publication manifest, and completed operation/authority/refusal/applicability/stopped-result/final-state/reconciliation evidence; `hkwtia.pr6.publication-completion.v2` then binds those records and that projection without any reciprocal hash reference. This completion is a closed `status: "PASSED" | "NOT_PASSED"` union. `PASSED` requires exact non-null PR number/URL/title/body/final-state identity, `lastSuccessfulMutationKind: "CREATE"`, the successful create receipt, the resolvable final-state receipt, `retryAttempted: false`, and `continuedMutation: false` while forbidding every failure/reconciliation record field. `NOT_PASSED` requires `failureKind: "PRE_MUTATION" | "RECONCILED_UNCERTAINTY" | "POST_MUTATION_VALIDATION"`, the exact stopped-command entry identity plus its `hkwtia.pr6.command-result-projection.v1` path/hash appearing exactly once in the closed PUBLICATION manifest with `outcome: "NOT_PASSED"`, `retryAttempted: false`, `continuedMutation: false`, conditionally null PR identity, and no final-state receipt. That result has closed publication-stop fields duplicating `failureKind` and canonical nullable `lastSuccessfulMutationKind`, plus nullable `uncertainMutationReceiptPath`/`uncertainMutationReceiptSha256`: they resolve one `hkwtia.pr6.publication-operation-outcome.v1` with `status: "OUTCOME_UNCERTAIN"` exactly for `RECONCILED_UNCERTAINTY` and are canonical null for both other failure kinds. `PRE_MUTATION` requires canonical-null `lastSuccessfulMutationKind` and forbids PR identity/create completion; `POST_MUTATION_VALIDATION` requires last successful mutation `PUSH` or `CREATE`, with PR identity/create completion required exactly for `CREATE` and forbidden for `PUSH`; `RECONCILED_UNCERTAINTY` permits null/`PUSH`/`CREATE`, binds its canonical nullable discovered PR identity, and permits a create-completion receipt exactly when an earlier create had already completed. Only `RECONCILED_UNCERTAINTY` requires the exact reconciliation descriptor/result receipt, while the other failure kinds forbid both. No branch may invent success. The capability-free terminal shell validates either branch and closes. Trusted launcher/CI then freezes `hkwtia.pr6.terminal-attestation-command-evidence-manifest.v1`, independently recomputes its exact partition/full-evidence closure, and returns that immutable manifest plus completion status as the terminal handoff. No sixth verifier command is launched merely to verify the terminal manifest, preventing a recursive evidence cycle. Every resolver recomputes canonical template, slot, vector, entry, result, source, cleanup, partition, manifest, review, checkpoint, provider, retained, field, and publication aggregates; all cross-record duplicates are byte-equal. Unknown fields, missing schemas, unresolved paths, reused evidence identities, partition gaps/overlap, or aggregate drift fail. No record contains a token, credential, raw provider response, or raw review prose. Trusted launcher/CI issues each record only after its prerequisite exists; no record is reused for another schema, head, partition, epoch, or shell.
...
- Remote mutation additionally requires exact non-secret `hkwtia.pr6.publication-approval.v1` bound to `github.com`, repository, head/base/ref/OIDs, PR5/spec/approved-plan record and literal plan/blob raw-hash identity, reviewed SHA, complete immutable Git/HTTPS/shell/credential-helper/GitHub-CLI closures, canonical GitHub config directory, fixed argument allowlists/deadlines, and approval reference. It contains no credential. A distinct `hkwtia.pr6.publication-reconciliation.v1` is required after an uncertain mutation; it binds repository, reviewed head, one exact `hkwtia.pr6.publication-operation-outcome.v1` path/hash, and the exact ordered read-only `ls-remote`, `gh pr list`, and `gh pr view` operation templates plus their canonical aggregate needed for that receipt. Each brokered reconciliation read creates one immutable `hkwtia.pr6.publication-read-operation-receipt.v1` binding repository/reviewed head, reconciliation descriptor and uncertain-operation-outcome hashes, exact read kind/argument-vector hash, positive child identity/start/end/native exit, sanitized classification, `mutationAttempted: false`, and its own receipt identity. A launcher-issued immutable `hkwtia.pr6.publication-reconciliation-result.v1` closes every reconciliation shell and binds repository, reviewed head, the stopped-command entry identity and `hkwtia.pr6.command-result-projection.v1` path/hash with `outcome: "NOT_PASSED"` plus exact failure/last-successful-mutation discriminants, the exact underlying `hkwtia.pr6.publication-operation-outcome.v1` path/hash with `status: "OUTCOME_UNCERTAIN"`, reconciliation-descriptor path/hash, the descriptor-bound read-operation-template aggregate, exact ordered reconciliation-read receipt paths/hashes and sanitized classifications plus separately recomputed receipt/classification aggregates, canonical nullable discovered PR number/URL, `status: "NOT_PASSED"`, `mutationAttempted: false`, `retryAttempted: false`, `continuedMutation: false`, and a canonical terminal timestamp. Unknown fields, an unbound read result, or any write-capable operation invalidates it. The reconciliation descriptor and result forbid push/create/close and cannot be substituted for the publication descriptor.
...
- Before every remote-mutating `push`, `pr create`, or `pr close` child is created, the invoker atomically writes one immutable, non-secret `hkwtia.pr6.publication-operation.v1` STARTED receipt outside the worktree. It records a UUID operation ID, operation kind, repository, exact base/head/ref and reviewed SHA, captured PR number when already known, start timestamp, publication-descriptor hash, and hash of the fixed argument vector. A clean zero exit creates a separate immutable `hkwtia.pr6.publication-operation-completion.v1` SUCCEEDED receipt bound to the STARTED hash; for `pr create`, SUCCEEDED additionally requires exactly one canonical stdout URL/number and records that number, otherwise success cannot be claimed. Timeout, cancellation, broken pipe, lost transport, killed helper/descendant, or any exit whose remote effect cannot be proven instead creates a separate immutable `hkwtia.pr6.publication-operation-outcome.v1` with `status: "OUTCOME_UNCERTAIN"`, bound to the exact STARTED path/hash, operation identity/kind/argument hash, repository/base/head/ref/reviewed SHA, sanitized terminal reason, and end timestamp. All three schemas are closed and receipts are never rewritten or appended. The uncertain outcome closes the complete process tree/handles, revokes the publication invokers, prints only its path/hash, and throws before caller code can run another command.
- Authority metadata belongs to the required-command entry, not to a successful receipt. `separateAuthorityPolicy: "REQUIRED"` therefore requires nonempty root `separateAuthorityScope`, `separateAuthorityIssuer`, and `authorizedOperationClass` for every allowed disposition. `LAUNCHED` additionally resolves the unique positive `separate-authority-receipt.v1`; `REFUSED_BEFORE_LAUNCH` and `PROVEN_NOT_APPLICABLE` retain the three root values but require the positive receipt path/hash canonical null. The refusal receipt duplicates the exact expected scope, issuer, and operation class before proving `MISSING_SEPARATE_AUTHORITY`; the applicability receipt binds the same required-command entry. A policy other than `REQUIRED` requires all three root values and every positive-authority receipt field canonical null. Tests cover required launched, required refused, required inapplicable, and unrequired branches, including each scope/issuer/class mismatch.
- Each mutation STARTED receipt also duplicates the executing PUBLICATION `entryIdentitySha256`, `requiredCommandIdentitySha256`, `commandProjectionSha256`, `commandTemplateSha256`, `runtimeSlotValuesSha256`, `resolvedCommandSha256`, the distinct `resolvedArgumentVectorSha256`, verified tool, and execution mode. Its `argumentVectorSha256` equals `resolvedArgumentVectorSha256`, never the full resolved-command hash; operation kind and exact base/head/ref/reviewed-head/captured-PR/publication-descriptor fields must equal that entry's resolved typed slots and the publication input. `publication-operation-outcome.v1` duplicates every one of those fields plus the exact STARTED path/hash. The terminal resolver compares the complete tuple outcome-to-STARTED and STARTED-to-the-one stopped manifest entry; matching only repository, reviewed head, operation ID, or the wrong hash domain is invalid. Wrong entry, required-command identity, tool, mode, kind, vector, full command, base, head, ref, captured PR, descriptor, or STARTED path/hash is a negative contract case.
- The root `commandClass` closed union is extended with `RECONCILIATION_READ`. This specific class supersedes the earlier generic class restrictions only for descriptor-bound read entries in the PUBLICATION partition: it requires `separateAuthorityPolicy: "REQUIRED"`, conditional applicability, exact reconciliation template ordinal/kind/vector fields, and allowed tuples for `LAUNCHED` with PASSED/zero or NOT_PASSED/nonzero, `REFUSED_BEFORE_LAUNCH` with NOT_PASSED/no native exit, and `PROVEN_NOT_APPLICABLE` with NOT_APPLICABLE/no native exit. Thus one fixed matrix entry can be launched in one run, refused before launch in another, or form the inapplicable suffix in another. `RECONCILIATION_READ` is illegal in every other partition; no other class gains a disposition. The root, phase-projection, command-evidence, result, source, cleanup, authority/refusal/applicability, manifest, and terminal resolvers all enforce this subtype, and tests reject a GATE/PUBLICATION substitution or any missing/extra disposition.
- PUBLICATION command-evidence entries expose closed projection fields needed by the terminal joins. Every mutation or reconciliation-read entry requires a distinct `resolvedArgumentVectorSha256` in addition to `resolvedCommandSha256`. A mutation entry also requires `operationKind`, canonical nullable `capturedPrNumber`, and `publicationDescriptorSha256`; a `RECONCILIATION_READ` entry also requires `reconciliationTemplateOrdinal` and `reconciliationReadKind`, with `resolvedArgumentVectorSha256` equal to the descriptor template's argument-vector hash. Fields owned by another subtype are canonical null. These are projections of already approved typed slots, not new ambient inputs, and the resolver includes them in the entry identity and full-evidence aggregate.
- Reconciliation schemas use an ordered-prefix union. `publication-reconciliation.v1.readOperationTemplates` is a nonempty closed array of `{ordinal, readKind, argumentVectorSha256, allowedResultTuples}`, with one-based contiguous ordinals and a recomputed full-template aggregate; each unique allowed tuple fixes `launchDisposition`, `nativeExitClass: "ZERO" | "NONZERO" | "NO_NATIVE_EXIT"`, `status`, sanitized classification, and `continuationDisposition: "CONTINUE" | "STOP"`. Actual read receipts form an exact nonempty prefix. The closed `publication-read-operation-receipt.v1` common arm contains and identity-hashes repository/reviewed head; descriptor path/hash; uncertain-outcome path/hash; `commandEntryIdentitySha256`; `requiredCommandIdentitySha256`; `commandRunId`; canonical nullable `commandExecutionHead`; `commandResultProjectionPath`/`commandResultProjectionSha256`; `commandCleanupReceiptPath`/`commandCleanupReceiptSha256`; exact nullable separate-authority/refusal/applicability receipt path/hash fields; ordinal; read kind; argument-vector hash; launch disposition; child identity/start/end/native exit; status; classification; continuation disposition; `mutationAttempted: false`; and its unique receipt identity. Trusted launcher/CI freezes the command result projection and cleanup receipt before this read receipt, so these references are acyclic. `LAUNCHED` requires positive child identity, ordered start/end times, a native exit, no refusal/applicability receipt, and `status: "PASSED"` iff both command evidence and receipt exit zero or `status: "NOT_PASSED"` iff both are nonzero; timeout/cancellation/tree termination is normalized only after cleanup to the same truthful nonzero terminal exit and `NOT_PASSED` in every record. `REFUSED_BEFORE_LAUNCH` requires `status: "NOT_PASSED"`, `continuationDisposition: "STOP"`, canonical-null child/times/native exit/positive-authority/applicability fields, and one exact `launcher-refusal-receipt.v1` proving no child and no mutation. Every receipt tuple must occur exactly once in its descriptor template's allowed table. Each nonfinal actual receipt is PASSED/CONTINUE; the final actual receipt is STOP, whether a successful classification answered the question or the read failed.
- Every remaining descriptor ordinal maps in order to its exact `RECONCILIATION_READ` command entry with `PROVEN_NOT_APPLICABLE` and unique applicability receipt. That receipt adds exact `producingReconciliationReadReceiptPath`/`producingReconciliationReadReceiptSha256` fields and binds them, plus the producing entry/result projection, to the earlier terminal STOP decision; it cannot cite only a free-standing result. `publication-reconciliation-result.v1` and `publication-command-evidence-manifest.v1` both carry exact parallel arrays `reconciliationReadOrdinals`, `reconciliationReadKinds`, `reconciliationReadArgumentVectorSha256s`, `reconciliationReadLaunchDispositions`, `reconciliationReadStatuses`, `reconciliationReadContinuationDispositions`, `reconciliationReadReceiptPaths`, `reconciliationReadReceiptSha256s`, and `reconciliationReadClassifications`, plus `readOperationTemplateAggregateSha256`, `executedReadOperationTemplateAggregateSha256`, and the existing receipt/classification aggregates. They also carry `unexecutedReadTemplateOrdinals`, `unexecutedReadApplicabilityReceiptPaths`, `unexecutedReadApplicabilityReceiptSha256s`, and nullable `unexecutedReadOperationTemplateAggregateSha256`. The resolver recomputes every aggregate; terminal validation requires result/manifest equality, elementwise receipt-to-template and receipt-to-command-result/cleanup equality, exact prefix cardinality, the CONTINUE/STOP rule, and exact suffix applicability closure. A first-read refusal, nonzero exit, or normalized timeout therefore closes truthfully as `NOT_PASSED` with a one-receipt prefix and proved-inapplicable suffix; it cannot disappear, be reported as success, or permit another read. Every non-reconciliation branch requires all these arrays empty and aggregates canonical null.
- These discriminated reconciliation-read rules replace the earlier “positive child identity/start/end/native exit” shorthand for `publication-read-operation-receipt.v1`: positive child/times apply only to its `LAUNCHED` arm, while the `REFUSED_BEFORE_LAUNCH` arm is intentionally a truthful no-child receipt. There is no implicit success-only interpretation.
- The command-evidence dependency graph is a strict one-way issuance DAG and this rule supersedes any earlier shorthand that says a refusal/applicability decision proves its target result/cleanup tuple or that a result projection repeats its cleanup hash. `entryIdentitySha256` is a pre-result logical identity over only the root/required-command/partition ordinal, phase projection, template/typed-slot/resolved-command identities, run identity, and nullable execution head; its identity preimage explicitly excludes every command-source/result/cleanup/authority/refusal/applicability path or hash. Trusted launcher/CI freezes records in this exact topological order: phase projection and entry identity; positive authority receipt when required for a launched child; prelaunch command-source manifest; refusal or applicability decision receipt when that arm applies; command result projection; cleanup receipt; then the command-evidence entry, partition manifest, aggregate, and review package. `hkwtia.pr6.launcher-refusal-receipt.v1` and `hkwtia.pr6.command-applicability-receipt.v1` are closed pre-result decision schemas: both bind repository, entry/required-command identity, command template, typed-slot values, command projection, resolved command and any separately required resolved argument vector, run identity, and nullable execution head. The refusal decision additionally binds the expected authority scope/issuer/operation class, `MISSING_SEPARATE_AUTHORITY`, no child/native exit, and no mutation; the applicability decision binds `NOT_APPLICABLE` plus only already frozen producing-entry/result evidence and, for a reconciliation suffix, the exact earlier terminal-STOP read receipt. Neither decision schema contains its target result-projection or cleanup-receipt path/hash. The later result projection binds the already frozen source plus authority/refusal/applicability hash but contains no cleanup-receipt path/hash. The still-later cleanup receipt binds that result-projection hash and repeats the prior authority/refusal/applicability hash. The final entry/manifest binds all independently addressable records. Closed-schema validation rejects any forbidden future-record field, reciprocal content-hash edge, self-reference, or order violation.
- Uncertainty always ends the publication happy path. A fresh shell may resolve only the separately approved reconciliation descriptor. For push: remote head absent means “not landed,” exact reviewed SHA means “landed,” and any other/multiple result means manual inspection. For create: the exact owner/head open-PR count 0 means “not created,” 1 requires exact `pr view` and is reported as discovered, and greater than 1/ambiguous means manual inspection. For close: exact captured PR state CLOSED or OPEN is reported. Every reconciliation result stops publication as `NOT_PASSED`; no automatic retry, continued publication, duplicate creation, automatic close, or mutation is allowed in that run. Only capability-free evidence-manifest/completion/terminal-attestation closure may follow.
...
- [ ] **Step 4: Run the boundary probes under the verified pre-entry launcher**
    ...
    # RECORD-BARRIER: use the current externally approved kind/mode/source record; after any edit, complete Task 0's Refresh-Pr6ExecutionRecord protocol first.
    & $verifiedNode scripts/run-credential-free-verification.mjs unit -- tests/unit/verified-runtime-bootstrap.test.ts tests/unit/credential-free-verification-boundary.test.ts tests/unit/credential-free-build-boundary.test.ts tests/unit/managed-next-production-boundary.test.ts tests/unit/pr6-publication-continuation.test.ts tests/unit/wisetech-delivery-gates.test.ts

    Expected: PASS. Reproduce the trusted no-profile/noninteractive PowerShell launch, exact preamble bytes, separate framed control endpoint/peer/sequence, replacement environment, pre-field handshake, fake-profile/console/clipboard denial, approved-plan binding, and schema-owned immutable operation-input/script records; exact initial-RED baseline-source closure and structured RED/GREEN contract results without native exit state; install/runtime schemas; captured bootstrap/native blocks; complete PowerShell/Node/npm/Git/GitHub-CLI/helper/whole-`node_modules`/browser closures; binary-safe Git-blob hashing plus the post-commit attested-OID hash handoff; synthetic child-only native Git/`gh` credential delivery and zeroing; `--no-bin-links` with direct regular CLI targets; restricted-token/read-only-mount directory immutability; source/materialization/output graphs; run-owned Vitest cache and Playwright `.last-run.json`; non-inheritable Node capability delivery/zeroing; guarded distDir/generated tsconfig; controlled `next-env.d.ts` and redirected build-info writes; generated/traced closure; full browser isolation; atomic containment; and cleanup. Wrong kind/stale record, ambient profile/secret, preamble/channel/approved-plan drift, fake host/PATH, `.bin` link, source/module Vitest cache, worktree Playwright output, credential/config leakage, late file/directory/DLL/PAK/locale/package creation, DACL/mount relaxation, unexpected source write, root build-info, next-env replacement, loader/config/child drift, or capability/process escape fails before acceptance. Contract cases also prove: a final-head producer cannot be consumed until a fresh projected shell; reviewed PR-body raw/canonical identity or title drift fails before create/final success; POST_CREATE begins with no inherited scalar/function and must resolve its helper; a LAUNCHED entry missing child identity/times/native exit or its required authority receipt fails; required refused/inapplicable entries preserve root authority scope/issuer/class while keeping the positive receipt null, and refusal mismatches fail; result projections and cleanup receipts cannot drift on run/head/child/times/native-exit/authority/refusal/applicability fields; source projections and authority template/slot/scope/issuer/operation fields must match; an uncertain outcome with a different STARTED path, stopped entry, required-command identity, tool/mode, operation kind, vector, base/head/ref, captured PR, or descriptor fails; reconciliation descriptors/results and ordered read arrays/aggregates cannot cross-bind another PUBLICATION manifest; swapped ordinals/read kinds, wrong argument vectors, skipped prefix members, extra receipts, or a read after NOT_PASSED fail; first-read refusal, nonzero, and normalized timeout each retain one truthful NOT_PASSED receipt and exact inapplicable-suffix proof; entry/result/source/cleanup/authority arrays cannot omit, duplicate, or reorder evidence; the sole INITIAL_RED source union cannot masquerade as an execution record; field p75 rejects null, NaN, either infinity, negative, and insufficient values; and PASSED versus PRE_MUTATION/RECONCILED_UNCERTAINTY/POST_MUTATION_VALIDATION completion records close only their exact final-state or reconciliation branch.

    Additional negative matrix cases require `RECONCILIATION_READ`—and reject GATE/PUBLICATION—for every descriptor ordinal; exercise LAUNCHED, required-authority REFUSED_BEFORE_LAUNCH, and conditional PROVEN_NOT_APPLICABLE on the same fixed entry; reject a missing closed read-receipt command/result/cleanup/run/head field; distinguish `resolvedCommandSha256` from `resolvedArgumentVectorSha256`; and reject any read-receipt-versus-entry/result/cleanup mismatch in disposition, child ID, timestamps, native exit, outcome/status, authority, refusal, applicability, result, or cleanup identity. Suffix tests also reject an applicability receipt whose producing read-receipt path/hash is not the exact terminal STOP receipt. Dedicated topology tests freeze projection/entry → authority/source/decision → result → cleanup → manifest in that order; reject a result that names its cleanup, reject a refusal/applicability decision that names its target result or cleanup, reject a cleanup that omits or misbinds its earlier result, and reject every self-reference, reciprocal content-hash edge, forbidden future-record field, or out-of-order issuance attempt.

- [ ] **Step 5: Commit the execution prerequisite**

    & $verifiedGit add -- ':(literal)package.json' ':(literal)next.config.ts' ':(literal)vitest.config.ts' ':(literal)playwright.config.ts' ':(literal)README.md' ':(literal)AGENTS.md' ':(literal)CLAUDE.md' ':(literal).github/pull_request_template.md' ':(literal)docs/integration/wisetech-delivery-gates.md' ':(literal)tests/unit/wisetech-delivery-gates.test.ts' ':(literal)scripts/generate-pr6-execution-candidate.ps1' ':(literal)scripts/resolve-verified-runtime.ps1' ':(literal)scripts/invoke-verified-tool.ps1' ':(literal)scripts/pr6-native-process-boundary.ps1' ':(literal)scripts/managed-process-guardian.ps1' ':(literal)scripts/pr6-publication-state-assertions.ps1' ':(literal)scripts/managed-runtime-environment.mjs' ':(literal)scripts/managed-runtime-environment.d.mts' ':(literal)scripts/run-credential-free-verification.mjs' ':(literal)scripts/run-managed-playwright.mjs' ':(literal)scripts/run-managed-next.mjs' ':(literal)tests/contracts/pr6-unsafe-bootstrap.red.ps1' ':(literal)tests/fixtures/powershell-profile-probe.ps1' ':(literal)tests/fixtures/credential-free-process-env-probe.mjs' ':(literal)tests/fixtures/managed-next-env-probe.mjs' ':(literal)tests/fixtures/managed-process-creation-window-probe.mjs' ':(literal)tests/fixtures/materialization-late-file-probe.mjs' ':(literal)tests/unit/verified-runtime-bootstrap.test.ts' ':(literal)tests/unit/credential-free-verification-boundary.test.ts' ':(literal)tests/unit/credential-free-build-boundary.test.ts' ':(literal)tests/unit/managed-next-production-boundary.test.ts' ':(literal)tests/unit/pr6-publication-continuation.test.ts'
...
- [ ] **Step 6: Run the exact authorized-Preview Lighthouse and field-performance gates**
    ...
    `lighthouse-runner.test.ts` uses injected fetch/filesystem/hash/spawn/UUID seams to prove byte-bound guard-before-network, GET-only Vercel proof, exact local-record/descriptor/provider head equality, deployment/project/team/READY/Preview/generated-origin binding, rejection when the same provider-evidence SHA-256 is replayed with a different Preview-approval hash, deployment ID, project ID, nullable team ID, or canonical generated URL, strict alias/denylist behavior, parent-only capability handling, unique OS-temp ownership, complete LHCI closure, and staged materialized invocation. It covers the entire Chrome distribution manifest/root, executable/version, full-closure hold/revalidation before each launch and after exit, replacement of DLL/PAK/locale/snapshot/helper files, operational allowlists, rebuilt PATH, temp/profile confinement, exit propagation, three-state attempt receipts, all three completion booleans, conditional provider/run/retained evidence, retention-manifest failure, independent field-INP status/evidence, the two-input overall-gate truth table, and success/failure retention. Resolver contract cases present the same provider-sanitized-evidence path/SHA-256 with a mismatched Preview-approval hash, deployment ID, project ID, nullable team ID, or canonical generated URL and require both `hkwtia.pr6.verification-authoring-input.v1` and `hkwtia.pr6.lighthouse-commit-verification.v1` validation to fail. They also prove the `UNAUTHORIZED` union rejects every Preview identity field, while every authorized attempt resolves the exact approval descriptor and binds the attempt receipt to it even when failure occurs before provider proof. Both authoring and commit resolver cases reject `PASSED` with null, NaN, positive/negative infinity, negative p75, or insufficient samples, reject a non-null p75 when samples are insufficient, and accept only a sufficient finite nonnegative p75 at or below 200. `lighthouse-process-boundary.test.ts` verifies the complete replacement child map includes only approved non-secret markers plus the Chrome distribution-manifest hash. Config/declaration/real-loader tests prove exact routes, thresholds, filesystem output, full-browser identity, no local/public upload, and runtime/declaration parity. None copies implementation logic or touches real provider/filesystem/Chrome.
    ...
    Without Preview/Lighthouse approval, do not launch `collect`; the required-command matrix entry still produces one immutable trusted-launcher refusal receipt with `NOT_PASSED` and no child process. Retain the checkpoint, set the attempt and field statuses to `UNAUTHORIZED`, all three completion booleans false, and require every Preview approval/target field plus attempt/provider/run/retained/field evidence field canonical null. For every separately authorized attempt, resolve the exact Preview approval descriptor first; the trusted launcher issues an immutable sanitized attempt receipt duplicating its approval SHA-256, deployment/project/nullable-team/generated URL, last completed phase, and result/failure even when provider proof never completes. A failure at preflight, provider proof, Chrome, collection, assertion, or retention is `AUTHORIZED_FAILED`. Preserve provider evidence iff provider proof completed; bind/report the canonical external run root iff it was created; and require a retained-artifact manifest only iff manifest finalization completed. Thus a retention-manifest failure is truthfully recorded with `runCreated: true`, `retainedManifestCompleted: false`, the run root in the attempt receipt, and no fabricated manifest. Never discard failure evidence merely because the overall gate did not pass.
...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    End every suite/provider shell. Resolve the independently approved plan-derived `hkwtia.pr6.required-command-matrix.v2` root that existed before Task 0, but close only evidence that can exist now: the complete `TASK0` partition through its own manifest/zero-finding package and the complete `PRE_AUTHORING` partition through `hkwtia.pr6.suite-evidence-manifest.v3`. The trusted launcher/CI freezes every ordered Task 1-11 and Task 12-through-Step-6 entry, including unique no-child refusal/applicability and cleanup receipts, and issues exact `hkwtia.pr6.verification-authoring-input.v1` binding the root plus both partition counts/aggregates/manifests. The later `POST_COMMIT_REVIEW`, `PUBLICATION`, and `TERMINAL_ATTESTATION` partitions remain fixed templates but are deliberately not compared with this pre-authoring suite; dependency-ready phase projections resolve their typed runtime slots only before those later commands. The resolver requires exact partition count/order/identity/template/projection/resolved-vector/result equality, so no command can be omitted because it did not launch and no completed-receipt-derived inventory can redefine the requirement. The manifest proves at least one unique successful RED and GREEN for each Task 1-11, permits their exact GATE/CONTROL entries, binds one designated final review package per task to every ordered entry/source/result/cleanup projection, and preserves the complete nonempty pending Task 12 GATE/CONTROL/AGGREGATE set with at least one AGGREGATE. Lighthouse attempt, completion, field-INP, and overall-gate state remain separate: an authorized failure retains every phase that actually completed without changing the overall gate from `NOT_PASSED`. Start Step 7 in a fresh capability-free shell, rerun Task 0's core bootstrap, and resolve every value from that operation record:
    ...
    $requiredCommandMatrixPath = [string]$verificationAuthoringInput.requiredCommandMatrixPath
    $requiredCommandMatrixSha256 = [string]$verificationAuthoringInput.requiredCommandMatrixSha256
    $requiredCommandMatrixResults = @(& $runtimeInputRecordResolver -Path $requiredCommandMatrixPath -Sha256 $requiredCommandMatrixSha256 -ExpectedSchema "hkwtia.pr6.required-command-matrix.v2")
    if ($requiredCommandMatrixResults.Count -ne 1) { throw "PR6_REQUIRED_COMMAND_MATRIX_COUNT_INVALID" }
    $requiredCommandMatrix = $requiredCommandMatrixResults[0]
    $requiredCommandRootEntries = @($requiredCommandMatrix.entries)
    $requiredCommandPartitions = @($requiredCommandMatrix.partitions)
    $expectedPartitionNames = @("TASK0", "PRE_AUTHORING", "POST_COMMIT_REVIEW", "PUBLICATION", "TERMINAL_ATTESTATION")
    if ($requiredCommandPartitions.Count -ne $expectedPartitionNames.Count) { throw "PR6_REQUIRED_COMMAND_PARTITION_COUNT_INVALID" }
    for ($partitionIndex = 0; $partitionIndex -lt $expectedPartitionNames.Count; $partitionIndex += 1) {
      if ([string]$requiredCommandPartitions[$partitionIndex].name -cne $expectedPartitionNames[$partitionIndex] -or [int]$requiredCommandPartitions[$partitionIndex].entryCount -lt 1 -or [string]$requiredCommandPartitions[$partitionIndex].entryAggregateSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_REQUIRED_COMMAND_PARTITION_ORDER_OR_IDENTITY_INVALID" }
    }
    $requiredCommandMatrixRootEntryAggregateSha256 = [string]$requiredCommandMatrix.rootEntryAggregateSha256
    if ([string]$requiredCommandMatrix.repository -cne [string]$task12Target.repository -or [string]$requiredCommandMatrix.approvedPlanCommit -cne [string]$verificationAuthoringInput.approvedPlanCommit -or [int]$requiredCommandMatrix.rootEntryCount -ne $requiredCommandRootEntries.Count -or [int]$verificationAuthoringInput.requiredCommandMatrixRootEntryCount -ne $requiredCommandRootEntries.Count -or $requiredCommandRootEntries.Count -lt 5 -or $requiredCommandMatrixRootEntryAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$verificationAuthoringInput.requiredCommandMatrixRootEntryAggregateSha256 -cne $requiredCommandMatrixRootEntryAggregateSha256) { throw "PR6_REQUIRED_COMMAND_MATRIX_ROOT_BINDING_INVALID" }

    $task0Partition = $requiredCommandPartitions[0]
    $preAuthoringPartition = $requiredCommandPartitions[1]
    $requiredCommandEntries = @($requiredCommandRootEntries | Where-Object { [string]$_.partition -ceq "PRE_AUTHORING" })
    $requiredCommandPartitionEntryAggregateSha256 = [string]$preAuthoringPartition.entryAggregateSha256
    if ([int]$preAuthoringPartition.entryCount -ne $requiredCommandEntries.Count -or [int]$verificationAuthoringInput.requiredCommandPartitionEntryCount -ne $requiredCommandEntries.Count -or [string]$verificationAuthoringInput.requiredCommandPartitionEntryAggregateSha256 -cne $requiredCommandPartitionEntryAggregateSha256) { throw "PR6_REQUIRED_PRE_AUTHORING_PARTITION_BINDING_INVALID" }

    $task0ManifestResults = @(& $runtimeInputRecordResolver -Path ([string]$verificationAuthoringInput.task0CommandEvidenceManifestPath) -Sha256 ([string]$verificationAuthoringInput.task0CommandEvidenceManifestSha256) -ExpectedSchema "hkwtia.pr6.task0-command-evidence-manifest.v2")
    $task0ReviewResults = @(& $runtimeInputRecordResolver -Path ([string]$verificationAuthoringInput.task0ReviewPackagePath) -Sha256 ([string]$verificationAuthoringInput.task0ReviewPackageSha256) -ExpectedSchema "hkwtia.pr6.task-review-package.v3")
    if ($task0ManifestResults.Count -ne 1 -or $task0ReviewResults.Count -ne 1) { throw "PR6_TASK0_EVIDENCE_OR_REVIEW_COUNT_INVALID" }
    $task0Manifest = $task0ManifestResults[0]
    $task0ReviewPackage = $task0ReviewResults[0]
    $task0Entries = @($task0Manifest.entries)
    $task0InitialRedEntries = @($task0Entries | Where-Object { [string]$_.commandClass -ceq "INITIAL_RED" })
    if ($task0InitialRedEntries.Count -ne 1) { throw "PR6_TASK0_INITIAL_RED_CARDINALITY_INVALID" }
    $task0InitialRedEntry = $task0InitialRedEntries[0]
    if ([string]$task0InitialRedEntry.task -cne "0" -or [string]$task0InitialRedEntry.commandSourceKind -cne "INITIAL_RED" -or [string]$task0InitialRedEntry.launchDisposition -cne "EVALUATED_IN_LAUNCH_SHELL" -or [string]$task0InitialRedEntry.outcome -cne "EXPECTED_RED" -or $null -ne $task0InitialRedEntry.nativeExitCode -or $null -ne $task0InitialRedEntry.executionHead -or $null -ne $task0InitialRedEntry.childProcessId -or $null -ne $task0InitialRedEntry.childStartedAt -or $null -ne $task0InitialRedEntry.childEndedAt -or [string]::IsNullOrWhiteSpace([string]$task0InitialRedEntry.redCause)) { throw "PR6_TASK0_INITIAL_RED_TUPLE_INVALID" }
    if ([string]$task0Manifest.repository -cne [string]$task12Target.repository -or [string]$task0Manifest.partition -cne "TASK0" -or [string]$task0Manifest.requiredCommandMatrixPath -cne $requiredCommandMatrixPath -or [string]$task0Manifest.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$task0Manifest.entryCount -ne [int]$task0Partition.entryCount -or $task0Entries.Count -ne [int]$task0Partition.entryCount -or [string]$task0Manifest.entryAggregateSha256 -cne [string]$task0Partition.entryAggregateSha256) { throw "PR6_TASK0_PARTITION_MANIFEST_BINDING_INVALID" }
    $task0ReviewedEntryIdentitySha256s = @($task0ReviewPackage.entryIdentitySha256s)
    $task0ReviewedSourceSha256s = @($task0ReviewPackage.commandSourceManifestSha256s)
    $task0ReviewedResultSha256s = @($task0ReviewPackage.resultProjectionSha256s)
    $task0ReviewedCleanupSha256s = @($task0ReviewPackage.cleanupReceiptSha256s)
    $task0ReviewedAuthoritySha256s = @($task0ReviewPackage.separateAuthorityReceiptSha256s)
    $task0ReviewedRefusalSha256s = @($task0ReviewPackage.launcherRefusalReceiptSha256s)
    $task0ReviewedApplicabilitySha256s = @($task0ReviewPackage.applicabilityReceiptSha256s)
    if ([string]$task0ReviewPackage.task -cne "0" -or [string]$task0ReviewPackage.partition -cne "TASK0" -or [string]$task0ReviewPackage.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [string]$task0ReviewPackage.fullEntryEvidenceAggregateSha256 -cne [string]$task0Manifest.fullEntryEvidenceAggregateSha256 -or [int]$task0ReviewPackage.findings.critical -ne 0 -or [int]$task0ReviewPackage.findings.important -ne 0 -or [int]$task0ReviewPackage.findings.minor -ne 0 -or $task0ReviewedEntryIdentitySha256s.Count -ne $task0Entries.Count -or $task0ReviewedSourceSha256s.Count -ne $task0Entries.Count -or $task0ReviewedResultSha256s.Count -ne $task0Entries.Count -or $task0ReviewedCleanupSha256s.Count -ne $task0Entries.Count -or $task0ReviewedAuthoritySha256s.Count -ne $task0Entries.Count -or $task0ReviewedRefusalSha256s.Count -ne $task0Entries.Count -or $task0ReviewedApplicabilitySha256s.Count -ne $task0Entries.Count) { throw "PR6_TASK0_REVIEW_PACKAGE_BINDING_INVALID" }
    for ($task0EntryIndex = 0; $task0EntryIndex -lt $task0Entries.Count; $task0EntryIndex += 1) {
      if ([string]$task0ReviewedEntryIdentitySha256s[$task0EntryIndex] -cne [string]$task0Entries[$task0EntryIndex].entryIdentitySha256 -or [string]$task0ReviewedSourceSha256s[$task0EntryIndex] -cne [string]$task0Entries[$task0EntryIndex].commandSourceManifestSha256 -or [string]$task0ReviewedResultSha256s[$task0EntryIndex] -cne [string]$task0Entries[$task0EntryIndex].resultProjectionSha256 -or [string]$task0ReviewedCleanupSha256s[$task0EntryIndex] -cne [string]$task0Entries[$task0EntryIndex].cleanupReceiptSha256 -or -not [Object]::Equals($task0ReviewedAuthoritySha256s[$task0EntryIndex], $task0Entries[$task0EntryIndex].separateAuthorityReceiptSha256) -or -not [Object]::Equals($task0ReviewedRefusalSha256s[$task0EntryIndex], $task0Entries[$task0EntryIndex].launcherRefusalReceiptSha256) -or -not [Object]::Equals($task0ReviewedApplicabilitySha256s[$task0EntryIndex], $task0Entries[$task0EntryIndex].applicabilityReceiptSha256)) { throw "PR6_TASK0_REVIEW_ORDERED_EVIDENCE_MISMATCH" }
    }
    ...
    $suiteEvidenceManifests = @(& $runtimeInputRecordResolver -Path $suiteEvidenceManifestPath -Sha256 $suiteEvidenceManifestSha256 -ExpectedSchema "hkwtia.pr6.suite-evidence-manifest.v3")
    ...
    if ([string]$suiteEvidenceManifest.repository -cne [string]$task12Target.repository -or [string]$suiteEvidenceManifest.authoringHead -cne $preTask12Head -or [string]$suiteEvidenceManifest.approvedPlanCommit -cne [string]$verificationAuthoringInput.approvedPlanCommit -or [string]$suiteEvidenceManifest.partition -cne "PRE_AUTHORING" -or [string]$suiteEvidenceManifest.requiredCommandMatrixPath -cne $requiredCommandMatrixPath -or [string]$suiteEvidenceManifest.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$suiteEvidenceManifest.requiredCommandMatrixRootEntryCount -ne $requiredCommandRootEntries.Count -or [string]$suiteEvidenceManifest.requiredCommandMatrixRootEntryAggregateSha256 -cne $requiredCommandMatrixRootEntryAggregateSha256 -or [int]$suiteEvidenceManifest.requiredCommandPartitionEntryCount -ne $requiredCommandEntries.Count -or [string]$suiteEvidenceManifest.requiredCommandPartitionEntryAggregateSha256 -cne $requiredCommandPartitionEntryAggregateSha256 -or [int]$suiteEvidenceManifest.commandProjectionEntryCount -ne $requiredCommandEntries.Count -or [string]$suiteEvidenceManifest.commandProjectionAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEvidenceManifest.lighthouseAttemptStatus -cne $lighthouseAttemptStatus -or [bool]$suiteEvidenceManifest.providerProofCompleted -ne $providerProofCompleted -or [bool]$suiteEvidenceManifest.runCreated -ne $runCreated -or [bool]$suiteEvidenceManifest.retainedManifestCompleted -ne $retainedManifestCompleted -or [string]$suiteEvidenceManifest.fieldInpStatus -cne $fieldInpStatus -or [string]$suiteEvidenceManifest.gateStatus -cne $lighthouseGateStatus -or $suiteEntries.Count -ne $requiredCommandEntries.Count) { throw "PR6_SUITE_EVIDENCE_MANIFEST_BINDING_INVALID" }

    $cleanupReceipts = @()
    $commandSourceManifests = @()
    $resultProjections = @()
    $separateAuthorityReceipts = @()
    $launcherRefusalReceipts = @()
    $applicabilityReceipts = @()
    $taskReviewPackages = @()
    $pendingTask12EntryCount = 0
    $pendingTask12AggregateCount = 0
    $pendingTask12EntryIdentityHashes = @()
    $pendingTask12SourceManifestHashes = @()
    $pendingTask12ResultProjectionHashes = @()
    $pendingTask12CleanupReceiptHashes = @()
    $pendingTask12AuthorityReceiptHashes = @()
    $pendingTask12RefusalReceiptHashes = @()
    $pendingTask12ApplicabilityReceiptHashes = @()
    $requiredLaunchedAuthorityCount = 0
    $seenSuiteEntryIdentities = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenSourceManifestIdentities = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenSourceManifestHashes = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenCleanupReceiptIdentities = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenCleanupReceiptHashes = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenResultProjectionIdentities = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenResultProjectionHashes = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenAuthorityReceiptIdentities = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenAuthorityReceiptHashes = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenRefusalReceiptIdentities = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenRefusalReceiptHashes = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenApplicabilityReceiptIdentities = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $seenApplicabilityReceiptHashes = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $priorSuiteResultsByEntryIdentity = @{}
    $taskPhaseCoverage = @{}
    $taskEntriesByTask = @{}
    foreach ($taskNumber in 1..11) {
      $taskKey = [string]$taskNumber
      $taskPhaseCoverage[$taskKey] = @{ RED = 0; GREEN = 0 }
      $taskEntriesByTask[$taskKey] = @()
    }
    ...
    for ($entryIndex = 0; $entryIndex -lt $suiteEntryCount; $entryIndex += 1) {
      ...
      $requiredCommandEntry = $requiredCommandEntries[$entryIndex]
      ...
      if ([string]$suiteEntry.requiredCommandIdentitySha256 -cne [string]$requiredCommandEntry.requiredCommandIdentitySha256 -or [string]$suiteEntry.requiredCommandIdentitySha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_SUITE_REQUIRED_COMMAND_IDENTITY_MISMATCH" }
      foreach ($matrixField in @("task", "step", "partition", "commandClass", "tool", "mode", "commandTemplateSha256", "runtimeSlotSchemaSha256")) {
        if ([string]$suiteEntry.$matrixField -cne [string]$requiredCommandEntry.$matrixField) { throw "PR6_SUITE_REQUIRED_COMMAND_MATRIX_MISMATCH" }
      }
      if ([string]$suiteEntry.partition -cne "PRE_AUTHORING" -or [string]$suiteEntry.commandTemplateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.runtimeSlotValuesSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.resolvedCommandSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]::IsNullOrWhiteSpace([string]$suiteEntry.resultProjectionPath) -or [string]$suiteEntry.resultProjectionSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_SUITE_COMMAND_PROJECTION_IDENTITY_INVALID" }

      $projectionResults = @(& $runtimeInputRecordResolver -Path ([string]$suiteEntry.commandProjectionPath) -Sha256 ([string]$suiteEntry.commandProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-phase-projection.v1")
      if ($projectionResults.Count -ne 1) { throw "PR6_COMMAND_PHASE_PROJECTION_COUNT_INVALID" }
      $projectionEntryMatches = @(@($projectionResults[0].entries) | Where-Object { [string]$_.requiredCommandIdentitySha256 -ceq [string]$suiteEntry.requiredCommandIdentitySha256 })
      if ($projectionEntryMatches.Count -ne 1) { throw "PR6_COMMAND_PHASE_PROJECTION_ENTRY_COUNT_INVALID" }
      $projectionEntry = $projectionEntryMatches[0]
      if ([string]$projectionResults[0].requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [string]$projectionResults[0].partition -cne "PRE_AUTHORING" -or [string]$projectionEntry.commandTemplateSha256 -cne [string]$suiteEntry.commandTemplateSha256 -or [string]$projectionEntry.runtimeSlotValuesSha256 -cne [string]$suiteEntry.runtimeSlotValuesSha256 -or [string]$projectionEntry.resolvedCommandSha256 -cne [string]$suiteEntry.resolvedCommandSha256) { throw "PR6_COMMAND_PHASE_PROJECTION_BINDING_INVALID" }

      $launchDisposition = [string]$suiteEntry.launchDisposition
      if ($launchDisposition -ceq "LAUNCHED") {
        if ($null -eq $suiteEntry.nativeExitCode -or [long]$suiteEntry.childProcessId -le 0 -or $null -eq $suiteEntry.childStartedAt -or $null -eq $suiteEntry.childEndedAt -or [DateTimeOffset]$suiteEntry.childEndedAt -lt [DateTimeOffset]$suiteEntry.childStartedAt -or $null -ne $suiteEntry.launcherRefusalReceiptPath -or $null -ne $suiteEntry.launcherRefusalReceiptSha256 -or $null -ne $suiteEntry.applicabilityReceiptPath -or $null -ne $suiteEntry.applicabilityReceiptSha256) { throw "PR6_SUITE_LAUNCHED_DISPOSITION_INVALID" }
        if ([string]$requiredCommandEntry.separateAuthorityPolicy -ceq "REQUIRED") {
          if ([string]::IsNullOrWhiteSpace([string]$suiteEntry.separateAuthorityReceiptPath) -or [string]$suiteEntry.separateAuthorityReceiptSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_SUITE_LAUNCHED_AUTHORITY_RECEIPT_REQUIRED" }
          $requiredLaunchedAuthorityCount += 1
        } elseif ($null -ne $suiteEntry.separateAuthorityReceiptPath -or $null -ne $suiteEntry.separateAuthorityReceiptSha256) {
          throw "PR6_SUITE_UNREQUIRED_AUTHORITY_RECEIPT_PRESENT"
        }
        $nativeExitClass = if ([int]$suiteEntry.nativeExitCode -eq 0) { "ZERO" } else { "NONZERO" }
      } elseif ($launchDisposition -ceq "EVALUATED_IN_LAUNCH_SHELL") {
        if ([string]$requiredCommandEntry.executionMode -cne "IN_PROCESS" -or [string]$suiteEntry.commandClass -cnotin @("CONTROL", "AGGREGATE", "REVIEW") -or $null -ne $suiteEntry.nativeExitCode -or $null -ne $suiteEntry.childProcessId -or $null -ne $suiteEntry.childStartedAt -or $null -ne $suiteEntry.childEndedAt -or $null -ne $suiteEntry.separateAuthorityReceiptPath -or $null -ne $suiteEntry.separateAuthorityReceiptSha256 -or $null -ne $suiteEntry.launcherRefusalReceiptPath -or $null -ne $suiteEntry.launcherRefusalReceiptSha256 -or $null -ne $suiteEntry.applicabilityReceiptPath -or $null -ne $suiteEntry.applicabilityReceiptSha256) { throw "PR6_SUITE_IN_PROCESS_DISPOSITION_INVALID" }
        $nativeExitClass = "NO_NATIVE_EXIT"
      } elseif ($launchDisposition -ceq "REFUSED_BEFORE_LAUNCH") {
        if ([string]$suiteEntry.commandClass -cne "GATE" -or [string]$requiredCommandEntry.separateAuthorityPolicy -cne "REQUIRED" -or [string]$suiteEntry.outcome -cne "NOT_PASSED" -or $null -ne $suiteEntry.nativeExitCode -or $null -ne $suiteEntry.childProcessId -or $null -ne $suiteEntry.childStartedAt -or $null -ne $suiteEntry.childEndedAt -or $null -ne $suiteEntry.applicabilityReceiptPath -or $null -ne $suiteEntry.applicabilityReceiptSha256 -or $null -ne $suiteEntry.separateAuthorityReceiptPath -or $null -ne $suiteEntry.separateAuthorityReceiptSha256) { throw "PR6_SUITE_REFUSED_DISPOSITION_INVALID" }
        $nativeExitClass = "NO_NATIVE_EXIT"
      } elseif ($launchDisposition -ceq "PROVEN_NOT_APPLICABLE") {
        if ([string]$suiteEntry.commandClass -cnotin @("CONTROL", "PUBLICATION") -or [bool]$requiredCommandEntry.conditionalApplicability -ne $true -or [string]$suiteEntry.outcome -cne "NOT_APPLICABLE" -or $null -ne $suiteEntry.nativeExitCode -or $null -ne $suiteEntry.childProcessId -or $null -ne $suiteEntry.childStartedAt -or $null -ne $suiteEntry.childEndedAt -or $null -ne $suiteEntry.launcherRefusalReceiptPath -or $null -ne $suiteEntry.launcherRefusalReceiptSha256 -or $null -ne $suiteEntry.separateAuthorityReceiptPath -or $null -ne $suiteEntry.separateAuthorityReceiptSha256) { throw "PR6_SUITE_NOT_APPLICABLE_DISPOSITION_INVALID" }
        $nativeExitClass = "NO_NATIVE_EXIT"
      } else {
        throw "PR6_SUITE_LAUNCH_DISPOSITION_INVALID"
      }
      $allowedResultMatches = @(@($requiredCommandEntry.allowedResults) | Where-Object { [string]$_.launchDisposition -ceq $launchDisposition -and [string]$_.outcome -ceq [string]$suiteEntry.outcome -and [string]$_.nativeExitClass -ceq $nativeExitClass })
      if ($allowedResultMatches.Count -ne 1) { throw "PR6_SUITE_REQUIRED_COMMAND_RESULT_TUPLE_INVALID" }

      if ([string]$suiteEntry.commandClass -ceq "RED") {
        if ($launchDisposition -cne "LAUNCHED" -or [string]$suiteEntry.outcome -cne "EXPECTED_RED" -or [int]$suiteEntry.nativeExitCode -eq 0 -or [string]::IsNullOrWhiteSpace([string]$suiteEntry.redCause)) { throw "PR6_SUITE_RED_EVIDENCE_INVALID" }
      } elseif ([string]$suiteEntry.commandClass -ceq "GREEN") {
        if ($launchDisposition -cne "LAUNCHED" -or [string]$suiteEntry.outcome -cne "PASSED" -or [int]$suiteEntry.nativeExitCode -ne 0 -or $null -ne $suiteEntry.redCause) { throw "PR6_SUITE_GREEN_EVIDENCE_INVALID" }
      } elseif ([string]$suiteEntry.commandClass -ceq "GATE") {
        if ($null -ne $suiteEntry.redCause -or ($launchDisposition -ceq "LAUNCHED" -and (($suiteEntry.outcome -ceq "PASSED" -and [int]$suiteEntry.nativeExitCode -ne 0) -or ($suiteEntry.outcome -ceq "NOT_PASSED" -and [int]$suiteEntry.nativeExitCode -eq 0)))) { throw "PR6_SUITE_GATE_EVIDENCE_INVALID" }
      } elseif ([string]$suiteEntry.commandClass -cnotin @("CONTROL", "AGGREGATE")) {
        throw "PR6_SUITE_PRE_AUTHORING_COMMAND_CLASS_INVALID"
      }
      ...
      $sourceResults = @(& $runtimeInputRecordResolver -Path ([string]$suiteEntry.commandSourceManifestPath) -Sha256 ([string]$suiteEntry.commandSourceManifestSha256) -ExpectedSchema "hkwtia.pr6.command-source-manifest.v1")
      $resultResults = @(& $runtimeInputRecordResolver -Path ([string]$suiteEntry.resultProjectionPath) -Sha256 ([string]$suiteEntry.resultProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-result-projection.v1")
      $receiptResults = @(& $runtimeInputRecordResolver -Path ([string]$suiteEntry.cleanupReceiptPath) -Sha256 ([string]$suiteEntry.cleanupReceiptSha256) -ExpectedSchema "hkwtia.pr6.cleanup-receipt.v3")
      if ($sourceResults.Count -ne 1 -or $resultResults.Count -ne 1 -or $receiptResults.Count -ne 1) { throw "PR6_SUITE_REFERENCED_EVIDENCE_COUNT_INVALID" }
      $sourceManifest = $sourceResults[0]
      $resultProjection = $resultResults[0]
      $receipt = $receiptResults[0]
      if ([string]$resultProjection.projectionIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $seenResultProjectionIdentities.Add([string]$resultProjection.projectionIdentitySha256) -or -not $seenResultProjectionHashes.Add([string]$suiteEntry.resultProjectionSha256)) { throw "PR6_SUITE_RESULT_PROJECTION_REUSED_OR_INVALID" }
      if ([string]$sourceManifest.manifestIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$receipt.receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.commandSourceManifestSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.cleanupReceiptSha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $seenSourceManifestIdentities.Add([string]$sourceManifest.manifestIdentitySha256) -or -not $seenSourceManifestHashes.Add([string]$suiteEntry.commandSourceManifestSha256) -or -not $seenCleanupReceiptIdentities.Add([string]$receipt.receiptIdentitySha256) -or -not $seenCleanupReceiptHashes.Add([string]$suiteEntry.cleanupReceiptSha256)) { throw "PR6_SUITE_SOURCE_OR_CLEANUP_RECEIPT_REUSED" }
      if ([string]$resultProjection.repository -cne [string]$task12Target.repository -or [string]$resultProjection.entryIdentitySha256 -cne [string]$suiteEntry.entryIdentitySha256 -or [string]$resultProjection.requiredCommandIdentitySha256 -cne [string]$suiteEntry.requiredCommandIdentitySha256 -or [string]$resultProjection.runId -cne [string]$suiteEntry.runId -or -not [Object]::Equals($resultProjection.executionHead, $suiteEntry.executionHead) -or [string]$resultProjection.launchDisposition -cne $launchDisposition -or -not [Object]::Equals($resultProjection.childProcessId, $suiteEntry.childProcessId) -or -not [Object]::Equals($resultProjection.childStartedAt, $suiteEntry.childStartedAt) -or -not [Object]::Equals($resultProjection.childEndedAt, $suiteEntry.childEndedAt) -or -not [Object]::Equals($resultProjection.nativeExitCode, $suiteEntry.nativeExitCode) -or [string]$resultProjection.outcome -cne [string]$suiteEntry.outcome -or -not [Object]::Equals($resultProjection.redCause, $suiteEntry.redCause) -or [string]$resultProjection.commandSourceManifestSha256 -cne [string]$suiteEntry.commandSourceManifestSha256 -or -not [Object]::Equals($resultProjection.separateAuthorityReceiptSha256, $suiteEntry.separateAuthorityReceiptSha256) -or -not [Object]::Equals($resultProjection.launcherRefusalReceiptSha256, $suiteEntry.launcherRefusalReceiptSha256) -or -not [Object]::Equals($resultProjection.applicabilityReceiptSha256, $suiteEntry.applicabilityReceiptSha256) -or [long]$resultProjection.totalCount -lt 0 -or $null -eq $resultProjection.warningCodes -or $null -eq $resultProjection.skipCodes) { throw "PR6_COMMAND_RESULT_PROJECTION_BINDING_INVALID" }
      if ([string]$suiteEntry.commandSourceKind -cne "EXECUTION" -or [string]$sourceManifest.sourceKind -cne "EXECUTION" -or $null -ne $sourceManifest.initialRedAttestationPath -or $null -ne $sourceManifest.initialRedAttestationSha256 -or $null -ne $sourceManifest.initialRedBaselinePath -or $null -ne $sourceManifest.initialRedBaselineSha256 -or [string]$sourceManifest.repository -cne [string]$task12Target.repository -or [string]$sourceManifest.entryIdentitySha256 -cne [string]$suiteEntry.entryIdentitySha256 -or [string]$sourceManifest.requiredCommandIdentitySha256 -cne [string]$suiteEntry.requiredCommandIdentitySha256 -or [string]$sourceManifest.executionHead -cne [string]$suiteEntry.executionHead -or [string]$sourceManifest.commandTemplateSha256 -cne [string]$suiteEntry.commandTemplateSha256 -or [string]$sourceManifest.runtimeSlotValuesSha256 -cne [string]$suiteEntry.runtimeSlotValuesSha256 -or [string]$sourceManifest.resolvedCommandSha256 -cne [string]$suiteEntry.resolvedCommandSha256 -or [string]$sourceManifest.commandProjectionSha256 -cne [string]$suiteEntry.commandProjectionSha256 -or -not [Object]::Equals($sourceManifest.separateAuthorityReceiptSha256, $suiteEntry.separateAuthorityReceiptSha256)) { throw "PR6_COMMAND_SOURCE_MANIFEST_BINDING_INVALID" }
      if ([string]$receipt.repository -cne [string]$task12Target.repository -or [string]$receipt.entryIdentitySha256 -cne [string]$suiteEntry.entryIdentitySha256 -or [string]$receipt.requiredCommandIdentitySha256 -cne [string]$suiteEntry.requiredCommandIdentitySha256 -or [string]$receipt.task -cne [string]$suiteEntry.task -or [string]$receipt.step -cne [string]$suiteEntry.step -or [string]$receipt.commandClass -cne [string]$suiteEntry.commandClass -or [string]$receipt.launchDisposition -cne $launchDisposition -or [string]$receipt.runId -cne [string]$suiteEntry.runId -or [string]$receipt.executionHead -cne [string]$suiteEntry.executionHead -or [string]$receipt.outcome -cne [string]$suiteEntry.outcome -or -not [Object]::Equals($receipt.childProcessId, $suiteEntry.childProcessId) -or -not [Object]::Equals($receipt.childStartedAt, $suiteEntry.childStartedAt) -or -not [Object]::Equals($receipt.childEndedAt, $suiteEntry.childEndedAt) -or -not [Object]::Equals($receipt.nativeExitCode, $suiteEntry.nativeExitCode) -or [string]$receipt.resultProjectionSha256 -cne [string]$suiteEntry.resultProjectionSha256 -or -not [Object]::Equals($receipt.separateAuthorityReceiptSha256, $suiteEntry.separateAuthorityReceiptSha256) -or -not [Object]::Equals($receipt.launcherRefusalReceiptSha256, $suiteEntry.launcherRefusalReceiptSha256) -or -not [Object]::Equals($receipt.applicabilityReceiptSha256, $suiteEntry.applicabilityReceiptSha256) -or [string]$receipt.evidenceSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_CLEANUP_RECEIPT_BINDING_INVALID" }

      if ($launchDisposition -ceq "LAUNCHED" -and [string]$requiredCommandEntry.separateAuthorityPolicy -ceq "REQUIRED") {
        $authorityResults = @(& $runtimeInputRecordResolver -Path ([string]$suiteEntry.separateAuthorityReceiptPath) -Sha256 ([string]$suiteEntry.separateAuthorityReceiptSha256) -ExpectedSchema "hkwtia.pr6.separate-authority-receipt.v1")
        if ($authorityResults.Count -ne 1) { throw "PR6_SEPARATE_AUTHORITY_RECEIPT_COUNT_INVALID" }
        $authorityReceipt = $authorityResults[0]
        if ([string]$authorityReceipt.receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $seenAuthorityReceiptIdentities.Add([string]$authorityReceipt.receiptIdentitySha256) -or -not $seenAuthorityReceiptHashes.Add([string]$suiteEntry.separateAuthorityReceiptSha256) -or [string]$authorityReceipt.entryIdentitySha256 -cne [string]$suiteEntry.entryIdentitySha256 -or [string]$authorityReceipt.requiredCommandIdentitySha256 -cne [string]$suiteEntry.requiredCommandIdentitySha256 -or [string]$authorityReceipt.commandTemplateSha256 -cne [string]$suiteEntry.commandTemplateSha256 -or [string]$authorityReceipt.runtimeSlotValuesSha256 -cne [string]$suiteEntry.runtimeSlotValuesSha256 -or [string]$authorityReceipt.commandProjectionSha256 -cne [string]$suiteEntry.commandProjectionSha256 -or [string]$authorityReceipt.resolvedCommandSha256 -cne [string]$suiteEntry.resolvedCommandSha256 -or [string]$authorityReceipt.approvalScope -cne [string]$requiredCommandEntry.separateAuthorityScope -or [string]$authorityReceipt.issuer -cne [string]$requiredCommandEntry.separateAuthorityIssuer -or [string]$authorityReceipt.authorizedOperationClass -cne [string]$requiredCommandEntry.authorizedOperationClass -or [string]::IsNullOrWhiteSpace([string]$authorityReceipt.approvalReference) -or [string]::IsNullOrWhiteSpace([string]$authorityReceipt.approvalScope) -or [string]::IsNullOrWhiteSpace([string]$authorityReceipt.issuer) -or [string]::IsNullOrWhiteSpace([string]$authorityReceipt.authorizedOperationClass) -or [DateTimeOffset]$authorityReceipt.expiresAt -le [DateTimeOffset]$authorityReceipt.issuedAt -or [string]$resultProjection.separateAuthorityReceiptSha256 -cne [string]$suiteEntry.separateAuthorityReceiptSha256) { throw "PR6_SEPARATE_AUTHORITY_RECEIPT_BINDING_INVALID" }
        $separateAuthorityReceipts += $authorityReceipt
      } elseif ([string]$requiredCommandEntry.separateAuthorityPolicy -ceq "REQUIRED") {
        if ($launchDisposition -cnotin @("REFUSED_BEFORE_LAUNCH", "PROVEN_NOT_APPLICABLE") -or [string]::IsNullOrWhiteSpace([string]$requiredCommandEntry.separateAuthorityScope) -or [string]::IsNullOrWhiteSpace([string]$requiredCommandEntry.separateAuthorityIssuer) -or [string]::IsNullOrWhiteSpace([string]$requiredCommandEntry.authorizedOperationClass) -or $null -ne $resultProjection.separateAuthorityReceiptSha256) { throw "PR6_REQUIRED_AUTHORITY_DISPOSITION_INVALID" }
      } elseif ($null -ne $requiredCommandEntry.separateAuthorityScope -or $null -ne $requiredCommandEntry.separateAuthorityIssuer -or $null -ne $requiredCommandEntry.authorizedOperationClass -or $null -ne $resultProjection.separateAuthorityReceiptSha256) {
        throw "PR6_RESULT_UNEXPECTED_AUTHORITY_RECEIPT"
      }

      if ($launchDisposition -ceq "REFUSED_BEFORE_LAUNCH") {
        $refusalResults = @(& $runtimeInputRecordResolver -Path ([string]$suiteEntry.launcherRefusalReceiptPath) -Sha256 ([string]$suiteEntry.launcherRefusalReceiptSha256) -ExpectedSchema "hkwtia.pr6.launcher-refusal-receipt.v1")
        if ($refusalResults.Count -ne 1) { throw "PR6_LAUNCHER_REFUSAL_RECEIPT_COUNT_INVALID" }
        $refusalReceipt = $refusalResults[0]
        if ([string]$refusalReceipt.receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.launcherRefusalReceiptSha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $seenRefusalReceiptIdentities.Add([string]$refusalReceipt.receiptIdentitySha256) -or -not $seenRefusalReceiptHashes.Add([string]$suiteEntry.launcherRefusalReceiptSha256) -or [string]$refusalReceipt.repository -cne [string]$task12Target.repository -or [string]$refusalReceipt.entryIdentitySha256 -cne [string]$suiteEntry.entryIdentitySha256 -or [string]$refusalReceipt.requiredCommandIdentitySha256 -cne [string]$suiteEntry.requiredCommandIdentitySha256 -or [string]$refusalReceipt.commandTemplateSha256 -cne [string]$suiteEntry.commandTemplateSha256 -or [string]$refusalReceipt.runtimeSlotValuesSha256 -cne [string]$suiteEntry.runtimeSlotValuesSha256 -or [string]$refusalReceipt.commandProjectionSha256 -cne [string]$suiteEntry.commandProjectionSha256 -or [string]$refusalReceipt.resolvedCommandSha256 -cne [string]$suiteEntry.resolvedCommandSha256 -or [string]$refusalReceipt.runId -cne [string]$suiteEntry.runId -or -not [Object]::Equals($refusalReceipt.executionHead, $suiteEntry.executionHead) -or [string]$refusalReceipt.expectedAuthorityScope -cne [string]$requiredCommandEntry.separateAuthorityScope -or [string]$refusalReceipt.expectedAuthorityIssuer -cne [string]$requiredCommandEntry.separateAuthorityIssuer -or [string]$refusalReceipt.expectedAuthorizedOperationClass -cne [string]$requiredCommandEntry.authorizedOperationClass -or [string]$refusalReceipt.reason -cne "MISSING_SEPARATE_AUTHORITY" -or [bool]$refusalReceipt.childCreated -ne $false -or $null -ne $refusalReceipt.nativeExitCode -or [bool]$refusalReceipt.providerOrDataMutation -ne $false) { throw "PR6_LAUNCHER_REFUSAL_RECEIPT_BINDING_INVALID" }
        $launcherRefusalReceipts += $refusalReceipt
      } elseif ($launchDisposition -ceq "PROVEN_NOT_APPLICABLE") {
        $applicabilityResults = @(& $runtimeInputRecordResolver -Path ([string]$suiteEntry.applicabilityReceiptPath) -Sha256 ([string]$suiteEntry.applicabilityReceiptSha256) -ExpectedSchema "hkwtia.pr6.command-applicability-receipt.v1")
        if ($applicabilityResults.Count -ne 1) { throw "PR6_COMMAND_APPLICABILITY_RECEIPT_COUNT_INVALID" }
        $applicabilityReceipt = $applicabilityResults[0]
        if ([string]$applicabilityReceipt.receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.applicabilityReceiptSha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $seenApplicabilityReceiptIdentities.Add([string]$applicabilityReceipt.receiptIdentitySha256) -or -not $seenApplicabilityReceiptHashes.Add([string]$suiteEntry.applicabilityReceiptSha256) -or [string]$applicabilityReceipt.repository -cne [string]$task12Target.repository -or [string]$applicabilityReceipt.entryIdentitySha256 -cne [string]$suiteEntry.entryIdentitySha256 -or [string]$applicabilityReceipt.requiredCommandIdentitySha256 -cne [string]$suiteEntry.requiredCommandIdentitySha256 -or [string]$applicabilityReceipt.commandTemplateSha256 -cne [string]$suiteEntry.commandTemplateSha256 -or [string]$applicabilityReceipt.runtimeSlotValuesSha256 -cne [string]$suiteEntry.runtimeSlotValuesSha256 -or [string]$applicabilityReceipt.commandProjectionSha256 -cne [string]$suiteEntry.commandProjectionSha256 -or [string]$applicabilityReceipt.resolvedCommandSha256 -cne [string]$suiteEntry.resolvedCommandSha256 -or [string]$applicabilityReceipt.runId -cne [string]$suiteEntry.runId -or -not [Object]::Equals($applicabilityReceipt.executionHead, $suiteEntry.executionHead) -or [string]$applicabilityReceipt.outcome -cne "NOT_APPLICABLE" -or [string]$applicabilityReceipt.producingEntryIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $priorSuiteResultsByEntryIdentity.ContainsKey([string]$applicabilityReceipt.producingEntryIdentitySha256) -or [string]$applicabilityReceipt.producingResultProjectionSha256 -cne [string]$priorSuiteResultsByEntryIdentity[[string]$applicabilityReceipt.producingEntryIdentitySha256]) { throw "PR6_COMMAND_APPLICABILITY_RECEIPT_BINDING_INVALID" }
        $applicabilityReceipts += $applicabilityReceipt
      }

      $priorSuiteResultsByEntryIdentity[[string]$suiteEntry.entryIdentitySha256] = [string]$suiteEntry.resultProjectionSha256

      if ([string]$suiteEntry.reviewState -ceq "ZERO_FINDING") {
        $taskKey = [string]$suiteEntry.task
        if (-not $taskPhaseCoverage.ContainsKey($taskKey) -or [string]$suiteEntry.commandClass -ceq "AGGREGATE" -or [string]$suiteEntry.taskReviewedHead -cnotmatch "^[0-9a-f]{40}$") { throw "PR6_SUITE_ZERO_FINDING_REVIEW_STATE_INVALID" }
        if ([string]$suiteEntry.commandClass -ceq "RED" -or [string]$suiteEntry.commandClass -ceq "GREEN") { $taskPhaseCoverage[$taskKey][[string]$suiteEntry.commandClass] += 1 }
        $taskEntriesByTask[$taskKey] += $suiteEntry
      } elseif ([string]$suiteEntry.reviewState -ceq "PENDING_FINAL_REVIEW") {
        if ([string]$suiteEntry.task -cne "12" -or [string]$suiteEntry.commandClass -cnotin @("GATE", "CONTROL", "AGGREGATE") -or [string]$suiteEntry.taskBaseHead -cne $preTask12Head -or [string]$suiteEntry.executionHead -cne $preTask12Head -or $null -ne $suiteEntry.taskReviewedHead -or $null -ne $suiteEntry.taskReviewPackagePath -or $null -ne $suiteEntry.taskReviewPackageSha256) { throw "PR6_SUITE_PENDING_TASK12_REVIEW_STATE_INVALID" }
        $pendingTask12EntryCount += 1
        if ([string]$suiteEntry.commandClass -ceq "AGGREGATE") { $pendingTask12AggregateCount += 1 }
        $pendingTask12EntryIdentityHashes += [string]$suiteEntry.entryIdentitySha256
        $pendingTask12SourceManifestHashes += [string]$suiteEntry.commandSourceManifestSha256
        $pendingTask12ResultProjectionHashes += [string]$suiteEntry.resultProjectionSha256
        $pendingTask12CleanupReceiptHashes += [string]$suiteEntry.cleanupReceiptSha256
        $pendingTask12AuthorityReceiptHashes += $suiteEntry.separateAuthorityReceiptSha256
        $pendingTask12RefusalReceiptHashes += $suiteEntry.launcherRefusalReceiptSha256
        $pendingTask12ApplicabilityReceiptHashes += $suiteEntry.applicabilityReceiptSha256
      } else {
        throw "PR6_SUITE_REVIEW_STATE_INVALID"
      }

      $commandSourceManifests += $sourceManifest
      $resultProjections += $resultProjection
      $cleanupReceipts += $receipt
    }

    $taskReviewClosures = @($suiteEvidenceManifest.taskReviewClosures)
    if ($taskReviewClosures.Count -ne 11) { throw "PR6_TASK_REVIEW_CLOSURE_SET_COUNT_INVALID" }
    $seenTaskReviewClosures = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach ($taskNumber in 1..11) {
      $taskKey = [string]$taskNumber
      $taskEntries = @($taskEntriesByTask[$taskKey])
      if ([int]$taskPhaseCoverage[$taskKey].RED -lt 1 -or [int]$taskPhaseCoverage[$taskKey].GREEN -lt 1 -or $taskEntries.Count -lt 2) { throw "PR6_SUITE_TASK_RED_GREEN_COVERAGE_INCOMPLETE" }
      $taskReviewClosure = $taskReviewClosures[$taskNumber - 1]
      if ([string]$taskReviewClosure.task -cne $taskKey -or -not $seenTaskReviewClosures.Add($taskKey)) { throw "PR6_TASK_REVIEW_CLOSURE_ORDER_OR_CARDINALITY_INVALID" }
      foreach ($closureAggregateField in @("entryIdentityAggregateSha256", "commandSourceManifestAggregateSha256", "resultProjectionAggregateSha256", "cleanupReceiptAggregateSha256", "separateAuthorityReceiptAggregateSha256", "launcherRefusalReceiptAggregateSha256", "applicabilityReceiptAggregateSha256", "fullEntryEvidenceAggregateSha256")) {
        if ([string]$taskReviewClosure.$closureAggregateField -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_TASK_REVIEW_CLOSURE_AGGREGATE_INVALID" }
      }
      if ([string]$taskReviewClosure.taskBaseHead -cnotmatch "^[0-9a-f]{40}$" -or [string]$taskReviewClosure.finalReviewedHead -cnotmatch "^[0-9a-f]{40}$" -or [string]$taskReviewClosure.packageSha256 -cnotmatch "^[0-9a-f]{64}$" -or [int]$taskReviewClosure.entryCount -ne $taskEntries.Count) { throw "PR6_TASK_REVIEW_CLOSURE_IDENTITY_INVALID" }
      $reviewResults = @(& $runtimeInputRecordResolver -Path ([string]$taskReviewClosure.packagePath) -Sha256 ([string]$taskReviewClosure.packageSha256) -ExpectedSchema "hkwtia.pr6.task-review-package.v3")
      if ($reviewResults.Count -ne 1) { throw "PR6_TASK_REVIEW_PACKAGE_COUNT_INVALID" }
      $taskReviewPackage = $reviewResults[0]
      $reviewedEntryIdentitySha256s = @($taskReviewPackage.entryIdentitySha256s)
      $reviewedSourceManifestSha256s = @($taskReviewPackage.commandSourceManifestSha256s)
      $reviewedResultProjectionSha256s = @($taskReviewPackage.resultProjectionSha256s)
      $reviewedCleanupReceiptSha256s = @($taskReviewPackage.cleanupReceiptSha256s)
      $reviewedAuthorityReceiptSha256s = @($taskReviewPackage.separateAuthorityReceiptSha256s)
      $reviewedRefusalReceiptSha256s = @($taskReviewPackage.launcherRefusalReceiptSha256s)
      $reviewedApplicabilityReceiptSha256s = @($taskReviewPackage.applicabilityReceiptSha256s)
      if ([string]$taskReviewPackage.repository -cne [string]$task12Target.repository -or [string]$taskReviewPackage.task -cne $taskKey -or [string]$taskReviewPackage.partition -cne "PRE_AUTHORING" -or [string]$taskReviewPackage.taskBaseHead -cne [string]$taskReviewClosure.taskBaseHead -or [string]$taskReviewPackage.reviewedHead -cne [string]$taskReviewClosure.finalReviewedHead -or [string]$taskReviewPackage.entryIdentityAggregateSha256 -cne [string]$taskReviewClosure.entryIdentityAggregateSha256 -or [string]$taskReviewPackage.commandSourceManifestAggregateSha256 -cne [string]$taskReviewClosure.commandSourceManifestAggregateSha256 -or [string]$taskReviewPackage.resultProjectionAggregateSha256 -cne [string]$taskReviewClosure.resultProjectionAggregateSha256 -or [string]$taskReviewPackage.cleanupReceiptAggregateSha256 -cne [string]$taskReviewClosure.cleanupReceiptAggregateSha256 -or [string]$taskReviewPackage.separateAuthorityReceiptAggregateSha256 -cne [string]$taskReviewClosure.separateAuthorityReceiptAggregateSha256 -or [string]$taskReviewPackage.launcherRefusalReceiptAggregateSha256 -cne [string]$taskReviewClosure.launcherRefusalReceiptAggregateSha256 -or [string]$taskReviewPackage.applicabilityReceiptAggregateSha256 -cne [string]$taskReviewClosure.applicabilityReceiptAggregateSha256 -or [string]$taskReviewPackage.fullEntryEvidenceAggregateSha256 -cne [string]$taskReviewClosure.fullEntryEvidenceAggregateSha256 -or [string]$taskReviewPackage.specOid -cne "8c83969e9f2244dadf8f9c9e3bc4d4431320c94a" -or [string]$taskReviewPackage.approvedPlanCommit -cne [string]$verificationAuthoringInput.approvedPlanCommit -or [int]$taskReviewPackage.findings.critical -ne 0 -or [int]$taskReviewPackage.findings.important -ne 0 -or [int]$taskReviewPackage.findings.minor -ne 0 -or $reviewedEntryIdentitySha256s.Count -ne $taskEntries.Count -or $reviewedSourceManifestSha256s.Count -ne $taskEntries.Count -or $reviewedResultProjectionSha256s.Count -ne $taskEntries.Count -or $reviewedCleanupReceiptSha256s.Count -ne $taskEntries.Count -or $reviewedAuthorityReceiptSha256s.Count -ne $taskEntries.Count -or $reviewedRefusalReceiptSha256s.Count -ne $taskEntries.Count -or $reviewedApplicabilityReceiptSha256s.Count -ne $taskEntries.Count) { throw "PR6_TASK_REVIEW_PACKAGE_BINDING_INVALID" }
      for ($taskEntryIndex = 0; $taskEntryIndex -lt $taskEntries.Count; $taskEntryIndex += 1) {
        $taskEntry = $taskEntries[$taskEntryIndex]
        if ([string]$taskEntry.taskBaseHead -cne [string]$taskReviewClosure.taskBaseHead -or [string]$taskEntry.taskReviewedHead -cne [string]$taskReviewClosure.finalReviewedHead -or [string]$taskEntry.taskReviewPackagePath -cne [string]$taskReviewClosure.packagePath -or [string]$taskEntry.taskReviewPackageSha256 -cne [string]$taskReviewClosure.packageSha256 -or [string]$reviewedEntryIdentitySha256s[$taskEntryIndex] -cne [string]$taskEntry.entryIdentitySha256 -or [string]$reviewedSourceManifestSha256s[$taskEntryIndex] -cne [string]$taskEntry.commandSourceManifestSha256 -or [string]$reviewedResultProjectionSha256s[$taskEntryIndex] -cne [string]$taskEntry.resultProjectionSha256 -or [string]$reviewedCleanupReceiptSha256s[$taskEntryIndex] -cne [string]$taskEntry.cleanupReceiptSha256 -or -not [Object]::Equals($reviewedAuthorityReceiptSha256s[$taskEntryIndex], $taskEntry.separateAuthorityReceiptSha256) -or -not [Object]::Equals($reviewedRefusalReceiptSha256s[$taskEntryIndex], $taskEntry.launcherRefusalReceiptSha256) -or -not [Object]::Equals($reviewedApplicabilityReceiptSha256s[$taskEntryIndex], $taskEntry.applicabilityReceiptSha256)) { throw "PR6_TASK_REVIEW_PACKAGE_ORDERED_EVIDENCE_MISMATCH" }
        & $verifiedGit merge-base --is-ancestor ([string]$taskEntry.executionHead) ([string]$taskReviewClosure.finalReviewedHead)
        if ($LASTEXITCODE -ne 0) { throw "PR6_SUITE_EXECUTION_REVIEW_ANCESTRY_INVALID" }
      }
      & $verifiedGit merge-base --is-ancestor ([string]$taskReviewClosure.finalReviewedHead) $preTask12Head
    ...
    $pendingTask12ResultProjectionAggregateSha256 = [string]$suiteEvidenceManifest.pendingTask12ResultProjectionAggregateSha256
    $pendingTask12CleanupReceiptAggregateSha256 = [string]$suiteEvidenceManifest.pendingTask12CleanupReceiptAggregateSha256
    $pendingTask12AuthorityReceiptAggregateSha256 = [string]$suiteEvidenceManifest.pendingTask12AuthorityReceiptAggregateSha256
    $pendingTask12RefusalReceiptAggregateSha256 = [string]$suiteEvidenceManifest.pendingTask12RefusalReceiptAggregateSha256
    $pendingTask12ApplicabilityReceiptAggregateSha256 = [string]$suiteEvidenceManifest.pendingTask12ApplicabilityReceiptAggregateSha256
    $pendingTask12FullEntryEvidenceAggregateSha256 = [string]$suiteEvidenceManifest.pendingTask12FullEntryEvidenceAggregateSha256
    if ($pendingTask12EntryCount -lt 1 -or $pendingTask12AggregateCount -lt 1 -or [int]$suiteEvidenceManifest.pendingTask12EntryCount -ne $pendingTask12EntryCount -or [int]$suiteEvidenceManifest.pendingTask12AggregateCount -ne $pendingTask12AggregateCount -or $pendingTask12EntryIdentityHashes.Count -ne $pendingTask12EntryCount -or $pendingTask12SourceManifestHashes.Count -ne $pendingTask12EntryCount -or $pendingTask12ResultProjectionHashes.Count -ne $pendingTask12EntryCount -or $pendingTask12CleanupReceiptHashes.Count -ne $pendingTask12EntryCount -or $pendingTask12AuthorityReceiptHashes.Count -ne $pendingTask12EntryCount -or $pendingTask12RefusalReceiptHashes.Count -ne $pendingTask12EntryCount -or $pendingTask12ApplicabilityReceiptHashes.Count -ne $pendingTask12EntryCount -or $cleanupReceipts.Count -ne $suiteEntryCount -or $commandSourceManifests.Count -ne $suiteEntryCount -or $resultProjections.Count -ne $suiteEntryCount -or $separateAuthorityReceipts.Count -ne $requiredLaunchedAuthorityCount -or $taskReviewPackages.Count -ne 11 -or $pendingTask12EntryIdentityAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or $pendingTask12SourceManifestAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or $pendingTask12ResultProjectionAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or $pendingTask12CleanupReceiptAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or $pendingTask12AuthorityReceiptAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or $pendingTask12RefusalReceiptAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or $pendingTask12ApplicabilityReceiptAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or $pendingTask12FullEntryEvidenceAggregateSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_SUITE_REFERENCED_EVIDENCE_SET_INVALID" }
    ...
    if ($lighthouseAttemptStatus -ceq "UNAUTHORIZED") {
      if ($providerProofCompleted -or $runCreated -or $retainedManifestCompleted -or $fieldInpStatus -cne "UNAUTHORIZED" -or $null -ne $verificationAuthoringInput.previewApprovalPath -or $null -ne $verificationAuthoringInput.previewApprovalSha256 -or $null -ne $verificationAuthoringInput.deploymentId -or $null -ne $verificationAuthoringInput.projectId -or $null -ne $verificationAuthoringInput.teamId -or $null -ne $verificationAuthoringInput.generatedUrl -or $null -ne $verificationAuthoringInput.lighthouseAttemptReceiptPath -or $null -ne $verificationAuthoringInput.lighthouseAttemptReceiptSha256 -or $null -ne $verificationAuthoringInput.providerSanitizedEvidencePath -or $null -ne $verificationAuthoringInput.providerSanitizedEvidenceSha256 -or $null -ne $verificationAuthoringInput.runRoot -or $null -ne $verificationAuthoringInput.retainedArtifactManifestPath -or $null -ne $verificationAuthoringInput.retainedArtifactManifestSha256 -or $null -ne $verificationAuthoringInput.fieldInpEvidencePath -or $null -ne $verificationAuthoringInput.fieldInpEvidenceSha256) { throw "PR6_LIGHTHOUSE_UNAUTHORIZED_AUTHORING_EVIDENCE_PRESENT" }
    } elseif ($lighthouseAttemptStatus -ceq "AUTHORIZED_FAILED" -or $lighthouseAttemptStatus -ceq "PASSED") {
      $previewApprovalResults = @(& $runtimeInputRecordResolver -Path ([string]$verificationAuthoringInput.previewApprovalPath) -Sha256 ([string]$verificationAuthoringInput.previewApprovalSha256) -ExpectedSchema "hkwtia.pr6.lighthouse-preview-approval.v1")
      if ($previewApprovalResults.Count -ne 1) { throw "PR6_LIGHTHOUSE_PREVIEW_APPROVAL_COUNT_INVALID" }
      $previewApproval = $previewApprovalResults[0]
      if ([string]$verificationAuthoringInput.previewApprovalSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]::IsNullOrWhiteSpace([string]$verificationAuthoringInput.deploymentId) -or [string]::IsNullOrWhiteSpace([string]$verificationAuthoringInput.projectId) -or [string]::IsNullOrWhiteSpace([string]$verificationAuthoringInput.generatedUrl) -or [string]$previewApproval.repository -cne [string]$task12Target.repository -or [string]$previewApproval.approvedPlanCommit -cne [string]$verificationAuthoringInput.approvedPlanCommit -or [string]$previewApproval.preTask12Head -cne $preTask12Head -or [string]$previewApproval.deploymentId -cne [string]$verificationAuthoringInput.deploymentId -or [string]$previewApproval.projectId -cne [string]$verificationAuthoringInput.projectId -or -not [Object]::Equals($previewApproval.teamId, $verificationAuthoringInput.teamId) -or [string]$previewApproval.generatedUrl -cne [string]$verificationAuthoringInput.generatedUrl) { throw "PR6_LIGHTHOUSE_PREVIEW_APPROVAL_BINDING_INVALID" }
      ...
      if ([string]$lighthouseAttemptReceipt.repository -cne [string]$task12Target.repository -or [string]$lighthouseAttemptReceipt.evidenceHead -cne $preTask12Head -or [string]$lighthouseAttemptReceipt.previewApprovalSha256 -cne [string]$verificationAuthoringInput.previewApprovalSha256 -or [string]$lighthouseAttemptReceipt.deploymentId -cne [string]$verificationAuthoringInput.deploymentId -or [string]$lighthouseAttemptReceipt.projectId -cne [string]$verificationAuthoringInput.projectId -or -not [Object]::Equals($lighthouseAttemptReceipt.teamId, $verificationAuthoringInput.teamId) -or [string]$lighthouseAttemptReceipt.generatedUrl -cne [string]$verificationAuthoringInput.generatedUrl -or [string]$lighthouseAttemptReceipt.lighthouseAttemptStatus -cne $lighthouseAttemptStatus -or [bool]$lighthouseAttemptReceipt.providerProofCompleted -ne $providerProofCompleted -or [bool]$lighthouseAttemptReceipt.runCreated -ne $runCreated -or [bool]$lighthouseAttemptReceipt.retainedManifestCompleted -ne $retainedManifestCompleted -or [string]::IsNullOrWhiteSpace([string]$lighthouseAttemptReceipt.lastCompletedPhase)) { throw "PR6_LIGHTHOUSE_ATTEMPT_RECEIPT_BINDING_INVALID" }
      ...
      if ($providerProofCompleted) {
        ...
        if ([string]$providerSanitizedEvidence.repository -cne [string]$task12Target.repository -or [string]$providerSanitizedEvidence.evidenceHead -cne $preTask12Head -or [string]$lighthouseAttemptReceipt.providerSanitizedEvidenceSha256 -cne [string]$verificationAuthoringInput.providerSanitizedEvidenceSha256 -or [string]$providerSanitizedEvidence.previewApprovalSha256 -cne [string]$verificationAuthoringInput.previewApprovalSha256 -or [string]$providerSanitizedEvidence.deploymentId -cne [string]$verificationAuthoringInput.deploymentId -or [string]$providerSanitizedEvidence.projectId -cne [string]$verificationAuthoringInput.projectId -or -not [Object]::Equals($providerSanitizedEvidence.teamId, $verificationAuthoringInput.teamId) -or [string]$providerSanitizedEvidence.generatedUrl -cne [string]$verificationAuthoringInput.generatedUrl) { throw "PR6_LIGHTHOUSE_PROVIDER_EVIDENCE_BINDING_INVALID" }
    ...
    if ($fieldInpStatus -ceq "UNAUTHORIZED") {
      ...
      $fieldInpSampleSufficient = [bool]$fieldInpEvidence.sampleSufficient
      $fieldInpP75 = $fieldInpEvidence.p75Milliseconds
      $fieldInpP75Valid = ($null -ne $fieldInpP75 -and [double]::IsFinite([double]$fieldInpP75) -and [double]$fieldInpP75 -ge 0)
      if (($fieldInpSampleSufficient -and -not $fieldInpP75Valid) -or (-not $fieldInpSampleSufficient -and $null -ne $fieldInpP75)) { throw "PR6_FIELD_INP_P75_SHAPE_INVALID" }
      $fieldInpPassCondition = ($fieldInpSampleSufficient -and $fieldInpP75Valid -and [double]$fieldInpP75 -le 200)
      if (-not $providerProofCompleted -or [string]$fieldInpEvidence.repository -cne [string]$task12Target.repository -or [string]$fieldInpEvidence.evidenceHead -cne $preTask12Head -or [string]$fieldInpEvidence.providerSanitizedEvidenceSha256 -cne [string]$verificationAuthoringInput.providerSanitizedEvidenceSha256 -or [string]$fieldInpEvidence.previewApprovalSha256 -cne [string]$verificationAuthoringInput.previewApprovalSha256 -or [string]$fieldInpEvidence.deploymentId -cne [string]$providerSanitizedEvidence.deploymentId -or [string]$fieldInpEvidence.deploymentId -cne [string]$verificationAuthoringInput.deploymentId -or [string]$fieldInpEvidence.projectId -cne [string]$providerSanitizedEvidence.projectId -or [string]$fieldInpEvidence.projectId -cne [string]$verificationAuthoringInput.projectId -or -not [Object]::Equals($fieldInpEvidence.teamId, $providerSanitizedEvidence.teamId) -or -not [Object]::Equals($fieldInpEvidence.teamId, $verificationAuthoringInput.teamId) -or [string]$fieldInpEvidence.generatedUrl -cne [string]$providerSanitizedEvidence.generatedUrl -or [string]$fieldInpEvidence.generatedUrl -cne [string]$verificationAuthoringInput.generatedUrl -or [string]$fieldInpEvidence.status -cne $fieldInpStatus -or [int]$fieldInpEvidence.thresholdMilliseconds -ne 200 -or ($fieldInpStatus -ceq "PASSED" -and -not $fieldInpPassCondition) -or ($fieldInpStatus -ceq "NOT_PASSED" -and $fieldInpPassCondition)) { throw "PR6_FIELD_INP_EVIDENCE_BINDING_INVALID" }
    }

    The launch transcript proves the shell's replacement environment contains no `LHCI_*`, `VERCEL_SHARE_TOKEN`, provider credential, or approval value, and every resolved schema rejects such fields; do not enumerate or print an ambient environment. The authoring input, independently approved root command matrix, exact Task 0 and PRE_AUTHORING partition manifests/projections, every command-source/result/refusal/applicability/cleanup record, completed Task 0/Tasks 1-11 review package, checkpoint evidence, conditional Lighthouse attempt receipt, and conditionally completed provider/run/retained/field records are held immutable through the edit and exact Git checks. Author the document solely from those validated objects—never by reopening an untyped receipt/artifact path. Missing, duplicated, reordered, reused, partition-mismatched, or drifted target/helper/checkpoint/root-matrix/projection/Task-0/suite/source/result/refusal/applicability/review/cleanup/attempt/retained/provider/field evidence; incomplete Tasks 1-11 RED/GREEN coverage; a cross-record repository/head/run/status/hash mismatch; an unauthorized evidence branch; discarded authorized-failure evidence; an invalid two-input overall gate; or any extra/missing status entry is `NOT PASSED`.

    - immutable PR5 base, the exact `hkwtia.pr6.verification-authoring-input.v1`, root-matrix, Task 0 manifest/review, and `hkwtia.pr6.suite-evidence-manifest.v3` path/hashes plus every resolved projection/command-source/result/refusal/applicability/task-review/cleanup identity, the exact `hkwtia.pr6.task12-target.v1` record/helper path/hashes and clean local `preTask12Head` captured before Task 12 editing, and the reviewed implementation head that existed before evidence authoring; the final PR6 commit SHA is intentionally captured after commit in the external immutable review package, not self-referenced inside this commit;
    - every Task 0 and PRE_AUTHORING command, disposition, nullable native exit, timestamps when a child launched, totals, warnings, skips, exact blocker/refusal/applicability receipt, template/typed-slot/resolved-vector identity, and separate operator-approval reference when an isolated suite ran; later Step 7/8 and final-review commands close the fixed `POST_COMMIT_REVIEW` partition through its own manifest instead of being backfilled into this committed document;
    - complete unique RED and GREEN coverage for every Task 1-11 reconstructed from the independently approved PRE_AUTHORING partition plus every GATE/CONTROL entry, exact template/slot/resolved-vector, entry/source/result/cleanup projections, refusal/applicability evidence, task base/reviewed head, and zero-finding full-evidence review package; plus the complete nonempty ordered Task 12 GATE/CONTROL/AGGREGATE set, at least one AGGREGATE, and its entry/source/result/cleanup/authority/refusal/applicability/full-evidence aggregates marked `PENDING_FINAL_REVIEW` without fabricated RED or impossible pre-commit review;
...
- [ ] **Step 8: Stage the remaining Task 12 paths, commit the complete index, then prove the immutable final range**
    ...
    if ($LASTEXITCODE -ne 0) { throw "PR6_FINAL_COMMIT_FAILED" }

    Close the commit shell immediately; do not run a same-shell command with the newly created head. The trusted launcher/CI resolves the commit entry's immutable `hkwtia.pr6.command-result-projection.v1`, requires its sanitized `postExecutionHead` and branch-ref head to be the same 40-hex commit created by that exact command, then issues a dependency-ready `POST_COMMIT_REVIEW` projection solely from that earlier result and one `hkwtia.pr6.final-head-verification-input.v1` embedding the projection. Start a fresh capability-free shell, rerun Task 0's runtime bootstrap, recreate the verified Git invoker, and resolve the input before any final-head-bearing argument:

    $finalHeadInputs = @(& $runtimeInputRecordResolver -Path $operationInputRecordPath -Sha256 $operationInputRecordSha256 -ExpectedSchema "hkwtia.pr6.final-head-verification-input.v1")
    if ($finalHeadInputs.Count -ne 1) { throw "PR6_FINAL_HEAD_INPUT_COUNT_INVALID" }
    $finalHeadInput = $finalHeadInputs[0]
    $finalHeadProjectionResults = @(& $runtimeInputRecordResolver -Path ([string]$finalHeadInput.commandPhaseProjectionPath) -Sha256 ([string]$finalHeadInput.commandPhaseProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-phase-projection.v1")
    $finalCommitResultResults = @(& $runtimeInputRecordResolver -Path ([string]$finalHeadInput.finalCommitResultProjectionPath) -Sha256 ([string]$finalHeadInput.finalCommitResultProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-result-projection.v1")
    if ($finalHeadProjectionResults.Count -ne 1 -or $finalCommitResultResults.Count -ne 1) { throw "PR6_FINAL_HEAD_REFERENCE_COUNT_INVALID" }
    $finalHeadProjection = $finalHeadProjectionResults[0]
    $finalCommitResult = $finalCommitResultResults[0]
    $finalHead = [string]$finalHeadInput.finalHead
    $approvedPlanCommit = [string]$finalHeadInput.approvedPlanCommit
    $verificationDocumentPath = [string]$finalHeadInput.verificationDocumentPath
    if ($finalHead -cnotmatch "^[0-9a-f]{40}$" -or $approvedPlanCommit -cnotmatch "^[0-9a-f]{40}$" -or $verificationDocumentPath -cne "docs/integration/wisetech-pr6-verification.md" -or [string]$finalHeadProjection.partition -cne "POST_COMMIT_REVIEW" -or [string]$finalCommitResult.postExecutionHead -cne $finalHead -or [string]$finalCommitResult.postExecutionBranchHead -cne $finalHead -or [string]$finalHeadInput.finalCommitResultProjectionSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$finalHeadInput.finalCommitEntryIdentitySha256 -cne [string]$finalCommitResult.entryIdentitySha256) { throw "PR6_FINAL_HEAD_INPUT_BINDING_INVALID" }
    $liveFinalHead = ((@(& $verifiedGit rev-parse HEAD)) -join "`n").Trim()
    $liveFinalBranchHead = ((@(& $verifiedGit rev-parse "refs/heads/codex/wisetech-pr6-join-portal-admin")) -join "`n").Trim()
    if ($LASTEXITCODE -ne 0 -or $liveFinalHead -cne $finalHead -or $liveFinalBranchHead -cne $finalHead) { throw "PR6_FINAL_HEAD_LIVE_BINDING_INVALID" }
    ...
    Send only the exact sanitized `{finalHead, verificationDocumentPath, verificationDocumentBlobOid}` over the launcher-owned control channel, then close the commit shell. The trusted launcher/CI requires the literal path, resolves a dependency-ready `POST_COMMIT_REVIEW` command-phase projection from that proposed final-head/blob identity, and issues one exact `hkwtia.pr6.verification-document-hash-input.v1` binding the projection plus repository/branch/base/spec/approved-plan. Start a fresh capability-free hash shell, rerun Task 0's runtime bootstrap, and resolve that operation record:
    ...
    $hashPhaseProjectionResults = @(& $runtimeInputRecordResolver -Path ([string]$documentHashInput.commandPhaseProjectionPath) -Sha256 ([string]$documentHashInput.commandPhaseProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-phase-projection.v1")
    if ($hashPhaseProjectionResults.Count -ne 1) { throw "PR6_HASH_PHASE_PROJECTION_COUNT_INVALID" }
    $hashPhaseProjection = $hashPhaseProjectionResults[0]
    if ([string]$hashPhaseProjection.requiredCommandMatrixPath -cne [string]$documentHashInput.requiredCommandMatrixPath -or [string]$hashPhaseProjection.requiredCommandMatrixSha256 -cne [string]$documentHashInput.requiredCommandMatrixSha256 -or [string]$hashPhaseProjection.partition -cne "POST_COMMIT_REVIEW" -or [int]$hashPhaseProjection.entryCount -lt 1 -or [string]$hashPhaseProjection.entryAggregateSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_HASH_PHASE_PROJECTION_BINDING_INVALID" }
    ...
    $postCommitPhaseProjectionResults = @(& $runtimeInputRecordResolver -Path ([string]$lighthouseVerificationInput.commandPhaseProjectionPath) -Sha256 ([string]$lighthouseVerificationInput.commandPhaseProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-phase-projection.v1")
    if ($postCommitPhaseProjectionResults.Count -ne 1) { throw "PR6_POST_COMMIT_PHASE_PROJECTION_COUNT_INVALID" }
    $postCommitPhaseProjection = $postCommitPhaseProjectionResults[0]
    if ([string]$postCommitPhaseProjection.requiredCommandMatrixPath -cne [string]$lighthouseVerificationInput.requiredCommandMatrixPath -or [string]$postCommitPhaseProjection.requiredCommandMatrixSha256 -cne [string]$lighthouseVerificationInput.requiredCommandMatrixSha256 -or [string]$postCommitPhaseProjection.partition -cne "POST_COMMIT_REVIEW" -or [int]$postCommitPhaseProjection.entryCount -lt 1 -or [string]$postCommitPhaseProjection.entryAggregateSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_POST_COMMIT_PHASE_PROJECTION_BINDING_INVALID" }
    ...
    $fieldInpEvidence = $null
    if ($lighthouseAttemptStatus -ceq "UNAUTHORIZED") {
      if ($providerProofCompleted -or $runCreated -or $retainedManifestCompleted -or $fieldInpStatus -cne "UNAUTHORIZED" -or $null -ne $lighthouseVerificationInput.previewApprovalPath -or $null -ne $lighthouseVerificationInput.previewApprovalSha256 -or $null -ne $lighthouseVerificationInput.deploymentId -or $null -ne $lighthouseVerificationInput.projectId -or $null -ne $lighthouseVerificationInput.teamId -or $null -ne $lighthouseVerificationInput.generatedUrl -or $null -ne $lighthouseVerificationInput.lighthouseAttemptReceiptPath -or $null -ne $lighthouseVerificationInput.lighthouseAttemptReceiptSha256 -or $null -ne $lighthouseVerificationInput.providerSanitizedEvidencePath -or $null -ne $lighthouseVerificationInput.providerSanitizedEvidenceSha256 -or $null -ne $lighthouseVerificationInput.runRoot -or $null -ne $lighthouseVerificationInput.retainedArtifactManifestPath -or $null -ne $lighthouseVerificationInput.retainedArtifactManifestSha256 -or $null -ne $lighthouseVerificationInput.fieldInpEvidencePath -or $null -ne $lighthouseVerificationInput.fieldInpEvidenceSha256) { throw "PR6_LIGHTHOUSE_UNAUTHORIZED_COMMIT_EVIDENCE_PRESENT" }
    } else {
      $previewApprovalResults = @(& $runtimeInputRecordResolver -Path ([string]$lighthouseVerificationInput.previewApprovalPath) -Sha256 ([string]$lighthouseVerificationInput.previewApprovalSha256) -ExpectedSchema "hkwtia.pr6.lighthouse-preview-approval.v1")
      if ($previewApprovalResults.Count -ne 1) { throw "PR6_LIGHTHOUSE_COMMIT_PREVIEW_APPROVAL_COUNT_INVALID" }
      $previewApproval = $previewApprovalResults[0]
      if ([string]$lighthouseVerificationInput.previewApprovalSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]::IsNullOrWhiteSpace([string]$lighthouseVerificationInput.deploymentId) -or [string]::IsNullOrWhiteSpace([string]$lighthouseVerificationInput.projectId) -or [string]::IsNullOrWhiteSpace([string]$lighthouseVerificationInput.generatedUrl) -or [string]$previewApproval.repository -cne [string]$lighthouseVerificationInput.repository -or [string]$previewApproval.approvedPlanCommit -cne [string]$lighthouseVerificationInput.approvedPlanCommit -or [string]$previewApproval.preTask12Head -cne $lighthouseEvidenceHead -or [string]$previewApproval.deploymentId -cne [string]$lighthouseVerificationInput.deploymentId -or [string]$previewApproval.projectId -cne [string]$lighthouseVerificationInput.projectId -or -not [Object]::Equals($previewApproval.teamId, $lighthouseVerificationInput.teamId) -or [string]$previewApproval.generatedUrl -cne [string]$lighthouseVerificationInput.generatedUrl) { throw "PR6_LIGHTHOUSE_COMMIT_PREVIEW_APPROVAL_BINDING_INVALID" }
      ...
      $lighthouseAttemptReceipt = $attemptResults[0]
      if ([string]$lighthouseAttemptReceipt.repository -cne [string]$lighthouseVerificationInput.repository -or [string]$lighthouseAttemptReceipt.evidenceHead -cne $lighthouseEvidenceHead -or [string]$lighthouseAttemptReceipt.previewApprovalSha256 -cne [string]$lighthouseVerificationInput.previewApprovalSha256 -or [string]$lighthouseAttemptReceipt.deploymentId -cne [string]$lighthouseVerificationInput.deploymentId -or [string]$lighthouseAttemptReceipt.projectId -cne [string]$lighthouseVerificationInput.projectId -or -not [Object]::Equals($lighthouseAttemptReceipt.teamId, $lighthouseVerificationInput.teamId) -or [string]$lighthouseAttemptReceipt.generatedUrl -cne [string]$lighthouseVerificationInput.generatedUrl -or [string]$lighthouseAttemptReceipt.lighthouseAttemptStatus -cne $lighthouseAttemptStatus -or [bool]$lighthouseAttemptReceipt.providerProofCompleted -ne $providerProofCompleted -or [bool]$lighthouseAttemptReceipt.runCreated -ne $runCreated -or [bool]$lighthouseAttemptReceipt.retainedManifestCompleted -ne $retainedManifestCompleted -or [string]::IsNullOrWhiteSpace([string]$lighthouseAttemptReceipt.lastCompletedPhase)) { throw "PR6_LIGHTHOUSE_COMMIT_ATTEMPT_RECEIPT_INVALID" }
      if ($lighthouseAttemptStatus -ceq "PASSED") {
      ...
      if ($providerProofCompleted) {
        ...
        $providerSanitizedEvidence = $providerResults[0]
        if ([string]$providerSanitizedEvidence.repository -cne [string]$lighthouseVerificationInput.repository -or [string]$providerSanitizedEvidence.evidenceHead -cne $lighthouseEvidenceHead -or [string]$lighthouseAttemptReceipt.providerSanitizedEvidenceSha256 -cne [string]$lighthouseVerificationInput.providerSanitizedEvidenceSha256 -or [string]$providerSanitizedEvidence.previewApprovalSha256 -cne [string]$lighthouseVerificationInput.previewApprovalSha256 -or [string]$providerSanitizedEvidence.deploymentId -cne [string]$lighthouseVerificationInput.deploymentId -or [string]$providerSanitizedEvidence.projectId -cne [string]$lighthouseVerificationInput.projectId -or -not [Object]::Equals($providerSanitizedEvidence.teamId, $lighthouseVerificationInput.teamId) -or [string]$providerSanitizedEvidence.generatedUrl -cne [string]$lighthouseVerificationInput.generatedUrl) { throw "PR6_LIGHTHOUSE_COMMIT_PROVIDER_EVIDENCE_INVALID" }
    ...
    if ($fieldInpStatus -ceq "UNAUTHORIZED") {
      ...
      if (($fieldInpSampleSufficient -and -not $fieldInpP75Valid) -or (-not $fieldInpSampleSufficient -and $null -ne $fieldInpP75)) { throw "PR6_FIELD_INP_P75_SHAPE_INVALID" }
      $fieldInpPassCondition = ($fieldInpSampleSufficient -and $fieldInpP75Valid -and [double]$fieldInpP75 -le 200)
      if (-not $providerProofCompleted -or [string]$fieldInpEvidence.repository -cne [string]$lighthouseVerificationInput.repository -or [string]$fieldInpEvidence.evidenceHead -cne $lighthouseEvidenceHead -or [string]$fieldInpEvidence.providerSanitizedEvidenceSha256 -cne [string]$lighthouseVerificationInput.providerSanitizedEvidenceSha256 -or [string]$fieldInpEvidence.previewApprovalSha256 -cne [string]$lighthouseVerificationInput.previewApprovalSha256 -or [string]$fieldInpEvidence.deploymentId -cne [string]$providerSanitizedEvidence.deploymentId -or [string]$fieldInpEvidence.deploymentId -cne [string]$lighthouseVerificationInput.deploymentId -or [string]$fieldInpEvidence.projectId -cne [string]$providerSanitizedEvidence.projectId -or [string]$fieldInpEvidence.projectId -cne [string]$lighthouseVerificationInput.projectId -or -not [Object]::Equals($fieldInpEvidence.teamId, $providerSanitizedEvidence.teamId) -or -not [Object]::Equals($fieldInpEvidence.teamId, $lighthouseVerificationInput.teamId) -or [string]$fieldInpEvidence.generatedUrl -cne [string]$providerSanitizedEvidence.generatedUrl -or [string]$fieldInpEvidence.generatedUrl -cne [string]$lighthouseVerificationInput.generatedUrl -or [string]$fieldInpEvidence.status -cne $fieldInpStatus -or [int]$fieldInpEvidence.thresholdMilliseconds -ne 200 -or ($fieldInpStatus -ceq "PASSED" -and -not $fieldInpPassCondition) -or ($fieldInpStatus -ceq "NOT_PASSED" -and $fieldInpPassCondition)) { throw "PR6_FIELD_INP_COMMIT_EVIDENCE_INVALID" }
    }
    ...
    Expected: approved PR6 files only, cohesive commits, no range whitespace errors, and a clean worktree after all applicable executable verification paths. The first `verify-commit` always revalidates the attested Node/Git/LHCI identities, checkpoint hash, effective exact-path attributes/config, deletion tombstone, exact evidence-head ancestry, and every final `HEAD:<path>` OID/blob byte hash against the credential-free checkpoint—even when collection was unauthorized or an authorized attempt failed before creating a run. Every authorized attempt additionally resolves the exact Preview approval descriptor and binds its immutable attempt receipt to that approval SHA-256 plus the exact deployment/project/nullable-team/generated-URL target before any provider-proof field is considered; provider evidence is required exactly when provider proof completed; the canonical run root is required exactly when created. The second call runs only when retention-manifest finalization completed and binds that manifest/root, attempt status, and available pre/post raw diagnostics/materialized staged bytes without turning a failure into a pass. A run-root creation followed by retention-manifest failure remains representable and reported through the receipt without opening an untyped root. Field-INP evidence is independently required and threshold-checked whenever its status is not `UNAUTHORIZED`; a sufficient sample must carry an actual finite nonnegative p75, while null/NaN/infinity/negative/insufficient data is never coerced into a pass. Any checkpoint, staged blob, final blob, attribute/config, executable, tombstone, conditional-evidence, field threshold, or collected-run mismatch fails. An unauthorized attempt invents no evidence; an authorized failure retains what completed; a successful Lighthouse attempt with missing/noncompliant field INP still leaves the overall gate `NOT_PASSED`; only both statuses `PASSED` map it to `PASSED`. Preserve `$finalHead` in the immutable review package and publication checks; do not amend the committed verification document merely to self-record its own SHA.

- [ ] **Step 9: Complete independent review and publish the stacked draft PR**

    Generate the final immutable review package from PR5 head to the committed PR6 head. The trusted launcher first issues the dependency-ready final `POST_COMMIT_REVIEW` phase projection for one fresh reviewer. That reviewer inspects the approved spec, this plan, complete diff, Task 0 and PRE_AUTHORING manifests, all per-task full-evidence packages, the complete pending Task 12 GATE/CONTROL/AGGREGATE set, verification record, authorization/import boundaries, cleanup, and rollback, then prints the exact reviewed SHA and C/I/M verdict. The launcher records that REVIEW entry's source/result/cleanup evidence, freezes `hkwtia.pr6.post-commit-command-evidence-manifest.v1`, and requires its ordered projections/entries to exactly exhaust the root `POST_COMMIT_REVIEW` partition. It recomputes the pending Task 12 entry/source/result/cleanup/authority/refusal/applicability/full-evidence aggregates and inspects them against the committed Task 12 diff and final verification-document blob. Only a zero Critical/Important/Minor verdict can issue the final package. A finding marks that head `NOT_PASSED`; any corrected head gets new typed-slot phase projections and a new final manifest, while rejected-attempt evidence remains immutable but outside the final partition closure.

    Serialize the zero verdict as immutable `hkwtia.pr6.zero-finding-review-package.v2`; it binds the exact root matrix, all five partition counts/aggregates, TASK0/PRE_AUTHORING/POST_COMMIT_REVIEW manifests, pending Task 12 entry/source/result/cleanup/authority/refusal/applicability/full-evidence aggregates, and final verification-document blob identity. After separate publication approval, trusted launcher/CI issues `hkwtia.pr6.publication-input.v2` duplicating that closure and binding the exact PUBLICATION partition plus its first dependency-ready phase projection and publication descriptor. Later publication epochs may resolve only typed slots produced by earlier immutable operation receipts, and the final completion record must prove exact partition exhaustion. Open the first fresh publication PowerShell epoch, rerun Task 0's core bootstrap, and resolve both launcher-delivered non-secret records without console or environment input. Every later epoch closes the preceding shell and rehydrates the same reviewed closure plus its new dependency-ready phase projection in another fresh shell:

    $publicationInputs = @(& $runtimeInputRecordResolver -Path $operationInputRecordPath -Sha256 $operationInputRecordSha256 -ExpectedSchema "hkwtia.pr6.publication-input.v2")
    if ($publicationInputs.Count -ne 1) { throw "PR6_PUBLICATION_INPUT_COUNT_INVALID" }
    $publicationInput = $publicationInputs[0]
    if ([string]$publicationInput.epoch -cne "INITIAL") { throw "PR6_PUBLICATION_INITIAL_EPOCH_REQUIRED" }
    $reviewPackages = @(& $runtimeInputRecordResolver -Path ([string]$publicationInput.reviewPackagePath) -Sha256 ([string]$publicationInput.reviewPackageSha256) -ExpectedSchema "hkwtia.pr6.zero-finding-review-package.v2")
    ...
    $expectedTitle = "feat: align WiseTech Join Portal and Admin"
    ...
    $task0CommandEvidenceManifestPath = [string]$reviewPackage.task0CommandEvidenceManifestPath
    $task0CommandEvidenceManifestSha256 = [string]$reviewPackage.task0CommandEvidenceManifestSha256
    $task0FullEntryEvidenceAggregateSha256 = [string]$reviewPackage.task0FullEntryEvidenceAggregateSha256
    $suiteEvidenceManifestPath = [string]$reviewPackage.suiteEvidenceManifestPath
    $suiteEvidenceManifestSha256 = [string]$reviewPackage.suiteEvidenceManifestSha256
    $preAuthoringFullEntryEvidenceAggregateSha256 = [string]$reviewPackage.preAuthoringFullEntryEvidenceAggregateSha256
    $postCommitFullEntryEvidenceAggregateSha256 = [string]$reviewPackage.postCommitFullEntryEvidenceAggregateSha256
    $postCommitCommandEvidenceManifestPath = [string]$reviewPackage.postCommitCommandEvidenceManifestPath
    $postCommitCommandEvidenceManifestSha256 = [string]$reviewPackage.postCommitCommandEvidenceManifestSha256
    $requiredCommandMatrixPath = [string]$reviewPackage.requiredCommandMatrixPath
    $requiredCommandMatrixSha256 = [string]$reviewPackage.requiredCommandMatrixSha256
    $requiredCommandMatrixRootEntryCount = [int]$reviewPackage.requiredCommandMatrixRootEntryCount
    $requiredCommandMatrixRootEntryAggregateSha256 = [string]$reviewPackage.requiredCommandMatrixRootEntryAggregateSha256
    $task0PartitionEntryCount = [int]$reviewPackage.task0PartitionEntryCount
    $task0PartitionEntryAggregateSha256 = [string]$reviewPackage.task0PartitionEntryAggregateSha256
    $preAuthoringPartitionEntryCount = [int]$reviewPackage.preAuthoringPartitionEntryCount
    $preAuthoringPartitionEntryAggregateSha256 = [string]$reviewPackage.preAuthoringPartitionEntryAggregateSha256
    $postCommitReviewPartitionEntryCount = [int]$reviewPackage.postCommitReviewPartitionEntryCount
    $postCommitReviewPartitionEntryAggregateSha256 = [string]$reviewPackage.postCommitReviewPartitionEntryAggregateSha256
    $publicationPartitionEntryCount = [int]$reviewPackage.publicationPartitionEntryCount
    $publicationPartitionEntryAggregateSha256 = [string]$reviewPackage.publicationPartitionEntryAggregateSha256
    $terminalAttestationPartitionEntryCount = [int]$reviewPackage.terminalAttestationPartitionEntryCount
    $terminalAttestationPartitionEntryAggregateSha256 = [string]$reviewPackage.terminalAttestationPartitionEntryAggregateSha256
    ...
    $pendingTask12ResultProjectionAggregateSha256 = [string]$reviewPackage.pendingTask12ResultProjectionAggregateSha256
    $pendingTask12CleanupReceiptAggregateSha256 = [string]$reviewPackage.pendingTask12CleanupReceiptAggregateSha256
    $pendingTask12AuthorityReceiptAggregateSha256 = [string]$reviewPackage.pendingTask12AuthorityReceiptAggregateSha256
    $pendingTask12RefusalReceiptAggregateSha256 = [string]$reviewPackage.pendingTask12RefusalReceiptAggregateSha256
    $pendingTask12ApplicabilityReceiptAggregateSha256 = [string]$reviewPackage.pendingTask12ApplicabilityReceiptAggregateSha256
    $pendingTask12FullEntryEvidenceAggregateSha256 = [string]$reviewPackage.pendingTask12FullEntryEvidenceAggregateSha256
    ...
    $prBodyPath = [string]$reviewPackage.prBodyPath
    $prBodyBlobOid = [string]$reviewPackage.prBodyBlobOid
    $prBodyBlobSha256 = [string]$reviewPackage.prBodyBlobSha256
    $prBodyBlobByteLength = [long]$reviewPackage.prBodyBlobByteLength
    $prBodyCanonicalTextSha256 = [string]$reviewPackage.prBodyCanonicalTextSha256
    $prBodyCanonicalTextByteLength = [long]$reviewPackage.prBodyCanonicalTextByteLength
    if ([string]$reviewPackage.prTitle -cne $expectedTitle) { throw "PR6_REVIEW_PACKAGE_TITLE_INVALID" }
    ...
    foreach ($sha256Value in @($reviewerEvidenceSha256, $task0CommandEvidenceManifestSha256, $task0FullEntryEvidenceAggregateSha256, $suiteEvidenceManifestSha256, $preAuthoringFullEntryEvidenceAggregateSha256, $postCommitCommandEvidenceManifestSha256, $postCommitFullEntryEvidenceAggregateSha256, $requiredCommandMatrixSha256, $requiredCommandMatrixRootEntryAggregateSha256, $task0PartitionEntryAggregateSha256, $preAuthoringPartitionEntryAggregateSha256, $postCommitReviewPartitionEntryAggregateSha256, $publicationPartitionEntryAggregateSha256, $terminalAttestationPartitionEntryAggregateSha256, $pendingTask12EntryIdentityAggregateSha256, $pendingTask12SourceManifestAggregateSha256, $pendingTask12ResultProjectionAggregateSha256, $pendingTask12CleanupReceiptAggregateSha256, $pendingTask12AuthorityReceiptAggregateSha256, $pendingTask12RefusalReceiptAggregateSha256, $pendingTask12ApplicabilityReceiptAggregateSha256, $pendingTask12FullEntryEvidenceAggregateSha256, $finalVerificationDocumentBlobSha256, $prBodyBlobSha256, $prBodyCanonicalTextSha256)) {
      if ([string]$sha256Value -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_FINAL_REVIEW_HASH_IDENTITY_INVALID" }
    }
    if ([string]::IsNullOrWhiteSpace($requiredCommandMatrixPath) -or [string]::IsNullOrWhiteSpace($task0CommandEvidenceManifestPath) -or [string]::IsNullOrWhiteSpace($suiteEvidenceManifestPath) -or [string]::IsNullOrWhiteSpace($postCommitCommandEvidenceManifestPath) -or $requiredCommandMatrixRootEntryCount -lt 5 -or $task0PartitionEntryCount -lt 1 -or $preAuthoringPartitionEntryCount -lt 1 -or $postCommitReviewPartitionEntryCount -lt 1 -or $publicationPartitionEntryCount -lt 1 -or $terminalAttestationPartitionEntryCount -lt 1 -or $pendingTask12EntryCount -lt 1 -or $finalVerificationDocumentPath -cne "docs/integration/wisetech-pr6-verification.md" -or $finalVerificationDocumentBlobOid -cnotmatch "^[0-9a-f]{40}$" -or $finalVerificationDocumentBlobByteLength -le 0 -or $prBodyPath -cne "docs/integration/wisetech-pr6-pr-body.md" -or $prBodyBlobOid -cnotmatch "^[0-9a-f]{40}$" -or $prBodyBlobByteLength -le 0 -or $prBodyCanonicalTextByteLength -le 0) { throw "PR6_FINAL_REVIEW_PARTITION_CLOSURE_INVALID" }
    ...
    $requiredCommandMatrixResults = @(& $runtimeInputRecordResolver -Path $requiredCommandMatrixPath -Sha256 $requiredCommandMatrixSha256 -ExpectedSchema "hkwtia.pr6.required-command-matrix.v2")
    $task0ManifestResults = @(& $runtimeInputRecordResolver -Path $task0CommandEvidenceManifestPath -Sha256 $task0CommandEvidenceManifestSha256 -ExpectedSchema "hkwtia.pr6.task0-command-evidence-manifest.v2")
    $preAuthoringManifestResults = @(& $runtimeInputRecordResolver -Path $suiteEvidenceManifestPath -Sha256 $suiteEvidenceManifestSha256 -ExpectedSchema "hkwtia.pr6.suite-evidence-manifest.v3")
    $postCommitManifestResults = @(& $runtimeInputRecordResolver -Path $postCommitCommandEvidenceManifestPath -Sha256 $postCommitCommandEvidenceManifestSha256 -ExpectedSchema "hkwtia.pr6.post-commit-command-evidence-manifest.v1")
    $publicationProjectionResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationInput.publicationCommandPhaseProjectionPath) -Sha256 ([string]$publicationInput.publicationCommandPhaseProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-phase-projection.v1")
    if ($requiredCommandMatrixResults.Count -ne 1 -or $task0ManifestResults.Count -ne 1 -or $preAuthoringManifestResults.Count -ne 1 -or $postCommitManifestResults.Count -ne 1 -or $publicationProjectionResults.Count -ne 1) { throw "PR6_PUBLICATION_PARTITION_INPUT_COUNT_INVALID" }
    $requiredCommandMatrix = $requiredCommandMatrixResults[0]
    $requiredCommandPartitions = @($requiredCommandMatrix.partitions)
    $task0Manifest = $task0ManifestResults[0]
    $preAuthoringManifest = $preAuthoringManifestResults[0]
    $postCommitManifest = $postCommitManifestResults[0]
    $publicationProjection = $publicationProjectionResults[0]
    if ([int]$requiredCommandMatrix.rootEntryCount -ne $requiredCommandMatrixRootEntryCount -or [string]$requiredCommandMatrix.rootEntryAggregateSha256 -cne $requiredCommandMatrixRootEntryAggregateSha256 -or $requiredCommandPartitions.Count -ne 5 -or [string]$requiredCommandPartitions[0].name -cne "TASK0" -or [int]$requiredCommandPartitions[0].entryCount -ne $task0PartitionEntryCount -or [string]$requiredCommandPartitions[0].entryAggregateSha256 -cne $task0PartitionEntryAggregateSha256 -or [string]$requiredCommandPartitions[1].name -cne "PRE_AUTHORING" -or [int]$requiredCommandPartitions[1].entryCount -ne $preAuthoringPartitionEntryCount -or [string]$requiredCommandPartitions[1].entryAggregateSha256 -cne $preAuthoringPartitionEntryAggregateSha256 -or [string]$requiredCommandPartitions[2].name -cne "POST_COMMIT_REVIEW" -or [int]$requiredCommandPartitions[2].entryCount -ne $postCommitReviewPartitionEntryCount -or [string]$requiredCommandPartitions[2].entryAggregateSha256 -cne $postCommitReviewPartitionEntryAggregateSha256 -or [string]$requiredCommandPartitions[3].name -cne "PUBLICATION" -or [int]$requiredCommandPartitions[3].entryCount -ne $publicationPartitionEntryCount -or [string]$requiredCommandPartitions[3].entryAggregateSha256 -cne $publicationPartitionEntryAggregateSha256 -or [string]$requiredCommandPartitions[4].name -cne "TERMINAL_ATTESTATION" -or [int]$requiredCommandPartitions[4].entryCount -ne $terminalAttestationPartitionEntryCount -or [string]$requiredCommandPartitions[4].entryAggregateSha256 -cne $terminalAttestationPartitionEntryAggregateSha256) { throw "PR6_PUBLICATION_ROOT_MATRIX_PARTITION_BINDING_INVALID" }
    if ([string]$task0Manifest.repository -cne $expectedRepo -or [string]$task0Manifest.partition -cne "TASK0" -or [string]$task0Manifest.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$task0Manifest.entryCount -ne $task0PartitionEntryCount -or [string]$task0Manifest.entryAggregateSha256 -cne $task0PartitionEntryAggregateSha256 -or [string]$task0Manifest.fullEntryEvidenceAggregateSha256 -cne $task0FullEntryEvidenceAggregateSha256) { throw "PR6_PUBLICATION_TASK0_MANIFEST_BINDING_INVALID" }
    if ([string]$preAuthoringManifest.repository -cne $expectedRepo -or [string]$preAuthoringManifest.partition -cne "PRE_AUTHORING" -or [string]$preAuthoringManifest.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$preAuthoringManifest.entryCount -ne $preAuthoringPartitionEntryCount -or [string]$preAuthoringManifest.entryAggregateSha256 -cne $preAuthoringPartitionEntryAggregateSha256 -or [string]$preAuthoringManifest.fullEntryEvidenceAggregateSha256 -cne $preAuthoringFullEntryEvidenceAggregateSha256) { throw "PR6_PUBLICATION_PRE_AUTHORING_MANIFEST_BINDING_INVALID" }
    if ([string]$postCommitManifest.repository -cne $expectedRepo -or [string]$postCommitManifest.reviewedHead -cne $reviewedHead -or [string]$postCommitManifest.partition -cne "POST_COMMIT_REVIEW" -or [string]$postCommitManifest.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$postCommitManifest.entryCount -ne $postCommitReviewPartitionEntryCount -or [string]$postCommitManifest.entryAggregateSha256 -cne $postCommitReviewPartitionEntryAggregateSha256 -or [string]$postCommitManifest.fullEntryEvidenceAggregateSha256 -cne $postCommitFullEntryEvidenceAggregateSha256) { throw "PR6_POST_COMMIT_MANIFEST_BINDING_INVALID" }
    if ([string]$publicationProjection.partition -cne "PUBLICATION" -or [string]$publicationProjection.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$publicationProjection.entryCount -lt 1 -or [string]$publicationProjection.entryAggregateSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_PUBLICATION_PHASE_PROJECTION_BINDING_INVALID" }

    $closureFields = @("task0CommandEvidenceManifestPath", "task0CommandEvidenceManifestSha256", "task0FullEntryEvidenceAggregateSha256", "suiteEvidenceManifestPath", "suiteEvidenceManifestSha256", "preAuthoringFullEntryEvidenceAggregateSha256", "postCommitFullEntryEvidenceAggregateSha256", "postCommitCommandEvidenceManifestPath", "postCommitCommandEvidenceManifestSha256", "requiredCommandMatrixPath", "requiredCommandMatrixSha256", "requiredCommandMatrixRootEntryCount", "requiredCommandMatrixRootEntryAggregateSha256", "task0PartitionEntryCount", "task0PartitionEntryAggregateSha256", "preAuthoringPartitionEntryCount", "preAuthoringPartitionEntryAggregateSha256", "postCommitReviewPartitionEntryCount", "postCommitReviewPartitionEntryAggregateSha256", "publicationPartitionEntryCount", "publicationPartitionEntryAggregateSha256", "terminalAttestationPartitionEntryCount", "terminalAttestationPartitionEntryAggregateSha256", "pendingTask12EntryCount", "pendingTask12EntryIdentityAggregateSha256", "pendingTask12SourceManifestAggregateSha256", "pendingTask12ResultProjectionAggregateSha256", "pendingTask12CleanupReceiptAggregateSha256", "pendingTask12AuthorityReceiptAggregateSha256", "pendingTask12RefusalReceiptAggregateSha256", "pendingTask12ApplicabilityReceiptAggregateSha256", "pendingTask12FullEntryEvidenceAggregateSha256", "verificationDocumentPath", "verificationDocumentBlobOid", "verificationDocumentBlobSha256", "verificationDocumentBlobByteLength", "prBodyPath", "prBodyBlobOid", "prBodyBlobSha256", "prBodyBlobByteLength", "prBodyCanonicalTextSha256", "prBodyCanonicalTextByteLength", "prTitle")
    foreach ($closureField in $closureFields) {
      if ([string]$publicationInput.$closureField -cne [string]$reviewPackage.$closureField) { throw "PR6_PUBLICATION_REVIEW_CLOSURE_FIELD_MISMATCH" }
    }
    if ([string]$reviewPackage.approvedPlanRecordPath -cne ([string]$publicationInput.approvedPlanRecordPath) -or [string]$reviewPackage.approvedPlanRecordSha256 -cne ([string]$publicationInput.approvedPlanRecordSha256) -or [string]$reviewPackage.approvedPlanCommit -cne $expectedPlan -or [string]$reviewPackage.approvedPlanPath -cne $expectedPlanPath -or [string]$reviewPackage.approvedPlanBlobOid -cne $expectedPlanBlobOid -or [string]$reviewPackage.approvedPlanBlobSha256 -cne $expectedPlanSha256 -or [string]$reviewPackage.approvedCheckoutSha256 -cne $expectedPlanCheckoutSha256 -or [string]$reviewPackage.effectiveAttributesSha256 -cne $expectedPlanAttributesSha256) { throw "PR6_REVIEW_PACKAGE_BINDING_MISMATCH" }
    if ([string]$publicationInput.repository -cne $expectedRepo -or [string]$publicationInput.headBranch -cne $expectedBranch -or [string]$publicationInput.baseBranch -cne $expectedBaseBranch -or [string]$publicationInput.baseOid -cne $expectedBase -or [string]$publicationInput.specOid -cne $expectedSpec -or [string]$publicationInput.approvedPlanCommit -cne $expectedPlan -or [string]$publicationInput.reviewedHead -cne $reviewedHead -or [string]$publicationInput.planPath -cne $expectedPlanPath -or [string]$publicationInput.planBlobOid -cne $expectedPlanBlobOid -or [string]$publicationInput.planBlobSha256 -cne $expectedPlanSha256 -or [string]$publicationInput.planCheckoutSha256 -cne $expectedPlanCheckoutSha256 -or [string]$publicationInput.planAttributesSha256 -cne $expectedPlanAttributesSha256) { throw "PR6_PUBLICATION_INPUT_BINDING_MISMATCH" }
    ...
    $livePrBodyBlobOid = ((@(& $verifiedGit rev-parse ($reviewedHead + ":" + $prBodyPath))) -join "`n").Trim()
    if ($LASTEXITCODE -ne 0 -or $livePrBodyBlobOid -cne $prBodyBlobOid) { throw "PR6_REVIEWED_BODY_BLOB_OID_MISMATCH" }
    $livePrBodyHashResults = @(& $verifiedGitBlobSha256 cat-file blob $livePrBodyBlobOid)
    if ($livePrBodyHashResults.Count -ne 1 -or [string]$livePrBodyHashResults[0].sha256 -cne $prBodyBlobSha256 -or [long]$livePrBodyHashResults[0].byteLength -ne $prBodyBlobByteLength) { throw "PR6_REVIEWED_BODY_RAW_BLOB_MISMATCH" }
    ...
    $expectedBodyBytes = [Text.UTF8Encoding]::new($false, $true).GetBytes($expectedBody)
    $expectedBodySha256 = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($expectedBodyBytes)).ToLowerInvariant()
    $expectedBodyByteLength = [long]$expectedBodyBytes.Length
    [Array]::Clear($expectedBodyBytes, 0, $expectedBodyBytes.Length)
    if ($expectedBodySha256 -cne $prBodyCanonicalTextSha256 -or $expectedBodyByteLength -ne $prBodyCanonicalTextByteLength) { throw "PR6_REVIEWED_BODY_CANONICAL_TEXT_MISMATCH" }
    ...
    $createOutput = @(& $verifiedGh pr create --repo $expectedRepo --draft --base $expectedBaseBranch --head $expectedBranch --title $expectedTitle --body $expectedBody)
    ...
    Close the create epoch immediately after the exact SUCCEEDED receipt and captured URL/number reach the launcher. The launcher validates that receipt, resolves the PR-number runtime slot declared by the next PUBLICATION template, issues a new `epoch: "POST_CREATE"` `hkwtia.pr6.publication-input.v2` plus command-phase projection, and starts a fresh capability-free publication shell. No mutable shell variable or function crosses epochs. Rerun Task 0 bootstrap; resolve the continuation, review package, create receipt, publication helper, and projection; rehydrate every scalar solely from those immutable records; recreate the verified invokers and helper closures; and recompute the reviewed-blob body before any post-create read:

    $continuationInputs = @(& $runtimeInputRecordResolver -Path $operationInputRecordPath -Sha256 $operationInputRecordSha256 -ExpectedSchema "hkwtia.pr6.publication-input.v2")
    if ($continuationInputs.Count -ne 1) { throw "PR6_PUBLICATION_CONTINUATION_COUNT_INVALID" }
    $publicationInput = $continuationInputs[0]
    if ([string]$publicationInput.epoch -cne "POST_CREATE") { throw "PR6_PUBLICATION_POST_CREATE_EPOCH_REQUIRED" }
    $reviewPackages = @(& $runtimeInputRecordResolver -Path ([string]$publicationInput.reviewPackagePath) -Sha256 ([string]$publicationInput.reviewPackageSha256) -ExpectedSchema "hkwtia.pr6.zero-finding-review-package.v2")
    $createReceiptResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationInput.createCompletionReceiptPath) -Sha256 ([string]$publicationInput.createCompletionReceiptSha256) -ExpectedSchema "hkwtia.pr6.publication-operation-completion.v1")
    $continuationProjectionResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationInput.publicationCommandPhaseProjectionPath) -Sha256 ([string]$publicationInput.publicationCommandPhaseProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-phase-projection.v1")
    $publicationHelperResults = @(& $runtimeApprovedScriptResolver -Path ([string]$publicationInput.publicationStateAssertionsPath) -AllowedRoot ([string]$publicationInput.publicationStateAssertionsRoot) -Sha256 ([string]$publicationInput.publicationStateAssertionsSha256))
    if ($reviewPackages.Count -ne 1 -or $createReceiptResults.Count -ne 1 -or $continuationProjectionResults.Count -ne 1 -or $publicationHelperResults.Count -ne 1) { throw "PR6_PUBLICATION_CONTINUATION_REFERENCE_COUNT_INVALID" }
    $reviewPackage = $reviewPackages[0]
    $createReceipt = $createReceiptResults[0]
    $continuationProjection = $continuationProjectionResults[0]
    $expectedOwner = "YNWAforever"
    $expectedRepo = "YNWAforever/wisetech"
    $expectedGitRemote = "https://github.com/YNWAforever/wisetech.git"
    $expectedBranch = [string]$publicationInput.headBranch
    $expectedBaseBranch = [string]$publicationInput.baseBranch
    $expectedBase = [string]$publicationInput.baseOid
    $reviewedHead = [string]$publicationInput.reviewedHead
    $requiredCommandMatrixSha256 = [string]$publicationInput.requiredCommandMatrixSha256
    $publicationPartitionEntryCount = [int]$publicationInput.publicationPartitionEntryCount
    $publicationPartitionEntryAggregateSha256 = [string]$publicationInput.publicationPartitionEntryAggregateSha256
    $publicationApprovalPath = [string]$publicationInput.publicationApprovalPath
    $publicationApprovalSha256 = [string]$publicationInput.publicationApprovalSha256
    $bodyPath = [string]$publicationInput.prBodyPath
    $prBodyBlobOid = [string]$publicationInput.prBodyBlobOid
    $prBodyBlobSha256 = [string]$publicationInput.prBodyBlobSha256
    $prBodyBlobByteLength = [long]$publicationInput.prBodyBlobByteLength
    $prBodyCanonicalTextSha256 = [string]$publicationInput.prBodyCanonicalTextSha256
    $prBodyCanonicalTextByteLength = [long]$publicationInput.prBodyCanonicalTextByteLength
    $expectedTitle = [string]$publicationInput.prTitle
    $createdPrNumber = [string]$createReceipt.prNumber
    $prUrl = [string]$createReceipt.prUrl
    $remoteHeadRef = "refs/heads/$expectedBranch"
    if ([string]$continuationProjection.partition -cne "PUBLICATION" -or [string]$continuationProjection.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [string]$createReceipt.reviewedHead -cne $reviewedHead -or [string]$createReceipt.prNumber -cne [string]$publicationInput.prNumber -or [string]$createReceipt.prUrl -cne [string]$publicationInput.prUrl -or $bodyPath -cne "docs/integration/wisetech-pr6-pr-body.md" -or $prBodyBlobOid -cnotmatch "^[0-9a-f]{40}$" -or $prBodyBlobSha256 -cnotmatch "^[0-9a-f]{64}$" -or $prBodyCanonicalTextSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_PUBLICATION_CONTINUATION_BINDING_INVALID" }
    $verifiedGit = { & $runtimeInvoker -NativeProcessBoundaryScriptBlock $runtimeNativeProcessBoundary -GuardianScriptBlock $runtimeGuardian -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -OperationInputRecordPath $operationInputRecordPath -OperationInputRecordSha256 $operationInputRecordSha256 -Tool Git -PublicationApprovalPath $publicationApprovalPath -PublicationApprovalSha256 $publicationApprovalSha256 -Arguments @($args) }.GetNewClosure()
    $verifiedGitBlobSha256 = { & $runtimeInvoker -NativeProcessBoundaryScriptBlock $runtimeNativeProcessBoundary -GuardianScriptBlock $runtimeGuardian -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -OperationInputRecordPath $operationInputRecordPath -OperationInputRecordSha256 $operationInputRecordSha256 -Tool GitBlobSha256 -PublicationApprovalPath $publicationApprovalPath -PublicationApprovalSha256 $publicationApprovalSha256 -Arguments @($args) }.GetNewClosure()
    $verifiedGh = { & $runtimeInvoker -NativeProcessBoundaryScriptBlock $runtimeNativeProcessBoundary -GuardianScriptBlock $runtimeGuardian -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -OperationInputRecordPath $operationInputRecordPath -OperationInputRecordSha256 $operationInputRecordSha256 -Tool GitHubCli -PublicationApprovalPath $publicationApprovalPath -PublicationApprovalSha256 $publicationApprovalSha256 -Arguments @($args) }.GetNewClosure()
    $publicationAssertions = & $publicationHelperResults[0] -VerifiedGit $verifiedGit -VerifiedGitBlobSha256 $verifiedGitBlobSha256 -VerifiedGh $verifiedGh -PublicationInput $publicationInput -ReviewPackage $reviewPackage
    if ($publicationAssertions.Count -ne 1) { throw "PR6_PUBLICATION_ASSERTION_HELPER_RESULT_COUNT_INVALID" }
    $AssertPr6LocalState = $publicationAssertions[0].AssertLocalState
    $AssertPr6RemoteBase = $publicationAssertions[0].AssertRemoteBase
    $GetPr6RemoteOid = $publicationAssertions[0].GetRemoteOid
    $GetPr6OpenHeadPrs = $publicationAssertions[0].GetOpenHeadPrs
    $bodySpec = "${reviewedHead}:$bodyPath"
    $bodyLines = @(& $verifiedGit show --no-ext-diff --no-textconv $bodySpec)
    if ($LASTEXITCODE -ne 0) { throw "PR6_CONTINUATION_BODY_READ_FAILED" }
    $expectedBody = (($bodyLines -join "`n") -replace "`r`n", "`n").TrimEnd("`r", "`n")
    $expectedBodyBytes = [Text.UTF8Encoding]::new($false, $true).GetBytes($expectedBody)
    $expectedBodySha256 = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($expectedBodyBytes)).ToLowerInvariant()
    $expectedBodyByteLength = [long]$expectedBodyBytes.Length
    [Array]::Clear($expectedBodyBytes, 0, $expectedBodyBytes.Length)
    if ([string]::IsNullOrWhiteSpace($expectedBody) -or $expectedBodySha256 -cne $prBodyCanonicalTextSha256 -or $expectedBodyByteLength -ne $prBodyCanonicalTextByteLength) { throw "PR6_CONTINUATION_BODY_BINDING_INVALID" }

    function Assert-Pr6LocalState([string] $phase) { & $AssertPr6LocalState $phase }
    function Assert-Pr6RemoteBase([string] $phase) { & $AssertPr6RemoteBase $phase }
    function Get-Pr6RemoteOid([string] $ref, [string] $failure) { & $GetPr6RemoteOid $ref $failure }
    function Get-Pr6OpenHeadPrs([string] $phase) { @(& $GetPr6OpenHeadPrs $phase) }

    The helper is captured/hash-checked before evaluation, closes only over the newly created invokers and immutable input objects, and returns no mutable path. Contract tests launch the POST_CREATE shell with an empty scope and prove every missing scalar/function, helper drift, inherited variable/function, body mismatch, or create-receipt mismatch fails before GitHub read.
    ...
    $viewJsonLines = @(& $verifiedGh pr view $createdPrNumber --repo $expectedRepo --json url,title,state,isDraft,baseRefName,baseRefOid,headRefName,headRefOid,headRepositoryOwner,body,mergeStateStatus,statusCheckRollup)
    ...
    if ($published.title -cne $expectedTitle) { throw "PR6_PR_TITLE_MISMATCH" }
    ...
    On the uninterrupted path, send only the sanitized validated PR state and the exact final view/base/head/open-set result-receipt identities to the launcher, then close the POST_CREATE shell. The launcher validates those identities against the PUBLICATION projection and freezes `hkwtia.pr6.publication-final-state.v1` before any completion input is issued. That receipt is the sole success-state source for the terminal shell; title, body hashes and byte lengths, draft/open state, base/head/owner, remote head, unique open-head set, and create receipt must all agree.

    Whether the publication path succeeded, stopped before or after mutation, or ran reconciliation, send only its sanitized outcome plus ordered publication operation/authority/refusal/applicability/final-state/reconciliation receipt hashes over the launcher channel and close the last publication or reconciliation shell. Trusted launcher/CI freezes `hkwtia.pr6.publication-command-evidence-manifest.v1`; its projections and entries must exactly, once each and in root order, exhaust PUBLICATION, including every branch proved not applicable. It first issues the dependency-ready TERMINAL_ATTESTATION projection from only the immutable review package, publication manifest, and completed receipts. It then issues acyclic `hkwtia.pr6.publication-completion.v2` with a closed PASSED/NOT_PASSED union and starts one capability-free evidence shell. Rerun Task 0 bootstrap and resolve only that record and its referenced immutable evidence:

    $publicationCompletionInputs = @(& $runtimeInputRecordResolver -Path $operationInputRecordPath -Sha256 $operationInputRecordSha256 -ExpectedSchema "hkwtia.pr6.publication-completion.v2")
    if ($publicationCompletionInputs.Count -ne 1) { throw "PR6_PUBLICATION_COMPLETION_COUNT_INVALID" }
    $publicationCompletion = $publicationCompletionInputs[0]
    $completionReviewPackageResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.reviewPackagePath) -Sha256 ([string]$publicationCompletion.reviewPackageSha256) -ExpectedSchema "hkwtia.pr6.zero-finding-review-package.v2")
    $publicationEvidenceManifestResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.publicationCommandEvidenceManifestPath) -Sha256 ([string]$publicationCompletion.publicationCommandEvidenceManifestSha256) -ExpectedSchema "hkwtia.pr6.publication-command-evidence-manifest.v1")
    $terminalAttestationProjectionResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.terminalAttestationCommandPhaseProjectionPath) -Sha256 ([string]$publicationCompletion.terminalAttestationCommandPhaseProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-phase-projection.v1")
    if ($completionReviewPackageResults.Count -ne 1 -or $publicationEvidenceManifestResults.Count -ne 1 -or $terminalAttestationProjectionResults.Count -ne 1) { throw "PR6_PUBLICATION_COMPLETION_REFERENCE_COUNT_INVALID" }
    $completionReviewPackage = $completionReviewPackageResults[0]
    $publicationEvidenceManifest = $publicationEvidenceManifestResults[0]
    $terminalAttestationProjection = $terminalAttestationProjectionResults[0]
    $publicationEvidenceEntries = @($publicationEvidenceManifest.entries)
    $expectedRepo = "YNWAforever/wisetech"
    $expectedTitle = "feat: align WiseTech Join Portal and Admin"
    $expectedOwner = "YNWAforever"
    $expectedBaseBranch = "codex/wisetech-pr5-public-journeys"
    $expectedBase = "3856dd71842f9a2e1d9c4b7a46521416a5bd83ae"
    $expectedBranch = "codex/wisetech-pr6-join-portal-admin"
    $expectedRemoteRef = "refs/heads/codex/wisetech-pr6-join-portal-admin"
    $reviewedHead = [string]$completionReviewPackage.reviewedHead
    $requiredCommandMatrixPath = [string]$completionReviewPackage.requiredCommandMatrixPath
    $requiredCommandMatrixSha256 = [string]$completionReviewPackage.requiredCommandMatrixSha256
    $publicationPartitionEntryCount = [int]$completionReviewPackage.publicationPartitionEntryCount
    $publicationPartitionEntryAggregateSha256 = [string]$completionReviewPackage.publicationPartitionEntryAggregateSha256
    $terminalAttestationPartitionEntryCount = [int]$completionReviewPackage.terminalAttestationPartitionEntryCount
    $terminalAttestationPartitionEntryAggregateSha256 = [string]$completionReviewPackage.terminalAttestationPartitionEntryAggregateSha256
    $completionStatus = [string]$publicationCompletion.status
    if ($completionStatus -cnotin @("PASSED", "NOT_PASSED")) { throw "PR6_PUBLICATION_COMPLETION_STATUS_INVALID" }
    if ([string]$publicationCompletion.repository -cne $expectedRepo -or [string]$publicationCompletion.reviewedHead -cne $reviewedHead -or [string]$publicationCompletion.requiredCommandMatrixPath -cne $requiredCommandMatrixPath -or [string]$publicationCompletion.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$publicationCompletion.publicationPartitionEntryCount -ne $publicationPartitionEntryCount -or [string]$publicationCompletion.publicationPartitionEntryAggregateSha256 -cne $publicationPartitionEntryAggregateSha256 -or [int]$publicationCompletion.terminalAttestationPartitionEntryCount -ne $terminalAttestationPartitionEntryCount -or [string]$publicationCompletion.terminalAttestationPartitionEntryAggregateSha256 -cne $terminalAttestationPartitionEntryAggregateSha256 -or [string]$publicationCompletion.prTitle -cne $expectedTitle -or [string]$publicationCompletion.prBodyPath -cne [string]$completionReviewPackage.prBodyPath -or [string]$publicationCompletion.prBodyBlobOid -cne [string]$completionReviewPackage.prBodyBlobOid -or [string]$publicationCompletion.prBodyBlobSha256 -cne [string]$completionReviewPackage.prBodyBlobSha256 -or [long]$publicationCompletion.prBodyBlobByteLength -ne [long]$completionReviewPackage.prBodyBlobByteLength -or [string]$publicationCompletion.prBodyCanonicalTextSha256 -cne [string]$completionReviewPackage.prBodyCanonicalTextSha256 -or [long]$publicationCompletion.prBodyCanonicalTextByteLength -ne [long]$completionReviewPackage.prBodyCanonicalTextByteLength) { throw "PR6_PUBLICATION_COMPLETION_COMMON_BINDING_INVALID" }
    if ([string]$publicationEvidenceManifest.partition -cne "PUBLICATION" -or [string]$publicationEvidenceManifest.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$publicationEvidenceManifest.entryCount -ne $publicationPartitionEntryCount -or $publicationEvidenceEntries.Count -ne $publicationPartitionEntryCount -or [string]$publicationEvidenceManifest.entryAggregateSha256 -cne $publicationPartitionEntryAggregateSha256 -or [string]$publicationEvidenceManifest.fullEntryEvidenceAggregateSha256 -cne [string]$publicationCompletion.fullEntryEvidenceAggregateSha256) { throw "PR6_PUBLICATION_MANIFEST_COMPLETION_BINDING_INVALID" }
    if (-not [Object]::Equals($publicationEvidenceManifest.finalStateReceiptPath, $publicationCompletion.finalStateReceiptPath) -or -not [Object]::Equals($publicationEvidenceManifest.finalStateReceiptSha256, $publicationCompletion.finalStateReceiptSha256) -or -not [Object]::Equals($publicationEvidenceManifest.stoppedCommandEntryIdentitySha256, $publicationCompletion.stoppedCommandEntryIdentitySha256) -or -not [Object]::Equals($publicationEvidenceManifest.stoppedCommandResultProjectionPath, $publicationCompletion.stoppedCommandResultProjectionPath) -or -not [Object]::Equals($publicationEvidenceManifest.stoppedCommandResultProjectionSha256, $publicationCompletion.stoppedCommandResultProjectionSha256) -or -not [Object]::Equals($publicationEvidenceManifest.uncertainMutationReceiptPath, $publicationCompletion.uncertainMutationReceiptPath) -or -not [Object]::Equals($publicationEvidenceManifest.uncertainMutationReceiptSha256, $publicationCompletion.uncertainMutationReceiptSha256) -or -not [Object]::Equals($publicationEvidenceManifest.reconciliationDescriptorPath, $publicationCompletion.reconciliationDescriptorPath) -or -not [Object]::Equals($publicationEvidenceManifest.reconciliationDescriptorSha256, $publicationCompletion.reconciliationDescriptorSha256) -or -not [Object]::Equals($publicationEvidenceManifest.reconciliationResultReceiptPath, $publicationCompletion.reconciliationResultReceiptPath) -or -not [Object]::Equals($publicationEvidenceManifest.reconciliationResultReceiptSha256, $publicationCompletion.reconciliationResultReceiptSha256) -or -not [Object]::Equals($publicationEvidenceManifest.lastSuccessfulMutationKind, $publicationCompletion.lastSuccessfulMutationKind)) { throw "PR6_PUBLICATION_MANIFEST_TERMINAL_SOURCE_MISMATCH" }
    if ([string]$terminalAttestationProjection.partition -cne "TERMINAL_ATTESTATION" -or [string]$terminalAttestationProjection.requiredCommandMatrixPath -cne $requiredCommandMatrixPath -or [string]$terminalAttestationProjection.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$terminalAttestationProjection.entryCount -ne $terminalAttestationPartitionEntryCount -or [string]$terminalAttestationProjection.entryAggregateSha256 -cne $terminalAttestationPartitionEntryAggregateSha256) { throw "PR6_TERMINAL_ATTESTATION_PROJECTION_INVALID" }

    $completionArrays = @(
      @($publicationCompletion.entryIdentitySha256s),
      @($publicationCompletion.commandSourceManifestSha256s),
      @($publicationCompletion.resultProjectionSha256s),
      @($publicationCompletion.cleanupReceiptSha256s),
      @($publicationCompletion.separateAuthorityReceiptSha256s),
      @($publicationCompletion.launcherRefusalReceiptSha256s),
      @($publicationCompletion.applicabilityReceiptSha256s)
    )
    foreach ($completionArray in $completionArrays) {
      if ($completionArray.Count -ne $publicationEvidenceEntries.Count) { throw "PR6_PUBLICATION_COMPLETION_EVIDENCE_CARDINALITY_INVALID" }
    }
    for ($publicationEntryIndex = 0; $publicationEntryIndex -lt $publicationEvidenceEntries.Count; $publicationEntryIndex += 1) {
      $publicationEntry = $publicationEvidenceEntries[$publicationEntryIndex]
      if ([string]$publicationCompletion.entryIdentitySha256s[$publicationEntryIndex] -cne [string]$publicationEntry.entryIdentitySha256 -or [string]$publicationCompletion.commandSourceManifestSha256s[$publicationEntryIndex] -cne [string]$publicationEntry.commandSourceManifestSha256 -or [string]$publicationCompletion.resultProjectionSha256s[$publicationEntryIndex] -cne [string]$publicationEntry.resultProjectionSha256 -or [string]$publicationCompletion.cleanupReceiptSha256s[$publicationEntryIndex] -cne [string]$publicationEntry.cleanupReceiptSha256 -or -not [Object]::Equals($publicationCompletion.separateAuthorityReceiptSha256s[$publicationEntryIndex], $publicationEntry.separateAuthorityReceiptSha256) -or -not [Object]::Equals($publicationCompletion.launcherRefusalReceiptSha256s[$publicationEntryIndex], $publicationEntry.launcherRefusalReceiptSha256) -or -not [Object]::Equals($publicationCompletion.applicabilityReceiptSha256s[$publicationEntryIndex], $publicationEntry.applicabilityReceiptSha256)) { throw "PR6_PUBLICATION_COMPLETION_ORDERED_EVIDENCE_MISMATCH" }
    }

    $prIdentityPresent = ($null -ne $publicationCompletion.prNumber -or $null -ne $publicationCompletion.prUrl)
    if ($prIdentityPresent -and ($null -eq $publicationCompletion.prNumber -or $null -eq $publicationCompletion.prUrl)) { throw "PR6_PUBLICATION_COMPLETION_PARTIAL_PR_IDENTITY" }
    $createCompletionReceiptPresent = ($null -ne $publicationCompletion.createCompletionReceiptPath -or $null -ne $publicationCompletion.createCompletionReceiptSha256)
    if ($createCompletionReceiptPresent -and ([string]::IsNullOrWhiteSpace([string]$publicationCompletion.createCompletionReceiptPath) -or [string]$publicationCompletion.createCompletionReceiptSha256 -cnotmatch "^[0-9a-f]{64}$")) { throw "PR6_PUBLICATION_COMPLETION_PARTIAL_CREATE_RECEIPT" }
    $createCompletionReceipt = $null
    if ($createCompletionReceiptPresent) {
      if (-not $prIdentityPresent) { throw "PR6_PUBLICATION_CREATE_RECEIPT_WITHOUT_PR_IDENTITY" }
      $createCompletionReceiptResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.createCompletionReceiptPath) -Sha256 ([string]$publicationCompletion.createCompletionReceiptSha256) -ExpectedSchema "hkwtia.pr6.publication-operation-completion.v1")
      if ($createCompletionReceiptResults.Count -ne 1) { throw "PR6_PUBLICATION_CREATE_COMPLETION_COUNT_INVALID" }
      $createCompletionReceipt = $createCompletionReceiptResults[0]
      if ([string]$createCompletionReceipt.reviewedHead -cne $reviewedHead -or [string]$createCompletionReceipt.prNumber -cne [string]$publicationCompletion.prNumber -or [string]$createCompletionReceipt.prUrl -cne [string]$publicationCompletion.prUrl) { throw "PR6_PUBLICATION_CREATE_COMPLETION_BINDING_INVALID" }
    }

    if ($completionStatus -ceq "PASSED") {
      if (-not $prIdentityPresent -or -not $createCompletionReceiptPresent -or [string]$publicationCompletion.lastSuccessfulMutationKind -cne "CREATE" -or [string]$publicationCompletion.finalState -cne "OPEN_DRAFT_EXACT" -or [string]::IsNullOrWhiteSpace([string]$publicationCompletion.finalStateReceiptPath) -or [string]$publicationCompletion.finalStateReceiptSha256 -cnotmatch "^[0-9a-f]{64}$" -or $null -ne $publicationCompletion.failureKind -or $null -ne $publicationCompletion.stoppedCommandEntryIdentitySha256 -or $null -ne $publicationCompletion.stoppedCommandResultProjectionPath -or $null -ne $publicationCompletion.stoppedCommandResultProjectionSha256 -or $null -ne $publicationCompletion.uncertainMutationReceiptPath -or $null -ne $publicationCompletion.uncertainMutationReceiptSha256 -or $null -ne $publicationCompletion.reconciliationDescriptorPath -or $null -ne $publicationCompletion.reconciliationDescriptorSha256 -or $null -ne $publicationCompletion.reconciliationResultReceiptPath -or $null -ne $publicationCompletion.reconciliationResultReceiptSha256 -or -not [Object]::Equals($publicationCompletion.retryAttempted, $false) -or -not [Object]::Equals($publicationCompletion.continuedMutation, $false)) { throw "PR6_PUBLICATION_PASSED_UNION_INVALID" }
      $finalStateReceiptResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.finalStateReceiptPath) -Sha256 ([string]$publicationCompletion.finalStateReceiptSha256) -ExpectedSchema "hkwtia.pr6.publication-final-state.v1")
      if ($finalStateReceiptResults.Count -ne 1) { throw "PR6_PUBLICATION_FINAL_STATE_COUNT_INVALID" }
      $finalStateReceipt = $finalStateReceiptResults[0]
      if ([string]$finalStateReceipt.status -cne "PASSED" -or [string]$finalStateReceipt.repository -cne $expectedRepo -or [string]$finalStateReceipt.reviewedHead -cne $reviewedHead -or [string]$finalStateReceipt.prNumber -cne [string]$publicationCompletion.prNumber -or [string]$finalStateReceipt.prUrl -cne [string]$publicationCompletion.prUrl -or [string]$finalStateReceipt.prTitle -cne $expectedTitle -or [string]$finalStateReceipt.prBodyPath -cne [string]$completionReviewPackage.prBodyPath -or [string]$finalStateReceipt.prBodyBlobOid -cne [string]$completionReviewPackage.prBodyBlobOid -or [string]$finalStateReceipt.prBodyBlobSha256 -cne [string]$completionReviewPackage.prBodyBlobSha256 -or [long]$finalStateReceipt.prBodyBlobByteLength -ne [long]$completionReviewPackage.prBodyBlobByteLength -or [string]$finalStateReceipt.prBodyCanonicalTextSha256 -cne [string]$completionReviewPackage.prBodyCanonicalTextSha256 -or [long]$finalStateReceipt.prBodyCanonicalTextByteLength -ne [long]$completionReviewPackage.prBodyCanonicalTextByteLength -or [string]$finalStateReceipt.state -cne "OPEN" -or [bool]$finalStateReceipt.isDraft -ne $true -or [string]$finalStateReceipt.baseRefName -cne $expectedBaseBranch -or [string]$finalStateReceipt.baseRefOid -cne $expectedBase -or [string]$finalStateReceipt.headRefName -cne $expectedBranch -or [string]$finalStateReceipt.headRefOid -cne $reviewedHead -or [string]$finalStateReceipt.headRepositoryOwner -cne $expectedOwner -or [string]$finalStateReceipt.remoteHeadOid -cne $reviewedHead -or [int]$finalStateReceipt.openHeadPrCount -ne 1 -or [string]$finalStateReceipt.createCompletionReceiptPath -cne [string]$publicationCompletion.createCompletionReceiptPath -or [string]$finalStateReceipt.createCompletionReceiptSha256 -cne [string]$publicationCompletion.createCompletionReceiptSha256 -or [string]$finalStateReceipt.finalReadResultReceiptAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$finalStateReceipt.finalReadResultReceiptAggregateSha256 -cne [string]$publicationEvidenceManifest.finalReadResultReceiptAggregateSha256) { throw "PR6_PUBLICATION_FINAL_STATE_BINDING_INVALID" }
    } else {
      $allowedFailureKinds = @("PRE_MUTATION", "RECONCILED_UNCERTAINTY", "POST_MUTATION_VALIDATION")
      if ([string]$publicationCompletion.failureKind -cnotin $allowedFailureKinds -or $null -ne $publicationEvidenceManifest.finalReadResultReceiptAggregateSha256 -or [string]$publicationCompletion.stoppedCommandEntryIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]::IsNullOrWhiteSpace([string]$publicationCompletion.stoppedCommandResultProjectionPath) -or [string]$publicationCompletion.stoppedCommandResultProjectionSha256 -cnotmatch "^[0-9a-f]{64}$" -or -not [Object]::Equals($publicationCompletion.retryAttempted, $false) -or -not [Object]::Equals($publicationCompletion.continuedMutation, $false) -or $null -ne $publicationCompletion.finalState -or $null -ne $publicationCompletion.finalStateReceiptPath -or $null -ne $publicationCompletion.finalStateReceiptSha256) { throw "PR6_PUBLICATION_NOT_PASSED_UNION_INVALID" }
      $lastSuccessfulMutationKind = $publicationCompletion.lastSuccessfulMutationKind
      if ($null -ne $lastSuccessfulMutationKind -and [string]$lastSuccessfulMutationKind -cnotin @("PUSH", "CREATE")) { throw "PR6_PUBLICATION_LAST_MUTATION_KIND_INVALID" }
      if ([string]$publicationCompletion.failureKind -ceq "PRE_MUTATION" -and ($null -ne $lastSuccessfulMutationKind -or $prIdentityPresent -or $createCompletionReceiptPresent)) { throw "PR6_PUBLICATION_PRE_MUTATION_IDENTITY_INVALID" }
      if ([string]$publicationCompletion.failureKind -ceq "POST_MUTATION_VALIDATION" -and $null -eq $lastSuccessfulMutationKind) { throw "PR6_PUBLICATION_POST_MUTATION_KIND_MISSING" }
      if ([string]$lastSuccessfulMutationKind -ceq "CREATE" -and (-not $prIdentityPresent -or -not $createCompletionReceiptPresent)) { throw "PR6_PUBLICATION_COMPLETED_CREATE_IDENTITY_INVALID" }
      if ([string]$lastSuccessfulMutationKind -cne "CREATE" -and $createCompletionReceiptPresent) { throw "PR6_PUBLICATION_UNEXPECTED_CREATE_COMPLETION" }
      if ([string]$publicationCompletion.failureKind -ceq "POST_MUTATION_VALIDATION" -and [string]$lastSuccessfulMutationKind -ceq "PUSH" -and $prIdentityPresent) { throw "PR6_PUBLICATION_POST_PUSH_PR_IDENTITY_INVALID" }
      $stoppedCommandResultProjectionResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.stoppedCommandResultProjectionPath) -Sha256 ([string]$publicationCompletion.stoppedCommandResultProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-result-projection.v1")
      $stoppedCommandEntries = @($publicationEvidenceEntries | Where-Object { [string]$_.entryIdentitySha256 -ceq [string]$publicationCompletion.stoppedCommandEntryIdentitySha256 -and [string]$_.resultProjectionSha256 -ceq [string]$publicationCompletion.stoppedCommandResultProjectionSha256 })
      if ($stoppedCommandResultProjectionResults.Count -ne 1 -or $stoppedCommandEntries.Count -ne 1 -or [string]$stoppedCommandEntries[0].resultProjectionPath -cne [string]$publicationCompletion.stoppedCommandResultProjectionPath -or [string]$stoppedCommandResultProjectionResults[0].entryIdentitySha256 -cne [string]$publicationCompletion.stoppedCommandEntryIdentitySha256 -or [string]$stoppedCommandResultProjectionResults[0].outcome -cne "NOT_PASSED" -or [string]$stoppedCommandResultProjectionResults[0].failureKind -cne [string]$publicationCompletion.failureKind -or -not [Object]::Equals($stoppedCommandResultProjectionResults[0].lastSuccessfulMutationKind, $publicationCompletion.lastSuccessfulMutationKind)) { throw "PR6_PUBLICATION_STOPPED_COMMAND_RESULT_INVALID" }
      $stoppedCommandResultProjection = $stoppedCommandResultProjectionResults[0]
      $uncertainMutationReceiptPresent = ($null -ne $publicationCompletion.uncertainMutationReceiptPath -or $null -ne $publicationCompletion.uncertainMutationReceiptSha256)
      if ($uncertainMutationReceiptPresent -and ([string]::IsNullOrWhiteSpace([string]$publicationCompletion.uncertainMutationReceiptPath) -or [string]$publicationCompletion.uncertainMutationReceiptSha256 -cnotmatch "^[0-9a-f]{64}$")) { throw "PR6_PUBLICATION_UNCERTAIN_MUTATION_RECEIPT_PARTIAL" }
      if (-not [Object]::Equals($stoppedCommandResultProjection.uncertainMutationReceiptPath, $publicationCompletion.uncertainMutationReceiptPath) -or -not [Object]::Equals($stoppedCommandResultProjection.uncertainMutationReceiptSha256, $publicationCompletion.uncertainMutationReceiptSha256)) { throw "PR6_PUBLICATION_STOPPED_COMMAND_UNCERTAIN_RECEIPT_MISMATCH" }
      if ([string]$publicationCompletion.failureKind -ceq "RECONCILED_UNCERTAINTY") {
        if (-not $uncertainMutationReceiptPresent) { throw "PR6_PUBLICATION_UNCERTAIN_MUTATION_RECEIPT_REQUIRED" }
        $uncertainMutationReceiptResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.uncertainMutationReceiptPath) -Sha256 ([string]$publicationCompletion.uncertainMutationReceiptSha256) -ExpectedSchema "hkwtia.pr6.publication-operation-outcome.v1")
        if ($uncertainMutationReceiptResults.Count -ne 1) { throw "PR6_PUBLICATION_UNCERTAIN_MUTATION_OUTCOME_COUNT_INVALID" }
        $uncertainStartedOperationResults = @(& $runtimeInputRecordResolver -Path ([string]$uncertainMutationReceiptResults[0].startedOperationReceiptPath) -Sha256 ([string]$uncertainMutationReceiptResults[0].startedOperationReceiptSha256) -ExpectedSchema "hkwtia.pr6.publication-operation.v1")
        $reconciliationDescriptorResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.reconciliationDescriptorPath) -Sha256 ([string]$publicationCompletion.reconciliationDescriptorSha256) -ExpectedSchema "hkwtia.pr6.publication-reconciliation.v1")
        $reconciliationResultResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.reconciliationResultReceiptPath) -Sha256 ([string]$publicationCompletion.reconciliationResultReceiptSha256) -ExpectedSchema "hkwtia.pr6.publication-reconciliation-result.v1")
        if ($uncertainStartedOperationResults.Count -ne 1) { throw "PR6_PUBLICATION_UNCERTAIN_STARTED_COUNT_INVALID" }
        $uncertainMutationOutcome = $uncertainMutationReceiptResults[0]
        $uncertainStartedOperation = $uncertainStartedOperationResults[0]
        $stoppedCommandEntry = $stoppedCommandEntries[0]
        foreach ($operationDuplicateField in @("operationId", "operationKind", "argumentVectorSha256", "entryIdentitySha256", "requiredCommandIdentitySha256", "commandProjectionSha256", "commandTemplateSha256", "runtimeSlotValuesSha256", "resolvedCommandSha256", "resolvedArgumentVectorSha256", "verifiedTool", "executionMode", "repository", "baseRefName", "baseRefOid", "headRefName", "headRefOid", "remoteRef", "reviewedHead", "capturedPrNumber", "publicationDescriptorSha256")) {
          if (-not [Object]::Equals($uncertainMutationOutcome.$operationDuplicateField, $uncertainStartedOperation.$operationDuplicateField)) { throw "PR6_PUBLICATION_UNCERTAIN_OUTCOME_STARTED_FIELD_MISMATCH" }
        }
        if ([string]::IsNullOrWhiteSpace([string]$uncertainMutationOutcome.startedOperationReceiptPath) -or [string]$uncertainMutationOutcome.startedOperationReceiptSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_PUBLICATION_UNCERTAIN_STARTED_IDENTITY_INVALID" }
        if ([string]$uncertainStartedOperation.entryIdentitySha256 -cne [string]$stoppedCommandEntry.entryIdentitySha256 -or [string]$uncertainStartedOperation.requiredCommandIdentitySha256 -cne [string]$stoppedCommandEntry.requiredCommandIdentitySha256 -or [string]$uncertainStartedOperation.commandProjectionSha256 -cne [string]$stoppedCommandEntry.commandProjectionSha256 -or [string]$uncertainStartedOperation.commandTemplateSha256 -cne [string]$stoppedCommandEntry.commandTemplateSha256 -or [string]$uncertainStartedOperation.runtimeSlotValuesSha256 -cne [string]$stoppedCommandEntry.runtimeSlotValuesSha256 -or [string]$uncertainStartedOperation.resolvedCommandSha256 -cne [string]$stoppedCommandEntry.resolvedCommandSha256 -or [string]$uncertainStartedOperation.resolvedArgumentVectorSha256 -cne [string]$stoppedCommandEntry.resolvedArgumentVectorSha256 -or [string]$uncertainStartedOperation.argumentVectorSha256 -cne [string]$stoppedCommandEntry.resolvedArgumentVectorSha256 -or [string]$uncertainStartedOperation.verifiedTool -cne [string]$stoppedCommandEntry.tool -or [string]$uncertainStartedOperation.executionMode -cne [string]$stoppedCommandEntry.mode -or [string]$uncertainStartedOperation.operationKind -cne [string]$stoppedCommandEntry.operationKind -or -not [Object]::Equals($uncertainStartedOperation.capturedPrNumber, $stoppedCommandEntry.capturedPrNumber) -or [string]$uncertainStartedOperation.publicationDescriptorSha256 -cne [string]$stoppedCommandEntry.publicationDescriptorSha256) { throw "PR6_PUBLICATION_UNCERTAIN_STOPPED_COMMAND_MISMATCH" }
        if ([string]$uncertainStartedOperation.repository -cne $expectedRepo -or [string]$uncertainStartedOperation.baseRefName -cne $expectedBaseBranch -or [string]$uncertainStartedOperation.baseRefOid -cne $expectedBase -or [string]$uncertainStartedOperation.headRefName -cne $expectedBranch -or [string]$uncertainStartedOperation.headRefOid -cne $reviewedHead -or [string]$uncertainStartedOperation.remoteRef -cne $expectedRemoteRef -or [string]$uncertainStartedOperation.reviewedHead -cne $reviewedHead) { throw "PR6_PUBLICATION_UNCERTAIN_TARGET_MISMATCH" }
        if ($uncertainStartedOperationResults.Count -ne 1 -or [string]$uncertainStartedOperationResults[0].status -cne "STARTED" -or [string]$uncertainStartedOperationResults[0].repository -cne $expectedRepo -or [string]$uncertainStartedOperationResults[0].reviewedHead -cne $reviewedHead -or [string]$uncertainStartedOperationResults[0].operationId -cne [string]$uncertainMutationReceiptResults[0].operationId -or [string]$uncertainMutationReceiptResults[0].status -cne "OUTCOME_UNCERTAIN" -or [string]$uncertainMutationReceiptResults[0].repository -cne $expectedRepo -or [string]$uncertainMutationReceiptResults[0].reviewedHead -cne $reviewedHead -or $reconciliationDescriptorResults.Count -ne 1 -or [string]$reconciliationDescriptorResults[0].repository -cne $expectedRepo -or [string]$reconciliationDescriptorResults[0].reviewedHead -cne $reviewedHead -or [string]$reconciliationDescriptorResults[0].operationOutcomeReceiptPath -cne [string]$publicationCompletion.uncertainMutationReceiptPath -or [string]$reconciliationDescriptorResults[0].operationOutcomeReceiptSha256 -cne [string]$publicationCompletion.uncertainMutationReceiptSha256 -or $reconciliationResultResults.Count -ne 1 -or [string]$reconciliationResultResults[0].repository -cne $expectedRepo -or [string]$reconciliationResultResults[0].reviewedHead -cne $reviewedHead -or -not [Object]::Equals($reconciliationResultResults[0].lastSuccessfulMutationKind, $publicationCompletion.lastSuccessfulMutationKind) -or [string]$reconciliationResultResults[0].stoppedCommandEntryIdentitySha256 -cne [string]$publicationCompletion.stoppedCommandEntryIdentitySha256 -or [string]$reconciliationResultResults[0].stoppedCommandResultProjectionPath -cne [string]$publicationCompletion.stoppedCommandResultProjectionPath -or [string]$reconciliationResultResults[0].stoppedCommandResultProjectionSha256 -cne [string]$publicationCompletion.stoppedCommandResultProjectionSha256 -or [string]$reconciliationResultResults[0].uncertainMutationReceiptPath -cne [string]$publicationCompletion.uncertainMutationReceiptPath -or [string]$reconciliationResultResults[0].uncertainMutationReceiptSha256 -cne [string]$publicationCompletion.uncertainMutationReceiptSha256 -or [string]$reconciliationResultResults[0].reconciliationDescriptorPath -cne [string]$publicationCompletion.reconciliationDescriptorPath -or [string]$reconciliationResultResults[0].reconciliationDescriptorSha256 -cne [string]$publicationCompletion.reconciliationDescriptorSha256 -or -not [Object]::Equals($reconciliationResultResults[0].prNumber, $publicationCompletion.prNumber) -or -not [Object]::Equals($reconciliationResultResults[0].prUrl, $publicationCompletion.prUrl) -or [string]$reconciliationResultResults[0].status -cne "NOT_PASSED" -or -not [Object]::Equals($reconciliationResultResults[0].mutationAttempted, $false) -or -not [Object]::Equals($reconciliationResultResults[0].retryAttempted, $false) -or -not [Object]::Equals($reconciliationResultResults[0].continuedMutation, $false)) { throw "PR6_PUBLICATION_RECONCILIATION_BINDING_INVALID" }
        $descriptorReadTemplates = @($reconciliationDescriptorResults[0].readOperationTemplates)
        $manifestReadOrdinals = @($publicationEvidenceManifest.reconciliationReadOrdinals)
        $manifestReadKinds = @($publicationEvidenceManifest.reconciliationReadKinds)
        $manifestReadArgumentVectorSha256s = @($publicationEvidenceManifest.reconciliationReadArgumentVectorSha256s)
        $manifestReadLaunchDispositions = @($publicationEvidenceManifest.reconciliationReadLaunchDispositions)
        $manifestReadStatuses = @($publicationEvidenceManifest.reconciliationReadStatuses)
        $manifestReadContinuations = @($publicationEvidenceManifest.reconciliationReadContinuationDispositions)
        $manifestReadReceiptPaths = @($publicationEvidenceManifest.reconciliationReadReceiptPaths)
        $manifestReadReceiptSha256s = @($publicationEvidenceManifest.reconciliationReadReceiptSha256s)
        $manifestReadClassifications = @($publicationEvidenceManifest.reconciliationReadClassifications)
        $resultReadOrdinals = @($reconciliationResultResults[0].reconciliationReadOrdinals)
        $resultReadKinds = @($reconciliationResultResults[0].reconciliationReadKinds)
        $resultReadArgumentVectorSha256s = @($reconciliationResultResults[0].reconciliationReadArgumentVectorSha256s)
        $resultReadLaunchDispositions = @($reconciliationResultResults[0].reconciliationReadLaunchDispositions)
        $resultReadStatuses = @($reconciliationResultResults[0].reconciliationReadStatuses)
        $resultReadContinuations = @($reconciliationResultResults[0].reconciliationReadContinuationDispositions)
        $resultReadReceiptPaths = @($reconciliationResultResults[0].reconciliationReadReceiptPaths)
        $resultReadReceiptSha256s = @($reconciliationResultResults[0].reconciliationReadReceiptSha256s)
        $resultReadClassifications = @($reconciliationResultResults[0].reconciliationReadClassifications)
        $manifestSuffixOrdinals = @($publicationEvidenceManifest.unexecutedReadTemplateOrdinals)
        $manifestSuffixApplicabilityPaths = @($publicationEvidenceManifest.unexecutedReadApplicabilityReceiptPaths)
        $manifestSuffixApplicabilitySha256s = @($publicationEvidenceManifest.unexecutedReadApplicabilityReceiptSha256s)
        $resultSuffixOrdinals = @($reconciliationResultResults[0].unexecutedReadTemplateOrdinals)
        $resultSuffixApplicabilityPaths = @($reconciliationResultResults[0].unexecutedReadApplicabilityReceiptPaths)
        $resultSuffixApplicabilitySha256s = @($reconciliationResultResults[0].unexecutedReadApplicabilityReceiptSha256s)
        $readCount = $manifestReadReceiptPaths.Count
        if ($descriptorReadTemplates.Count -lt 1 -or $readCount -lt 1 -or $readCount -gt $descriptorReadTemplates.Count) { throw "PR6_PUBLICATION_RECONCILIATION_PREFIX_CARDINALITY_INVALID" }
        foreach ($manifestReadArray in @($manifestReadOrdinals, $manifestReadKinds, $manifestReadArgumentVectorSha256s, $manifestReadLaunchDispositions, $manifestReadStatuses, $manifestReadContinuations, $manifestReadReceiptSha256s, $manifestReadClassifications)) {
          if ($manifestReadArray.Count -ne $readCount) { throw "PR6_PUBLICATION_RECONCILIATION_MANIFEST_READ_CARDINALITY_INVALID" }
        }
        foreach ($resultReadArray in @($resultReadOrdinals, $resultReadKinds, $resultReadArgumentVectorSha256s, $resultReadLaunchDispositions, $resultReadStatuses, $resultReadContinuations, $resultReadReceiptPaths, $resultReadReceiptSha256s, $resultReadClassifications)) {
          if ($resultReadArray.Count -ne $readCount) { throw "PR6_PUBLICATION_RECONCILIATION_RESULT_READ_CARDINALITY_INVALID" }
        }
        for ($descriptorIndex = 0; $descriptorIndex -lt $descriptorReadTemplates.Count; $descriptorIndex += 1) {
          $descriptorTemplate = $descriptorReadTemplates[$descriptorIndex]
          if ([int]$descriptorTemplate.ordinal -ne ($descriptorIndex + 1) -or [string]::IsNullOrWhiteSpace([string]$descriptorTemplate.readKind) -or [string]$descriptorTemplate.argumentVectorSha256 -cnotmatch "^[0-9a-f]{64}$" -or @($descriptorTemplate.allowedResultTuples).Count -lt 1) { throw "PR6_PUBLICATION_RECONCILIATION_DESCRIPTOR_TEMPLATE_INVALID" }
        }
        if ([string]$reconciliationDescriptorResults[0].readOperationTemplateAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$reconciliationResultResults[0].readOperationTemplateAggregateSha256 -cne [string]$reconciliationDescriptorResults[0].readOperationTemplateAggregateSha256 -or [string]$publicationEvidenceManifest.readOperationTemplateAggregateSha256 -cne [string]$reconciliationDescriptorResults[0].readOperationTemplateAggregateSha256 -or [string]$publicationEvidenceManifest.executedReadOperationTemplateAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$reconciliationResultResults[0].executedReadOperationTemplateAggregateSha256 -cne [string]$publicationEvidenceManifest.executedReadOperationTemplateAggregateSha256 -or [string]$publicationEvidenceManifest.reconciliationReadReceiptAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$publicationEvidenceManifest.reconciliationReadClassificationAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$reconciliationResultResults[0].reconciliationReadReceiptAggregateSha256 -cne [string]$publicationEvidenceManifest.reconciliationReadReceiptAggregateSha256 -or [string]$reconciliationResultResults[0].reconciliationReadClassificationAggregateSha256 -cne [string]$publicationEvidenceManifest.reconciliationReadClassificationAggregateSha256) { throw "PR6_PUBLICATION_RECONCILIATION_READ_AGGREGATE_INVALID" }
        $seenReconciliationReadReceiptHashes = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
        $seenReconciliationReadReceiptIdentities = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
        $terminalReadCommandEntry = $null
        $terminalReadReceiptPath = $null
        $terminalReadReceiptSha256 = $null
        for ($reconciliationReadIndex = 0; $reconciliationReadIndex -lt $readCount; $reconciliationReadIndex += 1) {
          $expectedReadTemplate = $descriptorReadTemplates[$reconciliationReadIndex]
          $expectedReadOrdinal = $reconciliationReadIndex + 1
          if ([int]$manifestReadOrdinals[$reconciliationReadIndex] -ne $expectedReadOrdinal -or [int]$resultReadOrdinals[$reconciliationReadIndex] -ne $expectedReadOrdinal -or [string]$manifestReadKinds[$reconciliationReadIndex] -cne [string]$expectedReadTemplate.readKind -or [string]$resultReadKinds[$reconciliationReadIndex] -cne [string]$manifestReadKinds[$reconciliationReadIndex] -or [string]$manifestReadArgumentVectorSha256s[$reconciliationReadIndex] -cne [string]$expectedReadTemplate.argumentVectorSha256 -or [string]$resultReadArgumentVectorSha256s[$reconciliationReadIndex] -cne [string]$manifestReadArgumentVectorSha256s[$reconciliationReadIndex] -or [string]$resultReadLaunchDispositions[$reconciliationReadIndex] -cne [string]$manifestReadLaunchDispositions[$reconciliationReadIndex] -or [string]$resultReadStatuses[$reconciliationReadIndex] -cne [string]$manifestReadStatuses[$reconciliationReadIndex] -or [string]$resultReadContinuations[$reconciliationReadIndex] -cne [string]$manifestReadContinuations[$reconciliationReadIndex] -or [string]$resultReadReceiptPaths[$reconciliationReadIndex] -cne [string]$manifestReadReceiptPaths[$reconciliationReadIndex] -or [string]$resultReadReceiptSha256s[$reconciliationReadIndex] -cne [string]$manifestReadReceiptSha256s[$reconciliationReadIndex] -or [string]$resultReadClassifications[$reconciliationReadIndex] -cne [string]$manifestReadClassifications[$reconciliationReadIndex]) { throw "PR6_PUBLICATION_RECONCILIATION_TEMPLATE_MAPPING_INVALID" }
          if ([string]::IsNullOrWhiteSpace([string]$manifestReadReceiptPaths[$reconciliationReadIndex]) -or [string]$manifestReadReceiptSha256s[$reconciliationReadIndex] -cnotmatch "^[0-9a-f]{64}$" -or -not $seenReconciliationReadReceiptHashes.Add([string]$manifestReadReceiptSha256s[$reconciliationReadIndex]) -or [string]::IsNullOrWhiteSpace([string]$manifestReadClassifications[$reconciliationReadIndex])) { throw "PR6_PUBLICATION_RECONCILIATION_READ_ORDER_INVALID" }
          $reconciliationReadReceiptResults = @(& $runtimeInputRecordResolver -Path ([string]$manifestReadReceiptPaths[$reconciliationReadIndex]) -Sha256 ([string]$manifestReadReceiptSha256s[$reconciliationReadIndex]) -ExpectedSchema "hkwtia.pr6.publication-read-operation-receipt.v1")
          if ($reconciliationReadReceiptResults.Count -ne 1) { throw "PR6_PUBLICATION_RECONCILIATION_READ_RECEIPT_COUNT_INVALID" }
          $reconciliationReadReceipt = $reconciliationReadReceiptResults[0]
          if ([string]$reconciliationReadReceipt.receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $seenReconciliationReadReceiptIdentities.Add([string]$reconciliationReadReceipt.receiptIdentitySha256) -or [string]$reconciliationReadReceipt.repository -cne $expectedRepo -or [string]$reconciliationReadReceipt.reviewedHead -cne $reviewedHead -or [string]$reconciliationReadReceipt.reconciliationDescriptorPath -cne [string]$publicationCompletion.reconciliationDescriptorPath -or [string]$reconciliationReadReceipt.reconciliationDescriptorSha256 -cne [string]$publicationCompletion.reconciliationDescriptorSha256 -or [string]$reconciliationReadReceipt.operationOutcomeReceiptPath -cne [string]$publicationCompletion.uncertainMutationReceiptPath -or [string]$reconciliationReadReceipt.operationOutcomeReceiptSha256 -cne [string]$publicationCompletion.uncertainMutationReceiptSha256 -or [int]$reconciliationReadReceipt.ordinal -ne $expectedReadOrdinal -or [string]$reconciliationReadReceipt.readKind -cne [string]$expectedReadTemplate.readKind -or [string]$reconciliationReadReceipt.argumentVectorSha256 -cne [string]$expectedReadTemplate.argumentVectorSha256 -or [string]$reconciliationReadReceipt.launchDisposition -cne [string]$manifestReadLaunchDispositions[$reconciliationReadIndex] -or [string]$reconciliationReadReceipt.status -cne [string]$manifestReadStatuses[$reconciliationReadIndex] -or [string]$reconciliationReadReceipt.continuationDisposition -cne [string]$manifestReadContinuations[$reconciliationReadIndex] -or [string]$reconciliationReadReceipt.classification -cne [string]$manifestReadClassifications[$reconciliationReadIndex] -or -not [Object]::Equals($reconciliationReadReceipt.mutationAttempted, $false)) { throw "PR6_PUBLICATION_RECONCILIATION_READ_RECEIPT_INVALID" }
          $readCommandEntries = @($publicationEvidenceEntries | Where-Object { [string]$_.commandClass -ceq "RECONCILIATION_READ" -and [int]$_.reconciliationTemplateOrdinal -eq $expectedReadOrdinal -and [string]$_.reconciliationReadKind -ceq [string]$expectedReadTemplate.readKind -and [string]$_.resolvedArgumentVectorSha256 -ceq [string]$expectedReadTemplate.argumentVectorSha256 -and [string]$_.launchDisposition -ceq [string]$reconciliationReadReceipt.launchDisposition })
          if ($readCommandEntries.Count -ne 1) { throw "PR6_PUBLICATION_RECONCILIATION_READ_COMMAND_ENTRY_INVALID" }
          $readCommandEntry = $readCommandEntries[0]
          if ([string]$reconciliationReadReceipt.commandEntryIdentitySha256 -cne [string]$readCommandEntry.entryIdentitySha256 -or [string]$reconciliationReadReceipt.requiredCommandIdentitySha256 -cne [string]$readCommandEntry.requiredCommandIdentitySha256 -or [string]$reconciliationReadReceipt.commandRunId -cne [string]$readCommandEntry.runId -or -not [Object]::Equals($reconciliationReadReceipt.commandExecutionHead, $readCommandEntry.executionHead) -or [string]$reconciliationReadReceipt.commandResultProjectionPath -cne [string]$readCommandEntry.resultProjectionPath -or [string]$reconciliationReadReceipt.commandResultProjectionSha256 -cne [string]$readCommandEntry.resultProjectionSha256 -or [string]$reconciliationReadReceipt.commandCleanupReceiptPath -cne [string]$readCommandEntry.cleanupReceiptPath -or [string]$reconciliationReadReceipt.commandCleanupReceiptSha256 -cne [string]$readCommandEntry.cleanupReceiptSha256 -or -not [Object]::Equals($reconciliationReadReceipt.separateAuthorityReceiptPath, $readCommandEntry.separateAuthorityReceiptPath) -or -not [Object]::Equals($reconciliationReadReceipt.separateAuthorityReceiptSha256, $readCommandEntry.separateAuthorityReceiptSha256) -or -not [Object]::Equals($reconciliationReadReceipt.launcherRefusalReceiptPath, $readCommandEntry.launcherRefusalReceiptPath) -or -not [Object]::Equals($reconciliationReadReceipt.launcherRefusalReceiptSha256, $readCommandEntry.launcherRefusalReceiptSha256) -or -not [Object]::Equals($reconciliationReadReceipt.applicabilityReceiptPath, $readCommandEntry.applicabilityReceiptPath) -or -not [Object]::Equals($reconciliationReadReceipt.applicabilityReceiptSha256, $readCommandEntry.applicabilityReceiptSha256)) { throw "PR6_PUBLICATION_RECONCILIATION_READ_COMMAND_BINDING_INVALID" }
          $readCommandResultProjectionResults = @(& $runtimeInputRecordResolver -Path ([string]$reconciliationReadReceipt.commandResultProjectionPath) -Sha256 ([string]$reconciliationReadReceipt.commandResultProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-result-projection.v1")
          $readCommandCleanupReceiptResults = @(& $runtimeInputRecordResolver -Path ([string]$reconciliationReadReceipt.commandCleanupReceiptPath) -Sha256 ([string]$reconciliationReadReceipt.commandCleanupReceiptSha256) -ExpectedSchema "hkwtia.pr6.cleanup-receipt.v3")
          if ($readCommandResultProjectionResults.Count -ne 1 -or $readCommandCleanupReceiptResults.Count -ne 1) { throw "PR6_PUBLICATION_RECONCILIATION_READ_EVIDENCE_COUNT_INVALID" }
          $readCommandResultProjection = $readCommandResultProjectionResults[0]
          $readCommandCleanupReceipt = $readCommandCleanupReceiptResults[0]
          foreach ($readProcessField in @("childProcessId", "childStartedAt", "childEndedAt", "nativeExitCode")) {
            if (-not [Object]::Equals($reconciliationReadReceipt.$readProcessField, $readCommandEntry.$readProcessField) -or -not [Object]::Equals($reconciliationReadReceipt.$readProcessField, $readCommandResultProjection.$readProcessField) -or -not [Object]::Equals($reconciliationReadReceipt.$readProcessField, $readCommandCleanupReceipt.$readProcessField)) { throw "PR6_PUBLICATION_RECONCILIATION_READ_PROCESS_TUPLE_MISMATCH" }
          }
          if ([string]$readCommandEntry.outcome -cne [string]$reconciliationReadReceipt.status -or [string]$readCommandResultProjection.outcome -cne [string]$reconciliationReadReceipt.status -or [string]$readCommandCleanupReceipt.outcome -cne [string]$reconciliationReadReceipt.status -or [string]$readCommandResultProjection.repository -cne $expectedRepo -or [string]$readCommandResultProjection.entryIdentitySha256 -cne [string]$readCommandEntry.entryIdentitySha256 -or [string]$readCommandResultProjection.requiredCommandIdentitySha256 -cne [string]$readCommandEntry.requiredCommandIdentitySha256 -or [string]$readCommandResultProjection.runId -cne [string]$readCommandEntry.runId -or -not [Object]::Equals($readCommandResultProjection.executionHead, $readCommandEntry.executionHead) -or [string]$readCommandResultProjection.launchDisposition -cne [string]$readCommandEntry.launchDisposition -or -not [Object]::Equals($readCommandResultProjection.separateAuthorityReceiptSha256, $readCommandEntry.separateAuthorityReceiptSha256) -or -not [Object]::Equals($readCommandResultProjection.launcherRefusalReceiptSha256, $readCommandEntry.launcherRefusalReceiptSha256) -or -not [Object]::Equals($readCommandResultProjection.applicabilityReceiptSha256, $readCommandEntry.applicabilityReceiptSha256) -or [string]$readCommandCleanupReceipt.repository -cne $expectedRepo -or [string]$readCommandCleanupReceipt.entryIdentitySha256 -cne [string]$readCommandEntry.entryIdentitySha256 -or [string]$readCommandCleanupReceipt.requiredCommandIdentitySha256 -cne [string]$readCommandEntry.requiredCommandIdentitySha256 -or [string]$readCommandCleanupReceipt.runId -cne [string]$readCommandEntry.runId -or -not [Object]::Equals($readCommandCleanupReceipt.executionHead, $readCommandEntry.executionHead) -or [string]$readCommandCleanupReceipt.launchDisposition -cne [string]$readCommandEntry.launchDisposition -or [string]$readCommandCleanupReceipt.resultProjectionSha256 -cne [string]$readCommandEntry.resultProjectionSha256 -or -not [Object]::Equals($readCommandCleanupReceipt.separateAuthorityReceiptSha256, $readCommandEntry.separateAuthorityReceiptSha256) -or -not [Object]::Equals($readCommandCleanupReceipt.launcherRefusalReceiptSha256, $readCommandEntry.launcherRefusalReceiptSha256) -or -not [Object]::Equals($readCommandCleanupReceipt.applicabilityReceiptSha256, $readCommandEntry.applicabilityReceiptSha256)) { throw "PR6_PUBLICATION_RECONCILIATION_READ_EVIDENCE_BINDING_INVALID" }
          if ([string]$reconciliationReadReceipt.launchDisposition -ceq "LAUNCHED") {
            $expectedReadStatus = if ([int]$reconciliationReadReceipt.nativeExitCode -eq 0) { "PASSED" } else { "NOT_PASSED" }
            $readNativeExitClass = if ([int]$reconciliationReadReceipt.nativeExitCode -eq 0) { "ZERO" } else { "NONZERO" }
            if ([long]$reconciliationReadReceipt.childProcessId -le 0 -or $null -eq $reconciliationReadReceipt.childStartedAt -or $null -eq $reconciliationReadReceipt.childEndedAt -or [DateTimeOffset]$reconciliationReadReceipt.childEndedAt -lt [DateTimeOffset]$reconciliationReadReceipt.childStartedAt -or $null -eq $reconciliationReadReceipt.nativeExitCode -or [string]$reconciliationReadReceipt.status -cne $expectedReadStatus -or [string]::IsNullOrWhiteSpace([string]$reconciliationReadReceipt.separateAuthorityReceiptPath) -or [string]$reconciliationReadReceipt.separateAuthorityReceiptSha256 -cnotmatch "^[0-9a-f]{64}$" -or $null -ne $reconciliationReadReceipt.launcherRefusalReceiptPath -or $null -ne $reconciliationReadReceipt.launcherRefusalReceiptSha256 -or $null -ne $reconciliationReadReceipt.applicabilityReceiptPath -or $null -ne $reconciliationReadReceipt.applicabilityReceiptSha256) { throw "PR6_PUBLICATION_RECONCILIATION_LAUNCHED_READ_INVALID" }
          } elseif ([string]$reconciliationReadReceipt.launchDisposition -ceq "REFUSED_BEFORE_LAUNCH") {
            $readNativeExitClass = "NO_NATIVE_EXIT"
            if ([string]$reconciliationReadReceipt.status -cne "NOT_PASSED" -or [string]$reconciliationReadReceipt.continuationDisposition -cne "STOP" -or $null -ne $reconciliationReadReceipt.childProcessId -or $null -ne $reconciliationReadReceipt.childStartedAt -or $null -ne $reconciliationReadReceipt.childEndedAt -or $null -ne $reconciliationReadReceipt.nativeExitCode -or $null -ne $reconciliationReadReceipt.separateAuthorityReceiptPath -or $null -ne $reconciliationReadReceipt.separateAuthorityReceiptSha256 -or [string]::IsNullOrWhiteSpace([string]$reconciliationReadReceipt.launcherRefusalReceiptPath) -or [string]$reconciliationReadReceipt.launcherRefusalReceiptSha256 -cnotmatch "^[0-9a-f]{64}$" -or $null -ne $reconciliationReadReceipt.applicabilityReceiptPath -or $null -ne $reconciliationReadReceipt.applicabilityReceiptSha256) { throw "PR6_PUBLICATION_RECONCILIATION_REFUSED_READ_INVALID" }
            $readRefusalResults = @(& $runtimeInputRecordResolver -Path ([string]$reconciliationReadReceipt.launcherRefusalReceiptPath) -Sha256 ([string]$reconciliationReadReceipt.launcherRefusalReceiptSha256) -ExpectedSchema "hkwtia.pr6.launcher-refusal-receipt.v1")
            if ($readRefusalResults.Count -ne 1 -or [string]$readRefusalResults[0].repository -cne $expectedRepo -or [string]$readRefusalResults[0].entryIdentitySha256 -cne [string]$readCommandEntry.entryIdentitySha256 -or [string]$readRefusalResults[0].requiredCommandIdentitySha256 -cne [string]$readCommandEntry.requiredCommandIdentitySha256 -or [string]$readRefusalResults[0].commandTemplateSha256 -cne [string]$readCommandEntry.commandTemplateSha256 -or [string]$readRefusalResults[0].runtimeSlotValuesSha256 -cne [string]$readCommandEntry.runtimeSlotValuesSha256 -or [string]$readRefusalResults[0].commandProjectionSha256 -cne [string]$readCommandEntry.commandProjectionSha256 -or [string]$readRefusalResults[0].resolvedCommandSha256 -cne [string]$readCommandEntry.resolvedCommandSha256 -or [string]$readRefusalResults[0].resolvedArgumentVectorSha256 -cne [string]$readCommandEntry.resolvedArgumentVectorSha256 -or [string]$readRefusalResults[0].runId -cne [string]$readCommandEntry.runId -or -not [Object]::Equals($readRefusalResults[0].executionHead, $readCommandEntry.executionHead) -or [string]$readRefusalResults[0].receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$readCommandEntry.launcherRefusalReceiptPath -cne [string]$reconciliationReadReceipt.launcherRefusalReceiptPath -or [string]$readCommandEntry.launcherRefusalReceiptSha256 -cne [string]$reconciliationReadReceipt.launcherRefusalReceiptSha256 -or [string]$readRefusalResults[0].reason -cne "MISSING_SEPARATE_AUTHORITY" -or -not [Object]::Equals($readRefusalResults[0].childCreated, $false) -or $null -ne $readRefusalResults[0].nativeExitCode -or -not [Object]::Equals($readRefusalResults[0].providerOrDataMutation, $false)) { throw "PR6_PUBLICATION_RECONCILIATION_READ_REFUSAL_BINDING_INVALID" }
          } else {
            throw "PR6_PUBLICATION_RECONCILIATION_READ_DISPOSITION_INVALID"
          }
          $allowedReadResultMatches = @(@($expectedReadTemplate.allowedResultTuples) | Where-Object { [string]$_.launchDisposition -ceq [string]$reconciliationReadReceipt.launchDisposition -and [string]$_.nativeExitClass -ceq $readNativeExitClass -and [string]$_.status -ceq [string]$reconciliationReadReceipt.status -and [string]$_.classification -ceq [string]$reconciliationReadReceipt.classification -and [string]$_.continuationDisposition -ceq [string]$reconciliationReadReceipt.continuationDisposition })
          if ($allowedReadResultMatches.Count -ne 1 -or ([string]$reconciliationReadReceipt.status -ceq "NOT_PASSED" -and [string]$reconciliationReadReceipt.continuationDisposition -cne "STOP")) { throw "PR6_PUBLICATION_RECONCILIATION_READ_RESULT_TUPLE_INVALID" }
          if ($reconciliationReadIndex -lt ($readCount - 1)) {
            if ([string]$reconciliationReadReceipt.status -cne "PASSED" -or [string]$reconciliationReadReceipt.continuationDisposition -cne "CONTINUE") { throw "PR6_PUBLICATION_RECONCILIATION_EARLY_STOP_INVALID" }
          } elseif ([string]$reconciliationReadReceipt.continuationDisposition -cne "STOP") {
            throw "PR6_PUBLICATION_RECONCILIATION_TERMINAL_READ_MISSING"
          }
          $terminalReadCommandEntry = $readCommandEntry
          $terminalReadReceiptPath = [string]$manifestReadReceiptPaths[$reconciliationReadIndex]
          $terminalReadReceiptSha256 = [string]$manifestReadReceiptSha256s[$reconciliationReadIndex]
        }
        $expectedSuffixCount = $descriptorReadTemplates.Count - $readCount
        foreach ($suffixArray in @($manifestSuffixOrdinals, $manifestSuffixApplicabilityPaths, $manifestSuffixApplicabilitySha256s, $resultSuffixOrdinals, $resultSuffixApplicabilityPaths, $resultSuffixApplicabilitySha256s)) {
          if ($suffixArray.Count -ne $expectedSuffixCount) { throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_CARDINALITY_INVALID" }
        }
        if ($expectedSuffixCount -eq 0) {
          if ($null -ne $publicationEvidenceManifest.unexecutedReadOperationTemplateAggregateSha256 -or $null -ne $reconciliationResultResults[0].unexecutedReadOperationTemplateAggregateSha256) { throw "PR6_PUBLICATION_RECONCILIATION_EMPTY_SUFFIX_AGGREGATE_INVALID" }
        } elseif ([string]$publicationEvidenceManifest.unexecutedReadOperationTemplateAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$reconciliationResultResults[0].unexecutedReadOperationTemplateAggregateSha256 -cne [string]$publicationEvidenceManifest.unexecutedReadOperationTemplateAggregateSha256) {
          throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_AGGREGATE_INVALID"
        }
        for ($suffixIndex = 0; $suffixIndex -lt $expectedSuffixCount; $suffixIndex += 1) {
          $descriptorSuffixIndex = $readCount + $suffixIndex
          $suffixTemplate = $descriptorReadTemplates[$descriptorSuffixIndex]
          $expectedSuffixOrdinal = $descriptorSuffixIndex + 1
          if ([int]$manifestSuffixOrdinals[$suffixIndex] -ne $expectedSuffixOrdinal -or [int]$resultSuffixOrdinals[$suffixIndex] -ne $expectedSuffixOrdinal -or [string]$resultSuffixApplicabilityPaths[$suffixIndex] -cne [string]$manifestSuffixApplicabilityPaths[$suffixIndex] -or [string]$resultSuffixApplicabilitySha256s[$suffixIndex] -cne [string]$manifestSuffixApplicabilitySha256s[$suffixIndex] -or [string]::IsNullOrWhiteSpace([string]$manifestSuffixApplicabilityPaths[$suffixIndex]) -or [string]$manifestSuffixApplicabilitySha256s[$suffixIndex] -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_MAPPING_INVALID" }
          $suffixCommandEntries = @($publicationEvidenceEntries | Where-Object { [string]$_.commandClass -ceq "RECONCILIATION_READ" -and [int]$_.reconciliationTemplateOrdinal -eq $expectedSuffixOrdinal -and [string]$_.reconciliationReadKind -ceq [string]$suffixTemplate.readKind -and [string]$_.resolvedArgumentVectorSha256 -ceq [string]$suffixTemplate.argumentVectorSha256 -and [string]$_.launchDisposition -ceq "PROVEN_NOT_APPLICABLE" -and [string]$_.applicabilityReceiptPath -ceq [string]$manifestSuffixApplicabilityPaths[$suffixIndex] -and [string]$_.applicabilityReceiptSha256 -ceq [string]$manifestSuffixApplicabilitySha256s[$suffixIndex] })
          if ($suffixCommandEntries.Count -ne 1) { throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_COMMAND_INVALID" }
          $suffixCommandEntry = $suffixCommandEntries[0]
          $suffixApplicabilityResults = @(& $runtimeInputRecordResolver -Path ([string]$manifestSuffixApplicabilityPaths[$suffixIndex]) -Sha256 ([string]$manifestSuffixApplicabilitySha256s[$suffixIndex]) -ExpectedSchema "hkwtia.pr6.command-applicability-receipt.v1")
          $suffixResultResults = @(& $runtimeInputRecordResolver -Path ([string]$suffixCommandEntry.resultProjectionPath) -Sha256 ([string]$suffixCommandEntry.resultProjectionSha256) -ExpectedSchema "hkwtia.pr6.command-result-projection.v1")
          $suffixCleanupResults = @(& $runtimeInputRecordResolver -Path ([string]$suffixCommandEntry.cleanupReceiptPath) -Sha256 ([string]$suffixCommandEntry.cleanupReceiptSha256) -ExpectedSchema "hkwtia.pr6.cleanup-receipt.v3")
          if ($suffixApplicabilityResults.Count -ne 1 -or $suffixResultResults.Count -ne 1 -or $suffixCleanupResults.Count -ne 1) { throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_EVIDENCE_COUNT_INVALID" }
          $suffixApplicability = $suffixApplicabilityResults[0]
          $suffixResult = $suffixResultResults[0]
          $suffixCleanup = $suffixCleanupResults[0]
          if ([string]$suffixApplicability.repository -cne $expectedRepo -or [string]$suffixApplicability.entryIdentitySha256 -cne [string]$suffixCommandEntry.entryIdentitySha256 -or [string]$suffixApplicability.requiredCommandIdentitySha256 -cne [string]$suffixCommandEntry.requiredCommandIdentitySha256 -or [string]$suffixApplicability.commandTemplateSha256 -cne [string]$suffixCommandEntry.commandTemplateSha256 -or [string]$suffixApplicability.runtimeSlotValuesSha256 -cne [string]$suffixCommandEntry.runtimeSlotValuesSha256 -or [string]$suffixApplicability.commandProjectionSha256 -cne [string]$suffixCommandEntry.commandProjectionSha256 -or [string]$suffixApplicability.resolvedCommandSha256 -cne [string]$suffixCommandEntry.resolvedCommandSha256 -or [string]$suffixApplicability.resolvedArgumentVectorSha256 -cne [string]$suffixCommandEntry.resolvedArgumentVectorSha256 -or [string]$suffixApplicability.runId -cne [string]$suffixCommandEntry.runId -or -not [Object]::Equals($suffixApplicability.executionHead, $suffixCommandEntry.executionHead) -or [string]$suffixApplicability.producingEntryIdentitySha256 -cne [string]$terminalReadCommandEntry.entryIdentitySha256 -or [string]$suffixApplicability.producingResultProjectionSha256 -cne [string]$terminalReadCommandEntry.resultProjectionSha256 -or [string]$suffixApplicability.producingReconciliationReadReceiptPath -cne $terminalReadReceiptPath -or [string]$suffixApplicability.producingReconciliationReadReceiptSha256 -cne $terminalReadReceiptSha256 -or [string]$suffixApplicability.outcome -cne "NOT_APPLICABLE") { throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_APPLICABILITY_INVALID" }
          if ([string]$suffixCommandEntry.outcome -cne "NOT_APPLICABLE" -or $null -ne $suffixCommandEntry.childProcessId -or $null -ne $suffixCommandEntry.childStartedAt -or $null -ne $suffixCommandEntry.childEndedAt -or $null -ne $suffixCommandEntry.nativeExitCode -or $null -ne $suffixCommandEntry.separateAuthorityReceiptPath -or $null -ne $suffixCommandEntry.separateAuthorityReceiptSha256 -or $null -ne $suffixCommandEntry.launcherRefusalReceiptPath -or $null -ne $suffixCommandEntry.launcherRefusalReceiptSha256) { throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_ENTRY_INVALID" }
          if ([string]$suffixResult.repository -cne $expectedRepo -or [string]$suffixResult.entryIdentitySha256 -cne [string]$suffixCommandEntry.entryIdentitySha256 -or [string]$suffixResult.requiredCommandIdentitySha256 -cne [string]$suffixCommandEntry.requiredCommandIdentitySha256 -or [string]$suffixResult.runId -cne [string]$suffixCommandEntry.runId -or -not [Object]::Equals($suffixResult.executionHead, $suffixCommandEntry.executionHead) -or [string]$suffixResult.launchDisposition -cne "PROVEN_NOT_APPLICABLE" -or [string]$suffixResult.outcome -cne "NOT_APPLICABLE" -or $null -ne $suffixResult.childProcessId -or $null -ne $suffixResult.childStartedAt -or $null -ne $suffixResult.childEndedAt -or $null -ne $suffixResult.nativeExitCode -or [string]$suffixResult.commandSourceManifestSha256 -cne [string]$suffixCommandEntry.commandSourceManifestSha256 -or $null -ne $suffixResult.separateAuthorityReceiptSha256 -or $null -ne $suffixResult.launcherRefusalReceiptSha256 -or [string]$suffixResult.applicabilityReceiptSha256 -cne [string]$suffixCommandEntry.applicabilityReceiptSha256) { throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_RESULT_INVALID" }
          if ([string]$suffixCleanup.repository -cne $expectedRepo -or [string]$suffixCleanup.entryIdentitySha256 -cne [string]$suffixCommandEntry.entryIdentitySha256 -or [string]$suffixCleanup.requiredCommandIdentitySha256 -cne [string]$suffixCommandEntry.requiredCommandIdentitySha256 -or [string]$suffixCleanup.runId -cne [string]$suffixCommandEntry.runId -or -not [Object]::Equals($suffixCleanup.executionHead, $suffixCommandEntry.executionHead) -or [string]$suffixCleanup.launchDisposition -cne "PROVEN_NOT_APPLICABLE" -or [string]$suffixCleanup.outcome -cne "NOT_APPLICABLE" -or $null -ne $suffixCleanup.childProcessId -or $null -ne $suffixCleanup.childStartedAt -or $null -ne $suffixCleanup.childEndedAt -or $null -ne $suffixCleanup.nativeExitCode -or [string]$suffixCleanup.resultProjectionSha256 -cne [string]$suffixCommandEntry.resultProjectionSha256 -or $null -ne $suffixCleanup.separateAuthorityReceiptSha256 -or $null -ne $suffixCleanup.launcherRefusalReceiptSha256 -or [string]$suffixCleanup.applicabilityReceiptSha256 -cne [string]$suffixCommandEntry.applicabilityReceiptSha256) { throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_CLEANUP_INVALID" }
        }
      } elseif ($uncertainMutationReceiptPresent -or $null -ne $publicationCompletion.reconciliationDescriptorPath -or $null -ne $publicationCompletion.reconciliationDescriptorSha256 -or $null -ne $publicationCompletion.reconciliationResultReceiptPath -or $null -ne $publicationCompletion.reconciliationResultReceiptSha256 -or @($publicationEvidenceManifest.reconciliationReadOrdinals).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadKinds).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadArgumentVectorSha256s).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadLaunchDispositions).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadStatuses).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadContinuationDispositions).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadReceiptPaths).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadReceiptSha256s).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadClassifications).Count -ne 0 -or @($publicationEvidenceManifest.unexecutedReadTemplateOrdinals).Count -ne 0 -or @($publicationEvidenceManifest.unexecutedReadApplicabilityReceiptPaths).Count -ne 0 -or @($publicationEvidenceManifest.unexecutedReadApplicabilityReceiptSha256s).Count -ne 0 -or $null -ne $publicationEvidenceManifest.readOperationTemplateAggregateSha256 -or $null -ne $publicationEvidenceManifest.executedReadOperationTemplateAggregateSha256 -or $null -ne $publicationEvidenceManifest.unexecutedReadOperationTemplateAggregateSha256 -or $null -ne $publicationEvidenceManifest.reconciliationReadReceiptAggregateSha256 -or $null -ne $publicationEvidenceManifest.reconciliationReadClassificationAggregateSha256) {
        throw "PR6_PUBLICATION_UNEXPECTED_RECONCILIATION_EVIDENCE"
      }
    }

    Send only the sanitized terminal validation result—including PASSED/NOT_PASSED and canonical nullable PR identity—over the launcher channel and close this shell. Trusted launcher/CI freezes `hkwtia.pr6.terminal-attestation-command-evidence-manifest.v1`, requires its one dependency-ready projection and ordered evidence entries to exactly exhaust TERMINAL_ATTESTATION, recomputes its unique entry/source/result/cleanup/authority/refusal/applicability arrays and `fullEntryEvidenceAggregateSha256`, and returns its immutable path/hash plus the validated publication-completion identity. This launcher-issued terminal manifest is deliberately not consumed by another repository command; starting such a verifier would create an unbounded self-verification cycle.

    Expected: both terminal branches close completely. On the uninterrupted happy path, `publication-completion.v2` is `PASSED` and binds one `publication-final-state.v1` proving one OPEN draft PR at the captured URL with exact reviewed-commit title/body, base name/OID `codex/wisetech-pr5-public-journeys`/`3856dd71842f9a2e1d9c4b7a46521416a5bd83ae`, and local branch/head plus remote/published head all equal to the zero-finding `$reviewedHead`; each mutation has immutable STARTED/SUCCEEDED receipts. A pre-mutation failure, post-mutation validation failure, or uncertain mutation instead produces `publication-completion.v2` `NOT_PASSED`; uncertainty is closed by the fresh read-only reconciliation descriptor/result, never retries or continues, and invents no success. Both paths freeze the exact PUBLICATION manifest, validate the acyclic completion record in the capability-free terminal shell, and freeze the non-recursive TERMINAL_ATTESTATION manifest. Report pending/failing checks separately. Do not merge, deploy, mutate providers, or convert an external gate into PASS.
