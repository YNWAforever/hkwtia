...
- The external execution record is populated by the operator or CI approval system before each shell and is never inferred from PATH, a package shim, the repository, or ambient configuration. Its exact JSON schema is the discriminated union `hkwtia.pr6.execution-attestation.v1` with required `recordKind: "install-only" | "runtime"`; unknown, missing, duplicate, case-colliding, or kind-incompatible fields fail. Both kinds require canonical `repositoryRoot`; exact path/allowed-root/SHA-256 entries for the generator, resolver, invoker, native process adapter, guardian, and bootstrap contract; the complete current PowerShell runtime distribution `{path, allowedRoot, sha256, version, distributionManifestSha256}`; the complete Node distribution; the complete npm package/CLI distribution; an ordered mode-specific source manifest; and the lowercase SHA-256 of the canonical UTF-8 record. `install-only` requires exactly the bootstrap/install/generator source closure and permits only `install`; it forbids `gitDir`, Git/GitHub CLI, `node_modules`, installed CLIs, browser/Chrome/provider/database/Preview/publication fields, and every non-install mode. `runtime` forbids `install` and requires canonical `gitDir`, the complete Git distribution, the entire worktree-local `node_modules` tree as one ordered path/length/SHA-256 manifest bound to `package-lock.json`, all runtime source manifests, and every installed CLI used by an allowed mode. A runtime mode that opens Playwright additionally requires a separately approved complete browser-and-FFmpeg distribution manifest plus exact browser executable path/version; a non-browser mode forbids those fields. Publication adds GitHub CLI only through its separate descriptor. The npm manifest covers the complete package root including `lib/cli.js`; publication Git covers `git-remote-https`, its shell/transport closure, and the one exact `gh auth git-credential` helper chain. Every distribution manifest includes every non-OS executable, library, script, locale, resource, PAK/snapshot, dynamically resolved package, and configuration file that can load before or while a capability is available, with an immutable-root policy. The record contains no token, password, cookie, database URL, or provider value.
- The approval supplies five independent non-secret resolver-bootstrap fields—record path/hash and resolver path/allowed-root/hash—so the repository resolver is checked before it parses the record. Initial RED additionally supplies three independent contract fields—contract path/allowed-root/hash—because the mutable repository path is never executable. A missing file, unmanifested dependency, alternate host, mutable relative path, or schema/version drift fails.
- Before invoking `resolve-verified-runtime.ps1`, the exact shell preamble below uses PowerShell/.NET only to canonicalize the separately approved resolver path without a reparse component and compare its captured bytes to the separately approved resolver hash. The attested resolver hashes and duplicate-key-validates the exact record, enforces the kind-specific required/forbidden fields, canonicalizes roots, resolves every recorded component without reparse ambiguity, and verifies filesystem identity/SHA-256/version plus every ordered distribution/source manifest. It reads the invoker, native process adapter, guardian, and bootstrap contract exactly once, verifies and strict-UTF-8 decodes the captured byte arrays, and returns immutable `InvokerScriptBlock`, `NativeProcessBoundaryScriptBlock`, `GuardianScriptBlock`, and `BootstrapContractScriptBlock` values plus the validated descriptor path/hash/kind. It never returns or later invokes a mutable bootstrap path and never enumerates or prints the environment.
- `invoke-verified-tool.ps1` is the only top-level process-launch boundary. For a Node mode, its dependency-free parent reads every approved repository source and the complete approved `node_modules` tree exactly once, hashes those captured bytes, and materializes the original relative layout into one unique absent OS-temp root. On Windows every materialized file and complete executable/browser distribution file is held with `FileShare.Read` only—denying write and delete—through the whole contained tree's exit; on POSIX the equivalent recorded read-only mount/file identity is held. ESM, CJS, Next, Vitest, Playwright, TypeScript, declarations, dynamic imports, and package resolution execute only from this byte-equal immutable tree. The original checkout and original `node_modules` are never reopened by a child. Materialization order, duplicate/case collisions, links/reparse points, missing dynamic resolutions, and any source/config/package byte mismatch fail before process creation.
- Node starts with a cleared environment. The anonymous control/capability pipes are non-inheritable except for the one explicitly duplicated child endpoint; every descendant receives an explicitly constructed environment and handle list. After all pre-capability imports load from the immutable tree, the child returns the descriptor/source/materialization hashes, mode, entrypoint, `process.execPath`, immutable root, and parent nonce. Only then may the parent send the exact retained capability payload; immediately after child acknowledgement, the parent zeroes its capability buffer and closes its pipe endpoint. A missing, incorrect, duplicate, or late handshake kills the non-breakaway contained tree. Trusted attested Node packages may create necessary descendants only inside that same Job/process group; the native adapter remains the only top-level launch boundary. Every unexpected descendant image, inherited capability handle, or process outside the container fails evidence. Ambient `NODE_OPTIONS`, `NODE_PATH`, preload/import flags, dynamic-loader variables, `GIT_*`, `GH_*`, askpass, helpers, hooks, proxies, writable PATH entries, and shell shims are absent before the first child instruction.
...
- Every child map starts empty. The install-only mode executes the complete attested npm distribution through the handle-bound attested Node distribution with no shell, empty run-owned user/global configs, and the public registry. Before `ci`, require `node_modules` to be absent or a regular non-reparse directory wholly inside this worktree; inventory and revalidate its complete ancestor/tree identity immediately before and after npm. Require every lockfile `resolved` host to equal `registry.npmjs.org`, execute exactly `ci --ignore-scripts --audit=false --fund=false`, never resolve an ancestor/shared dependency tree, and run no browser download or lifecycle script. If a required artifact is unavailable, the gate is `NOT PASSED`. After `ci`, the dependency-free generator records the entire worktree-local `node_modules` tree—not a guessed CLI subgraph—plus lockfile identity. Close the install-only shell; external approval must bind a fresh `runtime` record before any unit, E2E, build, audit, Git, browser, or managed mode. Reinstalling invalidates it immediately. Audit is exact `audit --omit=dev --audit-level=high` and is runtime-only.
- The `build` and `e2e` modes reserve one loopback port and derive `http://127.0.0.1:<port>`. They create an unguessable run ID and pass fixed internal `PR6_NEXT_DIST_DIR=.next/pr6-<uuid>` to both `next.config.ts` evaluation and build/start inside the immutable materialization root. The config accepts that value only when the byte-bound internal runner attestation matches the runtime record/mode/run ID, requires this exact relative grammar, rejects absolute/traversal/symlink forms, and otherwise uses the normal production `distDir`; no CLI option or ambient fallback selects it. Build receives only the owned `NEXT_PUBLIC_SITE_URL`, internal run projection, and fixed telemetry/loader markers. Start receives the exact same run ID/distDir; mismatch fails before server code.
- After the capability-free build and before any database/Auth/provider capability is retained, the dependency-free parent enumerates and hashes every generated Next output file and every `.nft.json` output-file-tracing dependency. Every traced dependency must resolve under the already immutable source or complete `node_modules` root; escape, link, omission, late creation, or replacement fails. The parent captures generated output into the same uniquely owned immutable runtime tree, holds every output/traced dependency without write/delete sharing through server exit, and runs metadata/robots/sitemap/structured-data origin and capability-canary scans there. Replacement probes swap a generated chunk and external traced dependencies such as `pg`, `sharp`, and AWS modules between build/manifest/start; none may execute or survive. The original build directory is never reopened by `next start`.
...
- [ ] **Step 1: Record an executable dependency-free RED without launching Node, a browser, or provider**
    ...
    Add `tests/contracts/pr6-unsafe-bootstrap.red.ps1`. Using PowerShell/.NET file reads only, it must fail on the current raw npm/npx/PATH routes, ambient Node preloads, ambient Git/GitHub configuration, missing record-kind discrimination, missing complete PowerShell/Node/npm/Git/browser/source closures and post-handshake capability handoff, mutable bootstrap/launcher/config execution, stale source records, dependency lifecycle execution, unsafe `node_modules`/Next-output containment, ambient browser discovery, `PLAYWRIGHT_BASE_URL`, `reuseExistingServer: !CI`, missing `forbidOnly`, inherited browser environment, development Next server, non-atomic Job assignment, late dependency bootstrap, and absent creation-window/parent-death cleanup. Before executing the new contract, the operator/CI approval supplies its exact path/allowed-root/SHA-256. Define the exact dependency-free `New-Pr6ApprovedScriptBlock` helper printed in Step 3 in this shell, capture the contract once, and execute only that immutable block; no repository path invocation is permitted. Run:
...
- [ ] **Step 4: Run the boundary probes under the verified pre-entry launcher**
    ...
    Expected: PASS. The actual pre-entry/child key sets; exact install-only/runtime required-and-forbidden schemas; independently captured resolver/contract/generator/native-adapter/guardian blocks; complete PowerShell/Node/npm/Git/whole-`node_modules`/browser distributions; immutable source/materialization/output-tracing graphs; non-inheritable post-handshake capability delivery and buffer zeroing; clean npm/Git configuration; guarded `distDir`/run-ID equality; generated/external-module capture; real build URLs; production selection; full browser isolation; deadlines; atomic Job membership or POSIX parent-death PID recheck; abrupt-parent cleanup; and real post-readiness dotenv proof all pass. Wrong kind, stale source record, fake PATH/`ComSpec`, loader injection, raw config/CLI, mutable contract path, same-path replacement at capture/materialization/handshake/spawn/output/browser barriers, immediate-grandchild escape, unmanifested DLL/PAK/locale/FFmpeg, stale localhost, reparse/dotenv race, lifecycle attempt, unsupported argument, and child failure all fail closed.

- [ ] **Step 5: Commit the execution prerequisite**

    & $verifiedGit add -- ':(literal)package.json' ':(literal)next.config.ts' ':(literal)playwright.config.ts' ':(literal)README.md' ':(literal)AGENTS.md' ':(literal)CLAUDE.md' ':(literal).github/pull_request_template.md' ':(literal)docs/integration/wisetech-delivery-gates.md' ':(literal)tests/unit/wisetech-delivery-gates.test.ts' ':(literal)scripts/generate-pr6-execution-candidate.ps1' ':(literal)scripts/resolve-verified-runtime.ps1' ':(literal)scripts/invoke-verified-tool.ps1' ':(literal)scripts/pr6-native-process-boundary.ps1' ':(literal)scripts/managed-process-guardian.ps1' ':(literal)scripts/managed-runtime-environment.mjs' ':(literal)scripts/managed-runtime-environment.d.mts' ':(literal)scripts/run-credential-free-verification.mjs' ':(literal)scripts/run-managed-playwright.mjs' ':(literal)scripts/run-managed-next.mjs' ':(literal)tests/contracts/pr6-unsafe-bootstrap.red.ps1' ':(literal)tests/fixtures/credential-free-process-env-probe.mjs' ':(literal)tests/fixtures/managed-next-env-probe.mjs' ':(literal)tests/fixtures/managed-process-creation-window-probe.mjs' ':(literal)tests/unit/verified-runtime-bootstrap.test.ts' ':(literal)tests/unit/credential-free-verification-boundary.test.ts' ':(literal)tests/unit/credential-free-build-boundary.test.ts' ':(literal)tests/unit/managed-next-production-boundary.test.ts'
...
- Task 4 extends Task 0's shared test-only runtime for mutating M1, M2, M3, M4B, M5, M6, M7, final authenticated-Axe, guarded PostgreSQL, and read-only M4C suites. `scripts/managed-runtime-environment.d.mts` remains the one exact sibling declaration for the `.mjs` implementation and owns every exported constant, type, and function below; TypeScript consumers import that single runtime module with no duplicate TS implementation.
  ...
  `m4c-readonly-preview` is a distinct sanitized branch owned here rather than an assumed legacy guard. The original parent requires exact `M4C_READONLY_PREVIEW_APPROVED=M4C_APPROVED_READONLY_PREVIEW_ONLY`, exact path plus lowercase SHA-256 for a non-secret `hkwtia.pr6.m4c-preview-approval.v1` descriptor, exact path/hash for Task 12's immutable `hkwtia.pr6.task12-target.v1` local-head record, parent-only `M4C_VERCEL_READ_TOKEN`, optional `VERCEL_SHARE_TOKEN`, and absent ambient `PLAYWRIGHT_BASE_URL`. The target record was captured through verified Git from a clean local branch before Task 12 edits and binds the exact 40-hex `preTask12Head`. The M4C descriptor must bind the same head, branch, repository `YNWAforever/wisetech`, deployment/project/optional-team, generated HTTPS origin, nonempty production-origin denylist, and SHA-256 of `M4C_SAFE_REQUEST_MANIFEST`. Before any browser/share-token handoff, the parent performs one bounded no-redirect authenticated GET to `https://api.vercel.com/v13/deployments/{encodedDeploymentId}` with only the exact optional team query. Require HTTP 200, exact deployment/project/team, `readyState === "READY"`, `target === null`, exact Git org/repo/ref, and provider Git SHA equal to both descriptor `reviewedHead` and the independently recorded local `preTask12Head`; descriptor origin must equal `https://` plus provider `url`. Reject production/loopback/Unicode origins and denylist/alias collisions. Only token-stripped provider-response hash, local-head-record hash, and validated projections enter `M4CReadonlyPreviewTarget`; tokens/raw response never enter runner/browser. Timeout, redirect, ambiguity, descriptor/hash/head mismatch is `NOT PASSED` without retry or browser launch.

  `M4C_SAFE_REQUEST_MANIFEST` permits documents only at the current `localePrefix: "as-needed"` canonical paths: `/ai-ops`, `/zh/ai-ops`, `/sitemap.xml`, `/news/m4-runtime-and-concierge`, `/zh/news/m4-runtime-and-concierge`, `/news/m4-public-ai-ops`, and `/zh/news/m4-public-ai-ops`. Source tests set English `localePaths` prefix to `""`, reject every `/en/...` redirect as noncanonical, and retain `/zh` for Traditional Chinese. When a share token exists, permit one initial `/?_vercel_share=<opaque>` protection bootstrap on the generated host; its response stays same-origin, the token is stripped before application observation, and it cannot replay.
  After provider/local-head proof and before browser creation, the dependency-free parent performs bounded same-origin GETs for that exact safe document set (after the optional one-time share bootstrap), with redirects disabled and strict response/byte/count/time budgets. It verifies each page's source/graph read-only contract, parses only exact same-origin `/_next/static/`, stylesheet, font, and image URLs present in the returned documents, then recursively fetches same-origin CSS dependencies under fixed depth/file/byte budgets. It rejects RSC/client-navigation endpoints, scripts that require discovery beyond those documents, query-bearing assets outside one source-tested immutable-asset query allowlist, unknown origins, and any `/api` path. The resulting ordered `M4CAssetManifest` records path/query, media type, length, and SHA-256; its hash and exact allowlist projection enter the runner, never response bodies or credentials. There is no assumed provider build manifest.
  `m4c-readonly-preview-safety.ts` creates the only context with `javaScriptEnabled: false`, uses direct document navigations rather than client navigation, sets `serviceWorkers: "block"`, disables trace/screenshot/video/downloads, and permits only the exact document plus preflighted asset manifest on the provider-bound origin. Every `/api/`, `/showcase/`, Portal/Admin/Join path, unknown GET, form/action/RPC request, and method other than GET/HEAD/OPTIONS is denied and recorded. A source/graph contract explicitly catches state-changing `GET /api/showcase/[slug]/view`; no broad “GET is safe” rule exists. Unit/source/subprocess tests cover descriptor/key/hash/head collision and drift, wrong sentinel, provider failures, canonical-English routing, safe/asset-manifest drift, CSS recursion budgets, unknown/RSC/client-navigation requests, Showcase view counter, share-token exfiltration/replay, mutation methods, service worker, extra credential, alternate spec, and direct-context bypass. Any denial is `NOT PASSED` and cannot count as M4C acceptance.
...
- [ ] **Step 1: Define final credential-free and authenticated browser verification matrices**
    ...
    Persist those non-secret projections outside the worktree in an externally approved immutable `hkwtia.pr6.task12-target.v1` record with exact record path/hash, repository `YNWAforever/wisetech`, branch, `preTask12Head`, PR5 base, capture timestamp, and verified Git descriptor hash. M4C and Lighthouse approval descriptors and Vercel responses must later equal this independently captured local head. A dirty tree, branch/head drift, missing record, or descriptor mismatch is `NOT PASSED` before provider access.
...
- [ ] **Step 5: Prove safety contracts, then run isolated M1-M7 only with separate authority**
    ...
    - M4C: exact `M4C_READONLY_PREVIEW_APPROVED=M4C_APPROVED_READONLY_PREVIEW_ONLY`, exact non-secret M4C approval-descriptor path/hash, exact Task 12 local-head-record path/hash, parent-only `M4C_VERCEL_READ_TOKEN`, optional `VERCEL_SHARE_TOKEN`, and absent ambient `PLAYWRIGHT_BASE_URL`. The bounded GET-only Vercel proof requires descriptor Git head and provider Git SHA to equal the independently recorded clean local `preTask12Head`. Before browser creation the parent preflights the exact canonical documents (`/ai-ops` and `/news/...` for English; `/zh/...` for Traditional Chinese), recursively captures bounded same-origin CSS/static/font/image dependencies into the hashed immutable `M4CAssetManifest`, and gives the runner only its hash/allowlist. The helper performs direct document navigation, blocks service workers/RSC/client navigation, cross-origin or redirect exfiltration, every API or unknown same-origin request, and every method except GET/HEAD/OPTIONS; the suite still asserts the private canary absent.
...
- [ ] **Step 6: Run the exact authorized-Preview Lighthouse and field-performance gates**
    ...
    Use the fail-closed wrapper/config contracts implemented before Step 2. Task 0 captures and verifies the complete `prepare-index` launcher/support graph before Node starts, launches it without retained capability, and requires the post-load source-manifest handshake. `prepare-index` revalidates the Node/Git distributions and creates the source/index checkpoint below; all `LHCI_*`/Vercel approval variables must be absent and no network/Chrome action occurs. The materialized `collect` mode revalidates the checkpoint before reading any capability. Before a run root or Chrome process exists, it requires one approved `hkwtia.pr6.lighthouse-preview-approval.v1` descriptor path/hash bound to the exact Task 12 local-head-record path/hash, `LHCI_PREVIEW_DEPLOYMENT_ID`/URL/project/optional team, production-origin denylist, `LHCI_REVIEWED_HEAD`, exact branch, parent-only `LHCI_VERCEL_READ_TOKEN`, exact `LHCI_CHROME_PATH`, exact `LHCI_CHROME_VERSION`, exact `LHCI_CHROME_DISTRIBUTION_MANIFEST_PATH`, lowercase `LHCI_CHROME_DISTRIBUTION_MANIFEST_SHA256`, and canonical nonempty `LHCI_CHROME_ALLOWED_ROOTS_JSON`. The descriptor's reviewed head and `LHCI_REVIEWED_HEAD` must both equal the independently recorded clean `preTask12Head` before any provider request.
    Perform one bounded no-redirect authenticated GET to the exact Vercel deployment endpoint and require HTTP 200, deployment/project/optional team, `readyState === "READY"`, `target === null`, Git org/repo/ref, and provider Git SHA all equal the descriptor and local `preTask12Head`. Require the generated deployment URL to match the response. Reject production/loopback origins, the complete denylist, malformed/Unicode/wildcard/duplicate aliases, redirects, ambiguity, and non-GET attempts. The Vercel token/raw response remain parent-only; only validated non-secret projections and token-stripped response hash enter evidence. This proof never creates, promotes, aliases, or mutates a deployment.

    Only after provider/local-head proof, the wrapper validates the complete Task 0-attested local `@lhci/cli` closure and the separately approved complete Chrome distribution without a shell. The Chrome manifest is an ordered exact-case relative path/length/SHA-256 inventory rooted under one approved canonical directory and includes the selected executable plus every DLL/shared library, helper executable, ICU/snapshot file, PAK, locale, resource, SwiftShader component, sandbox, and other code/data file Chrome can load. Resolve the executable and every manifest member without symlink/reparse ambiguity; require no unmanifested file beneath the distribution roots and hold every member without write/delete sharing (or the recorded read-only mount identity) through version preflight and LHCI/browser-tree exit. Hash/re-stat the whole distribution immediately before version preflight, before LHCI spawn, and after exit. A bounded no-shell `--version` must return the exact approved version. Ambient PATH/registry/channel/cache discovery, partial executable-only hashing, resource fallback, replacement, or drift is `NOT PASSED`.
    ...
    Expected when separately authorized: the runner/config contracts, revalidated sanitized Git/index checkpoint, exact local-head/descriptor/provider-SHA equality, read-only Vercel deployment proof, and immutable-Preview collection PASS for the exact four public URLs. Without approval, do not run `collect`; retain the checkpoint, mark Lighthouse/field performance `NOT PASSED`, and continue with staged source. The verification record binds deployment/generated URL/project/reviewed local head/branch/READY/Preview, token-stripped provider hash, aliases/production denylist, canonical Chrome root/executable/version, complete distribution manifest path/hash and every preflight/pre-spawn/post-exit member aggregate, operational environment fingerprint, run-owned temp/profile root, and pre/post Lighthouse execution-manifest aggregate. Require only `<runRoot>/.lighthouseci` and `<runRoot>/upload/manifest.json`, exact URL/run allowlist, retained artifact hashes, and unchanged worktree status. Record LCP at most 2.5 seconds and CLS at most 0.1; INP at most 200 ms p75 only from separately authorized current-deployment field evidence, otherwise `NOT PASSED`. Retain/report the unique external run root and never stage, overwrite, reuse, or claim cleanup of it.
...
- [ ] **Step 9: Complete independent review and publish the stacked draft PR**
    ...
    $verifiedGit = { & $runtimeInvoker -NativeProcessBoundaryScriptBlock $runtimeNativeProcessBoundary -GuardianScriptBlock $runtimeGuardian -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -Tool Git -PublicationApprovalPath $publicationApprovalPath -PublicationApprovalSha256 $publicationApprovalSha256 -Arguments @($args) }.GetNewClosure()
    $verifiedGh = { & $runtimeInvoker -NativeProcessBoundaryScriptBlock $runtimeNativeProcessBoundary -GuardianScriptBlock $runtimeGuardian -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -Tool GitHubCli -PublicationApprovalPath $publicationApprovalPath -PublicationApprovalSha256 $publicationApprovalSha256 -Arguments @($args) }.GetNewClosure()

    The publication descriptor is exact `hkwtia.pr6.publication-approval.v1` and must bind `github.com`, `YNWAforever/wisetech`, expected head/base branches/base OID/reviewed head, complete immutable Git executable/HTTPS transport/shell/helper and GitHub CLI distributions, approved canonical config directory, fixed operation allowlists/deadlines, and approval reference. It contains no credential. The invoker clears ambient Git/GH/proxy/config/helper/hook state, holds the complete closure through exit, and permits the verified GitHub CLI credential helper only for exact GitHub HTTPS operations. The separate reconciliation descriptor and immutable receipt pair have the exact Task 0 schemas and cannot mutate.
...
- Verification coverage: Task 0 supplies discriminated install/runtime records, independently captured generator/resolver/contract/invoker/native-adapter/guardian blocks, mandatory post-edit source refresh, complete PowerShell/Node/npm/Git/GitHub-CLI/whole-`node_modules`/browser closures, byte-equal immutable materialization, non-inheritable post-handshake capability handoff, guarded Next `distDir` plus complete generated/output-traced closure, script-free install, atomic Job/POSIX parent-death containment, and bounded cleanup tests. Task 3 adds the exact loopback-only credential-free null-session seam without masking operational Auth errors. Task 8 withholds every company-scoped Dashboard/Company/Listing/Seats/Billing projection under ambiguous context. Task 12 binds M4C and Lighthouse to an independently captured clean local pre-Task12 head; M4C uses canonical as-needed English routes plus a bounded immutable asset manifest, and Lighthouse holds the complete Chrome code/resource distribution. Managed M1-M7/M4B/M5/Axe and PostgreSQL remain separate guarded gates with real session/data restoration. Publication binds the zero-finding SHA through immutable tool closures, per-mutation STARTED/SUCCEEDED receipts, fail-stop OUTCOME_UNCERTAIN behavior, and a separate read-only reconciliation descriptor; no uncertain operation retries or continues.
