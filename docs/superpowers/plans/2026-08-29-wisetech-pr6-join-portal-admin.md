...
- Each mutation STARTED receipt also duplicates the executing PUBLICATION `entryIdentitySha256`, `requiredCommandIdentitySha256`, `commandProjectionSha256`, `commandTemplateSha256`, `runtimeSlotValuesSha256`, `resolvedCommandSha256`, verified tool, and execution mode. Its `argumentVectorSha256` is the same canonical resolved argument-vector identity carried by that stopped entry's phase projection; operation kind and exact base/head/ref/reviewed-head/captured-PR/publication-descriptor fields must equal that entry's resolved typed slots and the publication input. `publication-operation-outcome.v1` duplicates every one of those fields plus the exact STARTED path/hash. The terminal resolver compares the complete tuple outcome-to-STARTED and STARTED-to-the-one stopped manifest entry; matching only repository, reviewed head, or operation ID is invalid. Wrong entry, required-command identity, tool, mode, kind, vector, base, head, ref, captured PR, descriptor, or STARTED path/hash is a negative contract case.
- PUBLICATION command-evidence entries expose closed projection fields needed by that terminal join. A mutation entry requires `operationKind`, canonical nullable `capturedPrNumber`, and `publicationDescriptorSha256`; a reconciliation-read entry requires `reconciliationTemplateOrdinal` and `reconciliationReadKind`, with its existing `resolvedCommandSha256` equal to the descriptor template's argument-vector hash. Fields owned by another subtype are canonical null. These are projections of already approved typed slots, not new ambient inputs, and the resolver includes them in the entry identity and full-evidence aggregate.
- Reconciliation schemas use an ordered-prefix union. `publication-reconciliation.v1.readOperationTemplates` is a nonempty closed array of `{ordinal, readKind, argumentVectorSha256, allowedResultTuples}`, with one-based contiguous ordinals and a recomputed full-template aggregate; each allowed tuple fixes `launchDisposition`, `nativeExitClass: "ZERO" | "NONZERO" | "NO_NATIVE_EXIT"`, `status`, sanitized classification, and `continuationDisposition: "CONTINUE" | "STOP"`. Actual read receipts form an exact nonempty prefix. Each `publication-read-operation-receipt.v1` duplicates its descriptor path/hash and uncertain-outcome path/hash, ordinal, read kind, argument-vector hash, classification, continuation disposition, `mutationAttempted: false`, and unique identity, then discriminates on `launchDisposition`: `LAUNCHED` requires positive child identity, ordered start/end times, a native exit, no refusal receipt, and `status: "PASSED"` iff exit zero or `status: "NOT_PASSED"` iff nonzero; timeout/cancellation/tree termination is normalized only after cleanup to a truthful nonzero terminal exit and `NOT_PASSED`. `REFUSED_BEFORE_LAUNCH` requires `status: "NOT_PASSED"`, `continuationDisposition: "STOP"`, canonical-null child/times/native exit, and one exact `launcher-refusal-receipt.v1` proving no child and no mutation. Every receipt tuple must occur exactly once in its descriptor template's allowed table. Each nonfinal actual receipt is PASSED/CONTINUE; the final actual receipt is STOP, whether a successful classification has already answered the reconciliation question or the read failed. Every remaining descriptor ordinal then maps in order to its exact PUBLICATION command entry with `PROVEN_NOT_APPLICABLE` and unique applicability receipt whose producing entry/result is the earlier STOP decision.
- `publication-reconciliation-result.v1` and `publication-command-evidence-manifest.v1` both carry the descriptor's recomputed full-template aggregate; the actual prefix's ordered `{ordinal, readKind, argumentVectorSha256, launchDisposition, status, continuationDisposition, receiptSha256, classification}` mapping and recomputed executed-template aggregate; and the ordered unexecuted-suffix ordinals/applicability-receipt path/hash pairs plus recomputed suffix aggregate. They also retain the existing receipt/classification arrays and aggregates. The terminal resolver requires result and manifest equality, elementwise receipt-to-template equality, exact prefix cardinality, the CONTINUE/STOP rule, and exact suffix applicability closure. A first-read refusal, nonzero exit, or normalized timeout therefore closes truthfully as `NOT_PASSED` with a one-receipt prefix and proved-inapplicable suffix; it cannot disappear, be reported as success, or permit another read. Every non-reconciliation completion branch requires all read/template/prefix/suffix arrays empty and all related aggregates canonical null.
...
- [ ] **Step 9: Complete independent review and publish the stacked draft PR**
    ...
    if ($completionStatus -ceq "PASSED") {
      ...
      if ([string]$publicationCompletion.failureKind -ceq "RECONCILED_UNCERTAINTY") {
        ...
        foreach ($operationDuplicateField in @("operationId", "operationKind", "argumentVectorSha256", "entryIdentitySha256", "requiredCommandIdentitySha256", "commandProjectionSha256", "commandTemplateSha256", "runtimeSlotValuesSha256", "resolvedCommandSha256", "verifiedTool", "executionMode", "repository", "baseRefName", "baseRefOid", "headRefName", "headRefOid", "remoteRef", "reviewedHead", "capturedPrNumber", "publicationDescriptorSha256")) {
        ...
        if ([string]$uncertainStartedOperation.entryIdentitySha256 -cne [string]$stoppedCommandEntry.entryIdentitySha256 -or [string]$uncertainStartedOperation.requiredCommandIdentitySha256 -cne [string]$stoppedCommandEntry.requiredCommandIdentitySha256 -or [string]$uncertainStartedOperation.commandProjectionSha256 -cne [string]$stoppedCommandEntry.commandProjectionSha256 -or [string]$uncertainStartedOperation.commandTemplateSha256 -cne [string]$stoppedCommandEntry.commandTemplateSha256 -or [string]$uncertainStartedOperation.runtimeSlotValuesSha256 -cne [string]$stoppedCommandEntry.runtimeSlotValuesSha256 -or [string]$uncertainStartedOperation.resolvedCommandSha256 -cne [string]$stoppedCommandEntry.resolvedCommandSha256 -or [string]$uncertainStartedOperation.argumentVectorSha256 -cne [string]$stoppedCommandEntry.resolvedCommandSha256 -or [string]$uncertainStartedOperation.verifiedTool -cne [string]$stoppedCommandEntry.tool -or [string]$uncertainStartedOperation.executionMode -cne [string]$stoppedCommandEntry.mode -or [string]$uncertainStartedOperation.operationKind -cne [string]$stoppedCommandEntry.operationKind -or -not [Object]::Equals($uncertainStartedOperation.capturedPrNumber, $stoppedCommandEntry.capturedPrNumber) -or [string]$uncertainStartedOperation.publicationDescriptorSha256 -cne [string]$stoppedCommandEntry.publicationDescriptorSha256) { throw "PR6_PUBLICATION_UNCERTAIN_STOPPED_COMMAND_MISMATCH" }
        ...
        for ($reconciliationReadIndex = 0; $reconciliationReadIndex -lt $readCount; $reconciliationReadIndex += 1) {
          ...
          $readCommandEntries = @($publicationEvidenceEntries | Where-Object { [int]$_.reconciliationTemplateOrdinal -eq $expectedReadOrdinal -and [string]$_.reconciliationReadKind -ceq [string]$expectedReadTemplate.readKind -and [string]$_.resolvedCommandSha256 -ceq [string]$expectedReadTemplate.argumentVectorSha256 -and [string]$_.launchDisposition -ceq [string]$reconciliationReadReceipt.launchDisposition })
          ...
          if ([string]$reconciliationReadReceipt.commandEntryIdentitySha256 -cne [string]$readCommandEntry.entryIdentitySha256 -or [string]$reconciliationReadReceipt.commandResultProjectionSha256 -cne [string]$readCommandEntry.resultProjectionSha256) { throw "PR6_PUBLICATION_RECONCILIATION_READ_COMMAND_BINDING_INVALID" }
        ...
        for ($suffixIndex = 0; $suffixIndex -lt $expectedSuffixCount; $suffixIndex += 1) {
          ...
          $suffixCommandEntries = @($publicationEvidenceEntries | Where-Object { [int]$_.reconciliationTemplateOrdinal -eq $expectedSuffixOrdinal -and [string]$_.reconciliationReadKind -ceq [string]$suffixTemplate.readKind -and [string]$_.resolvedCommandSha256 -ceq [string]$suffixTemplate.argumentVectorSha256 -and [string]$_.launchDisposition -ceq "PROVEN_NOT_APPLICABLE" -and [string]$_.applicabilityReceiptPath -ceq [string]$manifestSuffixApplicabilityPaths[$suffixIndex] -and [string]$_.applicabilityReceiptSha256 -ceq [string]$manifestSuffixApplicabilitySha256s[$suffixIndex] })
