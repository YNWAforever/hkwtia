...
- [ ] **Step 3: Implement durable billing and status projection**
    ...
    Before the journey, retrieve both configured Stripe test Prices and require `active === true`, `type === "recurring"`, `currency === "hkd"`, and `recurring.interval === "year"`. Tag every disposable database row with one generated `runId`. Maintain a test-only `M1StripeRunLedger` in `tests/fixtures/m1-live-acceptance.ts`: start from the exact Checkout Session ID attached to the run-owned billing attempt, retrieve that Session after completion, require its `client_reference_id` and Session/Subscription metadata to equal the run-owned membership/application/attempt IDs, and record the exact Session, Customer, Subscription, Invoice, PaymentIntent, and Charge IDs reachable from that lineage. Record the Billing Portal redirect origin and exact locale-correct return only as named sanitized residual evidence because the production adapter intentionally does not expose that immutable Session ID. This ledger, rather than an unsupported claim that every Stripe object is deletable or accepts metadata, proves provider ownership without adding test-only fields to the production adapter.

    Request the real test-mode Neon magic link, poll only the allowlisted controlled inbox for that recipient and request timestamp, open the returned callback, complete profile/company onboarding, prove the stored annual option and exact Startup or Corporate Price, complete Stripe test checkout, wait for the signed webhook to activate the durable membership, render active completion, open the locale-correct Billing Portal, verify receipts/seats/secondary pages, and sign out.

    `afterAll` must run after both success and failure. Collect and validate the ledger before deleting database rows, then apply these exact dispositions only to owned test objects: if the Checkout Session is `open`, expire it and verify `expired`; if already `expired`, verify only; if `complete`, retain it. Cancel a Subscription in `active`, `trialing`, `past_due`, `unpaid`, `paused`, or `incomplete` immediately and verify `canceled`; accept `canceled` or `incomplete_expired` as already terminal. After Subscription disposition, delete the disposable Customer and verify Stripe returns `deleted: true`. Completed Checkout Sessions, Invoices, PaymentIntents, and Charges are immutable Stripe test-mode financial/audit records: do not attempt to delete them, and record their IDs, type, ownership lineage, and `retained_immutable_test_record` disposition. Record the Portal Session as `retained_immutable_unaddressable_test_record` with sanitized redirect/return evidence, not a fabricated ID. Never mutate/delete either configured Price or the shared webhook endpoint. Finally delete only run-ID-owned database rows and verify zero remain. Fail the suite only when required Session expiration, Subscription cancellation/terminal verification, Customer deletion, or database cleanup is incomplete; named immutable residual evidence is an expected disposition, not a cleanup failure. Do not use a production email, customer, Price, endpoint, database, or catch-all deletion.
