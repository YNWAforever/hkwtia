...
- Task 4 owns the shared test-only managed-runtime boundary used by mutating M1, M2, M3, M4B, M5, M6, M7, and final authenticated-Axe suites:
  ...
  Any nonempty value for any sentinel name activates the no-reuse safety posture even when the value is wrong. Reject more than one active mutating suite, reject any present `PLAYWRIGHT_BASE_URL`, validate one canonical decimal `PLAYWRIGHT_PORT` (default `3000`), and derive exactly `http://localhost:<port>`. That origin is the sole `use.baseURL`, `webServer.url`, child `APP_URL`, and suite allowed origin. For every mutating run, `webServer.env` spreads defined ambient values first, then unconditionally overrides `DATABASE_URL <- DATABASE_URL_TEST` and `APP_URL <- managed origin` and resets `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_STARTUP_PRICE_ID`, `STRIPE_CORPORATE_PRICE_ID`, `CRON_SECRET`, `UNSUBSCRIBE_TOKEN_SECRET`, `NEON_AUTH_BASE_URL`, and `NEON_AUTH_COOKIE_SECRET` to empty strings. The active suite wrapper may then supply only its explicitly required test values: M1/M2 map all four `STRIPE_TEST_*` names, M2 maps `M2_TEST_CRON_SECRET` and `M2_TEST_UNSUBSCRIBE_TOKEN_SECRET`, M3 maps `M3_TEST_UNSUBSCRIBE_TOKEN_SECRET`, and every authenticated managed suite maps `NEON_AUTH_TEST_BASE_URL` and `NEON_AUTH_TEST_COOKIE_SECRET` only after the pure guard proves the canonical HTTPS base origin equals independent `NEON_AUTH_TEST_ALLOWED_ORIGIN` and is non-production. Missing suite-required test values fail configuration/guard before a Pool, browser, auth, provider, or mutation client is constructed; ambient standard database/provider/operational/auth names can never win. `reuseExistingServer` is `false` for any nonempty destructive sentinel so an occupied port fails instead of attaching to a stale server. Credential-free runs retain their existing behavior. M1/M2/M3/M4B/M5/M6/M7/Axe use this managed boundary; only M4C and the public Lighthouse gate remain separately guarded read-only Preview flows.
...
- [ ] **Step 1: Write failing durable-price, locked-row, completion, and locale tests**
    ...
    In `isolated-runtime-environment.test.ts`, prove external `PLAYWRIGHT_BASE_URL` rejection, canonical managed origin/port equality, multiple-suite rejection, ambient standard-variable override precedence, exact database/APP mapping, standard Stripe/cron/unsubscribe/Auth scrubbing, suite-only remapping, missing suite-required test variables, non-test Stripe/Auth inputs, and `reuseExistingServer: false` for every nonempty M1/M2/M3/M4B/M5/M6/M7/Axe sentinel including malformed values. In `m1-live-acceptance-safety.test.ts`, prove pure guards reject missing/wrong destructive or provider sentinels, absent `DATABASE_URL_TEST`, non-Neon/TLS-invalid/production-labelled/mismatched database hosts/projects, managed origin/`APP_URL`/`M1_E2E_ALLOWED_ORIGIN` mismatch, any present `PLAYWRIGHT_BASE_URL`, non-test Stripe credentials, invalid or mismatched `NEON_AUTH_TEST_BASE_URL`/`NEON_AUTH_TEST_ALLOWED_ORIGIN`, weak `NEON_AUTH_TEST_COOKIE_SECRET`, unallowlisted inbox/auth-link origins, missing owner/invitee/overflow emails, and normalized email collisions before Pool/browser/inbox/Auth/Stripe construction. Then prove the guarded read-only DB preflight rejects absent or non-member owner/invitee profile mappings, configured/profile email or auth/profile-ID collisions, and pre-existing owner Join/company context after only the guarded Pool exists but before any other client or mutation. Finally prove the post-company/pre-invitation checkpoint rejects an existing invitee membership or active count other than one for the exact run-owned company.
...
- [ ] **Step 1: Write failing grouped-navigation, source-owner, and CRM rendering tests**
    ...
    Extend the M2 safety/runtime tests to require `M2_ACCEPTANCE_ALLOW_DESTRUCTIVE=M2_ISOLATED_FIXTURES_ONLY`, `M2_E2E_ALLOWED_ORIGIN`, `APP_URL`, `NEON_PROJECT_ID`, `M2_TEST_NEON_PROJECT_ID`, `M2_TEST_NEON_HOST`, `M2_TEST_CRON_SECRET`, and `M2_TEST_UNSUBSCRIBE_TOKEN_SECRET` in the centralized contract while delegating standard DB/Stripe/APP mapping to Task 4's shared builder. The M2 wrapper then unconditionally maps `CRON_SECRET <- M2_TEST_CRON_SECRET` and `UNSUBSCRIBE_TOKEN_SECRET <- M2_TEST_UNSUBSCRIBE_TOKEN_SECRET` after the ambient spread; require both test secrets nonempty and distinct, with the unsubscribe secret at least 32 bytes, so ambient production names never win. Assert any present `PLAYWRIGHT_BASE_URL`, missing/wrong sentinel, managed-origin/APP/allowlist mismatch, missing project/test/secret variables, equal or undersized test secrets, production/non-Neon/mismatched/TLS-invalid DB, raw test DB mismatch after mapping, multiple mutating sentinels, or reusable/occupied server fails before `connect` or `seedM2`.
    Extend `resetM2AuthenticatedFixtures` and its unit tests with a complete three-identity lifecycle. After the guarded `beforeAll` reset, snapshot the full configured staff/member/company-admin profiles. After every browser context is closed, run the named mutation reset, poll the three profiles through a bounded quiet window, and compare-and-swap restore the snapshots against the full last-observed rows. Retry only an allowed in-window `lastLoginAt`/`updatedAt` touch, require a second byte-identical quiet window, preserve external drift, aggregate all identity/reset failures, and fail cleanup on any incomplete restoration. Inject a delayed `touchLastLogin` after the reset and after the first CAS in unit tests. The E2E source contract requires reset/snapshot in `beforeAll` and context-close/reset/quiescent restoration in `afterAll`.
...
- [ ] **Step 2: Run the focused tests and record the RED reason**
    ...
    Expected: FAIL because Admin navigation is flat, Dashboard is brand-only, CRM pages lack shared primitives, the M2 reset/browser path is not yet bound to the shared managed loopback/test-resource runtime, test-only cron/unsubscribe mappings and quiescent three-profile restoration are absent, and the browser suite lacks the complete 26-page and 19-API authority matrices.
...
- [ ] **Step 3: Adopt Admin shell and CRM primitives without moving authority**
    ...
    Add `M2_ACCEPTANCE_DESTRUCTIVE_SENTINEL = "M2_ISOLATED_FIXTURES_ONLY"` and the complete required M2 names. Keep `tests/fixtures/m2-runtime-env.ts` as an M2-specific missing-variable wrapper that delegates/re-exports Task 4's neutral builder, then maps only `M2_TEST_CRON_SECRET`/`M2_TEST_UNSUBSCRIBE_TOKEN_SECRET` to their standard child names. Require `PLAYWRIGHT_BASE_URL` absent, the canonical managed origin equal to `APP_URL` and `M2_E2E_ALLOWED_ORIGIN`, exact child `DATABASE_URL === DATABASE_URL_TEST`, standard Stripe names mapped only from `STRIPE_TEST_*`, standard cron/unsubscribe names mapped only from the distinct valid M2 test secrets, canonical non-production Neon/TLS/project identity, and `hkwtia.vercel.app` forbidden. Any nonempty destructive sentinel forces no server reuse; missing/wrong authorization skips as `NOT PASSED` before mutation. A separately approved valid run performs reset plus full identity snapshot before browser work and, after contexts close, named reset plus bounded quiet-window/CAS restoration in `afterAll`.
...
- [ ] **Step 4: Run GREEN and M2 CRM regressions**
    ...
    Expected: PASS with grouped active navigation, locale retention, unchanged CRM/security assertions, exact shared plus M2-only secret mappings, every unsafe M2 reset/target case blocked before connection or seed, and delayed login touches unable to survive quiescent three-profile cleanup.

    Run:

    npm.cmd run test:e2e -- tests/e2e/m2-admin-crm.spec.ts

    Expected: credential-free anonymous 26-page/two-locale Admin 404 PASS. The isolated anonymous/member/company-admin 19-API matrix, authenticated member/company-admin page matrix, and CRM cases run only after separate mutation approval and the complete M2 variable gate; all 26 pages deny both authenticated roles, every API preserves its route-specific denial/no-op status, the fingerprint is unchanged, all contexts close, and `afterAll` completes deterministic data reset plus quiet-window/CAS restoration of staff/member/company-admin profiles. Missing approval, sentinel, database identity, origin, test secrets, credentials, or complete cleanup remains `NOT PASSED` and never calls `seedM2` before the guard.
...
- [ ] **Step 1: Write failing CMS family source and rendering contracts**
    ...
    Add M7 safety RED cases for `M7_ACCEPTANCE_ALLOW_DESTRUCTIVE=M7_ISOLATED_FIXTURES_ONLY`, exact shared child `DATABASE_URL === DATABASE_URL_TEST`, `NEON_PROJECT_ID === M7_TEST_NEON_PROJECT_ID`, canonical TLS and exact non-production `M7_TEST_NEON_HOST`, absent `PLAYWRIGHT_BASE_URL`, canonical managed origin equal to `APP_URL`/`M7_E2E_ALLOWED_ORIGIN`, valid shared test-only Neon Auth mapping, one active mutating sentinel, forced no-reuse, and exact `M7_TEST_ADMIN_EMAIL`/`M7_TEST_ADMIN_PASSWORD`. Pure target/runtime failures occur before Pool/auth/browser; guarded DB identity checks occur after only the Pool exists and before auth/browser or mutation.
...
- [ ] **Step 3: Apply Operations presentation primitives**
    ...
    Implement `m6-browser-lifecycle.ts` and make `m6-launch-pad.spec.ts` call it in `beforeAll`/`afterAll`. Before seed, enforce the fixed-ID/unique-pair equality for all five cohort applications and exact one-row/expected-ID Showcase cardinality for the graduation company. The member application is owned only by exact cohort, controlled member company, DB-clock request bounds, and pre-run absence/baseline. Each stage audit is owned only by new ID, configured staff actor/type, action `cohort_application.stage_changed`, target type/id, database-clock request bounds, and exact `{fromStage, toStage}` chain. Snapshot and compare-and-swap restore every pair-owned application, all seed-upserted fixed rows, and the complete graduation-company Showcase set including `goneGlobal`/`updatedAt`; delete only a truly absent-before member application and its exact residue. Close contexts, drain/restore both profiles with the shared helper, then verify byte-identical pre-run database and identity baselines. Partial seed, application, transition, audit, Showcase, or one-identity failure cannot suppress another cleanup phase. Unexpected alternate IDs, cardinality change, clock-skew candidate, or drift is preserved and fails.
...
- Modify the public performance gate: `lighthouserc.js`. It must use only the exact authorized Preview route allowlist below, local filesystem output, and no noindex/authenticated URL.
...
- [ ] **Step 1: Define final credential-free and authenticated browser verification matrices**
    ...
    This suite has its own managed lifecycle: require `PR6_AUTHENTICATED_AXE_ALLOW_DESTRUCTIVE=PR6_ISOLATED_IDENTITIES_ONLY`, absent `PLAYWRIGHT_BASE_URL`, exact managed `APP_URL`/`PR6_AXE_E2E_ALLOWED_ORIGIN`, guarded `DATABASE_URL_TEST` mapping, `NEON_PROJECT_ID === PR6_AXE_TEST_NEON_PROJECT_ID`, exact non-production `PR6_AXE_TEST_DB_HOST`, valid shared test-only Neon Auth mapping, and exactly four pairwise-distinct route-matrix identities: M2 member/staff, M3 staff, and M7 Admin. After pure checks, snapshot every distinct full profile before login. Each real sign-in must return the exact expected profile/auth identity and advance that same guarded DB profile's `lastLoginAt` inside database `clock_timestamp()` request bounds, observationally binding target and database before continuing. Close every context and quiet-window/CAS restore all identities in an aggregate `afterAll`; a skip, binding failure, drift, or incomplete restoration is `NOT PASSED`.
    ...
    Refactor `m4b-e2e-safety.ts` and `m4b-agents.spec.ts` onto Task 4 managed runtime. Require `M4B_IDENTITY_RESTORE_ALLOW_DESTRUCTIVE=M4B_TEST_IDENTITIES_ONLY`, canonical `M4B_ACCEPTANCE_AS_OF`, absent `PLAYWRIGHT_BASE_URL`, exact canonical managed `APP_URL`/`M4B_E2E_ALLOWED_ORIGIN`, mapped `DATABASE_URL_TEST` with exact `M4B_TEST_DB_HOST`, valid shared test-only Neon Auth mapping, and exact distinct `M4B_TEST_STAFF_EMAIL`, `M4B_TEST_STAFF_PASSWORD`, `M4B_TEST_MEMBER_EMAIL`, and `M4B_TEST_MEMBER_PASSWORD` before Pool/browser/auth. A guarded read-only preflight derives the exact fixture window/source keys from `M4B_ACCEPTANCE_AS_OF` and requires exactly the three expected pending retention approvals plus one exact inert Board-report post and no ambiguous duplicate before login; it never calls `seedM4B`, an agent, a job route, or a provider. Snapshot both full profiles through Task 10 generic helper. Load both message catalogs and iterate locale prefixes `""` and `"/zh"`: staff must see the three retention approvals, reports, and inert Board preview in both locales; member must receive each locale's real 404 on approvals/reports. Close every context and independently quiet-window/CAS restore both profiles; missing/stale fixture evidence, one missing locale, skip, late touch, or restoration failure is `NOT PASSED`.

    Implement `m5-browser-lifecycle.ts` as a managed, advisory-locked, test-only lifecycle. Require `M5_ACCEPTANCE_ALLOW_DESTRUCTIVE=M5_ISOLATED_FIXTURES_ONLY`, absent `PLAYWRIGHT_BASE_URL`, exact managed `APP_URL`/`M5_E2E_ALLOWED_ORIGIN`, mapped `DATABASE_URL_TEST` with exact `M5_TEST_DB_HOST`, valid shared test-only Neon Auth mapping, exact distinct `M5_TEST_MANAGER_EMAIL`, `M5_TEST_MANAGER_PASSWORD`, `M5_TEST_MEMBER_EMAIL`, `M5_TEST_MEMBER_PASSWORD`, `M5_TEST_STAFF_EMAIL`, and `M5_TEST_STAFF_PASSWORD`, plus exact `M5_TEST_COMPANY_ID` and `M5_TEST_LISTING_ID` before Pool/browser/auth. Guarded DB preflight requires manager/member application profiles with role `member` and staff with role `staff`; normalized email/auth/profile IDs are pairwise distinct; manager has exactly one active company context at the target with company role `owner` or `admin`; ordinary member has exactly one at the same company with role `member`; and the listing ID is absent or belongs only to that company with no natural-key collision. Snapshot all three full profiles and the complete listing row or prior absence, then compare-and-swap initialize one exact draft run value. Through separate fresh contexts, manager saves a real draft and submits review, ordinary member sees the same listing read-only with no save/submit controls, and staff rejects the exact pending row with one nonempty run reason; verify every durable transition against the guarded DB. After contexts close, CAS restore/delete the listing to its exact baseline, terminate the managed target so no stale cache is claimed, and quiet-window/CAS restore all three profiles independently. Unit/source tests cover pure guard-before-Pool/browser/auth, origin mismatch, role/company ambiguity, alternate listing ownership, prior-present/prior-absent rows, every partial transition, late login touches, external drift, and aggregate cleanup.

    Before Step 2, create `tests/unit/lighthouse-config.test.ts` and replace `lighthouserc.js` with the exact fail-closed configuration specified in Step 6. This is Task 12 configuration work, not an external gate claim: the unit contract must be executable in the focused aggregate without network access, while the authorized Preview collection remains deferred to Step 6.

- [ ] **Step 2: Run the focused PR6 cross-surface aggregate**

    Run:

    npm.cmd test -- tests/unit/membership-catalog.test.ts tests/unit/membership-public-catalog.test.ts tests/unit/membership-links.test.tsx tests/unit/join-schema.test.ts tests/unit/join-navigation.test.ts tests/unit/join-service.test.ts tests/unit/join-actions.test.ts tests/unit/member-login-page.test.tsx tests/unit/portal-sign-out-button.test.tsx tests/unit/isolated-runtime-environment.test.ts tests/unit/checkout-service.test.ts tests/unit/checkout-recovery-service.test.ts tests/unit/billing-recovery-cas.test.ts tests/unit/join-billing-pages.test.tsx tests/unit/m1-live-acceptance-safety.test.ts tests/unit/seat-invitation-routes.test.tsx tests/unit/seat-service.test.ts tests/unit/internal-navigation.test.tsx tests/unit/internal-shell.test.tsx tests/unit/wisetech-pr6-route-inventory.test.ts tests/unit/wisetech-pr6-join-shell.test.tsx tests/unit/locale-switcher.test.tsx tests/unit/wisetech-pr6-portal-shell.test.tsx tests/unit/portal-company-context.test.ts tests/unit/wisetech-pr6-admin-crm-shell.test.tsx tests/unit/m2-auth-reset.test.ts tests/unit/m2-runtime-environment.test.ts tests/unit/m2-browser-acceptance-contract.test.ts tests/unit/m3-e2e-safety.test.ts tests/unit/m3-browser-lifecycle.test.ts tests/unit/authenticated-identity-safety.test.ts tests/unit/wisetech-pr6-admin-cms-shell.test.tsx tests/unit/m7-acceptance-safety.test.ts tests/unit/m7-browser-acceptance-contract.test.ts tests/unit/wisetech-pr6-admin-operations-shell.test.tsx tests/unit/m6-e2e-safety-contract.test.ts tests/unit/m5-browser-lifecycle.test.ts tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts tests/unit/lighthouse-config.test.ts
...
- [ ] **Step 5: Prove safety contracts, then run isolated M1-M7 only with separate authority**
    ...
    npm.cmd test -- tests/unit/isolated-runtime-environment.test.ts tests/unit/authenticated-identity-safety.test.ts tests/unit/m1-live-acceptance-safety.test.ts tests/unit/m2-auth-reset.test.ts tests/unit/m2-runtime-environment.test.ts tests/unit/m2-browser-acceptance-contract.test.ts tests/unit/m3-acceptance-isolation.test.ts tests/unit/m3-acceptance-safety.test.ts tests/unit/m3-e2e-safety.test.ts tests/unit/m3-browser-lifecycle.test.ts tests/unit/m4b-runtime-guard.test.ts tests/unit/m4b-e2e-safety.test.ts tests/unit/m5-browser-lifecycle.test.ts tests/unit/m6-e2e-safety-contract.test.ts tests/unit/m7-acceptance-safety.test.ts tests/unit/m7-browser-acceptance-contract.test.ts tests/unit/pr6-authenticated-accessibility-lifecycle.test.ts

    Expected: PASS with shared managed-loopback/resource mapping, managed identity target-to-DB binding, standard database/provider/operational/Auth secret scrubbing, no-reuse behavior, and every destructive/provider/database/target/identity guard failing closed at its specified pure or guarded-DB phase before unauthorized mutation.
    ...
    - M6: `PLAYWRIGHT_BASE_URL` absent, optional `PLAYWRIGHT_PORT`, exact managed `APP_URL`/`M6_E2E_ALLOWED_ORIGIN`, `M6_ACCEPTANCE_SEED=true`, fixed canonical `M6_ACCEPTANCE_AS_OF`, `DATABASE_URL_TEST` mapped to child `DATABASE_URL`, exact `M6_ACCEPTANCE_DATABASE_HOST_ALLOWLIST`, the shared test-only Neon Auth trio, exact `M6_TEST_MEMBER_EMAIL`/`M6_TEST_MEMBER_PASSWORD`, `M6_TEST_STAFF_EMAIL`/`M6_TEST_STAFF_PASSWORD`, `M6_TEST_MEMBER_COMPANY_DISPLAY_NAME`, and `M6_TEST_GRADUATE_COMPANY_DISPLAY_NAME`. Any nonempty seed value disables reuse; lifecycle restores seed rows, member application, audits, Showcase state, and identities.
    ...
    Each guard requires canonical origins, independent non-production resource identity, standard secret scrubbing, and test-only child mappings. M1 handles recoverable provider/webhook/database/profile cleanup; M2 deterministic reset plus quiescent three-profile restoration; M3 pre-seed login snapshots, complete seed-footprint restoration, test-only unsubscribe verification, and identity restoration; M4B exact read-only fixture preflight plus bilingual two-profile restoration; M5 manager/member/staff listing plus identity restoration; M6 fixed-pair/all-Showcase/audit/identity reset; M7 Page Copy/News/Media/audit/Admin-profile restoration; and authenticated Axe four-identity restoration. Any ambiguity, cleanup, snapshot, drift, audit ownership, identity quiet-window, seat residue, or public-baseline failure fails. M1/M2/M3/M4B/M5/M6/M7/Axe are managed loopback only; M4C is separately guarded read-only Preview. No external/provider/database mutation occurs without separate named approval.
    ...
    - M6 proves legal cohort transitions/stage audits and Showcase projection under deterministic managed reset, then restores exact seed rows, all fixed cohort/company pair identities, member application, database-clock-owned audit set, every graduation-company Showcase row, and both identities.
...
- [ ] **Step 6: Run the exact authorized-Preview Lighthouse and field-performance gates**
    ...
    Use the fail-closed `lighthouserc.js` and unit contract implemented before Step 2. The config requires canonical HTTPS `LHCI_BASE_URL` and independently supplied `LHCI_ALLOWED_ORIGIN` to be exact-equal, requires `VERCEL_ENV=preview` or `LHCI_PREVIEW_ONLY=true`, rejects localhost and production-labelled hosts, and never reads `PLAYWRIGHT_BASE_URL`. Collect exactly `/membership`, `/zh/membership`, `/join?plan=startup&interval=annual`, and `/zh/join?plan=startup&interval=annual` for three runs. Never include noindex `/member-login` or authenticated routes. Assert performance at least 0.90, accessibility at least 0.95, SEO at least 0.95, `largest-contentful-paint` at most 2,500 ms, and `cumulative-layout-shift` at most 0.1. Set upload target to local filesystem only; never use temporary public storage. `lighthouse-config.test.ts` imports the real config and proves exact route equality, guard-before-collect behavior, thresholds, run count, no local fallback, no private/noindex routes, and no public upload.

    Run:

    npm.cmd test -- tests/unit/lighthouse-config.test.ts
    npm.cmd run test:lighthouse

    Expected: the config contract and authorized Preview collection PASS for the exact four public URLs. Record LCP at most 2.5 seconds and CLS at most 0.1 from the local Lighthouse artifacts. Record INP at most 200 ms at p75 only from separately authorized field/Preview telemetry; if that evidence is absent, older than the reviewed deployment, bound to another origin, or above 200 ms, the INP/performance external gate is `NOT PASSED`. Delete/ignore generated local artifacts and never stage them.
...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    - exact disposable cleanup evidence for every authorized suite: M1 provider/webhook/profile ledgers and named immutable evidence; M2 data reset plus three-profile quiescence; M3 complete seed-predicate ledger and identity restoration; M4B bilingual/two-profile restoration; M5 listing/three-profile restoration; M6 pair/application/audit/all-Showcase/identity reset; M7 Page Copy/News/Media/audit/Admin-profile restoration; and authenticated-Axe all-profile restoration;
...
- [ ] **Step 9: Complete independent review and publish the stacked draft PR**
    ...
    The zero-finding verdict must print the exact 40-character reviewed commit SHA. In the publication PowerShell session, obtain `$reviewedHead` from that verdict—not from current `HEAD`—and fail closed if it is not a lowercase 40-hex commit that resolves locally:
    ...
    if ((git rev-parse HEAD).Trim() -cne $reviewedHead) { throw "PR6_HEAD_CHANGED_AFTER_REVIEW" }
    ...
    Push only the reviewed branch. Recheck immediately before the push; no command that can create a commit may intervene:

    if ((git rev-parse HEAD).Trim() -cne $reviewedHead) { throw "PR6_HEAD_CHANGED_BEFORE_PUSH" }
    ...
    if ($localHead -cne $reviewedHead) { throw "PR6_UNREVIEWED_LOCAL_HEAD" }
    ...
    if ($published.headRefName -cne "codex/wisetech-pr6-join-portal-admin" -or $published.headRefOid -cne $reviewedHead) { throw "PR6_PR_HEAD_MISMATCH" }
    ...
    if ($remoteHead -cne $reviewedHead) { throw "PR6_REMOTE_HEAD_MISMATCH" }

    Expected: OPEN draft PR, exact reviewed body, base name/OID `codex/wisetech-pr5-public-journeys`/`3856dd71842f9a2e1d9c4b7a46521416a5bd83ae`, and local/published/remote head all exactly equal to `$reviewedHead` from the zero-finding verdict. Report pending/failing remote checks separately. Do not merge, deploy, mutate providers, or convert an external gate into PASS.
...
- Verification coverage: Task 12 includes exact credential-free and 60-case authenticated matrices, 26-page/19-API denial proof, unit/safety aggregates, managed M1-M7/M4B/M5/Axe lifecycles with standard secret scrubbing and complete identity/data restoration, M1 bounded timeout and immutable-evidence contract, M2 test-secret/quiescence safety, M3 pre-seed identity plus complete seed ledger, M4B exact read-only fixture preflight, M6 conflict-key/all-Showcase restoration, M7 database-clock audit ownership, an exact four-route authorized-Preview Lighthouse config implemented before aggregate execution with numeric Web Vitals and local-only artifacts, stage-before-commit ordering, post-commit immutable range proof, and publication bound to the exact zero-finding reviewed SHA.
