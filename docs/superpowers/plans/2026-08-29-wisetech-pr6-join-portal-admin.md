...
- Exact non-secret operation records are closed schemas. `hkwtia.pr6.m4c-readonly-input.v1` binds the Task 12 target/helper/allowed-dirty identities, M4C approval descriptor, exact request manifest, repository/branch/PR5/spec/approved-plan/`preTask12Head`, and read-only provider operation. `hkwtia.pr6.lighthouse-collect-input.v1` binds the Task 12 target-record path/hash, captured Task 12 assertion-helper path/allowed-root/hash, exact allowed-dirty-path inventory, repository/branch/PR5/spec/approved-plan/`preTask12Head`, Preview-approval descriptor path/hash, and the six checkpoint/entrypoint/config identities. `hkwtia.pr6.verification-authoring-input.v1` binds those target/helper/head/checkpoint fields; exact gate status; an ordered closed `hkwtia.pr6.suite-evidence-manifest.v1` path/hash whose entries bind suite, sanitized argument hash, start/end timestamps, exit code, totals/warnings/skips, `PASSED | NOT_PASSED`, approval reference, and cleanup-receipt path/hash; and, iff collection passed, run-root/retained-artifact/provider-sanitized evidence identities. `hkwtia.pr6.lighthouse-commit-verification.v1` binds repository/branch/PR5/spec/approved-plan/final head, final verification-document `HEAD:path` blob OID/SHA-256, the six checkpoint identities, and `gateStatus: "PASSED" | "NOT_PASSED"`; run-root/artifact/collection identities are required iff `PASSED` and forbidden otherwise. `hkwtia.pr6.zero-finding-review-package.v1` binds repository/branch, reviewed head, PR5/spec OIDs, approved-plan record/commit/literal path/blob OID/raw blob-byte SHA-256/approved-checkout identity/docs-only origin, reviewer evidence hash, and exact totals `{critical: 0, important: 0, minor: 0}`. `hkwtia.pr6.publication-input.v1` binds that review package, exact approved-plan-record path/hash, publication descriptor, repository/head/base branches, base/spec/approved-plan/reviewed-head OIDs, and literal plan/blob identity; all duplicates must be byte-equal. No operation record contains a token, credential, provider response, or raw review prose. Trusted launcher/CI issues each only after its prerequisite exists; no record is reused for another schema, head, or shell.
...
- PowerShell never starts a credentialed Git/`gh` process itself. For every descriptor-authorized remote read or mutation, the operation-bound invoker sends one framed non-secret launch request and argument hash to the external launcher's native credential broker. After descriptor/receipt/allowlist equality, the broker creates the exact attested Git/HTTPS/helper or GitHub-CLI tree under the same restricted token, non-breakaway Job/process group, deadlines, and read-only executable/config roots. It builds a child-tree-only replacement environment: canonical OS runtime keys, read-only empty `GH_CONFIG_DIR`, and exact `GH_TOKEN` bytes copied from escrow only for the contained Git/`gh`/`gh auth git-credential` tree. The token is absent from the PowerShell and Node environments, command lines, stdin, stdout/stderr, config, receipts, and disk; no credential-store/login command is allowed. On final descendant exit or cancellation, the broker closes handles, destroys the environment block, zeroes every parent token buffer, and records only token-name absence plus synthetic-value hashes in tests. Source/subprocess tests use a fake broker token to prove exact helper/direct-`gh` success, shell/Node absence, read-only config, output redaction, no sibling inheritance, full-tree cleanup, and failure before network when broker/acknowledgement/zeroing drifts.
...
- [ ] **Step 6: Run the exact authorized-Preview Lighthouse and field-performance gates**
    ...
    $task12LiveResults = @(& $assertPr6LiveTask12Target -InputRecordResolverScriptBlock $runtimeInputRecordResolver -VerifiedGitInvoker $verifiedGit -TargetRecordPath ([string]$lighthouseCollectInput.task12TargetRecordPath) -TargetRecordSha256 ([string]$lighthouseCollectInput.task12TargetRecordSha256) -AllowedDirtyPaths @($lighthouseCollectInput.allowedDirtyPaths) -AllowedDirtyPathsSha256 ([string]$lighthouseCollectInput.allowedDirtyPathsSha256))
    if ($task12LiveResults.Count -ne 1) { throw "PR6_TASK12_LIVE_RESULT_COUNT_INVALID" }
    $task12LiveResult = $task12LiveResults[0]
    if ($task12LiveResult.schema -cne "hkwtia.pr6.task12-live-assertion-result.v1" -or $task12LiveResult.head -cne $preTask12Head -or $task12LiveResult.gitTreeClosed -ne $true -or [string]$task12LiveResult.evidenceSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_TASK12_LIVE_RESULT_INVALID" }
    $verifiedGit = $null
...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    $assertPr6LiveTask12Target = $task12AssertionBlocks[0]
    $verifiedGit = { & $runtimeInvoker -NativeProcessBoundaryScriptBlock $runtimeNativeProcessBoundary -GuardianScriptBlock $runtimeGuardian -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -OperationInputRecordPath $operationInputRecordPath -OperationInputRecordSha256 $operationInputRecordSha256 -Tool Git -Arguments @($args) }.GetNewClosure()
    $task12LiveResults = @(& $assertPr6LiveTask12Target -InputRecordResolverScriptBlock $runtimeInputRecordResolver -VerifiedGitInvoker $verifiedGit -TargetRecordPath ([string]$verificationAuthoringInput.task12TargetRecordPath) -TargetRecordSha256 ([string]$verificationAuthoringInput.task12TargetRecordSha256) -AllowedDirtyPaths @($verificationAuthoringInput.allowedDirtyPaths) -AllowedDirtyPathsSha256 ([string]$verificationAuthoringInput.allowedDirtyPathsSha256))
    ...
    The launch transcript proves the shell's replacement environment contains no `LHCI_*`, `VERCEL_SHARE_TOKEN`, provider credential, or approval value, and both resolved schemas reject such fields; do not enumerate or print an ambient environment. The authoring input and suite manifest are held immutable through the edit and exact Git checks. Missing/drifted target/helper/checkpoint/suite evidence, an unauthorized run-root branch, or any extra dirty path is `NOT PASSED`. Then, in `docs/integration/wisetech-pr6-verification.md`, record:
...
- [ ] **Step 8: Stage the remaining Task 12 paths, commit the complete index, then prove the immutable final range**
    ...
    if ($finalHead -cnotmatch "^[0-9a-f]{40}$" -or $approvedPlanCommit -cnotmatch "^[0-9a-f]{40}$" -or $approvedPlanCommit -cne ([string]$verifiedRuntime.ApprovedPlanCommit)) { throw "PR6_LIGHTHOUSE_VERIFICATION_HEAD_INVALID" }
    $verifiedGit = { & $runtimeInvoker -NativeProcessBoundaryScriptBlock $runtimeNativeProcessBoundary -GuardianScriptBlock $runtimeGuardian -AttestationPath $runtimeDescriptorPath -AttestationSha256 $runtimeDescriptorSha256 -OperationInputRecordPath $operationInputRecordPath -OperationInputRecordSha256 $operationInputRecordSha256 -Tool Git -Arguments @($args) }.GetNewClosure()
