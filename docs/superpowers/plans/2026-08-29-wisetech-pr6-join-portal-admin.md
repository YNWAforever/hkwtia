...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    for ($entryIndex = 0; $entryIndex -lt $suiteEntryCount; $entryIndex += 1) {
      ...
      if ($launchDisposition -ceq "LAUNCHED" -and [string]$requiredCommandEntry.separateAuthorityPolicy -ceq "REQUIRED") {
        ...
        if ([string]$authorityReceipt.receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $seenAuthorityReceiptIdentities.Add([string]$authorityReceipt.receiptIdentitySha256) -or -not $seenAuthorityReceiptHashes.Add([string]$suiteEntry.separateAuthorityReceiptSha256) -or [string]$authorityReceipt.entryIdentitySha256 -cne [string]$suiteEntry.entryIdentitySha256 -or [string]$authorityReceipt.requiredCommandIdentitySha256 -cne [string]$suiteEntry.requiredCommandIdentitySha256 -or [string]$authorityReceipt.commandProjectionSha256 -cne [string]$suiteEntry.commandProjectionSha256 -or [string]$authorityReceipt.resolvedCommandSha256 -cne [string]$suiteEntry.resolvedCommandSha256 -or [string]::IsNullOrWhiteSpace([string]$authorityReceipt.approvalReference) -or [DateTimeOffset]$authorityReceipt.expiresAt -le [DateTimeOffset]$authorityReceipt.issuedAt -or [string]$resultProjection.separateAuthorityReceiptSha256 -cne [string]$suiteEntry.separateAuthorityReceiptSha256) { throw "PR6_SEPARATE_AUTHORITY_RECEIPT_BINDING_INVALID" }
        $separateAuthorityReceipts += $authorityReceipt
      } elseif ($null -ne $resultProjection.separateAuthorityReceiptSha256) {
...
- [ ] **Step 9: Complete independent review and publish the stacked draft PR**
    ...
    if (-not [Object]::Equals($publicationEvidenceManifest.finalStateReceiptPath, $publicationCompletion.finalStateReceiptPath) -or -not [Object]::Equals($publicationEvidenceManifest.finalStateReceiptSha256, $publicationCompletion.finalStateReceiptSha256) -or -not [Object]::Equals($publicationEvidenceManifest.stoppedOperationReceiptPath, $publicationCompletion.stoppedOperationReceiptPath) -or -not [Object]::Equals($publicationEvidenceManifest.stoppedOperationReceiptSha256, $publicationCompletion.stoppedOperationReceiptSha256) -or -not [Object]::Equals($publicationEvidenceManifest.reconciliationDescriptorPath, $publicationCompletion.reconciliationDescriptorPath) -or -not [Object]::Equals($publicationEvidenceManifest.reconciliationDescriptorSha256, $publicationCompletion.reconciliationDescriptorSha256) -or -not [Object]::Equals($publicationEvidenceManifest.reconciliationResultReceiptPath, $publicationCompletion.reconciliationResultReceiptPath) -or -not [Object]::Equals($publicationEvidenceManifest.reconciliationResultReceiptSha256, $publicationCompletion.reconciliationResultReceiptSha256) -or -not [Object]::Equals($publicationEvidenceManifest.lastSuccessfulMutationKind, $publicationCompletion.lastSuccessfulMutationKind)) { throw "PR6_PUBLICATION_MANIFEST_TERMINAL_SOURCE_MISMATCH" }
    ...
    if ($completionStatus -ceq "PASSED") {
      ...
      $stoppedOperationResults = @(& $runtimeInputRecordResolver -Path ([string]$publicationCompletion.stoppedOperationReceiptPath) -Sha256 ([string]$publicationCompletion.stoppedOperationReceiptSha256) -ExpectedSchema "hkwtia.pr6.publication-operation.v1")
      if ($stoppedOperationResults.Count -ne 1 -or [string]$stoppedOperationResults[0].status -cnotin @("NOT_PASSED", "OUTCOME_UNCERTAIN")) { throw "PR6_PUBLICATION_STOPPED_OPERATION_RECEIPT_INVALID" }
