...
- Because this document cannot self-reference its own commit, the external written-plan approval must bind the exact docs-only plan commit, this path, its Git blob OID, and raw SHA-256. Task 0 begins only from that approved-plan commit; every implementation, final-review, and publication head must descend from it.
...
- Every “shell” in this plan means a process created by an operator/CI-owned launcher outside the repository under separately approved `hkwtia.pr6.powershell-launch.v1`. That exact schema binds the canonical PowerShell root/executable path/version, executable SHA-256, complete ordered distribution-manifest path/hash/member aggregate, fixed arguments `-NoLogo -NoProfile -NonInteractive -Command -`, replacement-environment allowlist, launcher approval reference, and one non-inheritable bootstrap/capability channel. The trusted launcher resolves every distribution member without link/reparse ambiguity, applies the same restricted-token/read-only-root directory-entry policy specified below so no new DLL/module/profile can appear, holds the complete distribution immutable, starts the exact executable with no profile and no repository file argument, and feeds the captured dependency-free bootstrap bytes through standard input. It rejects ambient `PSModulePath`, profile variables/scripts, command/path substitutions, and every unapproved environment key.
- Database, Auth, Stripe, Neon, Vercel, GitHub, share-token, and other provider values remain in the trusted launcher's memory escrow; they never exist in the PowerShell process environment, command line, profile, stdin, or output. Before receiving even the non-secret execution-record fields, the new PowerShell process must return an OS-verified handshake containing its process image identity, complete distribution hash, exact startup-argument vector, replacement-environment fingerprint, parent nonce, and proof that profiles were disabled. Only after that match does the launcher send the non-secret resolver-bootstrap fields. Any later text saying “original parent” means this launcher-owned escrow/map; a managed Node child receives its exact permitted subset only over the post-import non-inheritable capability pipe, never through the shell environment. If this external launcher/capability channel is unavailable, every local/provider/publication gate is `NOT PASSED` before repository code.
- The external execution record is populated by the operator or CI approval system before each shell and is never inferred from PATH, a shim, the repository, or ambient configuration. Its exact JSON schema is the discriminated union `hkwtia.pr6.execution-attestation.v1` with required `recordKind: "install-only" | "runtime"`; unknown, missing, duplicate, case-colliding, or kind-incompatible fields fail. Both kinds require canonical `repositoryRoot`; the exact approved PowerShell-launch-record path/hash and equality to the live shell handshake; exact path/allowed-root/SHA-256 entries for generator, resolver, invoker, native process adapter, guardian, and bootstrap contract; the complete PowerShell, Node, and npm distributions; an ordered mode-specific source manifest; and the lowercase SHA-256 of the canonical UTF-8 record. `install-only` requires exactly the bootstrap/install/generator source closure and permits only `install`; it forbids `gitDir`, Git/GitHub CLI, installed CLIs, browser/Chrome/provider/database/Preview/publication fields, and every non-install mode. `runtime` forbids `install` and requires canonical `gitDir`, complete Git distribution, the entire worktree-local `node_modules` tree as one ordered path/length/SHA-256 manifest bound to `package-lock.json`, all runtime source manifests, and every installed CLI used by an allowed mode. A Playwright mode additionally requires a separately approved complete browser-and-FFmpeg distribution plus exact executable/version; a non-browser mode forbids those fields. Publication adds GitHub CLI only through its separate descriptor. Every manifest includes all non-OS executables, libraries, scripts, locales, resources, PAK/snapshot, dynamic packages, and configs that can load while a capability exists. No record contains a credential.
- After the trusted PowerShell-launch handshake, the launcher supplies five independent non-secret core resolver-bootstrap fields—execution-record path/hash and resolver path/allowed-root/hash. Initial RED additionally supplies contract path/allowed-root/hash. An operation shell may additionally receive exactly two non-secret fields, `operationInputRecordPath` and lowercase `operationInputRecordSha256`, only when the approved launch record names one exact operation-input schema, its schema-owned referenced-record schemas, and immutable allowed roots. The live shell image/distribution/arguments/environment must equal the PowerShell launch record and execution record before any core or operation field is released. No supported shell reads console input, command-line values, ambient environment values, clipboard text, profiles, or repository files to discover bootstrap, continuation, approval, gate-status, or reviewed-head fields. A missing/unmanifested file, profile load, alternate host, mutable relative path, extra field, or schema/version drift fails.
- Before invoking `resolve-verified-runtime.ps1`, the captured dependency-free preamble first rechecks the live no-profile PowerShell process handshake and independently approved resolver bytes. The resolver hashes and duplicate-key-validates the record, enforces kind fields, rechecks the current process image/version/distribution/startup arguments/environment against the external launch record, canonicalizes roots, and verifies every distribution/source manifest. It reads invoker, native adapter, guardian, and bootstrap contract once, verifies/strict-UTF-8 decodes those arrays, and returns immutable `InvokerScriptBlock`, `NativeProcessBoundaryScriptBlock`, `GuardianScriptBlock`, `BootstrapContractScriptBlock`, and `InputRecordResolverScriptBlock` plus descriptor path/hash/kind. The input-record resolver accepts only the launch-record-named schema, canonical path under its approved read-only root, and exact lowercase hash; reads the bytes once; rejects links/reparse points, duplicate/case-colliding/unknown/missing fields and noncanonical JSON values; returns one deep-immutable object; and causes the launcher to hold that record plus every exact referenced file immutable until shell exit. It never returns/invokes a mutable path or enumerates the environment.
- Exact non-secret operation records are closed schemas. `hkwtia.pr6.lighthouse-collect-input.v1` binds the Task 12 target-record path/hash, repository/branch/PR5/spec/`preTask12Head`, Preview-approval descriptor path/hash, and the six checkpoint/materialized-entrypoint/materialized-config path/hash fields from `prepare-index`. `hkwtia.pr6.lighthouse-commit-verification.v1` binds repository/branch/PR5/spec/final head, final verification-document `HEAD:path` blob OID/SHA-256, the same six checkpoint identities, and `gateStatus: "PASSED" | "NOT_PASSED"`; `runRoot`, retained-artifact-manifest path/hash, and collection aggregate are required iff `PASSED` and forbidden iff `NOT_PASSED`. `hkwtia.pr6.zero-finding-review-package.v1` binds repository/branch, reviewed head, PR5/spec OIDs, approved-plan commit/path/blob OID/SHA-256, reviewer/verdict evidence hash, and exact finding totals `{critical: 0, important: 0, minor: 0}`. `hkwtia.pr6.publication-input.v1` binds that review-package path/hash, the exact publication-approval descriptor path/hash, repository/head/base branches, base/spec/approved-plan/reviewed-head OIDs, and plan blob identity; all duplicated fields must be byte-equal across the three records. These records contain no token, credential, provider response, or raw review prose. A trusted launcher/CI authority issues each only after its named prerequisite exists; an operation record is never reused for another schema, head, or shell.
...
- [ ] **Step 1: Record an executable dependency-free RED without launching Node, a browser, or provider**
    ...
    $unsafeResults = @(& $approvedUnsafeContract -ExpectedState UnsafeBaseline)
...
- [ ] **Step 4: Run the boundary probes under the verified pre-entry launcher**
    ...
    # RECORD-BARRIER: use the current externally approved kind/mode/source record; after any edit, complete Task 0's Refresh-Pr6ExecutionRecord protocol first.
    & $verifiedNode scripts/run-credential-free-verification.mjs unit -- tests/unit/verified-runtime-bootstrap.test.ts tests/unit/credential-free-verification-boundary.test.ts tests/unit/managed-next-production-boundary.test.ts

    Expected: PASS. Reproduce the trusted no-profile/noninteractive PowerShell launch, replacement environment, pre-field handshake, fake-profile/console/clipboard denial, and schema-owned immutable operation-input records; exact structured RED/GREEN contract results without native exit state; install/runtime schemas; captured bootstrap/native blocks; complete PowerShell/Node/npm/Git/whole-`node_modules`/browser closures; `--no-bin-links` with direct regular CLI targets; restricted-token/read-only-mount directory immutability; source/materialization/output graphs; non-inheritable capability delivery/zeroing; guarded distDir/generated tsconfig; controlled `next-env.d.ts` and redirected build-info writes; generated/traced closure; full browser isolation; atomic containment; and cleanup. Wrong kind/stale record, ambient profile/secret, fake host/PATH, `.bin` link, late file/directory/DLL/PAK/locale/package creation, DACL/mount relaxation, unexpected source write, root build-info, next-env replacement, loader/config/child drift, or capability/process escape fails before acceptance.
...
- [ ] **Step 1: Define final credential-free and authenticated browser verification matrices**
    ...
    Persist those non-secret projections outside the worktree in externally approved immutable `hkwtia.pr6.task12-target.v1` with record path/hash, repository, branch/ref/head, PR5/spec OIDs and successful ancestry-result hashes, capture timestamp, and verified Git descriptor hash. Define `Assert-Pr6LiveTask12Target` to re-read the record/hash, exact branch, `HEAD`, branch ref, and both `merge-base --is-ancestor` results through verified Git and require all equal `preTask12Head`. Capture calls it with a clean-status requirement. Immediately before every M4C or Lighthouse provider request, call it again while no provider capability has been released; only the enumerated Task 12 worktree edits may then be dirty. Close Git handles, and only after success may the trusted launcher deliver the read token. Final commit and publication have their own ancestry checks below.
...
- [ ] **Step 5: Prove safety contracts, then run isolated M1-M7 only with separate authority**
    ...
    - M4C: exact sentinel, M4C descriptor path/hash, Task 12 local-head-record path/hash, launcher-escrowed Vercel read token/optional share token, and absent ambient browser routing. Before token release call `Assert-Pr6LiveTask12Target` and require live branch/ref/HEAD plus PR5/spec ancestry equal `preTask12Head`. Provider Git SHA must equal that same live head. The parent then performs optional protection bootstrap and exact canonical-document/CSS/static capture, constructs typed ordered manifest entries plus the hash-bound in-memory response bundle, destroys token/cookie state, and sends only the bundle after handshake. The browser renders fulfilled captured bytes with network/JS/service workers disabled and asserts the private canary absent.
...
- [ ] **Step 6: Run the exact authorized-Preview Lighthouse and field-performance gates**
    ...
    Use the fail-closed wrapper/config contracts implemented before Step 2. `prepare-index` runs capability-free from the byte-bound source and creates the staged checkpoint below; every Lighthouse/Vercel approval value is absent. Collection requires exact approved `hkwtia.pr6.lighthouse-preview-approval.v1` path/hash. Its closed schema binds approval reference; Task 12 target-record path/hash; repository/branch/`preTask12Head`; deployment/project/optional-team/generated URL; production-origin denylist; and `chrome: {allowedRoot, executablePath, version, distributionManifestPath, distributionManifestSha256, memberAggregateSha256}`. All path/hash fields are canonical/lowercase and every Chrome member entry is covered by the aggregate. Unknown/ambient `LHCI_*` Chrome/head/deployment fields are rejected; the trusted launcher derives the exact non-secret projections from this descriptor and keeps the Vercel read token in escrow.
    Immediately before any Vercel request, while no read token has been released, revalidate the Task 12 record/hash, call `Assert-Pr6LiveTask12Target`, and require live branch ref/HEAD plus PR5/spec ancestry equal descriptor `preTask12Head`. Then close Git handles and deliver only the read-token capability to the already handshaken parent. A descriptor/local/live mismatch fails before provider access or Chrome.
    ...
    if ([string]$lighthouseCollectInput.preTask12Head -cne $preTask12Head) { throw "PR6_LIGHTHOUSE_COLLECT_HEAD_MISMATCH" }
    Assert-Pr6LiveTask12Target
...
- [ ] **Step 8: Stage the remaining Task 12 paths, commit the complete index, then prove the immutable final range**
    ...
    Send only the exact sanitized `$finalHead` and final verification-document blob identity over the launcher-owned control channel, then close the commit shell. The trusted launcher/CI combines those values with the retained checkpoint and optional collection evidence, issues one exact `hkwtia.pr6.lighthouse-commit-verification.v1`, and starts a fresh capability-free verification shell. Rerun Task 0's runtime bootstrap, then resolve the launcher-delivered input record without console or environment input:
    ...
    if ($finalHead -cnotmatch "^[0-9a-f]{40}$") { throw "PR6_LIGHTHOUSE_VERIFICATION_HEAD_INVALID" }
    ...
    } elseif ($lighthouseGateStatus -ceq "NOT_PASSED") {
      if ($null -ne $lighthouseVerificationInput.runRoot) { throw "PR6_LIGHTHOUSE_UNAUTHORIZED_RUN_ROOT_PRESENT" }
    } else {
...
- [ ] **Step 9: Complete independent review and publish the stacked draft PR**
    ...
    The zero-finding verdict must print the exact 40-character reviewed commit SHA and be serialized into immutable `hkwtia.pr6.zero-finding-review-package.v1`. After the separate publication approval, the trusted launcher/CI issues one `hkwtia.pr6.publication-input.v1` that binds that review package and the publication descriptor. Open one fresh publication PowerShell session, rerun Task 0's core bootstrap, and resolve both launcher-delivered non-secret records without console or environment input:
    ...
    $expectedPlan = [string]$reviewPackage.approvedPlanCommit
    $expectedPlanPath = [string]$reviewPackage.approvedPlanPath
    $expectedPlanBlobOid = [string]$reviewPackage.approvedPlanBlobOid
    $expectedPlanSha256 = [string]$reviewPackage.approvedPlanSha256
    $reviewedHead = [string]$reviewPackage.reviewedHead
    if ($reviewedHead -cnotmatch "^[0-9a-f]{40}$" -or $expectedPlan -cnotmatch "^[0-9a-f]{40}$" -or $expectedPlanBlobOid -cnotmatch "^[0-9a-f]{40}$" -or $expectedPlanSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_REVIEW_PACKAGE_IDENTITY_INVALID" }
    if ([int]$reviewPackage.findings.critical -ne 0 -or [int]$reviewPackage.findings.important -ne 0 -or [int]$reviewPackage.findings.minor -ne 0) { throw "PR6_REVIEW_PACKAGE_NOT_ZERO_FINDING" }
    if ([string]$publicationInput.repository -cne $expectedRepo -or [string]$publicationInput.headBranch -cne $expectedBranch -or [string]$publicationInput.baseBranch -cne $expectedBaseBranch -or [string]$publicationInput.baseOid -cne $expectedBase -or [string]$publicationInput.specOid -cne $expectedSpec -or [string]$publicationInput.approvedPlanCommit -cne $expectedPlan -or [string]$publicationInput.reviewedHead -cne $reviewedHead -or [string]$publicationInput.planPath -cne $expectedPlanPath -or [string]$publicationInput.planBlobOid -cne $expectedPlanBlobOid -or [string]$publicationInput.planSha256 -cne $expectedPlanSha256) { throw "PR6_PUBLICATION_INPUT_BINDING_MISMATCH" }
...
- Verification coverage: Task 0 requires an externally trusted, hash-bound, no-profile/noninteractive PowerShell launcher before repository bytes, uses structured pure-PowerShell contract results, closed non-secret operation-input/review records instead of prompts, discriminated install/runtime attestations, independently captured generator/resolver/contract/invoker/native-adapter/guardian blocks, mandatory post-edit source refresh, and complete PowerShell/Node/npm/Git/GitHub-CLI/whole-`node_modules`/browser closures. Install is the sole worktree-write exception, uses `--no-bin-links`, and resolves direct regular CLI targets; all later execution uses restricted-token/DACL or read-only-mount materialization with denied late directory entries. The guarded Next phase routes `distDir` and build info into run-owned output, narrowly permits and validates the source-pinned `next-env.d.ts` rewrite, then seals the full generated/output-traced closure before capability. Task 3 adds the exact loopback-only credential-free null-session seam without masking operational Auth errors. Task 8 withholds every company-scoped Dashboard/Company/Listing/Seats/Billing projection under ambiguous context. Task 12 binds M4C and Lighthouse to a live-revalidated clean local pre-Task12 head plus PR5/spec ancestry; M4C uses canonical as-needed routes and an exact typed response-body/header bundle, while Lighthouse binds a closed approval descriptor to the complete Chrome code/resource distribution. Managed M1-M7/M4B/M5/Axe and PostgreSQL remain separate guarded gates with real session/data restoration. Final verification and publication rehydrate immutable final-head/review/approved-plan identities in fresh shells, revalidate ancestry immediately before provider or remote mutation, and retain per-mutation STARTED/SUCCEEDED receipts, fail-stop OUTCOME_UNCERTAIN behavior, and a separate read-only reconciliation descriptor; no uncertain operation retries or continues.
