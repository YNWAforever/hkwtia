...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    foreach ($suiteEntry in @($suiteEvidenceManifest.entries)) {
      ...
      if ($sourceResults.Count -ne 1 -or $reviewResults.Count -ne 1 -or $receiptResults.Count -ne 1) { throw "PR6_SUITE_REFERENCED_EVIDENCE_COUNT_INVALID" }
    ...
    if ($cleanupReceipts.Count -ne @($suiteEvidenceManifest.entries).Count -or $executionSourceManifests.Count -ne @($suiteEvidenceManifest.entries).Count -or $taskReviewPackages.Count -ne @($suiteEvidenceManifest.entries).Count) { throw "PR6_SUITE_REFERENCED_EVIDENCE_SET_INVALID" }
    ...
    - RED and GREEN evidence for Tasks 1-11 reconstructed from each resolved sanitized command vector/hash, execution head and source manifest, exact RED cause, task base/reviewed head, zero-finding task-review package, exit/totals, and cleanup receipt, plus Task 12 aggregate verification without fabricated RED;
