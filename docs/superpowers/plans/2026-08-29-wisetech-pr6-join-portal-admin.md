...
- [ ] **Step 9: Complete independent review and publish the stacked draft PR**
    ...
    if ($completionStatus -ceq "PASSED") {
      ...
      if ([string]$publicationCompletion.failureKind -ceq "RECONCILED_UNCERTAINTY") {
        ...
        if ($manifestReadReceiptPaths.Count -lt 1 -or $manifestReadReceiptPaths.Count -ne $manifestReadReceiptSha256s.Count -or $manifestReadReceiptPaths.Count -ne $manifestReadClassifications.Count -or $resultReadReceiptPaths.Count -ne $manifestReadReceiptPaths.Count -or $resultReadReceiptSha256s.Count -ne $manifestReadReceiptPaths.Count -or $resultReadClassifications.Count -ne $manifestReadReceiptPaths.Count -or [string]$publicationEvidenceManifest.reconciliationReadReceiptAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$publicationEvidenceManifest.reconciliationReadClassificationAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$reconciliationResultResults[0].reconciliationReadReceiptAggregateSha256 -cne [string]$publicationEvidenceManifest.reconciliationReadReceiptAggregateSha256 -or [string]$reconciliationResultResults[0].reconciliationReadClassificationAggregateSha256 -cne [string]$publicationEvidenceManifest.reconciliationReadClassificationAggregateSha256 -or [string]$reconciliationDescriptorResults[0].readOperationTemplateAggregateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$reconciliationResultResults[0].readOperationTemplateAggregateSha256 -cne [string]$reconciliationDescriptorResults[0].readOperationTemplateAggregateSha256) { throw "PR6_PUBLICATION_RECONCILIATION_READ_AGGREGATE_INVALID" }
        $seenReconciliationReadReceiptHashes = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
        for ($reconciliationReadIndex = 0; $reconciliationReadIndex -lt $manifestReadReceiptPaths.Count; $reconciliationReadIndex += 1) {
          if ([string]::IsNullOrWhiteSpace([string]$manifestReadReceiptPaths[$reconciliationReadIndex]) -or [string]$manifestReadReceiptSha256s[$reconciliationReadIndex] -cnotmatch "^[0-9a-f]{64}$" -or -not $seenReconciliationReadReceiptHashes.Add([string]$manifestReadReceiptSha256s[$reconciliationReadIndex]) -or [string]::IsNullOrWhiteSpace([string]$manifestReadClassifications[$reconciliationReadIndex]) -or [string]$resultReadReceiptPaths[$reconciliationReadIndex] -cne [string]$manifestReadReceiptPaths[$reconciliationReadIndex] -or [string]$resultReadReceiptSha256s[$reconciliationReadIndex] -cne [string]$manifestReadReceiptSha256s[$reconciliationReadIndex] -or [string]$resultReadClassifications[$reconciliationReadIndex] -cne [string]$manifestReadClassifications[$reconciliationReadIndex]) { throw "PR6_PUBLICATION_RECONCILIATION_READ_ORDER_INVALID" }
          ...
          if ([string]$reconciliationReadReceipt.repository -cne $expectedRepo -or [string]$reconciliationReadReceipt.reviewedHead -cne $reviewedHead -or [string]$reconciliationReadReceipt.reconciliationDescriptorSha256 -cne [string]$publicationCompletion.reconciliationDescriptorSha256 -or [string]$reconciliationReadReceipt.operationOutcomeReceiptSha256 -cne [string]$publicationCompletion.uncertainMutationReceiptSha256 -or [string]$reconciliationReadReceipt.classification -cne [string]$manifestReadClassifications[$reconciliationReadIndex] -or [long]$reconciliationReadReceipt.childProcessId -le 0 -or [DateTimeOffset]$reconciliationReadReceipt.childEndedAt -lt [DateTimeOffset]$reconciliationReadReceipt.childStartedAt -or [int]$reconciliationReadReceipt.nativeExitCode -ne 0 -or -not [Object]::Equals($reconciliationReadReceipt.mutationAttempted, $false)) { throw "PR6_PUBLICATION_RECONCILIATION_READ_RECEIPT_INVALID" }
        }
      } elseif ($uncertainMutationReceiptPresent -or $null -ne $publicationCompletion.reconciliationDescriptorPath -or $null -ne $publicationCompletion.reconciliationDescriptorSha256 -or $null -ne $publicationCompletion.reconciliationResultReceiptPath -or $null -ne $publicationCompletion.reconciliationResultReceiptSha256 -or @($publicationEvidenceManifest.reconciliationReadReceiptPaths).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadReceiptSha256s).Count -ne 0 -or @($publicationEvidenceManifest.reconciliationReadClassifications).Count -ne 0 -or $null -ne $publicationEvidenceManifest.reconciliationReadReceiptAggregateSha256 -or $null -ne $publicationEvidenceManifest.reconciliationReadClassificationAggregateSha256) {
