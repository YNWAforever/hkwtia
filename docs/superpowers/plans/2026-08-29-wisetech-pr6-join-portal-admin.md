...
- Exact non-secret operation/evidence records are closed schemas. `hkwtia.pr6.m4c-readonly-input.v1` binds the Task 12 target/helper/exact status-entry identities, M4C approval descriptor, request manifest, repository/branch/PR5/spec/approved-plan/`preTask12Head`, and read-only provider operation. `hkwtia.pr6.lighthouse-collect-input.v1` binds the Task 12 target/helper, exact expected status entries/hash, repository/branch/PR5/spec/approved-plan/`preTask12Head`, Preview approval, and six checkpoint/entrypoint/config identities. `hkwtia.pr6.verification-authoring-input.v1` binds those target/helper/head/checkpoint fields; explicit `preAuthoringExpectedStatusEntries`/hash and `postAuthoringExpectedStatusEntries`/hash, where every ordered entry fixes literal path plus index/worktree status and the post set equals the pre set plus exactly index-clean/worktree-`?` `docs/integration/wisetech-pr6-verification.md` and `docs/integration/wisetech-pr6-pr-body.md` entries; exact Lighthouse attempt/completion, field-INP, and derived overall-gate state; the literal verification-document path and permission to return only its post-commit `HEAD:path` OID proposal; the independently approved root command inventory and its exact `PRE_AUTHORING` partition projection; the exact Task 0 command-evidence manifest/review closure; one `hkwtia.pr6.suite-evidence-manifest.v2`; one `hkwtia.pr6.lighthouse-checkpoint-evidence.v1`; and the conditionally present attempt/provider/run/retained/field records below. The Preview branch is a closed discriminated union: `lighthouseAttemptStatus: "UNAUTHORIZED"` requires `previewApprovalPath`, `previewApprovalSha256`, `deploymentId`, `projectId`, `teamId`, and `generatedUrl` all canonical null and forbids every attempt/provider/run/retained/field field; `AUTHORIZED_FAILED` or `PASSED` requires an exact `hkwtia.pr6.lighthouse-preview-approval.v1` path/hash plus byte-equal deployment/project/nullable-team/canonical-generated-URL fields, and the attempt receipt must duplicate all five identities even when failure occurs before provider proof. `providerProofCompleted` then requires provider-sanitized evidence with those same identities; `runCreated` requires the canonical run root; `retainedManifestCompleted` requires the retained manifest; and non-`UNAUTHORIZED` field INP requires field evidence. `PASSED` Lighthouse requires the attempt receipt and all three completion booleans true, but the overall gate is `PASSED` only when Lighthouse and field INP are both `PASSED`; `retainedManifestCompleted` implies `runCreated`, `runCreated` implies `providerProofCompleted`, and non-`UNAUTHORIZED` field INP implies provider proof. The independently approved `hkwtia.pr6.required-command-matrix.v2` root is issued outside the repository only after the approved-plan commit exists and before Task 0. It is a literal audit of every executable command block in Tasks 0-12 and contains exactly four ordered, nonempty partitions: `TASK0`, `PRE_AUTHORING`, `POST_COMMIT_REVIEW`, and `PUBLICATION`, each with its own entry count/aggregate plus one root count/aggregate. `TASK0` covers every Task 0 command including the one-use initial RED; `PRE_AUTHORING` covers every Task 1-11 command and every Task 12 command completed through Step 6 plus Step 7's pre-authoring checks; `POST_COMMIT_REVIEW` covers the remaining Step 7 authoring/commit commands, all Step 8 final-commit verification, and Step 9 final-review closure commands; `PUBLICATION` covers every Step 9 remote/local publication, conditional push/create/view, and reconciliation branch. No evidence manifest is ever required to equal a future partition. Each root entry fixes `requiredCommandIdentitySha256`, root/partition ordinal, task/step, partition, `commandClass: "RED" | "GREEN" | "GATE" | "CONTROL" | "AGGREGATE" | "REVIEW" | "PUBLICATION"`, verified tool/mode, canonical argument template and SHA-256, an ordered closed typed-runtime-slot schema/aggregate, separate-authority/applicability policy, and exact allowed `{launchDisposition, outcome, nativeExitClass}` tuples. A runtime slot fixes name, primitive/OID/path/hash type, canonicalization, nullability, exact source schema/path field, and producing earlier entry when applicable; no slot may read ambient state. Runtime-generated checkpoint paths/hashes, materialized identities, final/reviewed heads, blob OIDs, PR number, and operation-receipt identities therefore remain typed placeholders in the pre-Task-0 root rather than impossible concrete values. Before the first command of each dependency-ready epoch, trusted launcher/CI issues immutable `hkwtia.pr6.command-phase-projection.v1` resolving only that epoch's slots from already validated records; it binds the root path/hash/count/aggregate, partition count/aggregate, exact ordered root entries, template hashes, typed source fields/values, slot-value aggregates, and resolved sanitized argument-vector hashes. Every projection exists before its commands, cannot alter command class/order/policy, and all ordered projections must exactly and without overlap exhaust their partition. Task 0, pre-authoring, post-commit-review, and publication evidence are respectively closed by `hkwtia.pr6.task0-command-evidence-manifest.v1`, `hkwtia.pr6.suite-evidence-manifest.v2`, `hkwtia.pr6.post-commit-command-evidence-manifest.v1`, and `hkwtia.pr6.publication-command-evidence-manifest.v1`; each must match only its root partition in exact count/order/identity/template/projection/resolved-vector identity. In every manifest, `entryAggregateSha256` is the ordered root-partition identity/template aggregate and is deliberately distinct from the recomputed `fullEntryEvidenceAggregateSha256`, which covers the ordered entry/source/result/cleanup evidence tuples. Every command-evidence entry contains unique entry/source/cleanup identities; root and partition identity; template, slot-value, and resolved-vector hashes; task/step/class/tool/mode; run identity plus nullable child-start/end fields; `launchDisposition: "LAUNCHED" | "REFUSED_BEFORE_LAUNCH" | "PROVEN_NOT_APPLICABLE"`; nullable native exit; `outcome: "EXPECTED_RED" | "PASSED" | "NOT_PASSED" | "NOT_APPLICABLE"`; nullable RED cause; a recomputed `resultProjectionSha256`; immutable `hkwtia.pr6.execution-source-manifest.v1` and `hkwtia.pr6.cleanup-receipt.v2` path/hash; and conditionally exact refusal/applicability receipt path/hash. `LAUNCHED` requires a native exit and forbids refusal/applicability receipts. `REFUSED_BEFORE_LAUNCH` is legal only for `GATE` with separate authority required; it requires `NOT_PASSED`, canonical-null native exit/child/timing fields where no child exists, and immutable `hkwtia.pr6.launcher-refusal-receipt.v1` proving the exact entry/template/projection, policy reason `MISSING_SEPARATE_AUTHORITY`, `childCreated: false`, `nativeExitCode: null`, and no provider/data mutation. `PROVEN_NOT_APPLICABLE` is legal only for a matrix-declared conditional `CONTROL` or `PUBLICATION` branch and requires `NOT_APPLICABLE`, null native exit, and an immutable applicability receipt bound to the exact earlier result that made the branch unreachable. Even refused or inapplicable entries retain a prelaunch source manifest and no-child cleanup receipt, so every required matrix entry has one unique reviewable source/result/cleanup projection without fabricating a process. `RED` requires launched `EXPECTED_RED`, nonzero exit, and a nonempty exact RED cause; `GREEN` requires launched `PASSED`, exit 0, and null RED cause; launched `GATE` requires `PASSED`/0 or `NOT_PASSED`/nonzero; other classes must match their exact root allowed tuple. The resolver validates the Task 0 manifest against the complete `TASK0` partition and one zero-finding Task 0 review package before accepting later evidence. The pre-authoring suite matches only `PRE_AUTHORING`; Tasks 1-11 entries require `ZERO_FINDING`, at least one unique RED and GREEN per task, no AGGREGATE, and may also contain GATE/CONTROL; exactly 11 ordered designated final task-review closures cover Tasks 1-11. Each closure and resolved `hkwtia.pr6.task-review-package.v2` binds exact ordered arrays and recomputed aggregates for entry identities, execution-source hashes, result-projection hashes, and cleanup-receipt hashes plus one canonical full-entry-evidence aggregate; the resolver compares every array element to all task entries and forbids reused source/cleanup/refusal/applicability identities. Task 12 pre-authoring entries require `PENDING_FINAL_REVIEW`, may be GATE/CONTROL/AGGREGATE, require at least one AGGREGATE, and have no task-review package; the resolver binds the complete nonempty ordered Task 12 set by entry/source/result/cleanup counts and aggregates into final review. The source manifest binds repository, entry/matrix identity, execution head, exact prelaunch source bytes/aggregate, execution-attestation hash, template/slot/resolved-command hashes, and command projection. Each cleanup receipt binds repository, unique receipt identity, exact entry/matrix identity, task/step/class/disposition/run/execution head/outcome/native exit, and closed sanitized session/data/provider/artifact/no-child cleanup projections. Checkpoint and retained-artifact evidence retain their existing exact head/path/hash/run bindings. Provider-sanitized evidence binds repository, `evidenceHead`, exact `previewApprovalSha256`, Vercel `deploymentId`, `projectId`, nullable `teamId`, canonical `generatedUrl`, and only token-free head/Chrome projections. Field-INP evidence binds repository, `evidenceHead`, the exact provider-sanitized-evidence hash and `previewApprovalSha256`, byte-equal deployment/project/nullable-team/generated-URL identity, authorization/sample window, sample sufficiency, finite nonnegative p75 or canonical null, threshold 200, and status. `hkwtia.pr6.verification-document-hash-input.v1` retains its exact final document/head/blob binding and the dependency-ready `POST_COMMIT_REVIEW` command-phase projection for that hash shell. `hkwtia.pr6.lighthouse-commit-verification.v1` uses the same conditional Preview union as authoring, binds the exact final document raw blob identity plus pre-Task-12 evidence head/checkpoint and the dependency-ready `POST_COMMIT_REVIEW` command-phase projection for that shell, and revalidates every conditionally completed attempt/provider/run/retained/field record; an authorized attempt receipt always closes the Preview approval/target chain before provider proof. `hkwtia.pr6.zero-finding-review-package.v2` binds repository/branch/reviewed head, PR5/spec/approved-plan identity, reviewer evidence, the root command matrix and all `TASK0`, `PRE_AUTHORING`, and `POST_COMMIT_REVIEW` partition counts/aggregates/manifests, complete pending Task 12 entry/source/result/cleanup aggregates, final verification-document blob identity, and exact zero findings. `hkwtia.pr6.publication-input.v2` duplicates that closure and binds the exact `PUBLICATION` partition/projection plus publication descriptor; after verified publication, `hkwtia.pr6.publication-completion.v1` binds the exact publication evidence manifest and every ordered operation/refusal/applicability receipt. Every resolver recomputes canonical template, slot, vector, entry, result, source, cleanup, partition, manifest, review, checkpoint, provider, retained, field, and publication aggregates; all cross-record duplicates are byte-equal. Unknown fields, missing schemas, unresolved paths, reused evidence identities, partition gaps/overlap, or aggregate drift fail. No record contains a token, credential, raw provider response, or raw review prose. Trusted launcher/CI issues each record only after its prerequisite exists; no record is reused for another schema, head, partition, epoch, or shell.
...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    $expectedPartitionNames = @("TASK0", "PRE_AUTHORING", "POST_COMMIT_REVIEW", "PUBLICATION")
    ...
    $task0ManifestResults = @(& $runtimeInputRecordResolver -Path ([string]$verificationAuthoringInput.task0CommandEvidenceManifestPath) -Sha256 ([string]$verificationAuthoringInput.task0CommandEvidenceManifestSha256) -ExpectedSchema "hkwtia.pr6.task0-command-evidence-manifest.v1")
    $task0ReviewResults = @(& $runtimeInputRecordResolver -Path ([string]$verificationAuthoringInput.task0ReviewPackagePath) -Sha256 ([string]$verificationAuthoringInput.task0ReviewPackageSha256) -ExpectedSchema "hkwtia.pr6.task-review-package.v2")
    ...
    $task0ReviewedSourceSha256s = @($task0ReviewPackage.executionSourceManifestSha256s)
    ...
    if ([string]$task0ReviewPackage.task -cne "0" -or [string]$task0ReviewPackage.partition -cne "TASK0" -or [string]$task0ReviewPackage.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [string]$task0ReviewPackage.fullEntryEvidenceAggregateSha256 -cne [string]$task0Manifest.fullEntryEvidenceAggregateSha256 -or [int]$task0ReviewPackage.findings.critical -ne 0 -or [int]$task0ReviewPackage.findings.important -ne 0 -or [int]$task0ReviewPackage.findings.minor -ne 0 -or $task0ReviewedEntryIdentitySha256s.Count -ne $task0Entries.Count -or $task0ReviewedSourceSha256s.Count -ne $task0Entries.Count -or $task0ReviewedResultSha256s.Count -ne $task0Entries.Count -or $task0ReviewedCleanupSha256s.Count -ne $task0Entries.Count) { throw "PR6_TASK0_REVIEW_PACKAGE_BINDING_INVALID" }
    for ($task0EntryIndex = 0; $task0EntryIndex -lt $task0Entries.Count; $task0EntryIndex += 1) {
      if ([string]$task0ReviewedEntryIdentitySha256s[$task0EntryIndex] -cne [string]$task0Entries[$task0EntryIndex].entryIdentitySha256 -or [string]$task0ReviewedSourceSha256s[$task0EntryIndex] -cne [string]$task0Entries[$task0EntryIndex].executionSourceManifestSha256 -or [string]$task0ReviewedResultSha256s[$task0EntryIndex] -cne [string]$task0Entries[$task0EntryIndex].resultProjectionSha256 -or [string]$task0ReviewedCleanupSha256s[$task0EntryIndex] -cne [string]$task0Entries[$task0EntryIndex].cleanupReceiptSha256) { throw "PR6_TASK0_REVIEW_ORDERED_EVIDENCE_MISMATCH" }
    }
    ...
    $suiteEvidenceManifests = @(& $runtimeInputRecordResolver -Path $suiteEvidenceManifestPath -Sha256 $suiteEvidenceManifestSha256 -ExpectedSchema "hkwtia.pr6.suite-evidence-manifest.v2")
    ...
    for ($entryIndex = 0; $entryIndex -lt $suiteEntryCount; $entryIndex += 1) {
      ...
      if ([string]$suiteEntry.partition -cne "PRE_AUTHORING" -or [string]$suiteEntry.commandTemplateSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.runtimeSlotValuesSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.resolvedCommandSha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.resultProjectionSha256 -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_SUITE_COMMAND_PROJECTION_IDENTITY_INVALID" }
      ...
      if ($launchDisposition -ceq "LAUNCHED") {
        if ($null -eq $suiteEntry.nativeExitCode -or $null -ne $suiteEntry.launcherRefusalReceiptPath -or $null -ne $suiteEntry.launcherRefusalReceiptSha256 -or $null -ne $suiteEntry.applicabilityReceiptPath -or $null -ne $suiteEntry.applicabilityReceiptSha256) { throw "PR6_SUITE_LAUNCHED_DISPOSITION_INVALID" }
        ...
        if ([string]$suiteEntry.commandClass -cne "GATE" -or [string]$requiredCommandEntry.separateAuthorityPolicy -cne "REQUIRED" -or [string]$suiteEntry.outcome -cne "NOT_PASSED" -or $null -ne $suiteEntry.nativeExitCode -or $null -ne $suiteEntry.childProcessId -or $null -ne $suiteEntry.childStartedAt -or $null -ne $suiteEntry.childEndedAt -or $null -ne $suiteEntry.applicabilityReceiptPath -or $null -ne $suiteEntry.applicabilityReceiptSha256) { throw "PR6_SUITE_REFUSED_DISPOSITION_INVALID" }
        $nativeExitClass = "NO_NATIVE_EXIT"
      } elseif ($launchDisposition -ceq "PROVEN_NOT_APPLICABLE") {
        if ([string]$suiteEntry.commandClass -cnotin @("CONTROL", "PUBLICATION") -or [bool]$requiredCommandEntry.conditionalApplicability -ne $true -or [string]$suiteEntry.outcome -cne "NOT_APPLICABLE" -or $null -ne $suiteEntry.nativeExitCode -or $null -ne $suiteEntry.childProcessId -or $null -ne $suiteEntry.childStartedAt -or $null -ne $suiteEntry.childEndedAt -or $null -ne $suiteEntry.launcherRefusalReceiptPath -or $null -ne $suiteEntry.launcherRefusalReceiptSha256) { throw "PR6_SUITE_NOT_APPLICABLE_DISPOSITION_INVALID" }
        $nativeExitClass = "NO_NATIVE_EXIT"
      } else {
      ...
      $receiptResults = @(& $runtimeInputRecordResolver -Path ([string]$suiteEntry.cleanupReceiptPath) -Sha256 ([string]$suiteEntry.cleanupReceiptSha256) -ExpectedSchema "hkwtia.pr6.cleanup-receipt.v2")
    ...
    foreach ($taskNumber in 1..11) {
      ...
      foreach ($closureAggregateField in @("entryIdentityAggregateSha256", "executionSourceManifestAggregateSha256", "resultProjectionAggregateSha256", "cleanupReceiptAggregateSha256", "fullEntryEvidenceAggregateSha256")) {
      ...
      $reviewResults = @(& $runtimeInputRecordResolver -Path ([string]$taskReviewClosure.packagePath) -Sha256 ([string]$taskReviewClosure.packageSha256) -ExpectedSchema "hkwtia.pr6.task-review-package.v2")
      ...
      $reviewedSourceManifestSha256s = @($taskReviewPackage.executionSourceManifestSha256s)
...
- [ ] **Step 9: Complete independent review and publish the stacked draft PR**
    ...
    foreach ($sha256Value in @($reviewerEvidenceSha256, $task0CommandEvidenceManifestSha256, $task0FullEntryEvidenceAggregateSha256, $suiteEvidenceManifestSha256, $preAuthoringFullEntryEvidenceAggregateSha256, $postCommitCommandEvidenceManifestSha256, $postCommitFullEntryEvidenceAggregateSha256, $requiredCommandMatrixSha256, $requiredCommandMatrixRootEntryAggregateSha256, $task0PartitionEntryAggregateSha256, $preAuthoringPartitionEntryAggregateSha256, $postCommitReviewPartitionEntryAggregateSha256, $publicationPartitionEntryAggregateSha256, $pendingTask12EntryIdentityAggregateSha256, $pendingTask12SourceManifestAggregateSha256, $pendingTask12ResultProjectionAggregateSha256, $pendingTask12CleanupReceiptAggregateSha256, $finalVerificationDocumentBlobSha256)) {
      if ([string]$sha256Value -cnotmatch "^[0-9a-f]{64}$") { throw "PR6_FINAL_REVIEW_HASH_IDENTITY_INVALID" }
    }
    if ([string]::IsNullOrWhiteSpace($requiredCommandMatrixPath) -or [string]::IsNullOrWhiteSpace($task0CommandEvidenceManifestPath) -or [string]::IsNullOrWhiteSpace($suiteEvidenceManifestPath) -or [string]::IsNullOrWhiteSpace($postCommitCommandEvidenceManifestPath) -or $requiredCommandMatrixRootEntryCount -lt 4 -or $task0PartitionEntryCount -lt 1 -or $preAuthoringPartitionEntryCount -lt 1 -or $postCommitReviewPartitionEntryCount -lt 1 -or $publicationPartitionEntryCount -lt 1 -or $pendingTask12EntryCount -lt 1 -or $finalVerificationDocumentPath -cne "docs/integration/wisetech-pr6-verification.md" -or $finalVerificationDocumentBlobOid -cnotmatch "^[0-9a-f]{40}$" -or $finalVerificationDocumentBlobByteLength -le 0) { throw "PR6_FINAL_REVIEW_PARTITION_CLOSURE_INVALID" }
    ...
    $requiredCommandMatrixResults = @(& $runtimeInputRecordResolver -Path $requiredCommandMatrixPath -Sha256 $requiredCommandMatrixSha256 -ExpectedSchema "hkwtia.pr6.required-command-matrix.v2")
    $task0ManifestResults = @(& $runtimeInputRecordResolver -Path $task0CommandEvidenceManifestPath -Sha256 $task0CommandEvidenceManifestSha256 -ExpectedSchema "hkwtia.pr6.task0-command-evidence-manifest.v1")
    $preAuthoringManifestResults = @(& $runtimeInputRecordResolver -Path $suiteEvidenceManifestPath -Sha256 $suiteEvidenceManifestSha256 -ExpectedSchema "hkwtia.pr6.suite-evidence-manifest.v2")
    ...
    if ([int]$requiredCommandMatrix.rootEntryCount -ne $requiredCommandMatrixRootEntryCount -or [string]$requiredCommandMatrix.rootEntryAggregateSha256 -cne $requiredCommandMatrixRootEntryAggregateSha256 -or $requiredCommandPartitions.Count -ne 4 -or [string]$requiredCommandPartitions[0].name -cne "TASK0" -or [int]$requiredCommandPartitions[0].entryCount -ne $task0PartitionEntryCount -or [string]$requiredCommandPartitions[0].entryAggregateSha256 -cne $task0PartitionEntryAggregateSha256 -or [string]$requiredCommandPartitions[1].name -cne "PRE_AUTHORING" -or [int]$requiredCommandPartitions[1].entryCount -ne $preAuthoringPartitionEntryCount -or [string]$requiredCommandPartitions[1].entryAggregateSha256 -cne $preAuthoringPartitionEntryAggregateSha256 -or [string]$requiredCommandPartitions[2].name -cne "POST_COMMIT_REVIEW" -or [int]$requiredCommandPartitions[2].entryCount -ne $postCommitReviewPartitionEntryCount -or [string]$requiredCommandPartitions[2].entryAggregateSha256 -cne $postCommitReviewPartitionEntryAggregateSha256 -or [string]$requiredCommandPartitions[3].name -cne "PUBLICATION" -or [int]$requiredCommandPartitions[3].entryCount -ne $publicationPartitionEntryCount -or [string]$requiredCommandPartitions[3].entryAggregateSha256 -cne $publicationPartitionEntryAggregateSha256) { throw "PR6_PUBLICATION_ROOT_MATRIX_PARTITION_BINDING_INVALID" }
    ...
    Close the create epoch immediately after the exact SUCCEEDED receipt and captured URL/number reach the launcher. The launcher validates that receipt, resolves the PR-number runtime slot declared by the next PUBLICATION template, issues a new `hkwtia.pr6.publication-input.v2` continuation plus command-phase projection, and starts a fresh capability-free publication shell. Rerun Task 0 bootstrap, resolve the continuation, require every prior reviewed/matrix/partition/approval field byte-equal, rebuild the three verified invokers, and assign `$createdPrNumber`/`$prUrl` only from the immutable create receipt before continuing. No mutable shell variable crosses epochs.
    ...
    Send the exact sanitized final local/remote/PR identity and ordered publication operation/refusal/applicability receipt hashes over the launcher channel and close the last publication shell. Trusted launcher/CI freezes `hkwtia.pr6.publication-command-evidence-manifest.v1`; its phase projections and entries must exactly, once each and in root order, exhaust the PUBLICATION partition. It then issues `hkwtia.pr6.publication-completion.v1` and starts one final capability-free evidence shell. Rerun Task 0 bootstrap and resolve only that record plus its manifest:

    $publicationCompletionInputs = @(& $runtimeInputRecordResolver -Path $operationInputRecordPath -Sha256 $operationInputRecordSha256 -ExpectedSchema "hkwtia.pr6.publication-completion.v1")
    ...
    if ($completionReviewPackageResults.Count -ne 1 -or $createCompletionReceiptResults.Count -ne 1) { throw "PR6_PUBLICATION_COMPLETION_REFERENCE_COUNT_INVALID" }
    ...
    $createdPrNumber = [string]$createCompletionReceipt.prNumber
    $prUrl = [string]$createCompletionReceipt.prUrl
    ...
    if ($publicationEvidenceManifestResults.Count -ne 1) { throw "PR6_PUBLICATION_EVIDENCE_MANIFEST_COUNT_INVALID" }
    ...
    if ([string]$publicationCompletion.repository -cne $expectedRepo -or [string]$publicationCompletion.reviewedHead -cne $reviewedHead -or [string]$publicationCompletion.prUrl -cne $prUrl -or [string]$publicationCompletion.prNumber -cne $createdPrNumber -or [string]$publicationCompletion.status -cne "PASSED" -or [string]$publicationCompletion.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$publicationCompletion.publicationPartitionEntryCount -ne $publicationPartitionEntryCount -or [string]$publicationCompletion.publicationPartitionEntryAggregateSha256 -cne $publicationPartitionEntryAggregateSha256 -or [string]$publicationEvidenceManifest.partition -cne "PUBLICATION" -or [string]$publicationEvidenceManifest.requiredCommandMatrixSha256 -cne $requiredCommandMatrixSha256 -or [int]$publicationEvidenceManifest.entryCount -ne $publicationPartitionEntryCount -or $publicationEvidenceEntries.Count -ne $publicationPartitionEntryCount -or [string]$publicationEvidenceManifest.entryAggregateSha256 -cne $publicationPartitionEntryAggregateSha256 -or [string]$publicationEvidenceManifest.fullEntryEvidenceAggregateSha256 -cne [string]$publicationCompletion.fullEntryEvidenceAggregateSha256) { throw "PR6_PUBLICATION_COMPLETION_BINDING_INVALID" }
    if (@($publicationCompletion.entryIdentitySha256s).Count -ne $publicationEvidenceEntries.Count -or @($publicationCompletion.resultProjectionSha256s).Count -ne $publicationEvidenceEntries.Count -or @($publicationCompletion.cleanupReceiptSha256s).Count -ne $publicationEvidenceEntries.Count) { throw "PR6_PUBLICATION_COMPLETION_EVIDENCE_CARDINALITY_INVALID" }
    for ($publicationEntryIndex = 0; $publicationEntryIndex -lt $publicationEvidenceEntries.Count; $publicationEntryIndex += 1) {
      if ([string]$publicationCompletion.entryIdentitySha256s[$publicationEntryIndex] -cne [string]$publicationEvidenceEntries[$publicationEntryIndex].entryIdentitySha256 -or [string]$publicationCompletion.resultProjectionSha256s[$publicationEntryIndex] -cne [string]$publicationEvidenceEntries[$publicationEntryIndex].resultProjectionSha256 -or [string]$publicationCompletion.cleanupReceiptSha256s[$publicationEntryIndex] -cne [string]$publicationEvidenceEntries[$publicationEntryIndex].cleanupReceiptSha256) { throw "PR6_PUBLICATION_COMPLETION_ORDERED_EVIDENCE_MISMATCH" }
    }
