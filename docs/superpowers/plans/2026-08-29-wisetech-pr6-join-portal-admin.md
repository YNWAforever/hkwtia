...
- [ ] **Step 4: Run the boundary probes under the verified pre-entry launcher**
    ...
    Additional negative matrix cases require `RECONCILIATION_READ`—and reject GATE/PUBLICATION—for every descriptor ordinal; exercise LAUNCHED, required-authority REFUSED_BEFORE_LAUNCH, and conditional PROVEN_NOT_APPLICABLE on the same fixed entry; reject a missing closed receipt command/result/cleanup/run/head field; distinguish `resolvedCommandSha256` from `resolvedArgumentVectorSha256`; and reject any receipt-versus-entry/result/cleanup mismatch in disposition, child ID, timestamps, native exit, outcome/status, authority, refusal, applicability, result, or cleanup identity. Suffix tests also reject an applicability receipt whose producing read-receipt path/hash is not the exact terminal STOP receipt.
...
- [ ] **Step 7: Write the verification record and prove the committed range**
    ...
    for ($entryIndex = 0; $entryIndex -lt $suiteEntryCount; $entryIndex += 1) {
      ...
      if ($launchDisposition -ceq "REFUSED_BEFORE_LAUNCH") {
        ...
        if ([string]$refusalReceipt.receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.launcherRefusalReceiptSha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $seenRefusalReceiptIdentities.Add([string]$refusalReceipt.receiptIdentitySha256) -or -not $seenRefusalReceiptHashes.Add([string]$suiteEntry.launcherRefusalReceiptSha256) -or [string]$refusalReceipt.entryIdentitySha256 -cne [string]$suiteEntry.entryIdentitySha256 -or [string]$refusalReceipt.requiredCommandIdentitySha256 -cne [string]$suiteEntry.requiredCommandIdentitySha256 -or [string]$refusalReceipt.commandTemplateSha256 -cne [string]$suiteEntry.commandTemplateSha256 -or [string]$refusalReceipt.commandProjectionSha256 -cne [string]$suiteEntry.commandProjectionSha256 -or [string]$refusalReceipt.resultProjectionSha256 -cne [string]$suiteEntry.resultProjectionSha256 -or [string]$refusalReceipt.cleanupReceiptSha256 -cne [string]$suiteEntry.cleanupReceiptSha256 -or [string]$refusalReceipt.expectedAuthorityScope -cne [string]$requiredCommandEntry.separateAuthorityScope -or [string]$refusalReceipt.expectedAuthorityIssuer -cne [string]$requiredCommandEntry.separateAuthorityIssuer -or [string]$refusalReceipt.expectedAuthorizedOperationClass -cne [string]$requiredCommandEntry.authorizedOperationClass -or [string]$refusalReceipt.reason -cne "MISSING_SEPARATE_AUTHORITY" -or [bool]$refusalReceipt.childCreated -ne $false -or $null -ne $refusalReceipt.nativeExitCode -or [bool]$refusalReceipt.providerOrDataMutation -ne $false) { throw "PR6_LAUNCHER_REFUSAL_RECEIPT_BINDING_INVALID" }
        ...
        if ([string]$applicabilityReceipt.receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$suiteEntry.applicabilityReceiptSha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $seenApplicabilityReceiptIdentities.Add([string]$applicabilityReceipt.receiptIdentitySha256) -or -not $seenApplicabilityReceiptHashes.Add([string]$suiteEntry.applicabilityReceiptSha256) -or [string]$applicabilityReceipt.entryIdentitySha256 -cne [string]$suiteEntry.entryIdentitySha256 -or [string]$applicabilityReceipt.requiredCommandIdentitySha256 -cne [string]$suiteEntry.requiredCommandIdentitySha256 -or [string]$applicabilityReceipt.commandTemplateSha256 -cne [string]$suiteEntry.commandTemplateSha256 -or [string]$applicabilityReceipt.commandProjectionSha256 -cne [string]$suiteEntry.commandProjectionSha256 -or [string]$applicabilityReceipt.resultProjectionSha256 -cne [string]$suiteEntry.resultProjectionSha256 -or [string]$applicabilityReceipt.cleanupReceiptSha256 -cne [string]$suiteEntry.cleanupReceiptSha256 -or [string]$applicabilityReceipt.outcome -cne "NOT_APPLICABLE" -or [string]$applicabilityReceipt.producingEntryIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or -not $priorSuiteResultsByEntryIdentity.ContainsKey([string]$applicabilityReceipt.producingEntryIdentitySha256) -or [string]$applicabilityReceipt.producingResultProjectionSha256 -cne [string]$priorSuiteResultsByEntryIdentity[[string]$applicabilityReceipt.producingEntryIdentitySha256]) { throw "PR6_COMMAND_APPLICABILITY_RECEIPT_BINDING_INVALID" }
      ...
      if ([string]$publicationCompletion.failureKind -ceq "RECONCILED_UNCERTAINTY") {
        ...
        for ($reconciliationReadIndex = 0; $reconciliationReadIndex -lt $readCount; $reconciliationReadIndex += 1) {
          ...
          if ([string]$reconciliationReadReceipt.launchDisposition -ceq "LAUNCHED") {
            ...
            if ($readRefusalResults.Count -ne 1 -or [string]$readRefusalResults[0].entryIdentitySha256 -cne [string]$readCommandEntry.entryIdentitySha256 -or [string]$readRefusalResults[0].resultProjectionSha256 -cne [string]$readCommandEntry.resultProjectionSha256 -or [string]$readRefusalResults[0].receiptIdentitySha256 -cnotmatch "^[0-9a-f]{64}$" -or [string]$readCommandEntry.launcherRefusalReceiptPath -cne [string]$reconciliationReadReceipt.launcherRefusalReceiptPath -or [string]$readCommandEntry.launcherRefusalReceiptSha256 -cne [string]$reconciliationReadReceipt.launcherRefusalReceiptSha256 -or [string]$readRefusalResults[0].reason -cne "MISSING_SEPARATE_AUTHORITY" -or -not [Object]::Equals($readRefusalResults[0].childCreated, $false) -or -not [Object]::Equals($readRefusalResults[0].providerOrDataMutation, $false)) { throw "PR6_PUBLICATION_RECONCILIATION_READ_REFUSAL_BINDING_INVALID" }
          } else {
        ...
        for ($suffixIndex = 0; $suffixIndex -lt $expectedSuffixCount; $suffixIndex += 1) {
          ...
          if ($suffixApplicabilityResults.Count -ne 1 -or [string]$suffixApplicabilityResults[0].entryIdentitySha256 -cne [string]$suffixCommandEntries[0].entryIdentitySha256 -or [string]$suffixApplicabilityResults[0].resultProjectionSha256 -cne [string]$suffixCommandEntries[0].resultProjectionSha256 -or [string]$suffixApplicabilityResults[0].producingEntryIdentitySha256 -cne [string]$terminalReadCommandEntry.entryIdentitySha256 -or [string]$suffixApplicabilityResults[0].producingResultProjectionSha256 -cne [string]$terminalReadCommandEntry.resultProjectionSha256 -or [string]$suffixApplicabilityResults[0].producingReconciliationReadReceiptPath -cne $terminalReadReceiptPath -or [string]$suffixApplicabilityResults[0].producingReconciliationReadReceiptSha256 -cne $terminalReadReceiptSha256 -or [string]$suffixApplicabilityResults[0].outcome -cne "NOT_APPLICABLE") { throw "PR6_PUBLICATION_RECONCILIATION_SUFFIX_APPLICABILITY_INVALID" }
        }
