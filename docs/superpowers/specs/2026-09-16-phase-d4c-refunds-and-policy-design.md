...
| Provider succeeds, our commit fails | A commit failure leaves the order paid locally and sends no email. The same idempotency key can return the existing provider refund during its retention window. After that window, [Stripe refuses a second full refund](https://docs.stripe.com/api/refunds/create); the D-4d review correction verifies the exact fully refunded HKD charge and reconciles the local row on retry. An unverified provider state remains failed for staff review. |
