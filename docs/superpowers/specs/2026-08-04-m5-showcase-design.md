...
6. The request-intro server action parses Zod input, rejects a non-empty honeypot without revealing whether a lead exists, applies the process-local rate limiter keyed by the trusted-proxy client address before any database read, creates one `leads` row, sends a staff notification, and sends a localized transactional acknowledgement. Email delivery uses the existing configured/test transport and does not block the lead row from being durable if a provider call fails; delivery failures are logged with a safe error code for retry/inspection.
