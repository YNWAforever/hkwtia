...
- Plan deviation: the WP-8 execution plan stated nothing under `app/` or `components/` would change. `03904e3` (programme-card edition span) and `27065f2` (SME plan-card contrast) both touch product code — `components/marketing/programme-grid.tsx` and `app/styles/wisetech-shell.css` respectively — because the Preview run in §3 found real defects the plan had not anticipated, not because the plan was followed as written.
...
| `npm.cmd test` | 2026-09-07 | 1 in-suite | 452 files passed, 2 failed, 15 skipped (469); 3768 tests passed, 3 failed, 40 skipped (3811). The two failing files (`tests/unit/wt-pages/launchpad-page.test.tsx`, `tests/unit/wt-pages/news-page.test.tsx`) were rerun alone (`wp8-rerun-two-files.log`): exit 1, not 0 — 1 file / 1 test passed (`news-page.test.tsx`), 1 file / 2 tests failed (`launchpad-page.test.tsx`; 47.55 s). The first `launchpad-page` failure is the load-related 5 s timeout recorded for this machine since WP-3; the second (`gives each SectionHeading its own eyebrow, distinct from the section title`) is not an independent second defect — it is a `Found multiple elements with the text: Programme` cascade from the first test's timed-out render still being mounted when the second test queries the DOM. The same cascade signature appears inside the full-suite run at `wp8-gate-unit.log` line 151. That first isolated rerun was itself not isolated: it was started in the same batch as the delivery-gates/route-parity run below and the two vitest processes overlapped. A second rerun with nothing else running (`wp8-rerun-two-files-2.log`, 03:08 HKT, at `e79b4ad`): exit 0, 2 files / 3 tests passed (30.9 s) — consistent with the load-timeout reading |
...
| `npx vitest run tests/unit/wisetech-delivery-gates.test.ts tests/unit/wisetech-route-parity.test.ts` | 2026-09-07 | 0 | 2 files, 32 tests passed (4.34 s), run at the final tip named in `wp8-delivery-gates-final.log`'s last line (`exit: 0 at 22bae00`) — the two tests this checklist and gates correction itself must keep green |
...
| `npm.cmd test` (full unit suite) | after `7fdb423` | 3768 passed, 40 skipped (the 40 are the environment-gated Postgres integration cases) — as reported by the implementing agent at the time; no `wp8-*` log file backs this specific total |
...
4. The Lighthouse path did **not** work: Vercel's SSO check does not accept the session cookie when it arrives as an `extraHeaders` `Cookie` header. This was confirmed independently with `curl` — the five URLs probed (`/`, `/zh`, `/membership`, `/zh/membership`, `/robots.txt`) all answered `302` to the SSO endpoint with the header alone. See §3.4.
...
**Failures 1-3 — real product defect, fixed on this branch (`27065f2`), not yet re-verified on a Preview.** The three failing cases are `accessibility.spec.ts`'s `/membership`, `/zh/membership`, and "the current mobile group … keeps AA contrast", which runs axe on `/zh/membership` after opening the mobile menu. All three share one root cause: axe `color-contrast` (serious) on `.plan-grid > article:nth-child(5)`, the SME pathway card (D-7) that sits on the dark `--wt-ink` `#0f4c81` background. Two elements failed: `.card-index` at `#646464` (1.49:1) and the card's body copy at `rgba(255,255,255,.62)` (4.46:1). The defect is invisible locally because the managed test server's empty database renders the "membership unavailable" notice instead of the plan grid, so no local axe run had ever seen the card; the companion sheet's global `.card-index { color: var(--wt-steel) }` — WP-4's fix for the light cards — had landed on the dark card as well. `27065f2` changes `app/styles/wisetech-shell.css` only: index `#a8d4ec` and body copy `#c9d8e6` — `tests/unit/wisetech-css-port.test.ts` computes each colour out of the port and asserts the ratio `>= 4.5` (the ≥ 4.5:1 floor, not an exact figure); the 5.61:1 index and 6.09:1 body-copy ratios are stated in the source comment directly above these rules in `app/styles/wisetech-shell.css`. Because the Preview predates the fix, re-verification needs WP-8's own Preview (§8).
...
The `/zh` row's five figures were transcribed from that run's `lhr-*.json` report before `.lighthouseci/` was cleaned up — the Preview's raw console transcript (`wp8-lighthouse.log`) only dumps a run's JSON on failure, and the `/zh` run itself succeeded, so its full LHR does not survive in the repository or in any surviving log. That `/zh` report's `finalDisplayedUrl` was the Preview `/zh` page itself; the `/` and `/membership` reports' `finalDisplayedUrl` both ended on `vercel.com/login` (confirmed directly in `wp8-lighthouse.log` for `/membership`) and are discarded for that reason. The transcription itself lives outside this repository, in the session scratchpad's `wp8-lighthouse-extract.md` — a reader of this table must treat the five `/zh` figures as transcribed, not attached.
...
An earlier attempt at this same command aborted before any suite ran: `wp8-local-e2e.log` records `Error: Process from config.webServer was not able to start. Exit code: 3221225794` — the managed dev server itself failed to come up (a stale process still holding the port, per this branch's own gotchas), not a test failure. The suites below are the successful rerun.
...
Local production-build Lighthouse (`next start -p 3200` serving the build from §2, then `LHCI_BASE_URL=http://localhost:3200 npm run test:lighthouse` against it — not the rc's own `npm.cmd run start`, which is skipped whenever `LHCI_BASE_URL` is set) — evidence only, never a verifying run for rows 3.19 / 8.3:

| URL | Performance | Accessibility | SEO | LCP | CLS |
|---|---:|---:|---:|---:|---:|
| `/` | not measured | not measured | not measured | not measured | not measured |
| `/zh` | 0.63 (devtools throttling; not comparable) | 1.00 | 0.92 | 5302 | 0.000 — transcribed from that run's `lhr-*.json` before `.lighthouseci/` was cleaned up; see `wp8-lighthouse-extract.md` in the session scratchpad, outside this repository |
...
- With `--collect.settings.throttlingMethod=devtools` (real throttling, no Lantern simulation) the first URL (`finalDisplayedUrl`: `http://localhost:3200/zh`) completed and is the run recorded above under `/zh`: Lighthouse's Chrome on this zh-HK workstation sends a Chinese `Accept-Language`, so the request for `/` is answered by the locale redirect to `/zh`. The second URL — `/zh` itself, next in `lighthouserc.js`'s target list — did **not** fail with a second Lantern error: `wp8-local-lighthouse-2.log` shows it died of the same Windows temp-directory `EPERM` that killed the Preview run's Chrome teardown (`rmSync` failing to remove `lighthouse.<pid>` after `taskkill` could not find the already-exited process), after a partial audit dump (`first-contentful-paint` score 0.2, `largest-contentful-paint` score 0.42) had already reached the console. Devtools-throttled performance scores are not comparable with the programme's simulated-throttling thresholds (WP-3 recorded 0.89–0.97 on the same pages), so the 0.63 is reported, not judged.
...
- before: `git show 895216b:tests/e2e/__screenshots__/wisetech-visual-baseline/<same file name as the "after" cell it sits beside>`
- after: `tests/e2e/__screenshots__/wisetech-visual-baseline/<file>` in the working tree at this tip

The table's "Before" column below states only whether a before capture exists for that row — build the actual `git show` path from the rule above and the row's own file names, rather than repeating the command in every cell.
...
|---|---|---|---|---|---|---|
| about | en | yes (see note above) | `about-en-desktop-1440-win32-chromium.png` | `about-en-laptop-1120-win32-chromium.png` | `about-en-tablet-820-win32-chromium.png` | `about-en-mobile-390-win32-chromium.png` |
| about | zh | yes (see note above) | `about-zh-desktop-1440-win32-chromium.png` | `about-zh-laptop-1120-win32-chromium.png` | `about-zh-tablet-820-win32-chromium.png` | `about-zh-mobile-390-win32-chromium.png` |
| contact | en | yes (see note above) | `contact-en-desktop-1440-win32-chromium.png` | `contact-en-laptop-1120-win32-chromium.png` | `contact-en-tablet-820-win32-chromium.png` | `contact-en-mobile-390-win32-chromium.png` |
| contact | zh | yes (see note above) | `contact-zh-desktop-1440-win32-chromium.png` | `contact-zh-laptop-1120-win32-chromium.png` | `contact-zh-tablet-820-win32-chromium.png` | `contact-zh-mobile-390-win32-chromium.png` |
| events | en | yes (see note above) | `events-en-desktop-1440-win32-chromium.png` | `events-en-laptop-1120-win32-chromium.png` | `events-en-tablet-820-win32-chromium.png` | `events-en-mobile-390-win32-chromium.png` |
| events | zh | yes (see note above) | `events-zh-desktop-1440-win32-chromium.png` | `events-zh-laptop-1120-win32-chromium.png` | `events-zh-tablet-820-win32-chromium.png` | `events-zh-mobile-390-win32-chromium.png` |
| home | en | yes (see note above) | `home-en-desktop-1440-win32-chromium.png` | `home-en-laptop-1120-win32-chromium.png` | `home-en-tablet-820-win32-chromium.png` | `home-en-mobile-390-win32-chromium.png` |
| home | zh | yes (see note above) | `home-zh-desktop-1440-win32-chromium.png` | `home-zh-laptop-1120-win32-chromium.png` | `home-zh-tablet-820-win32-chromium.png` | `home-zh-mobile-390-win32-chromium.png` |
| launchpad | en | yes (see note above) | `launchpad-en-desktop-1440-win32-chromium.png` | `launchpad-en-laptop-1120-win32-chromium.png` | `launchpad-en-tablet-820-win32-chromium.png` | `launchpad-en-mobile-390-win32-chromium.png` |
| launchpad | zh | yes (see note above) | `launchpad-zh-desktop-1440-win32-chromium.png` | `launchpad-zh-laptop-1120-win32-chromium.png` | `launchpad-zh-tablet-820-win32-chromium.png` | `launchpad-zh-mobile-390-win32-chromium.png` |
| membership | en | yes (see note above) | `membership-en-desktop-1440-win32-chromium.png` | `membership-en-laptop-1120-win32-chromium.png` | `membership-en-tablet-820-win32-chromium.png` | `membership-en-mobile-390-win32-chromium.png` |
| membership | zh | yes (see note above) | `membership-zh-desktop-1440-win32-chromium.png` | `membership-zh-laptop-1120-win32-chromium.png` | `membership-zh-tablet-820-win32-chromium.png` | `membership-zh-mobile-390-win32-chromium.png` |
| news | en | yes (see note above) | `news-en-desktop-1440-win32-chromium.png` | `news-en-laptop-1120-win32-chromium.png` | `news-en-tablet-820-win32-chromium.png` | `news-en-mobile-390-win32-chromium.png` |
| news | zh | yes (see note above) | `news-zh-desktop-1440-win32-chromium.png` | `news-zh-laptop-1120-win32-chromium.png` | `news-zh-tablet-820-win32-chromium.png` | `news-zh-mobile-390-win32-chromium.png` |
| programs-asa | en | yes (see note above) | `programs-asa-en-desktop-1440-win32-chromium.png` | `programs-asa-en-laptop-1120-win32-chromium.png` | `programs-asa-en-tablet-820-win32-chromium.png` | `programs-asa-en-mobile-390-win32-chromium.png` |
| programs-asa | zh | yes (see note above) | `programs-asa-zh-desktop-1440-win32-chromium.png` | `programs-asa-zh-laptop-1120-win32-chromium.png` | `programs-asa-zh-tablet-820-win32-chromium.png` | `programs-asa-zh-mobile-390-win32-chromium.png` |
| showcase | en | yes (see note above) | `showcase-en-desktop-1440-win32-chromium.png` | `showcase-en-laptop-1120-win32-chromium.png` | `showcase-en-tablet-820-win32-chromium.png` | `showcase-en-mobile-390-win32-chromium.png` |
| showcase | zh | yes (see note above) | `showcase-zh-desktop-1440-win32-chromium.png` | `showcase-zh-laptop-1120-win32-chromium.png` | `showcase-zh-tablet-820-win32-chromium.png` | `showcase-zh-mobile-390-win32-chromium.png` |
