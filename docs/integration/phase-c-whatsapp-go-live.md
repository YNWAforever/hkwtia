...
| 7 | **Then, and only then, `RUN_LIVE_WOZTELL=1`.** Since C-9 it is parsed as `"0" \| "1"` by `aiEnv()`, so a typo is a startup error rather than a silent downgrade into mock mode — which used to record every send as delivered while nothing left the building. Nothing else needs a new variable: `WOZTELL_APPROVED_TEMPLATE_KEYS` is the empty-registry fallback only, and is inert after step 1's seed. **The flip turns on THREE outbound paths, not one:** the staff reply lane, the campaign queue, and — easy to forget, because no row used to name it — the **journey runner's WhatsApp leg** (`renewal_14`, `dunning_3`, `event_reminder_24h`, ten-minute cron). All three read both consent stores; see row 8. | The two signed-in walks below, **and** the journey leg of row 8. | blocked |
