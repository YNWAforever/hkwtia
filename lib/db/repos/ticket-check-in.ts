    ...
    transaction: async (work) => (await getDb()).transaction(async (tx) => work({
      lockSeat: async (seatId) => {
        const row = (await tx.select({
          ...
          // The seat row carries no event of its own: the event is read through
          // the order so a seat cannot disagree with its order about the event.
        ...
        return {...row, buyerLocale: row.buyerLocale === "zh-HK" ? "zh-HK" : "en"};
      },
