 ...
 * inside the panel, not by next-intl -- so `Admin.eventsMgmt.cancel.confirm` must be read with
 * `t.raw`, not `t`. `t` parses ICU MessageFormat, and a literal `{orders}` looks exactly like an
 * ICU argument nobody ever passed `values.orders` to; next-intl's development build throws a
 * `FORMATTING_ERROR` for it and falls back to the bare message key, the trap
 * `lib/admin/refund-confirm-message.ts` records for the same shape.
 *
...
export const FALLBACK_CANCEL_CONFIRM_MESSAGE = "{orders} · {amount} · {attendees}";

/** Guards the one contract `confirm` has with CancelEventPanel: all three placeholders present. */
