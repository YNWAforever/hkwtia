...
function isCheckoutActivation(type: SupportedStripeEventType): boolean { return type === "checkout.session.completed" || type === "checkout.session.async_payment_succeeded"; }
function metadataFor(type: SupportedStripeEventType, object: StripeObject) { const value = isCheckoutActivation(type) || type.startsWith("customer.subscription.") ? object.metadata : objectValue(object.subscription_details ?? subscriptionDetails(object.parent)).metadata; const parsed = metadataSchema.safeParse(value); if (!parsed.success) throw new WebhookInputError(); return parsed.data; }
...
function normalize(event: Stripe.Event): WebhookLifecycleCommand | null {
  ...
  if (isCheckoutActivation(eventType) && object.client_reference_id !== metadata.membershipId) throw new WebhookInputError();
  if (eventType === "checkout.session.completed" && object.payment_status === "unpaid") return null;
  ...
  const stripeCheckoutSessionId = isCheckoutActivation(eventType) ? stringId(object.id) : null;
  ...
  if (isCheckoutActivation(eventType) && object.payment_status !== "paid") throw new WebhookInputError();
...
function normalizeRefundFailure(event: Stripe.Event): RefundFailureCommand | null {
  ...
  if ((parsedOrderId !== null && !parsedOrderId.success) || object.status !== "failed" || object.currency !== "hkd" ||
      typeof amount !== "number" || !Number.isSafeInteger(amount) || amount <= 0) throw new WebhookInputError();
