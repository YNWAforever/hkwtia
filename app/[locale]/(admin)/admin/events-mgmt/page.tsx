...
export default async function AdminEventsPage({params}: Props) {
  ...
  const [events, mediaRows, pendingRows, t] = await Promise.all([
    ...
    // The queue degrades to empty rather than taking the whole page down.
    eventsRepository.listForReview(actor).catch(() => []),
  ...
  const reviewRows: ReviewRow[] = pendingRows.map((row) => ({
    id: row.id, slug: row.slug, titleEn: row.title_en, startsAt: row.starts_at, organiser: row.organiser_name ?? null,
