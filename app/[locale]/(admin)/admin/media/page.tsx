...
export default async function AdminMediaPage({ params }: Props) {
  ...
  const createAction = createMediaAction.bind(
    null,
    "/" + locale + "/admin/media",
    {
  ...
  return (
    <div className="space-y-8">
      ...
      <MediaUploadForm labels={uploadLabels} />
              ...
              >
                <span className="flex h-12 w-24 shrink-0 items-center justify-center overflow-hidden rounded border border-border/70 bg-muted/40">
                  <Image
                    ...
                    height={48}
                    src={entry.url}
                    unoptimized={isPrivateMediaDeliveryUrl(entry.url)}
                    width={96}
                  />
                </span>
                <Link
                  className="underline"
                  href={localizedPath(locale, `/admin/media/${entry.id}`)}
                >
